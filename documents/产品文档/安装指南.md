...
-   创建文件系统的NFS共享时，请配置对应的权限参数，确保允许客户端以root用户访问、保留root用户的权限。
...
-   在部署Cantian引擎时，若需要进行两个数据库服务器间的通信认证，请准备好两台数据库服务器各自的CA证书、CRT证书和CRT证书私钥，并确保两台数据库服务器的CA证书、CRT证书和CRT证书私钥内容分别保持一致。否则， 通信认证无法成功。
    ...
    >![](public_sys-resources/icon-notice.gif) **须知：** 
    >CA证书、CRT证书和CRT证书私钥必须存放至"/opt/certificate/"路径下，例如："/opt/certificate/ca.crt"、"/opt/certificate/mes.crt"、"/opt/certificate/mes.key"。
...
-   若数据库服务器的numa被关闭，请执行以下命令打开numa：
    1.  执行以下命令，查看numa是否处于开启状态。

        ```
        lscpu | grep -i numa
        ```

        -   若回显包含“NUMA”的详细信息，表示numa处于开启状态。
        -   若返回的信息为空，表示numa未开启，请参考[2](#li840811815342)开启numa。

        ```
        lscpu | grep -i numa
        [root@node0 action]# lscpu | grep -i numa
        NUMA node(s):                    4
    ...
    2.  <a name="li840811815342"></a>根据以下步骤，开启numa功能：

        >![](public_sys-resources/icon-note.gif) **说明：** 
        >-   进入BIOS对应的热键、numa选项的具体名称和位置可能会因设备型号和BIOS版本而有所不同，请以实际界面为准。
        >-   开启numa的操作步骤，可能会因系统、硬件和BIOS版本的不同而有所差异。在进行BIOS设置前，建议仔细阅读相关的用户手册获取准确的的操作指导。

        1.  重新启动数据库服务器，并在启动时按下对应的热键（如F2、F10、Delete或Esc）进入BIOS设置。
        2.  在BIOS主界面上，导航到“Advanced”或“Chipset Configuration”（或其他与内存相关的选项）等菜单中，找到“NUMA”选项并将其设置为“Enabled”。

            ![](figures/zh-cn_image_0000002103740472.png)

        3.  完成设置后，保存更改并退出BIOS，数据库服务器自动重启后、新的配置生效。
...
4.  <a name="zh-cn_topic_0000001801875845_zh-cn_topic_0000001690212877_zh-cn_topic_0000001521308384_li168751833144317"></a>执行以下命令，上传Cantian引擎安装包并解压。
    ...
    此处以软件包名称为“Cantian\_24.12\_x86\_64\_RELEASE.tgz”、将包放到目录“/ctdb/cantian\_install”为例进行说明。最终的参数配置以《编译指南》的“[版本编译](zh-cn_topic_0000001852743545.md)”中编译生成的软件包和实际的安装路径为准。

    ```
    [root@host ~]# mkdir /ctdb/cantian_install
    [root@host ~]# chmod 755 /ctdb/cantian_install
    [root@host ~]# cd /ctdb/cantian_install
    [root@host  ]# tar -zxvf Cantian_24.12_x86_64_RELEASE.tgz
    ```
...
5.  修改配置文件config\_params\_file.json，相关参数如[表1](#zh-cn_topic_0000001801875845_zh-cn_topic_0000001690212877_zh-cn_topic_0000001521308384_zh-cn_topic_0000001571256873_table111125312412)所示。
    ...
    **表 1**  config\_params\_file.json文件参数说明
    ...
    <tbody><tr id="row124738329328"><td class="cellrowborder" valign="top" width="19.869999999999997%" headers="mcps1.2.3.1.1 "><p id="p15397322144014"><a name="p15397322144014"></a><a name="p15397322144014"></a>deploy_policy</p>
    </td>
    <td class="cellrowborder" valign="top" width="80.13%" headers="mcps1.2.3.1.2 "><p id="p13276751112615"><a name="p13276751112615"></a><a name="p13276751112615"></a>不同场景的配置策略，目前支持配置为“ModeA”。</p>
    </td>
    </tr>
    ...
    <td class="cellrowborder" valign="top" width="80.13%" headers="mcps1.2.3.1.2 "><p id="zh-cn_topic_0000001801875845_p1841320388507"><a name="zh-cn_topic_0000001801875845_p1841320388507"></a><a name="zh-cn_topic_0000001801875845_p1841320388507"></a>挂载storage_dbstor_fs文件系统的逻辑端口的IP。该逻辑IP应在<a href="配置存储网络.md">配置存储网络</a>进行了设置，请填写设置的逻辑IP值。</p>
    </td>
    </tr>
    <tr id="row8704191415380"><td class="cellrowborder" valign="top" width="19.869999999999997%" headers="mcps1.2.3.1.1 "><p id="p18394111517381"><a name="p18394111517381"></a><a name="p18394111517381"></a>mes_ssl_switch</p>
    </td>
    <td class="cellrowborder" valign="top" width="80.13%" headers="mcps1.2.3.1.2 "><p id="p539420151385"><a name="p539420151385"></a><a name="p539420151385"></a>部署Cantian引擎时，数据库服务器间是否进行通信认证。“true”表示需要进行通信认证，“false”表示不需要进行通信认证。默认为“true”。</p>
    <div class="notice" id="note1314471419346"><a name="note1314471419346"></a><a name="note1314471419346"></a><span class="noticetitle"> 须知： </span><div class="noticebody"><p id="p18144181419348"><a name="p18144181419348"></a><a name="p18144181419348"></a>参数为“true”时，通信认证所需的CA证书、CRT证书和CRT证书私钥必须存放至"/opt/certificate/"路径下，例如："/opt/certificate/ca.crt"、"/opt/certificate/mes.crt"、"/opt/certificate/mes.key"。</p>
    </div></div>
    </td>
    </tr>
    ...
    <td class="cellrowborder" valign="top" width="80.13%" headers="mcps1.2.3.1.2 "><p id="zh-cn_topic_0000001801875845_zh-cn_topic_0000001690212877_zh-cn_topic_0000001521308384_p1952054275715"><a name="zh-cn_topic_0000001801875845_zh-cn_topic_0000001690212877_zh-cn_topic_0000001521308384_p1952054275715"></a><a name="zh-cn_topic_0000001801875845_zh-cn_topic_0000001690212877_zh-cn_topic_0000001521308384_p1952054275715"></a>单个数据库服务器归档的最大容量，建议设置为storage_archive_fs文件系统容量的45%（不能高于或等于50%）。若超过了该最大容量，将按归档日志生成的先后顺序、自动删除部分归档日志。</p>
    </td>
    </tr>
    <tr id="row128587229308"><td class="cellrowborder" valign="top" width="19.869999999999997%" headers="mcps1.2.3.1.1 "><p id="p1545472613302"><a name="p1545472613302"></a><a name="p1545472613302"></a>redo_num</p>
    </td>
    <td class="cellrowborder" valign="top" width="80.13%" headers="mcps1.2.3.1.2 "><p id="p204547269303"><a name="p204547269303"></a><a name="p204547269303"></a>单个数据库服务器的redo文件的数目。取值范围：3~256。</p>
    </td>
    </tr>
    <tr id="row259552510306"><td class="cellrowborder" valign="top" width="19.869999999999997%" headers="mcps1.2.3.1.1 "><p id="p1245472623013"><a name="p1245472623013"></a><a name="p1245472623013"></a>redo_size</p>
    </td>
    <td class="cellrowborder" valign="top" width="80.13%" headers="mcps1.2.3.1.2 "><p id="p645532693013"><a name="p645532693013"></a><a name="p645532693013"></a>单个数据库服务器的redo文件的容量。所填入的数值必须为以“G”结尾结尾的整数，例如：1G。</p>
    </td>
    </tr>
    </tbody>
    </table>
