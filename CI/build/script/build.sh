...
echo "WITH_CTC_STORAGE_ENGINE: ${WITH_CTC_STORAGE_ENGINE}"
...
function compileReleaseMysql() {
  ...
  if [ "${BUILD_MODE}" == "multiple" ]; then
    if [ "${LLT_TEST_TYPE}" = "ASAN" ] || [ "${LLT_TEST_TYPE}" = "GCOV" ]; then
      cmake .. -DCMAKE_CXX_COMPILER=/usr/bin/g++ -DCMAKE_C_COMPILER=/usr/bin/gcc -DWITH_ASAN=ON -DWITH_ASAN_SCOPE=ON -DENABLE_GCOV=1 -DWITH_CTC_STORAGE_ENGINE=${WITH_CTC_STORAGE_ENGINE} -DFEATURE_FOR_EVERSQL=${FEATURE_FOR_EVERSQL} -DCMAKE_BUILD_TYPE=Release -DWITH_BOOST=${BOOST_PATH} -DWITHOUT_SERVER=OFF -DCMAKE_INSTALL_PREFIX=${HACTC_LIBCTCPROXY_DIR}/tmp
    else
      if  [[ ${OS_ARCH} =~ "aarch64" ]]; then
        cmake .. -DWITH_CTC_STORAGE_ENGINE=${WITH_CTC_STORAGE_ENGINE} -DFEATURE_FOR_EVERSQL=${FEATURE_FOR_EVERSQL} -DCMAKE_BUILD_TYPE=Release -DWITH_BOOST=${BOOST_PATH} -DCMAKE_C_FLAGS="-g -march=armv8.2-a+crc+lse" -DCMAKE_CXX_FLAGS="-g -march=armv8.2-a+crc+lse" -DWITHOUT_SERVER=OFF -DCMAKE_INSTALL_PREFIX=${HACTC_LIBCTCPROXY_DIR}/tmp
      else
        cmake .. -DWITH_CTC_STORAGE_ENGINE=${WITH_CTC_STORAGE_ENGINE} -DFEATURE_FOR_EVERSQL=${FEATURE_FOR_EVERSQL} -DCMAKE_BUILD_TYPE=Release -DWITH_BOOST=${BOOST_PATH} -DCMAKE_C_FLAGS=-g -DCMAKE_CXX_FLAGS=-g -DWITHOUT_SERVER=OFF -DCMAKE_INSTALL_PREFIX=${HACTC_LIBCTCPROXY_DIR}/tmp
      fi
    fi
  elif [ "${BUILD_MODE}" == "single" ]; then
    if  [[ ${OS_ARCH} =~ "aarch64" ]]; then
      cmake .. -DWITH_CANTIAN=1 -DWITH_CTC_STORAGE_ENGINE=${WITH_CTC_STORAGE_ENGINE} -DFEATURE_FOR_EVERSQL=${FEATURE_FOR_EVERSQL} -DCMAKE_BUILD_TYPE=Release -DWITH_BOOST=${BOOST_PATH} -DCMAKE_C_FLAGS="-g -march=armv8.2-a+crc+lse" -DCMAKE_CXX_FLAGS="-g -march=armv8.2-a+crc+lse" -DWITHOUT_SERVER=OFF -DCMAKE_INSTALL_PREFIX=${HACTC_LIBCTCPROXY_DIR}/tmp
    else
      cmake .. -DWITH_CANTIAN=1 -DWITH_CTC_STORAGE_ENGINE=${WITH_CTC_STORAGE_ENGINE} -DFEATURE_FOR_EVERSQL=${FEATURE_FOR_EVERSQL} -DCMAKE_BUILD_TYPE=Release -DWITH_BOOST=${BOOST_PATH} -DCMAKE_C_FLAGS=-g -DCMAKE_CXX_FLAGS=-g -DWITHOUT_SERVER=OFF -DCMAKE_INSTALL_PREFIX=${HACTC_LIBCTCPROXY_DIR}/tmp
    fi
  fi
}
...
function compileDebugMysql() {
  ...
  if [ "${BUILD_MODE}" == "multiple" ]; then
    if [ "${LLT_TEST_TYPE}" = "ASAN" ] || [ "${LLT_TEST_TYPE}" = "GCOV" ]; then
      cmake .. -DCMAKE_CXX_COMPILER=/usr/bin/g++ -DCMAKE_C_COMPILER=/usr/bin/gcc -DWITH_ASAN=ON -DWITH_ASAN_SCOPE=ON -DENABLE_GCOV=1 -DWITH_CTC_STORAGE_ENGINE=${WITH_CTC_STORAGE_ENGINE} -DFEATURE_FOR_EVERSQL=${FEATURE_FOR_EVERSQL} -DCMAKE_BUILD_TYPE=Debug -DWITH_BOOST=${BOOST_PATH} -DCMAKE_C_FLAGS=-w -DCMAKE_CXX_FLAGS=-w -DWITHOUT_SERVER=OFF -DCMAKE_INSTALL_PREFIX=${HACTC_LIBCTCPROXY_DIR}/tmp
    else
      cmake .. -DWITH_CTC_STORAGE_ENGINE=${WITH_CTC_STORAGE_ENGINE} -DFEATURE_FOR_EVERSQL=${FEATURE_FOR_EVERSQL} -DCMAKE_BUILD_TYPE=Debug -DWITH_BOOST=${BOOST_PATH} -DCMAKE_C_FLAGS=-w -DCMAKE_CXX_FLAGS=-w -DWITHOUT_SERVER=OFF -DCMAKE_INSTALL_PREFIX=${HACTC_LIBCTCPROXY_DIR}/tmp
    fi
  elif [ "${BUILD_MODE}" == "single" ]; then
    cmake .. -DWITH_CANTIAN=1 -DWITH_CTC_STORAGE_ENGINE=${WITH_CTC_STORAGE_ENGINE} -DFEATURE_FOR_EVERSQL=${FEATURE_FOR_EVERSQL} -DCMAKE_BUILD_TYPE=Debug -DWITH_BOOST=${BOOST_PATH} -DCMAKE_C_FLAGS=-w -DCMAKE_CXX_FLAGS=-w -DWITHOUT_SERVER=OFF -DCMAKE_INSTALL_PREFIX=${HACTC_LIBCTCPROXY_DIR}/tmp
  fi
}
...
function compileReleaseAsanMysql() {
  ...
  if [ "${BUILD_MODE}" == "multiple" ]; then
    cmake .. -DCMAKE_CXX_COMPILER=/usr/bin/g++ -DCMAKE_C_COMPILER=/usr/bin/gcc -DWITH_ASAN=ON -DWITH_ASAN_SCOPE=ON \
          -DWITH_CTC_STORAGE_ENGINE=${WITH_CTC_STORAGE_ENGINE} -DFEATURE_FOR_EVERSQL=${FEATURE_FOR_EVERSQL} -DCMAKE_BUILD_TYPE=Release -DWITH_BOOST=${BOOST_PATH} \
          -DWITH_UBSAN=ON -DWITHOUT_SERVER=OFF -DCMAKE_INSTALL_PREFIX=${HACTC_LIBCTCPROXY_DIR}/tmp
  elif [ "${BUILD_MODE}" == "single" ]; then
    cmake .. -DWITH_CANTIAN=1 -DWITH_CTC_STORAGE_ENGINE=${WITH_CTC_STORAGE_ENGINE} -DFEATURE_FOR_EVERSQL=${FEATURE_FOR_EVERSQL} -DCMAKE_BUILD_TYPE=Release \
