...
init_ct_regress()
{
    ...
    echo ENABLE_FDSA = TRUE >> ${CTDB_HOME}/cfg/cantiand.ini
}
...
init_and_start_cms1()
{
	...
	su - cantiandba -c 'cms gcc -reset -f' 2>&1
	su - cantiandba -c 'cms node -add 0 node0 127.0.0.1 1720' 2>&1
	su - cantiandba -c 'cms res -add db -type db -attr "check_timeout=1000000000"' 2>&1
    su - cantiandba -c 'cms res -edit db -attr "HB_TIMEOUT=1000000000"' 2>&1
    su - cantiandba -c 'cms node -list' 2>&1
	su - cantiandba -c 'cms res -list' 2>&1
    echo "start to start cms 1"   
    nohup su - cantiandba -c 'cms server -start' 2>&1  &
}
...
make_code()
{
    ...
    sh Makefile.sh make_regress_test ${lcov_build_flag} CANTIAN_READ_WRITE=1 2>&1 
    if [ "$?" != "0" ]; then
    ...
    strip -N main ${CANTIANDB_LIB}/libzeserver.a 2>&1
    make -sj 8 2>&1 
...
run_dtc_test()
{
    ...
    su - cantiandba -c "${code_path}/output/bin/dtc_test --gtest_output=xml:${code_path}/gtest_result/" 2>&1 | tee -a ${GRUN_LOG}
    su - cantiandba -c "${code_path}/output/bin/mes_test --gtest_output=xml:${code_path}/gtest_result/" 2>&1 | tee -a ${GRUN_LOG}
