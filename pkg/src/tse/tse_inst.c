...
int init_mysql_inst(void)
{
    ...
    if (g_mysql_inst_infos == NULL) {
        GS_LOG_RUN_ERR("init mysql inst info err, can not alloc mem, inst_num(%lu)", g_ctc_max_inst_num);
        return GS_ERROR;
    }
    for (int i = 0; i < g_ctc_max_inst_num; i++) {
        mysql_inst_info_s *inst = &g_mysql_inst_infos[i];
        inst->inst_id = GS_INFINITE32;
        inst->empty = true;
...
int add_mysql_inst_ctx_res(uint32_t inst_id, tse_context_t *ptr)
{
    ...
    if (mysql_inst_pos >= g_ctc_max_inst_num) {
        GS_LOG_RUN_ERR("add_mysql_inst_ctx_res error inst id:%x, mysql_inst_pos:%u", inst_id, mysql_inst_pos);
        return GS_ERROR;
    }
...
int remove_mysql_inst_ctx_res(uint32_t inst_id, tse_context_t *ptr)
{
    ...
    uint32_t mysql_inst_pos = GET_MYSQL_INST_POS(inst_id);
    if (mysql_inst_pos >= g_ctc_max_inst_num) {
        GS_LOG_RUN_ERR("remove_mysql_inst_ctx_res error inst id:%x, mysql_inst_pos:%u", inst_id, mysql_inst_pos);
        return GS_ERROR;
    }
...
{
    GS_LOG_DEBUG_INF("wait for instance_startuped to complete begin.");
    while (is_instance_startuped()) {
        cm_sleep(1000); /* 1000 ms */
    }
    GS_LOG_DEBUG_INF("wait for instance_startuped to complete end.");
    return GS_SUCCESS;
}
...
int tse_release_inst_id(uint32_t inst_id)
{
    ...
    uint32_t mysql_inst_pos = GET_MYSQL_INST_POS(inst_id);
    if (mysql_inst_pos >= g_ctc_max_inst_num) {
        GS_LOG_RUN_ERR("tse_release_inst_id error inst id:%x, mysql_inst_pos:%u", inst_id, mysql_inst_pos);
        return GS_ERROR;
    }

    mysql_inst_info_s *inst = &g_mysql_inst_infos[mysql_inst_pos];
    if (inst->empty == true || inst->inst_id != inst_id) {
        GS_LOG_RUN_ERR("tse_release_inst_id error inst id:%x %x, empty:%d", inst->inst_id, inst_id, inst->empty);
        return GS_SUCCESS;  // 这个地方可能存在没有调用alloc然后来调用release的情况，检查到状态错误直接返回，做任何操作
    }
    clean_up_mysql_inst_ctx_list(inst);
    inst->empty = true;
    inst->inst_id = GS_INFINITE32;
    GS_LOG_RUN_INF("tse_release_inst_id release inst id:%x", inst_id);
    return GS_SUCCESS;
}
...
{
    /* 集群reform期间禁止新的Mysqld接入 */
    if (g_rc_ctx != NULL && RC_REFORM_IN_PROGRESS) {
        GS_LOG_RUN_ERR("[TSE_INIT]:Can't alloc tse_inst_id when Reforming. status:%d", g_rc_ctx->status);
        return GS_ERROR;
    }
    // inst_id 高16位为daac的nodeId,低16位为proc_id/mysql_instid
    uint32_t mysql_inst_pos = GET_MYSQL_INST_POS(*inst_id);
    if (mysql_inst_pos >= g_ctc_max_inst_num) {
        GS_LOG_RUN_ERR("tse_alloc_inst_id error inst id:%u, mysql_inst_pos:%u", *inst_id, mysql_inst_pos);
        return GS_ERROR;
    }
    ...
    mysql_inst_info_s *inst = &g_mysql_inst_infos[mysql_inst_pos];
    if (inst->empty == false || inst->inst_id != GS_INFINITE32) {
        GS_LOG_RUN_ERR("tse_alloc_inst_id slot in use inst id:%u, empty:%d, mysql_inst_pos:%u", *inst_id, inst->empty,
                       mysql_inst_pos);
        return GS_ERROR;
    }
    ...
    *inst_id = alloc_inst_id;
    inst->empty = false;
    GS_LOG_RUN_INF("tse_alloc_inst_id:%x", alloc_inst_id);
    return GS_SUCCESS;
}
...
int tse_get_inst_id(uint32_t shm_client_id, uint32_t *inst_id)
{
    ...
    if (mysql_inst_pos >= g_ctc_max_inst_num) {
        GS_LOG_RUN_ERR("tse_get_inst_id  invaild mysql_inst_pos:%u", mysql_inst_pos);
        return GS_ERROR;
    }
    mysql_inst_info_s *inst = &g_mysql_inst_infos[mysql_inst_pos];
    if (inst->empty == false && inst->inst_id != GS_INFINITE32) {
