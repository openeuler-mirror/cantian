...
#define DIFF_INT(var, v1, v2)                                           \
    do {                                                                \
        if ((v1) != (v2)) {                                             \
            CT_LOG_DEBUG_INF("PRINT_DIFF %s,%d != %d", var, v1, v2);    \
        }                                                               \
    } while (0)
#define DIFF_UINT(var, v1, v2)                                          \
    do {                                                                \
        if ((v1) != (v2)) {                                             \
            CT_LOG_DEBUG_INF("PRINT_DIFF %s:%u != %u", var, v1, v2);    \
        }                                                               \
    } while (0)
#define DIFF_INT64(var, v1, v2)                                          \
    do {                                                                 \
        if ((v1) != (v2)) {                                              \
            CT_LOG_DEBUG_INF("PRINT_DIFF %s:%lld != %lld", var, v1, v2); \
        }                                                                \
    } while (0)
#define DIFF_UINT64(var, v1, v2)                                         \
    do {                                                                 \
        if ((v1) != (v2)) {                                              \
            CT_LOG_DEBUG_INF("PRINT_DIFF %s:%llu != %llu", var, v1, v2); \
        }                                                                \
    } while (0)
#define DIFF_STR(var, v1, v2)                                           \
    do {                                                                \
        if (strcmp(v1, v2) != 0) {                                      \
            CT_LOG_DEBUG_INF("PRINT_DIFF %s:%s != %s", var, v1, v2);    \
        }                                                               \
    } while (0)
...
{
    if (test_column == NULL) {
        CT_LOG_DEBUG_INF("test_column NULL");
        return;
    }
    if (column == NULL) {
        CT_LOG_DEBUG_INF("column NULL");
        return;
    }
    CT_LOG_DEBUG_INF("diff_knl_column_def_t column_name:%s begin", T2S_EX(&column->name));
    ...
    CT_LOG_DEBUG_INF("diff_knl_column_def_t column_name:%s end", T2S_EX(&column->name));
}
void diff_knl_table_def(status_t test_status, knl_table_def_t *test_def, status_t status, knl_table_def_t *def)
{
    CT_LOG_DEBUG_INF("----------------diff_knl_table_def  begin----------------");
    ...
    CT_LOG_DEBUG_INF("----------------diff_knl_table_def  end------------------");
}
void print_knl_column_def_t(knl_column_def_t *column)
{
    if (column == NULL) {
        CT_LOG_DEBUG_INF("NULL COLUMN");
        return;
    }
    CT_LOG_DEBUG_INF("name:%s", T2S(&column->name));
    CT_LOG_DEBUG_INF("datatype:%d", column->datatype);
    CT_LOG_DEBUG_INF("size:%u", column->size);
    CT_LOG_DEBUG_INF("precision:%u", column->precision);
    CT_LOG_DEBUG_INF("scale:%u", column->scale);
    CT_LOG_DEBUG_INF("is_option_set:%x", column->is_option_set);
    CT_LOG_DEBUG_INF("col_id:%u", column->col_id);
    CT_LOG_DEBUG_INF("inl_pri_cons_name:%s", T2S(&column->inl_pri_cons_name));
    CT_LOG_DEBUG_INF("inl_chk_cons_name:%s", T2S(&column->inl_chk_cons_name));
    CT_LOG_DEBUG_INF("inl_uq_cons_name:%s", T2S(&column->inl_uq_cons_name));
    CT_LOG_DEBUG_INF("inl_ref_cons_name:%s", T2S(&column->inl_ref_cons_name));
    CT_LOG_DEBUG_INF("ref_user:%s", T2S(&column->ref_user));
    CT_LOG_DEBUG_INF("ref_table:%s", T2S(&column->ref_table));

    CT_LOG_DEBUG_INF("refactor:%d", column->refactor);
    CT_LOG_DEBUG_INF("default_text:%s", T2S(&column->default_text));
    CT_LOG_DEBUG_INF("check_text:%s", T2S(&column->check_text));
    CT_LOG_DEBUG_INF("comment:%s", T2S(&column->comment));
}

void print_knl_altable_def_t(knl_altable_def_t *def)
{
    CT_LOG_DEBUG_INF("----------------print_knl_altable_def_t  begin----------------");

    CT_LOG_DEBUG_INF("action:%u", def->action);
    CT_LOG_DEBUG_INF("options:%u", def->options);
    CT_LOG_DEBUG_INF("user:%s", T2S(&def->user));
    CT_LOG_DEBUG_INF("name:%s", T2S(&def->name));

    CT_LOG_DEBUG_INF("column_cnt:%d", def->column_defs.count);
    ...
    CT_LOG_DEBUG_INF("----------------print_knl_altable_def_t  end------------------");
}

void print_knl_table_def_t(knl_table_def_t *def)
{
    CT_LOG_DEBUG_INF("----------------print_knl_table_def_t  begin----------------");
    CT_LOG_DEBUG_INF("schema:%s name:%s, space:%s", T2S(&def->schema), T2S(&def->name), T2S(&def->space));
    
    CT_LOG_DEBUG_INF("column_cnt:%d", def->columns.count);
    for (uint32_t i = 0; i < def->columns.count; i++) {
        knl_column_def_t *column = (knl_column_def_t *)cm_galist_get(&def->columns, i);
        print_knl_column_def_t(column);
    }
    CT_LOG_DEBUG_INF("initrans:%u", def->initrans);
    CT_LOG_DEBUG_INF("maxtrans:%u", def->maxtrans);
    CT_LOG_DEBUG_INF("pctfree:%u", def->pctfree);
    CT_LOG_DEBUG_INF("parted:%u", def->parted);
    CT_LOG_DEBUG_INF("sysid:%u", def->sysid);
    CT_LOG_DEBUG_INF("type:%d", def->type);
    CT_LOG_DEBUG_INF("appendonly:%d", def->appendonly);

    CT_LOG_DEBUG_INF("pk_inline:%d", def->pk_inline);
    CT_LOG_DEBUG_INF("uq_inline:%d", def->uq_inline);
    CT_LOG_DEBUG_INF("rf_inline:%d", def->rf_inline);
    CT_LOG_DEBUG_INF("chk_inline:%d", def->chk_inline);

    CT_LOG_DEBUG_INF("options:%u", def->options);
    CT_LOG_DEBUG_INF("serial_start:%lld", def->serial_start);
    CT_LOG_DEBUG_INF("collate:%u", def->collate);
    CT_LOG_DEBUG_INF("charset:%u", def->charset);
    CT_LOG_DEBUG_INF("compress_type:%u", def->compress_type);
    CT_LOG_DEBUG_INF("----------------print_knl_table_def_t  end------------------");
}