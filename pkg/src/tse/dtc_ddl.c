...
status_t tse_send_data_retry(const void *msg_data, uint8 dst_inst)
{
    ...
    SYNC_POINT_GLOBAL_START(TSE_MES_OVERVIEW_SEND_FAIL, &status, GS_ERROR);
    ...
    while (status != GS_SUCCESS) {
        ...
        if (rc_bitmap64_exist(&view.bitmap, dst_inst)) {
            ...
            GS_LOG_RUN_WAR("[TSE_MES]:target instance %u is not alive", dst_inst);
            return GS_ERROR;
        }

        if (retry_time % TSE_RESEND_MSG_TIMES == 0) {
            GS_LOG_RUN_WAR("[TSE_MES]:send message failed times:%u", retry_time);
        }
    }
    return status;
}
...
void tse_deal_repeat_msg(mes_message_head_t *req_head, mes_command_t mes_cmd, knl_session_t *session)
{
    ...
    if (rsp.err_code == TSE_DDL_PROCESSING) {
        GS_LOG_RUN_ERR("[TSE_MES]: %d from src_sid=%u still in processing.", mes_cmd, req_head->src_sid);
        return;
    }

    mes_init_ack_head(req_head, &(rsp.head), mes_cmd, sizeof(msg_ddl_rsp_t), session->id);
    if (mes_send_data(&rsp) != GS_SUCCESS) {
        GS_LOG_RUN_ERR("[TSE_MES]: Response repeat message failed, mes_cmd=%d, src_sid=%u", mes_cmd, req_head->src_sid);
    }
}
                         ...
                         mes_command_t mes_cmd, uint32_t new_msg_num)
{
    ...
    if (old_msg_num == new_msg_num) {
        ...
        GS_LOG_RUN_WAR("[TSE_MES]: Remote node receive repeat msg. mes_cmd=%d, msg_num=%u", mes_cmd, new_msg_num);
        return true;
    }
...
void dtc_proc_msg_tse_lock_table_req(void *sess, mes_message_t *msg)
{
    ...
    if (is_exist_repeat_msg(&(req->head), session, MES_CMD_PREPARE_DDL_RSP, req->msg_num)) {
        GS_LOG_RUN_ERR("[TSE_LOCK_TABLE]: repeat msg, conn_id=%u, lock_info(db=%s, table=%s), tse_instance_id=%u",
    ...
    if (ret != GS_SUCCESS) {
        GS_LOG_RUN_ERR("[TSE_LOCK_TABLE]: remote node execute lock table failed,"
    ...
    if (mes_send_data(&rsp) != GS_SUCCESS) {
        GS_LOG_RUN_ERR("[TSE_LOCK_TABLE]: mes_send_data failed, table lock_info(db=%s, table=%s), "
...
void dtc_proc_msg_tse_execute_ddl_req(void *sess, mes_message_t *msg)
{
    ...
    if (is_exist_repeat_msg(&(req->head), session, MES_CMD_EXECUTE_DDL_RSP, req->msg_num)) {
        GS_LOG_RUN_ERR("[TSE_DDL]: repeat msg, conn_id=%u, sql_command=%u, sql=%s",
    ...
    if (rsp.err_code != GS_SUCCESS) {
        GS_LOG_RUN_ERR("[TSE_DDL]: remote node execute ddl failed. ret=%u, conn_id=%u, user_name=%s,"
                       "sql_command=%u, sql=%s.", rsp.err_code, req->thd_id,
                       req->broadcast_req.user_name, req->broadcast_req.sql_command,
                       sql_without_plaintext_password((req->broadcast_req.options & TSE_CURRENT_SQL_CONTAIN_PLAINTEXT_PASSWORD),
                                                      req->broadcast_req.sql_str, sizeof(req->broadcast_req.sql_str)));
    }
    ...
    if (mes_send_data(&rsp) != GS_SUCCESS) {
        GS_LOG_RUN_ERR("[TSE_DDL]: mes_send_data failed, conn_id=%u, user_name=%s, sql_command=%u, sql=%s.",
                       req->thd_id, req->broadcast_req.user_name, req->broadcast_req.sql_command,
                       sql_without_plaintext_password((req->broadcast_req.options & TSE_CURRENT_SQL_CONTAIN_PLAINTEXT_PASSWORD),
                                                      req->broadcast_req.sql_str, sizeof(req->broadcast_req.sql_str)));
    }
...
void dtc_proc_msg_tse_execute_rewrite_open_conn_req(void *sess, mes_message_t *msg)
{
    ...
    if (is_exist_repeat_msg(&(req->head), session, MES_CMD_REWRITE_OPEN_CONN_RSP, req->msg_num)) {
        GS_LOG_RUN_ERR("[TSE_REWRITE_CONN]: repeat msg, conn_id=%u, sql_command=%u, sql=%s",
                       req->thd_id, req->broadcast_req.sql_command,
                       sql_without_plaintext_password((req->broadcast_req.options & TSE_CURRENT_SQL_CONTAIN_PLAINTEXT_PASSWORD),
                                                      req->broadcast_req.sql_str, sizeof(req->broadcast_req.sql_str)));
        mes_release_message_buf(msg->buffer);
    ...
    if (rsp.err_code != GS_SUCCESS) {
        GS_LOG_RUN_ERR("[TSE_REWRITE_CONN]: remote node execute ddl failed. ret=%u, conn_id=%u, user_name=%s,"
                       "sql_command=%u, sql=%s.", rsp.err_code, req->thd_id,
                       req->broadcast_req.user_name, req->broadcast_req.sql_command,
                       sql_without_plaintext_password((req->broadcast_req.options & TSE_CURRENT_SQL_CONTAIN_PLAINTEXT_PASSWORD),
                                                      req->broadcast_req.sql_str, sizeof(req->broadcast_req.sql_str)));
    ...
    if (mes_send_data(&rsp) != GS_SUCCESS) {
        GS_LOG_RUN_ERR("[TSE_REWRITE_CONN]: mes_send_data failed, conn_id=%u, user_name=%s, sql_command=%u, sql=%s.",
                       req->thd_id, req->broadcast_req.user_name, req->broadcast_req.sql_command,
                       sql_without_plaintext_password((req->broadcast_req.options & TSE_CURRENT_SQL_CONTAIN_PLAINTEXT_PASSWORD),
                                                      req->broadcast_req.sql_str, sizeof(req->broadcast_req.sql_str)));
    }
...
void dtc_proc_msg_tse_commit_ddl_req(void *sess, mes_message_t *msg)
{
    ...
    if (is_exist_repeat_msg(&(req->head), session, MES_CMD_COMMIT_DDL_RSP, req->msg_num)) {
        GS_LOG_RUN_ERR("[TSE_UNLOCK_TABLE]: repeat msg, conn_id=%u, tse_instance_id=%u",
    ...
    rsp.err_code = tse_ddl_execute_unlock_tables(&(req->tch), req->mysql_inst_id);
    if (rsp.err_code != GS_SUCCESS) {
        GS_LOG_RUN_ERR("[TSE_UNLOCK_TABLE]:remote node execute unlock table failed,"
    ...
    if (mes_send_data(&rsp) != GS_SUCCESS) {
        GS_LOG_RUN_ERR("[TSE_UNLOCK_TABLE]: mes_send_data failed, conn_id=%u, tse_instance_id=%u",
                       req->tch.thd_id, req->tch.inst_id);
    }
...
void dtc_proc_msg_tse_close_mysql_conn_req(void *sess, mes_message_t *msg)
{
    ...
    if (is_exist_repeat_msg(&(req->head), session, MES_CMD_CLOSE_MYSQL_CONNECTION_RSP, req->msg_num)) {
        GS_LOG_RUN_ERR("[TSE_CLOSE_SESSION]: repeat msg, src_sid=%u, conn_id=%u, tse_inst_id=%u, msg_num=%u",
    ...
    if (rsp.err_code != GS_SUCCESS) {
        GS_LOG_RUN_ERR("[TSE_CLOSE_SESSION]: close_mysql_connection failed,"
    ...
    if (mes_send_data(&rsp) != GS_SUCCESS) {
        GS_LOG_RUN_ERR("[TSE_CLOSE_SESSION]: mes_send_data failed, src_sid=%u, conn_id=%u, tse_inst_id=%u, msg_num=%u",
                       req->head.src_sid, req->thd_id, req->mysql_inst_id, req->msg_num);
    }
