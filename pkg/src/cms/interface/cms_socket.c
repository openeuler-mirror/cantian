...
status_t cms_socket_open(socket_t* sock_out)
{
    ...
    while (GS_TRUE) {
        ...
        if (sock == CS_INVALID_SOCKET) {
            GS_LOG_RUN_ERR("create socket failed, errno %d[%s]", errno, strerror(errno));
            return GS_ERROR;
        }
#ifndef _WIN32
    ...
    cms_socket_setopt_blocking(sock, GS_FALSE);
...
status_t cms_socket_setopt_blocking(socket_t sockfd, bool32 flag)
{
    ...
    if (nDelayFlag == -1) {
        GS_LOG_RUN_ERR("socket getfl error, sock %d, errno %d[%s]", sockfd, errno, strerror(errno));
        return GS_ERROR;
    }
...
status_t cms_socket_setopt_close_exec(socket_t sockfd)
{
    ...
    if (flags == -1) {
        GS_LOG_RUN_ERR("socket getfd error, sock %d, errno %d[%s]", sockfd, errno, strerror(errno));
        return GS_ERROR;
    }
    flags |= FD_CLOEXEC;
...
status_t cms_uds_create_listener(const char* pszName, socket_t* sock_out)
{
    ...
    if (sock < 0) {
        GS_LOG_RUN_ERR("socket failed, sock %d, errno %d[%s]", sock, errno, strerror(errno));
        return GS_ERROR;
    }

    status_t status = cms_uds_build_addr(&un, pszName, &len);
    if (status == GS_ERROR) {
        GS_LOG_RUN_ERR("build uds address failed, sock %d, errno %d[%s]", sock, errno, strerror(errno));
    ...
    if (ret < 0) {
        GS_LOG_RUN_ERR("bind failed, ret %d, sun path %s, sock %d, errno %d[%s]", ret, un.sun_path,
    ...
    if (ret < 0) {
        GS_LOG_RUN_ERR("listen failed, ret %d, sock %d, errno %d[%s]", ret, sock, errno, strerror(errno));
        cms_socket_close(sock);
        return GS_ERROR;
    }
    *sock_out = sock;
...
status_t cms_uds_connect(const char* pszName, socket_t* sock_out)
{
    ...
    sock = socket(AF_UNIX, SOCK_STREAM, 0);
    if (sock < 0) {
        GS_LOG_RUN_ERR("socket failed, sock %d, uds path %s, errno %d[%s]", sock, pszName, errno, strerror(errno));
        return GS_ERROR;
    }
    ...
    if (ret < 0) {
        GS_LOG_RUN_ERR("connect failed, ret %d, uds path %s, errno %d[%s]", ret, un.sun_path, errno, strerror(errno));
...
status_t cms_socket_accept(socket_t sockfd, int32 timeout_ms, socket_t* sock)
{
    ...
    *sock = accept(sockfd, 0, 0);
    if (*sock == CS_INVALID_SOCKET) {
        GS_LOG_RUN_ERR("accept failed, errno %d[%s].", errno, strerror(errno));
        return GS_ERROR;
    }
...
status_t cms_socket_send_bytes(socket_t sockfd, const char* data, int32* dlen, int32 timeout_ms)
{
    ...
    while (counter < snd_bytes) {
        ...
        if (0 == cc) {
            *dlen = counter;
            GS_LOG_RUN_ERR("send failed, errno %d[%s].", errno, strerror(errno));
            return GS_ERROR;
        }

        if (errno != EINTR) {
            *dlen = counter;
            GS_LOG_RUN_ERR("send failed, ret %d, errno %d[%s].", cc, errno, strerror(errno));
            return GS_ERROR;
        }
    }
...
status_t cms_socket_recv_bytes(socket_t sockfd, char* buf, int32* buf_len, int32 timeout_ms, bool32 is_retry_conn)
{
    ...
    status_t ret = GS_SUCCESS;
    if (sockfd < 0) {
        GS_LOG_RUN_ERR("recv failed, sockfd is invalid, sockfd %d.", sockfd);
        *buf_len = counter;
        return GS_ERROR;
    }
    ...
    if (is_retry_conn) {
        ...
        if (setsockopt(sockfd, SOL_SOCKET, SO_RCVTIMEO, (char *)&timeout_recv, sizeof(struct timeval)) < 0) {
            GS_LOG_RUN_ERR("setsockopt failed");
            return GS_ERROR;
        }
    }
    while (counter < rcv_bytes) {
        ...
        if (ret != GS_SUCCESS) {
            GS_LOG_RUN_ERR("socket wait failed, ret = %d", ret);
            return ret;
        }
        ...
        if (0 == cc) {
            *buf_len = counter;
            GS_LOG_RUN_INF("connection is closed by peer, sock %d.", sockfd);
            return GS_ERROR_CONN_CLOSED;
        }

        if (errno != EINTR) {
            *buf_len = counter;
            GS_LOG_RUN_ERR("recv failed, ret %d, sock %d, errno %d[%s].", cc, sockfd, errno, strerror(errno));
            return GS_ERROR;
        }
    }
...
status_t cms_socket_recv_header(socket_t sockfd, char* buf, int32 size, int32 timeout_ms, bool32 is_retry_conn)
{
    ...
    if (size < header_len) {
        GS_LOG_RUN_ERR("message buffer is not enough.");
        return GS_ERROR;
    }

    ret = cms_socket_recv_bytes(sockfd, buf, &header_len, timeout_ms, is_retry_conn);
    if (ret != GS_SUCCESS) {
        GS_LOG_RUN_ERR("recv msg failed, ret %d, len %d.", ret, header_len);
        return ret;
    }
    return GS_SUCCESS;
}
...
status_t cms_socket_recv_body(socket_t sockfd, char* buf, int32 size, int32 timeout_ms)
{
    ...
    ret = cms_socket_recv_bytes(sockfd, buf, &body_len, timeout_ms, GS_FALSE);
    if (ret != GS_SUCCESS) {
        GS_LOG_RUN_ERR("recv msg failed, ret %d, len %d", ret, body_len);
        return ret;
    }
    return GS_SUCCESS;
}
...
status_t cms_socket_recv(socket_t sockfd, cms_packet_head_t* msg, int32 size, int32 timeout_ms, bool32 is_retry_conn)
{
    ...
    if (ret != GS_SUCCESS) {
        GS_LOG_RUN_ERR("recv msg header failed, ret %d", ret);
        return ret;
    }

    body_len = msg->msg_size - sizeof(cms_packet_head_t);
    if (body_len > size - sizeof(cms_packet_head_t)) {
        GS_LOG_RUN_ERR("message buffer is not enough, msg type %u, msg req %llu, src msg req %llu, msg size %u, "
    ...
    if (ret != GS_SUCCESS) {
        GS_LOG_RUN_ERR("recv msg body failed, ret %d, msg type %u, msg req %llu, src msg req %llu, msg size %u, "
    ...
    CMS_LOG_MSG(GS_LOG_DEBUG_INF, "receive msg succeed", msg);
    return GS_SUCCESS;
}
...
status_t cms_socket_send(socket_t sockfd, cms_packet_head_t* msg, int32 timeout_ms)
{
    ...
    GS_RETURN_IFERR(cms_socket_send_bytes(sockfd, (char*)msg, &size, timeout_ms));

    CMS_LOG_MSG(GS_LOG_DEBUG_INF, "send msg succeed", msg);
