...
static bool32               g_cli_hbt_term = GS_TRUE;
static thread_t             g_cli_recv_thread;
static bool32               g_cli_recvt_term = GS_TRUE;
static bool32               g_cli_conn_try = GS_TRUE;
static thread_t             g_cli_worker_thread;
static bool32               g_cli_workert_term = GS_TRUE;
static char                 g_cms_home[GS_MAX_PATH_LEN] = {0};
static uint16               g_node_id = -1;
static thread_lock_t        g_cli_lock;
static cms_que_t            g_cli_recv_que;
static config_item_t        g_cms_params[] = {
    {"NODE_ID", GS_TRUE, GS_TRUE, "", NULL, NULL, "-", "-", "GS_TYPE_INTEGER", NULL, 0, \
        EFFECT_REBOOT, CFG_INS, NULL, NULL},
    {"_IP", GS_TRUE, GS_FALSE, "", NULL, NULL, "-", "-", "GS_TYPE_STRING", NULL, 0, \
        EFFECT_REBOOT, CFG_INS, NULL, NULL},
    {"_PORT", GS_TRUE, GS_FALSE, "", NULL, NULL, "-", "-", "GS_TYPE_STRING", NULL, 0, \
        EFFECT_REBOOT, CFG_INS, NULL, NULL},
};
...
{
    bool32 is_home_exist = GS_TRUE;

    const char *cms_home_env = getenv(CMS_ENV_CMS_HOME);
    if (cms_home_env == NULL) {
        GS_LOG_RUN_ERR("env $CMS_HOME not exists");
        return GS_ERROR;
    }

    errno_t err = strcpy_s(cms_home, GS_MAX_PATH_LEN, cms_home_env);
    MEMS_RETURN_IFERR(err);

    is_home_exist = cm_dir_exist(cms_home);
    if (is_home_exist == GS_FALSE) {
        GS_LOG_RUN_ERR("CMS_HOME[%s] not exists", cms_home);
        return GS_ERROR;
    }
    ...
    if (path_len > GS_MAX_PATH_BUFFER_SIZE - 1) {
        GS_LOG_RUN_ERR("cms home[%s] is too long", cms_home);
        return GS_ERROR;
    }
    cm_trim_home_path(cms_home, path_len);

    GS_LOG_RUN_INF("get cms home succ, cms home %s", cms_home);
    return GS_SUCCESS;
}
...
static status_t cms_load_param(void)
{
    ...
    int64 size = 0;
    errno_t err = EOK;
    char config_name[GS_FILE_NAME_BUFFER_SIZE] = {0};

    // get config info
    err = snprintf_s(config_name, GS_FILE_NAME_BUFFER_SIZE, GS_FILE_NAME_BUFFER_SIZE - 1,
    ...
    cfg.ignore = GS_TRUE;
    if (cm_read_config(config_name, &cfg) != GS_SUCCESS) {
        GS_LOG_RUN_ERR("read config failed");
        return GS_ERROR;
    }

    value = cm_get_config_value(&cfg, "NODE_ID");
    if (value == NULL || cm_str2bigint(value, &size) != GS_SUCCESS) {
        GS_THROW_ERROR(ERR_GSS_INVALID_PARAM, "invalid parameter value of 'NODE_ID'");
        GS_LOG_RUN_ERR("invalid parameter value of 'NODE_ID'");
        return GS_ERROR;
    }

    if (size < 0 || size >= GS_MAX_INSTANCES) {
        GS_THROW_ERROR(ERR_GSS_INVALID_PARAM, "invalid parameter value[%lld] of 'NODE_ID'", size);
        GS_LOG_RUN_ERR("invalid parameter value[%lld] of 'NODE_ID'", size);
        return GS_ERROR;
    }

    g_node_id = (uint16)size;
    GS_LOG_RUN_INF("CMS NODE_ID:%d", (int32)g_node_id);
...
{
    if (g_cms_home[0] == 0) {
        if (cms_get_cms_home(g_cms_home) != GS_SUCCESS) {
            g_cms_home[0] = 0;
            GS_LOG_RUN_ERR("get cms home failed");
            return GS_ERROR;
        }

        if (cms_load_param() != GS_SUCCESS) {
            g_cms_home[0] = 0;
            GS_LOG_RUN_ERR("load param failed");
            return GS_ERROR;
        }
    }
...
status_t cms_cli_init(void)
{
    ...
    if (ret != GS_SUCCESS) {
        ...
        GS_LOG_RUN_ERR("env init failed, ret %d", ret);
        return ret;
    }
    ...
    if (ret != GS_SUCCESS) {
        cm_destroy_thread_lock(&g_cli_lock);
        GS_LOG_RUN_ERR("init cli recv queue failed, ret %d", ret);
        return ret;
    }
    ...
    if (ret != GS_SUCCESS) {
        cm_destroy_thread_lock(&g_cli_lock);
        GS_LOG_RUN_ERR("uds cli init failed, ret %d", ret);
        return ret;
    }
    GS_LOG_RUN_INF("cms cli init succ");
    return GS_SUCCESS;
}
...
static status_t cms_cli_hb(void)
{
    ...
    if (check_time - last_check_time > CMS_CLI_HB_INTERVAL) {
        ...
        if (ret != GS_SUCCESS) {
            GS_LOG_RUN_ERR("send hb msg failed");
        ...
        GS_LOG_DEBUG_INF("cms cli send hb msg, msg type %u, msg seq %llu", req.head.msg_type, req.head.msg_seq);
    }
...
static void cms_cli_proc_msg_res_stat_chg(cms_packet_head_t* msg)
{
    if (g_notify_func != NULL) {
        GS_LOG_RUN_INF_LIMIT(LOG_PRINT_INTERVAL_SECOND_20,
        ...
        GS_LOG_DEBUG_INF("begin invoke notify function");
        g_notify_func(&chg_msg->stat);
        GS_LOG_DEBUG_INF("end invoke notify function");
        GS_LOG_RUN_INF_LIMIT(LOG_PRINT_INTERVAL_SECOND_20,
...
{
    GS_LOG_DEBUG_INF("begin proc get res stat res msg, msg type %u, msg size %u, msg seq %llu, msg src seq %llu",
        msg->msg_type, msg->msg_size, msg->msg_seq, msg->src_msg_seq);

    if (msg->msg_size > sizeof(cms_cli_msg_res_get_res_stat_t)) {
        GS_LOG_RUN_ERR("invalid msg size, msg size = %u", msg->msg_size);
        return;
    }

    cms_cli_msg_res_get_res_stat_t* res = malloc(sizeof(cms_cli_msg_res_get_res_stat_t));
    if (res == NULL) {
        GS_LOG_RUN_ERR("malloc failed, size %d", (int32)sizeof(cms_cli_msg_res_get_res_stat_t));
        return;
    }
    ...
    if (ret != GS_SUCCESS) {
        GS_LOG_RUN_ERR("can't find wait sender, msg type %u, msg size %u, msg seq %llu, msg src seq %llu",
            res->head.msg_type, res->head.msg_size, res->head.msg_seq, res->head.src_msg_seq);
        CM_FREE_PTR(res);
        return;
    }
    GS_LOG_DEBUG_INF("end proc get res stat res msg, msg type %u, msg size %u, msg seq %llu, msg src seq %llu",
        msg->msg_type, msg->msg_size, msg->msg_seq, msg->src_msg_seq);
}

static void cms_cli_proc_msg_set_res_data(cms_packet_head_t* msg)
{
    GS_LOG_DEBUG_INF("begin proc set res data res msg, msg type %u, msg size %u, msg seq %llu, msg src seq %llu",
        msg->msg_type, msg->msg_size, msg->msg_seq, msg->src_msg_seq);
    ...
    if (res == NULL) {
        GS_LOG_RUN_ERR("malloc failed, size %d", (int32)sizeof(cms_cli_msg_res_set_data_t));
        return;
    }
    ...
    GS_LOG_DEBUG_INF("end proc set res data res msg, msg type %u, msg size %u, msg seq %llu, msg src seq %llu",
        msg->msg_type, msg->msg_size, msg->msg_seq, msg->src_msg_seq);
}

static void cms_cli_proc_msg_set_work_stat(cms_packet_head_t* msg)
{
    GS_LOG_DEBUG_INF("begin proc set work stat res msg, msg type %u, msg size %u, msg seq %llu, msg src seq %llu",
        msg->msg_type, msg->msg_size, msg->msg_seq, msg->src_msg_seq);
    ...
    if (res == NULL) {
        GS_LOG_RUN_ERR("malloc failed, size %d", (int32)sizeof(cms_cli_msg_res_set_work_stat_t));
        return;
    }
    ...
    GS_LOG_DEBUG_INF("end proc set work stat res msg, msg type %u, msg size %u, msg seq %llu, msg src seq %llu",
        msg->msg_type, msg->msg_size, msg->msg_seq, msg->src_msg_seq);
}

static void cms_cli_proc_msg_res_disconn(cms_packet_head_t* msg)
{
    GS_LOG_RUN_INF("begin proc disconn res msg, msg type %u, msg size %u, msg seq %llu, msg src seq %llu",
        msg->msg_type, msg->msg_size, msg->msg_seq, msg->src_msg_seq);
    ...
    if (res == NULL) {
        GS_LOG_RUN_ERR("malloc failed, size %d", (int32)sizeof(cms_cli_msg_res_dis_conn_t));
        return;
    }
    ...
    GS_LOG_RUN_INF("end proc disconn res msg, msg type %u, msg size %u, msg seq %llu, msg src seq %llu",
        msg->msg_type, msg->msg_size, msg->msg_seq, msg->src_msg_seq);
}

static void cms_cli_proc_msg_get_res_data(cms_packet_head_t* msg)
{
    GS_LOG_DEBUG_INF("begin proc get res data res msg, msg type %u, msg size %u, msg seq %llu, msg src seq %llu",
        msg->msg_type, msg->msg_size, msg->msg_seq, msg->src_msg_seq);
    ...
    if (res == NULL) {
        GS_LOG_RUN_ERR("malloc failed, size %d", (int32)sizeof(cms_cli_msg_res_get_data_t));
        return;
    }
    ...
    GS_LOG_DEBUG_INF("end proc get res data res msg, msg type %u, msg size %u, msg seq %llu, msg src seq %llu",
        msg->msg_type, msg->msg_size, msg->msg_seq, msg->src_msg_seq);
}

static void cms_cli_proc_msg_res_hb(cms_packet_head_t* msg)
{
    static uint64 version = GS_INVALID_ID64;
    GS_LOG_DEBUG_INF("begin proc hb res msg, msg type %u, msg size %u, msg seq %llu, msg src seq %llu",
        msg->msg_type, msg->msg_size, msg->msg_seq, msg->src_msg_seq);
    ...
    if (version != res->version && res->version != 0) {
        ...
        if (cms_get_res_stat_list(res_list) != GS_SUCCESS) {
            GS_LOG_RUN_ERR("cms get res stat list faild");
            cm_free(res_list);
            return;
        }
    ...
    GS_LOG_DEBUG_INF("end proc hb res msg, msg type %u, msg size %u, msg seq %llu, msg src seq %llu",
        msg->msg_type, msg->msg_size, msg->msg_seq, msg->src_msg_seq);
}
...
static status_t cms_cli_iof_kick_res(status_t result)
{
    ...
    if (ret != GS_SUCCESS) {
        GS_LOG_RUN_ERR("send iof kick res msg failed");
    ...
    GS_LOG_RUN_INF("send iof kick res msg succeed, msg type %u, msg seq %llu", res.head.msg_type, res.head.msg_seq);
    return GS_SUCCESS;
}
...
static void cms_cli_proc_msg_req_dbs_iof_kick(cms_packet_head_t* msg)
{
    ...
    GS_LOG_DEBUG_INF("begin proc msg req iof kick");
    iof.nodeid = req->node_id;
    iof.sn = req->sn;
    cm_dbs_cfg_s *cfg = cm_dbs_get_cfg();
    ret = cm_dbs_get_ns_id(DEV_TYPE_PGPOOL, &iof.nsid);
    if (ret != GS_SUCCESS) {
        GS_LOG_RUN_ERR("get dbstore page pool nsid failed, namespace name %s, ret %d.", cfg->ns, ret);
        return;
    }
    ...
    if (ret != GS_SUCCESS) {
        GS_LOG_RUN_ERR("proc msg req iof kick failed, kick node %u, namespace name %s, ret %d.", req->node_id, cfg->ns,
            ret);
    }

    (void)cms_cli_iof_kick_res(ret);
    if (ret != GS_SUCCESS) {
        GS_LOG_RUN_ERR("proc msg req iof kick failed.");
        return;
    }
    GS_LOG_DEBUG_INF("proc msg req iof kick succ");
}
...
static void cms_cli_proc_msg_req_file_iof_kick(cms_packet_head_t* msg)
{
    ...
    if (ret != GS_SUCCESS) {
        GS_LOG_RUN_ERR("proc msg req iof kick failed, kick node %u, ret %d.", req->node_id, ret);
    }
...
static void cms_cli_proc_msg(cms_packet_head_t* msg)
{
    switch (msg->msg_type) {
        ...
        default:
            GS_LOG_RUN_ERR("unknown message type:%d", (int32)msg->msg_type);
    }
}
...
static void cms_uds_cli_retry_conn(void)
{
    ...
    for (int32 i = 0; i < CMS_RETRY_CONN_COUNT; i++) {
        if (g_cli_conn_try == GS_FALSE) {
            return;
        }
        cms_uds_cli_info_t cms_uds_cli_info = { g_res_type, g_inst_id, GS_TRUE, CMS_CLI_RES };
        ret = cms_uds_cli_connect(&cms_uds_cli_info, NULL);
        if (ret == GS_SUCCESS) {
            GS_LOG_RUN_ERR("cms cli retry conn succ, i %d", i);
            return;
        }
        GS_LOG_RUN_ERR("cms cli retry conn failed, ret %d, i %d", ret, i);
...
{
    g_cli_hbt_term = GS_FALSE;
    thread->closed = GS_FALSE;
    status_t ret = GS_SUCCESS;

    GS_LOG_RUN_INF("start cli hb etnry thread");
    while (!thread->closed) {
        ret = cms_cli_hb();
        if (ret != GS_SUCCESS) {
            GS_LOG_RUN_ERR("cms cli send hb failed");
        }
        cm_sleep(CMS_CLI_UDS_HB_INTERVAL);
    }

    g_cli_hbt_term = GS_TRUE;
    GS_LOG_RUN_INF("end cli hb etnry thread");
}

static void cms_cli_recv_entry(thread_t *thread)
{
    g_cli_recvt_term = GS_FALSE;
    thread->closed = GS_FALSE;
    static char msg_buf[CMS_MAX_MSG_SIZE] = {0};
    status_t ret = GS_SUCCESS;

    GS_LOG_RUN_INF("start cli recv etnry thread");
    while (!thread->closed) {
        GS_LOG_DEBUG_INF("begin cms cli recv msg");
        ...
        GS_LOG_DEBUG_INF("cms cli recv msg, msg type %u, msg seg %llu, msg src seq %llu, msg size %u",
        ...
        if (node == NULL) {
            GS_LOG_RUN_ERR("cms que alloc node failed");
            continue;
        }
        cms_enque(&g_cli_recv_que, node);
    }

    g_cli_recvt_term = GS_TRUE;
    GS_LOG_RUN_INF("end cli recv etnry thread");
}

static void cms_cli_worker_entry(thread_t *thread)
{
    g_cli_workert_term = GS_FALSE;
    thread->closed = GS_FALSE;
    biqueue_node_t *node = NULL;
    cms_packet_head_t* msg = NULL;

    GS_LOG_DEBUG_INF("start cli worker entry thread");
    while (!thread->closed) {
        node = cms_deque(&g_cli_recv_que);
        if (node == NULL) {
            GS_LOG_DEBUG_INF("cms cli worker entry get nothing, continue to loop");
            continue;
        }

        msg = (cms_packet_head_t*)cms_que_node_data(node);
        GS_LOG_DEBUG_INF("cms cli worker etnry proc msg, msg type %u, msg seq %llu, msg src seq %llu",
            msg->msg_type, msg->msg_seq, msg->src_msg_seq);
        cms_cli_proc_msg(msg);
        cms_que_free_node(node);
    }

    g_cli_workert_term = GS_TRUE;
    GS_LOG_DEBUG_INF("end cli worker entry thread");
}
    ...
    res_init_info_t *res_init_info, cms_notify_func_t notify_func, cms_master_op_t master_func)
{
    ...
    if (g_inst_id == inst_id) {
        GS_LOG_RUN_ERR("resource is already registered, instance id %d", (int32)g_inst_id);
        return GS_ERROR;
    }
    ...
    cms_uds_cli_info_t cms_uds_cli_info = { res_type, inst_id, GS_FALSE, CMS_CLI_RES };
    ret = cms_uds_cli_connect(&cms_uds_cli_info, res_init_info);
    if (ret != GS_SUCCESS) {
        GS_LOG_RUN_ERR("cms cli connect to server failed, ret %d", ret);
        return ret;
    }

    ret = cm_create_thread(cms_cli_hb_entry, GS_DFLT_THREAD_STACK_SIZE, NULL, &g_cli_hb_thread);
    if (ret != GS_SUCCESS) {
        cms_uds_cli_sock_close();
        GS_LOG_RUN_ERR("start cms cli hb entry thread create failed, ret %d", ret);
        return ret;
    }

    ret = cm_create_thread(cms_cli_recv_entry, GS_DFLT_THREAD_STACK_SIZE, NULL, &g_cli_recv_thread);
    if (ret != GS_SUCCESS) {
        cms_uds_cli_sock_close();
        GS_LOG_RUN_ERR("start cms cli recv entry thread create failed, ret %d", ret);
        return ret;
    }

    for (uint32 i = 0; i < CMS_CLI_WORK_ENTRY_COUNT; i++) {
        ret = cm_create_thread(cms_cli_worker_entry, GS_DFLT_THREAD_STACK_SIZE, NULL, &g_cli_worker_thread);
        if (ret != GS_SUCCESS) {
            cms_uds_cli_sock_close();
            GS_LOG_RUN_ERR("start cms cli worker entry thread create failed, ret %d", ret);
            return ret;
        }
    }
    GS_LOG_RUN_INF("register res to cms succ");
    return GS_SUCCESS;
}
    ...
    cms_notify_func_t notify_func, cms_master_op_t master_func)
{
    ...
    if (ret != GS_SUCCESS) {
        GS_LOG_RUN_ERR("register resource failed, res type %s, instance id %d", g_res_type, (int)g_inst_id);
        g_res_type[0] = 0;
        g_inst_id = -1;
        g_notify_func = NULL;
        g_master_func = NULL;
    } else {
        GS_LOG_RUN_INF("register resource successful, res type %s, instance id %d", g_res_type, (int)g_inst_id);
    }
...
status_t cms_send_disconn_req(void)
{
    ...
    GS_LOG_RUN_INF("begin send disconnect req to cms");
    ...
    if (err != EOK) {
        GS_LOG_RUN_ERR("strcpy_s failed, err %d, errno %d[%s]", err, errno, strerror(errno));
        return GS_ERROR;
    }
    ...
    if (ret != GS_SUCCESS) {
        GS_LOG_RUN_ERR("cms cli uds request failed, ret %d", ret);
    ...
    if (res.result != GS_SUCCESS) {
        GS_LOG_RUN_ERR("send disconnect msg failed, result %d", res.result);
        return GS_ERROR;
    }

    GS_LOG_RUN_INF("disconnect from cms succ");
    return GS_SUCCESS;
}
...
status_t cms_res_inst_unregister_inner(void)
{
    ...
    g_cli_conn_try = GS_FALSE;
    ret = cms_send_disconn_req();
    if (ret != GS_SUCCESS) {
        GS_LOG_RUN_ERR("cms send disconnect req to cms failed, ret %d", ret);
    }

    g_cli_recv_thread.closed = GS_TRUE;
    while (!g_cli_recvt_term) {
        cm_sleep(CMS_CLI_SLEEP_INTERVAL);
    }

    g_cli_worker_thread.closed = GS_TRUE;
    while (!g_cli_workert_term) {
        cm_sleep(CMS_CLI_SLEEP_INTERVAL);
    }

    g_cli_hb_thread.closed = GS_TRUE;
    ...
    GS_LOG_RUN_INF("unregister from cms succ");
    return GS_SUCCESS;
}
...
status_t cms_res_inst_unregister(void)
{
    ...
    if (ret != GS_SUCCESS) {
        GS_LOG_RUN_ERR("unregister resource failed");
    } else {
        GS_LOG_RUN_INF("unregister resource successful");
    }
    cm_thread_unlock(&g_cli_lock);
    return ret;
}
...
{
    if (g_inst_id == -1) {
        GS_LOG_RUN_ERR("resource instance not be registered");
        return GS_ERROR;
    }
    ...
    status_t ret = cms_uds_cli_request(&req.head, &res.head, sizeof(cms_cli_msg_res_set_work_stat_t),
        CMS_CLIENT_REQUEST_TIMEOUT);
    if (ret != GS_SUCCESS) {
        GS_LOG_RUN_ERR("cms set res work stat msg failed");
    ...
    if (res.result != GS_SUCCESS) {
        GS_LOG_RUN_ERR("set data failed, work stat %d, msg type %u, msg req %llu",
            (int32)stat, res.head.msg_type, res.head.msg_seq);
        return GS_ERROR;
    }

    GS_LOG_RUN_INF("set work stat succ, stat %d", (int32)stat);
    return GS_SUCCESS;
}
...
status_t cms_get_res_stat_list1(const char* res_type, cms_res_status_list_t* res_list)
{
    ...
    status_t ret = cms_uds_cli_request(&req.head, &res_stat.head, sizeof(cms_cli_msg_res_get_res_stat_t),
        CMS_CLIENT_REQUEST_TIMEOUT);
    if (ret != GS_SUCCESS) {
        GS_LOG_RUN_ERR("cms cli uds request failed");
    ...
    if (res_stat.result != GS_SUCCESS) {
        GS_LOG_RUN_ERR("cms cli get res stat failed, result %d", res_stat.result);
        return GS_ERROR;
    }
    ...
    GS_LOG_DEBUG_INF("get res stat succ, version %lld, inst count %d", res_list->version, res_list->inst_count);
    for (int32 i = 0; i < res_list->inst_count; i++) {
        GS_LOG_DEBUG_INF("res stat, instid %d, node id %d, stat %d, wrok stat %d",
...
cms_cli_msg_req_set_data_t* cms_get_new_set_data_req(uint32 slot_id, char* data, uint32 size, uint64 old_version)
{
    ...
    if (req == NULL) {
        GS_LOG_RUN_ERR("malloc failed, size %u", (uint32)sizeof(cms_cli_msg_req_set_data_t));
        return NULL;
    }
    ...
    errno_t err = strcpy_s(req->res_type, CMS_MAX_RES_TYPE_LEN, g_res_type);
    if (err != EOK) {
        GS_LOG_RUN_ERR("strcpy_s failed, err %d", err);
        CM_FREE_PTR(req);
        return NULL;
    }
    err = memcpy_s(req->data, CMS_MAX_RES_DATA_SIZE, data, size);
    if (err != EOK) {
        GS_LOG_RUN_ERR("memcpy_s failed, err %d", err);
        CM_FREE_PTR(req);
        return NULL;
    }
    req->slot_id = slot_id;
...
status_t cms_set_res_data_new(uint32 slot_id, char* data, uint32 size, uint64 old_version)
{
    ...
    if (slot_id >= CMS_MAX_RES_SLOT_COUNT) {
        GS_LOG_RUN_ERR("invalid slot id, slot id %u", slot_id);
        return GS_ERROR;
    }

    if (size >= CMS_MAX_RES_DATA_SIZE) {
        GS_LOG_RUN_ERR("invalid size, size %u", size);
        return GS_ERROR;
    }

    cms_cli_msg_req_set_data_t* req = cms_get_new_set_data_req(slot_id, data, size, old_version);
    if (req == NULL) {
        GS_LOG_RUN_ERR("get new set data req failed");
        return GS_ERROR;
    }
    ...
    if (err != EOK) {
        GS_LOG_RUN_ERR("memset_s failed, err %d", err);
        CM_FREE_PTR(req);
        return GS_ERROR;
    }
    ...
    status_t ret = cms_uds_cli_request(&req->head, &res.head, sizeof(cms_cli_msg_res_set_data_t),
        CMS_CLIENT_REQUEST_TIMEOUT);
    if (ret != GS_SUCCESS) {
        GS_LOG_RUN_ERR("cms socket send msg failed, ret %d", ret);
        CM_FREE_PTR(req);
    ...
    if (res.result != GS_SUCCESS) {
        GS_LOG_RUN_ERR("set data failed, res info %s", res.info);
        return GS_ERROR;
    }
    ...
    uint32 *size, uint64 *version)
{
    ...
    if (res->result != GS_SUCCESS) {
        GS_LOG_RUN_ERR("get resource data failed");
        return GS_ERROR;
    }

    err = memcpy_s(data, max_size, res->data, res->data_size);
    if (err != EOK) {
        GS_LOG_RUN_ERR("memcpy_s failed, err %d, errno %d[%s]",
...
status_t cms_get_res_data_new(uint32 slot_id, char* data, uint32 max_size, uint32* size, uint64* new_version)
{
    ...
    cms_cli_msg_res_get_data_t* res = malloc(sizeof(cms_cli_msg_res_get_data_t));
    if (res == NULL) {
        GS_LOG_RUN_ERR("malloc failed, size %u", (uint32)sizeof(cms_cli_msg_res_get_data_t));
        return GS_ERROR;
    }
    ret = memset_s(res, sizeof(cms_cli_msg_res_get_data_t), 0, sizeof(cms_cli_msg_res_get_data_t));
    if (ret != EOK) {
        GS_LOG_RUN_ERR("memset_s failed, ret %d", ret);
        CM_FREE_PTR(res);
        return GS_ERROR;
    }
    ...
    if (ret != GS_SUCCESS) {
        GS_LOG_RUN_ERR("cms handle get data res failed, ret %d", ret);
        CM_FREE_PTR(res);
        return ret;
    }
    CM_FREE_PTR(res);
...
{
    return cms_set_res_data_new(slot_id, data, size, GS_INVALID_ID64);
}
