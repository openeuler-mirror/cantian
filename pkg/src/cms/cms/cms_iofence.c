...
status_t cms_iofence_kick(const char* res_name, uint32 node_id)
{
    ...
    if (ret != GS_SUCCESS) {
        CMS_LOG_ERR("get res id by name failed, res_name %s", res_name);
        return ret;
    }
...
status_t cms_send_msg_kick_node(cms_msg_req_iof_kick_t *req, cms_msg_res_iof_kick_t *res, iofence_type_t iofence_type)
{
    ...
    if (res_is_available(master_node, iofence_type)) {
        if (cms_mes_send_cmd_to_other((cms_packet_head_t*)req, (cms_packet_head_t*)res, master_node) == GS_SUCCESS) {
    ...
    for (uint32 i = 0; i < node_count; i++) {
        if (i != master_node && res_is_available(i, iofence_type)) {
            if (cms_mes_send_cmd_to_other((cms_packet_head_t*)req, (cms_packet_head_t*)res, i) == GS_SUCCESS) {
...
status_t cms_kick_node_by_ns(const char* name, uint32 node_id, iofence_type_t iofence_type)
{
    ...
    iof.termid = 0;
    status_t ret;
    CMS_SYNC_POINT_GLOBAL_START(CMS_IOFENCE_KICK_NODE_FAIL, &ret, GS_ERROR);
    ret = cm_dbs_iof_kick_by_ns(&iof);
    CMS_SYNC_POINT_GLOBAL_END;
    if (ret != GS_SUCCESS) {
        GS_LOG_RUN_WAR("dbstore iof failed, node_id : %u", node_id);
    }
    return ret;
}
...
status_t cms_kick_node(const char* name, uint32 node_id, iofence_type_t iofence_type)
{
    ...
    if (ret != GS_SUCCESS) {
        GS_LOG_RUN_ERR("send kick node msg failed, name %s, node_id %u", name, node_id);
        return GS_ERROR;
    }

    if (res.result != GS_SUCCESS) {
        GS_LOG_RUN_ERR("kick node msg exec failed, result info %s", res.info);
        return GS_ERROR;
    }
