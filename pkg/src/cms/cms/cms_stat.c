...
#define CMS_STAT_IS_REGISTER(stat) ((stat).session_id != 0 && (stat).session_id != GS_INVALID_ID64)
...
static bool32 res_is_active(cms_disk_lock_t* lock, uint64 inst_id)
{
    ...
    if (cms_get_res_by_id((uint32)lock->int64_param1, &res) != GS_SUCCESS) {
        CMS_LOG_DEBUG_ERR("get res failed, res_id=%lld", lock->int64_param1);
        return GS_TRUE;
    }

    cms_res_stat_t stat;
    if (get_res_stat((uint32)inst_id, (uint32)lock->int64_param1, &stat) != GS_SUCCESS) {
...
{
    if (stat_read(CMS_NODE_STAT_POS(node_id), node_stat, sizeof(cms_node_stat_t)) != GS_SUCCESS) {
...
status_t cms_is_all_restart(bool32 *all_restart)
{
    ...
    for (uint32 node_id = 0; node_id < node_count; node_id++) {
        ...
        GS_RETURN_IFERR(cms_get_node_stat(node_id, (char *)node_stat));
...
{
    if (cms_disk_try_lock(&g_cms_inst->master_lock, DISK_LOCK_WRITE) != GS_SUCCESS) {
...
{
    bool32 all_restart = GS_FALSE;
    bool32 vote_done = GS_FALSE;
    cms_node_stat_t* node_stat = CMS_CUR_NODE_STAT;
    node_stat->vote_info_status = CMS_INITING_VOTE_INFO;

    if (cms_disk_lock(&g_cms_inst->vote_info_lock, 0, DISK_LOCK_WRITE) != GS_SUCCESS) {
        CMS_LOG_ERR("cms_disk_lock failed.");
        return GS_ERROR;
    }
    
    for (;;) {
        GS_RETURN_IFERR(cms_is_all_restart(&all_restart));
        if (all_restart) {
            if (cms_try_be_new_master()) {
                CMS_LOG_INF("cms is full restart, cms master begin to init vote file");
                GS_RETURN_IFERR(cms_init_cluster_vote_info());
        ...
        GS_RETURN_IFERR(cms_is_vote_done(&vote_done));
        if (vote_done) {
...
status_t cms_vote_disk_init(void)
{
    ...
    GS_RETURN_IFERR(cms_disk_lock_init(g_cms_param->gcc_type, g_cms_param->gcc_home, CMS_VOTE_RES_LOCK_POS,
        g_cms_param->node_id, &g_cms_inst->vote_result_lock, NULL, 0));
    // init vote info lock
    GS_RETURN_IFERR(cms_disk_lock_init(g_cms_param->gcc_type, g_cms_param->gcc_home, CMS_VOTE_INFO_LOCK_POS,
        g_cms_param->node_id, &g_cms_inst->vote_info_lock, NULL, 0));
    // init vote_lock
    for (int node_id = 0; node_id < CMS_MAX_NODE_COUNT; node_id++) {
        for (int slot_id = 0; slot_id < CMS_MAX_VOTE_SLOT_COUNT; slot_id++) {
            GS_RETURN_IFERR(cms_disk_lock_init(g_cms_param->gcc_type, g_cms_param->gcc_home,
                CMS_VOTE_DATA_LOCK_POS(node_id, slot_id), g_cms_param->node_id,
                &g_cms_inst->vote_data_lock[node_id][slot_id], NULL, 0));
    ...
    if (g_cms_param->gcc_type == CMS_DEV_TYPE_FILE) {
        ...
        if (cm_open_file(file_name, O_CREAT | O_RDWR | O_BINARY | O_CLOEXEC | O_SYNC | O_DIRECT,
            &g_cms_inst->vote_file_fd) != GS_SUCCESS) {
            return GS_ERROR;
        }
    } else if (g_cms_param->gcc_type == CMS_DEV_TYPE_SD) {
        GS_RETURN_IFERR(cm_open_disk(g_cms_param->gcc_home, &g_cms_inst->vote_file_fd));
    } else {
        CMS_LOG_ERR("invalid device type:%d", g_cms_param->gcc_type);
        return GS_ERROR;
    }

    GS_RETURN_IFERR(cms_init_vote_info());
    return GS_SUCCESS;
}
...
status_t cms_init_stat(void)
{
    ...
    GS_RETURN_IFERR(cms_disk_lock_init(g_cms_param->gcc_type, g_cms_param->gcc_home, CMS_MASTER_LOCK_POS,
                                       g_cms_param->node_id, &g_cms_inst->master_lock, NULL, 0));
    GS_RETURN_IFERR(cms_disk_lock_init(g_cms_param->gcc_type, g_cms_param->gcc_home, CMS_STAT_LOCK_POS,
                                       g_cms_param->node_id, &g_cms_inst->stat_lock, NULL, CMS_DLOCK_THREAD));
    cms_disk_unlock(&g_cms_inst->stat_lock);
    GS_RETURN_IFERR(cms_disk_lock_init(g_cms_param->gcc_type, g_cms_param->gcc_home, CMS_RES_START_LOCK_POS,
                                       g_cms_param->node_id, &g_cms_inst->res_start_lock, NULL, CMS_DLOCK_THREAD));
    ...
    for (int32 node_id = 0; node_id < CMS_MAX_NODE_COUNT; node_id++) {
        for (int32 res_id = 0; res_id < CMS_RES_STAT_MAX_RESOURCE_COUNT; res_id++) {
            GS_RETURN_IFERR(cms_disk_lock_init(g_cms_param->gcc_type, g_cms_param->gcc_home,
                CMS_RES_STAT_LOCK_POS(node_id, res_id), g_cms_param->node_id,
                &g_cms_inst->res_stat_lock[node_id][res_id], NULL, 0));
    ...
    for (int32 res_id = 0; res_id < CMS_MAX_RESOURCE_COUNT; res_id++) {
        ...
        GS_RETURN_IFERR(stat_read(CMS_RES_STAT_POS(g_cms_param->node_id, res_id), (char *)CMS_CUR_RES_STAT(res_id),
                                  sizeof(cms_res_stat_t)));
        CMS_CUR_RES_STAT(res_id)->checking = 0;
        for (int32 slot_id = 0; slot_id < CMS_MAX_RES_SLOT_COUNT; slot_id++) {
            GS_RETURN_IFERR(cms_disk_lock_init(g_cms_param->gcc_type, g_cms_param->gcc_home,
                                               CMS_RES_DATA_LOCK_POS(res_id, slot_id), g_cms_param->node_id,
                                               &g_cms_inst->res_data_lock[res_id][slot_id], res_is_active, 0));
...
{
    if (cms_disk_lock(&g_cms_inst->stat_lock, STAT_LOCK_WAIT_TIMEOUT, lock_type) != GS_SUCCESS) {
        CMS_LOG_ERR("cms_lock_stat timeout.");
        return GS_ERROR;
    }
    return GS_SUCCESS;
}
...
{
    status_t ret;
    GS_RETURN_IFERR(cms_lock_stat(DISK_LOCK_WRITE));
...
static bool32 cms_res_stat_steady(cms_res_status_list_t *res_stat_list, cms_res_t *res)
{
    ...
    if (is_cur_res_offline(&res_stat_list->inst_list[g_cms_param->node_id]) == GS_FALSE) {
        return GS_FALSE;
    }
...
static status_t cms_check_cluster_reform_stat(uint32 res_id, bool32 *reform_done)
{
    ...
    if (cms_stat_get_res_data(res.type, RC_CMS_REMOTE_CURRENT, (char *)&cur_res_stat, sizeof(cms_res_status_list_t),
        &data_size, &data_version) != GS_SUCCESS) {
...
status_t cms_get_cluster_res_list(uint32 res_id, cms_res_status_list_t *stat)
{
    ...
    for (uint32 node_id = 0; node_id < node_count; node_id++) {
        ...
        if (cms_get_node_by_id(node_id, &node_def) != GS_SUCCESS) {
            continue;
        }
...
static bool32 res_is_full_restart(uint32 res_id)
{
    ...
    for (;;) {
        if (cms_get_cluster_res_list(res_id, &stat) != GS_SUCCESS) {
...
status_t wait_for_cluster_reform_done(uint32 res_id)
{
    ...
    for (;;) {
        ...
        if (cms_check_cluster_reform_stat(res_id, &reform_done) != GS_SUCCESS) {
            CMS_LOG_ERR("cms check cluster reform stat failed, res_id %u", res_id);
            return GS_ERROR;
        }

        if (reform_done == GS_FALSE) {
...
{
    uint64 inst_id = GS_INVALID_ID64;
    uint16 node_id = GS_INVALID_ID16;
    if (cms_disk_lock_get_inst(lock, &inst_id) != GS_SUCCESS) {
        CMS_LOG_ERR("cms get lock inst_id failed");
        return GS_ERROR;
    }
    if (inst_id == GS_INVALID_ID64) {
        node_id = GS_INVALID_ID16;
...
static status_t wait_for_node_joined(uint32 res_id, uint32 timeout_ms)
{
    ...
    for (;;) {
        ...
        if (res_stat.cur_stat == CMS_RES_ONLINE && res_stat.work_stat == RC_JOINED) {
            if (cms_clear_restart_count(res_id) != GS_SUCCESS) {
...
{
    if (cms_res_started(res_id) != GS_SUCCESS) {
...
status_t cms_res_start(uint32 res_id, uint32 timeout_ms)
{
    ...
    if (cms_get_res_start_lock(res_id) != GS_SUCCESS) {
    ...
    if (wait_for_cluster_reform_done(res_id) != GS_SUCCESS) {
        CMS_LOG_ERR("cms wait cluster reform done failed");
        cms_release_res_start_lock(res_id);
        return GS_ERROR;
    }
    CMS_LOG_INF("cms cluster reform done");
    if (cms_check_res_running(res_id) != GS_SUCCESS) {
        cms_release_res_start_lock(res_id);
        return GS_ERROR;
    }
...
status_t cms_res_check(uint32 res_id, bool32 *res_running)
{
    ...
    if (cm_atomic32_inc(&res_stat->checking) != 1) {
        ...
        CMS_LOG_WAR("resource is being checked, res_id=%u", res_id);
        return GS_SUCCESS;
    }
...
status_t update_res_target_stat(uint32 res_id, cms_stat_t target_stat)
{
    ...
    if (CMS_SYNC_CUR_RES_STAT(res_id, res_stat) != GS_SUCCESS) {
...
status_t get_res_stat(uint32 node_id, uint32 res_id, cms_res_stat_t* res_stat)
{
    ...
    if (!g_cms_inst->is_server || node_id != g_cms_param->node_id) {
        GS_RETURN_IFERR(stat_read(CMS_RES_STAT_POS(node_id, res_id), (char *)CMS_RES_STAT(node_id, res_id),
                                  sizeof(cms_res_stat_t)));
        errno_t err = memcpy_s(res_stat, sizeof(cms_res_stat_t), CMS_RES_STAT(node_id, res_id),
            sizeof(cms_res_stat_t));
...
void cms_stat_set(cms_res_stat_t* res_stat, cms_stat_t new_stat, bool32* isChanged)
{
    ...
    if (res_stat->cur_stat != new_stat) {
        ...
        if (new_stat == CMS_RES_OFFLINE) {
            res_stat->inst_id = GS_INVALID_ID64;
            res_stat->session_id = GS_INVALID_ID64;
    ...
    if (!full_restart) {
        ...
        for (;;) {
            if (cms_stat_get_res_data(res_type, RC_REFORM_TRIGGER_VERSION, (char *)&res_info->trigger_version,
                sizeof(uint64), &data_size, &data_version) != GS_SUCCESS) {
                cm_sleep(RETRY_SLEEP_TIME);
                continue;
            }
            break;
        ...
        for (;;) {
            if (cms_stat_get_res_data(res_type, RC_CMS_REMOTE_CURRENT, (char *)&res_info->res_stat,
                sizeof(cms_res_status_list_t), &data_size, &data_version) != GS_SUCCESS) {
                cm_sleep(RETRY_SLEEP_TIME);
                continue;
            }
            break;
...
status_t cms_res_connect(socket_t sock, cms_cli_msg_req_conn_t *req, cms_cli_msg_res_conn_t *res)
{
    ...
    // find resource
    if (cms_get_res_id_by_type(req->res_type, &res_id) != GS_SUCCESS) {
    ...
    if (req->is_retry_conn) {
        ...
        CMS_SYNC_POINT_GLOBAL_END;
        if (cms_res_register_conn(sock, req, res_id, res_stat, &version) != GS_SUCCESS) {
...
status_t cms_stat_get_uds(uint64 session_id, socket_t *uds_sock)
{
    ...
    cm_thread_lock(&g_res_session[session_id].lock);
    *uds_sock = g_res_session[session_id].uds_sock;
    cm_thread_unlock(&g_res_session[session_id].lock);
    return GS_SUCCESS;
}
...
status_t cms_res_set_workstat(const char* res_type, uint32 inst_id, uint8 work_stat)
{
    ...
    uint32 res_id;
    // find resource
    GS_RETURN_IFERR(cms_get_res_id_by_type(res_type, &res_id));
    ...
    CMS_LOG_INF("resource setworkstat, state changed, version=%lld, res_type=%s, session_id=%lld, inst_id=%lld, "
        "cur_stat=%s, work_stat=%d, pre_stat=%s, target_stat=%s", version, res_stat->res_type, res_stat->session_id,
        res_stat->inst_id, cms_stat_str(res_stat->cur_stat), (int32)res_stat->work_stat,
        cms_stat_str(res_stat->pre_stat), cms_stat_str(res_stat->target_stat));
...
status_t cms_get_cluster_stat_bytype(const char* res_type, uint64 version, cms_res_status_list_t* stat_list)
{
    status_t ret = GS_SUCCESS;
    uint32 res_id = -1;
    CMS_LOG_DEBUG_INF("begin get cluster stat by type, res type %s, version %llu", res_type, version);
    ret = cms_get_res_id_by_type(res_type, &res_id);
    if (ret != GS_SUCCESS) {
        CMS_LOG_ERR("get res id by type failed, ret %d, res type %s", ret, res_type);
        return GS_ERROR;
    }
    ...
    CMS_LOG_DEBUG_INF("get cluster stat by type succ, res type %s, stat version %llu, reformer %u", res_type,
    ...
    uint32 data_size, uint64 old_version)
{
    ...
    if (cm_read_disk(ctx->gcc_handle, offset, (char *)res_data, CMS_BLOCK_SIZE) != GS_SUCCESS) {
        CM_FREE_PTR(res_data);
        return GS_ERROR;
    }

    if (res_data->magic == CMS_RES_DATA_MAGIC) {
        if (old_version != GS_INVALID_ID64 && res_data->version != old_version) {
    ...
    if (cm_write_disk(ctx->gcc_handle, offset, res_data, align_size) != GS_SUCCESS) {
        CM_FREE_PTR(res_data);
        return GS_ERROR;
    }
...
status_t cms_stat_set_res_data(const char* res_type, uint32 slot_id, char* data, uint32 data_size, uint64 old_version)
{
    ...
    if (cms_stat_set_res_data_inner(res_id, slot_id, data, data_size, old_version) != GS_SUCCESS) {
        cms_disk_unlock(&g_cms_inst->res_data_lock[res_id][slot_id]);
        return GS_ERROR;
    }
    ...
    CMS_LOG_DEBUG_INF("resource set data succeed, notify to all instances, res_type=%s, slot=%u, old_version=%lld",
                      res_type, slot_id, old_version);
    ...
    uint32* data_size, uint64* data_version)
{
    ...
    if (total_size > CMS_BLOCK_SIZE) {
        if (cm_read_disk(ctx->gcc_handle, offset + CMS_BLOCK_SIZE, (char *)res_data + CMS_BLOCK_SIZE,
                         total_size - CMS_BLOCK_SIZE) != GS_SUCCESS) {
            CM_FREE_PTR(res_data);
            return GS_ERROR;
        }
    }
    ...
    uint32* data_size, uint64* data_version)
{
    ...
    if (cms_disk_lock(&g_cms_inst->res_data_lock[res_id][slot_id], STAT_LOCK_WAIT_TIMEOUT,
        DISK_LOCK_READ) != GS_SUCCESS) {
        CMS_LOG_ERR("cms_disk_lock timeout");

        return GS_ERROR;
    }

    if (cms_stat_get_res_data_inner(res_id, slot_id, data, max_size, data_size, data_version) != GS_SUCCESS) {
        cms_disk_unlock(&g_cms_inst->res_data_lock[res_id][slot_id]);
        return GS_ERROR;
    }
...
{
    cms_res_stat_t res_stat;
    bool32 do_elect = GS_FALSE;

    if (reformer == GS_INVALID_ID8) {
        do_elect = GS_TRUE;
        ...
        if (get_res_stat(reformer, res_id, &res_stat) != GS_SUCCESS) {
    ...
    *new_reformer = GS_INVALID_ID8;
    for (uint32 node_id = 0; node_id < CMS_MAX_NODE_COUNT; node_id++) {
        ...
        if (res_stat.cur_stat == CMS_RES_ONLINE && res_stat.work_stat == 0) {
            if (*new_reformer == GS_INVALID_ID8) {
                *new_reformer = node_id;
...
status_t cms_try_be_master(void)
{
    ...
    GS_RETURN_IFERR(cms_disk_lock_get_data(&g_cms_inst->master_lock, (char *)&reformers, sizeof(cms_res_reformer_t)));
    ...
    for (uint32 res_id = 0; res_id < CMS_MAX_RESOURCE_COUNT; res_id++) {
        ...
        if (reformers.magic == CMS_REFORMER_MAGIC) {
            ...
            (void)cms_elect_res_reformer(res_id, GS_INVALID_ID8, &reformers.reformer[res_id]);
        }
    }

    reformers.magic = CMS_REFORMER_MAGIC;

    GS_RETURN_IFERR(cms_disk_lock_set_data(&g_cms_inst->master_lock, (char *)&reformers, sizeof(cms_res_reformer_t)));

    uint64 inst_id = GS_INVALID_ID64;
    if (cms_disk_lock_get_inst(&g_cms_inst->master_lock, &inst_id) != GS_SUCCESS) {
...
{
    GS_RETURN_IFERR(cms_disk_lock_init(g_cms_param->gcc_type, g_cms_param->gcc_home, CMS_MASTER_LOCK_POS,
        g_cms_param->node_id, master_lock, NULL, 0));
    uint64 inst_id = GS_INVALID_ID64;
    if (cms_disk_lock_get_inst(master_lock, &inst_id) != GS_SUCCESS) {
...
{
    uint64 inst_id = GS_INVALID_ID64;
    GS_RETURN_IFERR(cms_disk_lock_get_inst(&g_cms_inst->master_lock, &inst_id));
    if (inst_id == GS_INVALID_ID64) {
        *node_id = GS_INVALID_ID16;
    } else {
        *node_id = (uint16)inst_id;
...
{
    uint16 node_id;
    if (cms_get_master_node(&node_id) != GS_SUCCESS) {
...
status_t cms_aync_update_res_hb(cms_res_stat_t *res_stat_disk, uint32 res_id)
{
    ...
    if (cms_disk_lock(&g_cms_inst->res_stat_lock[g_cms_param->node_id][res_id], DISK_LOCK_WAIT_TIMEOUT,
        DISK_LOCK_WRITE) != GS_SUCCESS) {
        CMS_LOG_ERR("cms_disk_lock timeout, node_id = %u, res_id = %u.", g_cms_param->node_id, res_id);
        return GS_ERROR;
    }
    if (stat_read(CMS_CUR_RES_STAT_POS(res_id), (char *)res_stat_disk, sizeof(cms_res_stat_t)) != GS_SUCCESS) {
    ...
    if (stat_write(CMS_CUR_RES_STAT_POS(res_id), (char *)res_stat_disk, sizeof(cms_res_stat_t)) != GS_SUCCESS) {
...
void cms_stat_aync_write_entry(thread_t * thread)
{
    ...
    while (!thread->closed) {
        ...
        if (cms_aync_update_res_hb(res_stat_disk, res_id) != GS_SUCCESS) {
...
status_t cms_stat_read_from_disk(uint32 node_id, uint32 res_id, cms_res_stat_t **resRef)
{
    ...
    if (stat_read(CMS_RES_STAT_POS(node_id, res_id), (char*)(stat), sizeof(cms_res_stat_t)) != GS_SUCCESS) {
...
{
    if (stat_write(CMS_RES_STAT_POS(node_id, res_id), (char*)(stat), sizeof(cms_res_stat_t)) != GS_SUCCESS) {
...
status_t cms_update_disk_hb(void)
{
    ...
    if (stat_write(CMS_NODE_STAT_POS(g_cms_param->node_id), (char *)node_stat, sizeof(cms_node_stat_t)) != GS_SUCCESS) {
...
{
    uint32 node_count = cms_get_gcc_node_count();
    bool32 cms_online = GS_FALSE;

    for (uint32 node_id = 0; node_id < node_count; node_id++) {
        if (cms_node_is_invalid(node_id)) {
            continue;
        }
        GS_RETURN_IFERR(cms_server_stat(node_id, &cms_online));
        if (cms_online) {
