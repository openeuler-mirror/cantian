 ...
 *
 * IDENTIFICATION
 * src/ctsql/function/func_others.h
 *
 * -------------------------------------------------------------------------
 */
...
#define ADVLCK_INIT_WITH_NAME_TIMEOUT(func, timeout, result)                                    \
    do {                                                                                        \
        ...
        CT_RETURN_IFERR(sql_func_alck_get_name(stmt, (func)->argument, &name));                   \
        SQL_CHECK_COLUMN_VAR(&name, (result));                                                  \
        if ((func)->argument->next) {                                                             \
            CT_RETURN_IFERR(sql_func_alck_get_timeout(stmt, (func)->argument->next, &(timeout))); \
