...
static inline status_t sql_check_pre_exec(sql_stmt_t *stmt)
{
    ...
    if ((stmt->context->type == CTSQL_TYPE_INSERT || stmt->context->type == CTSQL_TYPE_DELETE ||
        stmt->context->type == CTSQL_TYPE_UPDATE || stmt->context->type == CTSQL_TYPE_CREATE_PROC) &&
