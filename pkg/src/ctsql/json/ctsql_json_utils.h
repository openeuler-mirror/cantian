 ...
 *
 * IDENTIFICATION
 * src/ctsql/json/ctsql_json_utils.h
 *
 * -------------------------------------------------------------------------
 */
...
static inline status_t json_malloc_dyna(uint64 size, char **ptr)
{
    ...
    if (size > JSON_MAX_SIZE - json_mpool->used_json_dyn_buf) {
        ...
        CT_THROW_ERROR(ERR_ALLOC_MEMORY, size, "exceed json dyna memory area");
        return CT_ERROR;
    }
    ...
    *ptr = (char *)malloc((size_t)size);
    if (*ptr == NULL) {
        ...
        json_mpool->used_json_dyn_buf -= size;
        cm_spin_unlock(&(json_mpool->lock));
        CT_THROW_ERROR(ERR_ALLOC_MEMORY, size, "exceed json dynamic memory area");
        return CT_ERROR;
    }
    return CT_SUCCESS;
}
...
static inline status_t JSON_ALLOC_DYNA(json_assist_t *json_ass, uint32 size, void **ptr)
{
    ...
    if (jda->count >= JSON_MAX_MEM_AREA_SIZE) {
        CT_THROW_ERROR(ERR_ALLOC_MEMORY, (uint64)size, "exceed json dyna memory area");
        return CT_ERROR;
    }

    CT_RETURN_IFERR(json_malloc_dyna(size, (char **)ptr));
...
static inline status_t JSON_ALLOC_LARGE(json_large_area_t *jla, uint32 size, void **ptr)
{
    ...
    CM_ASSERT(size > 0 && size <= CT_LARGE_PAGE_SIZE);
    if (size <= jla->free_bytes) {
        char *large_page_addr = mpool_page_addr(&g_instance->sga.large_pool, jla->page_id[jla->count - 1]);
        *ptr = large_page_addr + CT_LARGE_PAGE_SIZE - jla->free_bytes;
        jla->free_bytes -= size;
        return CT_SUCCESS;
    }

    if (jla->count >= JSON_MAX_MEM_AREA_SIZE) {
        CT_THROW_ERROR(ERR_ALLOC_MEMORY, size, "exceed json large area");
        return CT_ERROR;
    }
    CT_RETURN_IFERR(mpool_alloc_page_wait(&g_instance->sga.large_pool, &large_page_id, CM_MPOOL_ALLOC_WAIT_TIME));
    *ptr = mpool_page_addr(&g_instance->sga.large_pool, large_page_id);
    jla->page_id[jla->count] = large_page_id;
    jla->count++;
    jla->free_bytes = CT_LARGE_PAGE_SIZE - size;
...
#define JSON_RETURN_ERROR_IF_STACK_OVERFLOW(json_ass, level)                                                    \
    do {                                                                                                  \
        if (sql_stack_safe((json_ass)->stmt) == CT_ERROR) {                                                     \
            (json_ass)->is_overflow = CT_TRUE;                                                                  \
            cm_reset_error();                                                                             \
            CT_THROW_ERROR_EX(ERR_JSON_INSUFFICIENT_MEMORY, "stack overflow(json level is %u)", (level)); \
            CT_LOG_DEBUG_INF("[JSON] CT-%05d, %s", cm_error_info()->code, cm_error_info()->message);      \
            return CT_ERROR;                                                                              \
        }                                                                                                 \
    } while (0)
...
#define JSON_CHECK_MAX_SIZE(size)                                                                                  \
    do {                                                                                                           \
        if ((uint64)(size) > JSON_MAX_SIZE) {                                                                      \
            CT_THROW_ERROR_EX(ERR_JSON_INSUFFICIENT_MEMORY, "exceed json max allowed dynamic size(maximum: %llu)", \
                JSON_MAX_SIZE);                                                                                    \
            return CT_ERROR;                                                                                       \
        }                                                                                                          \
    } while (0)
...
#define JSON_EXTEND_LOB_VMEM_IF_NEEDED(ja_vlob, stmt)                                                                  \
    do {                                                                                                               \
        ...
        if (_ja_vlob->last_free_size == 0) {                                                                           \
            CT_RETURN_IFERR(sql_extend_lob_vmem((stmt), _execute_vm_list, &_ja_vlob->vlob));                           \
            CT_RETURN_IFERR(                                                                                           \
                vm_open((stmt)->session, (stmt)->mtrl.pool, _execute_vm_list->last, &_ja_vlob->last_page));            \
            _ja_vlob->last_free_size = CT_VMEM_PAGE_SIZE;                                                              \
        }                                                                                                              \
    } while (0)
...
status_t json_merge_patch(json_assist_t *json_ass, json_value_t *jv_target, json_value_t *jv_patch, json_value_t **jv_result);
