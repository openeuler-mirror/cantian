...
status_t dtc_sync_ddl_internal(knl_handle_t knl_session, char * logic_log_buf, uint32 logic_log_size)
{
    ...
    GS_LOG_DEBUG_INF("dtc sync ddl type=%d, op_type=%d, size=%u, logic_log_size %u can ignore=%d", log->type, *op_type,
    ...
    while (offset < logic_log_size) {
        ...
        GS_LOG_DEBUG_INF("dtc sync ddl log type=%d op_type=%d, size=%d , total size %dcan ignore=%d", tmplog->type,
    ...
    mes_init_send_head(&info.head, MES_CMD_DDL_BROADCAST, (uint16)(sizeof(msg_ddl_info_t) + logic_log_size),
        GS_INVALID_ID32, session->kernel->dtc_attr.inst_id, 0, session->id, GS_INVALID_ID16);
    ...
    status = mes_broadcast_bufflist_and_wait_with_retry(session->id, target_bits, &info.head, sizeof(msg_ddl_info_t),
        log, DTC_WAIT_MES_TIMEOUT, DTC_MAX_RETRY_TIEMS);
    return status;
}
...
void dtc_refresh_ddl(knl_session_t *session, log_entry_t *log)
{
    ...
    if (should_ignore) {
        GS_LOG_RUN_WAR("dtc refresh ddl, ignore redo log,  type=%d, op_type=%d, size=%d", log->type, *op_type, log->size);
        return;
    }

    GS_LOG_DEBUG_INF("dtc refresh ddl type=%d, op_type=%d, size=%d", log->type, *op_type, log->size);
                                   ...
                                   bool32 is_splitted)
{
    ...
    mes_init_send_head(&bcast.head, MES_CMD_BROADCAST_DATA, msg_size, GS_INVALID_ID32, session->kernel->id,
                       GS_INVALID_ID8, session->id, GS_INVALID_ID16);
    bcast.type = BTREE_SPLITTING;

    knl_try_begin_session_wait(session, BROADCAST_BTREE_SPLIT, GS_TRUE);
    ret = mes_broadcast_bufflist_and_wait_with_retry(session->id, MES_BROADCAST_ALL_INST, &bcast.head,
        ...
        knl_try_end_session_wait(session, BROADCAST_BTREE_SPLIT);
    if (ret != GS_SUCCESS) {
        GS_LOG_RUN_ERR(
    ...
    if (ret == GS_SUCCESS && btree->is_splitting) {
        SYNC_POINT_GLOBAL_START(CANTIAN_BTREE_SPLIT_AFTER_BCAST_SPLITTING_ABORT, NULL, 0);
        SYNC_POINT_GLOBAL_END;
    }
    btree->split_owner = btree->is_splitting ? session->kernel->id : GS_INVALID_ID8;
...
status_t dtc_process_btree_splitting(knl_session_t *session, char *data, uint8 src_inst)
{
    ...
    if (knl_try_open_dc_by_id(session, bcast->uid, bcast->table_id, &dc) != GS_SUCCESS) {
        cm_reset_error();
        knl_panic_log(0, "[DTC] failed to open dc user id %u, table id %u, index id %u", bcast->uid, bcast->table_id,
                      bcast->index_id);
        return GS_SUCCESS;
    }
    ...
    if (entity == NULL) {
        cm_reset_error();
        GS_LOG_DEBUG_INF("[DTC] broadcast btree entity is null, uid/table_id/index_id/part/subpart:[%d-%d-%d-%u-%u]",
    ...
    switch (bcast->split_status) {
        case BTREE_IS_SPLITTING:
            if (btree->is_splitting) {
                GS_LOG_RUN_ERR("[DTC] btree is splitting, split status:%d, split_owner:%u, wait ticks:%u, "
                                    ...
                                    bool8 *is_splitting)
{
    ...
    uint16 msg_size = sizeof(msg_broadcast_data_t) + sizeof(msg_broadcast_btree_data_t);
    mes_init_send_head(&bcast.head, MES_CMD_BROADCAST_DATA, msg_size, GS_INVALID_ID32, session->kernel->id,
                       btree->split_owner, session->id, GS_INVALID_ID16);
    bcast.type = BTREE_SPLIT_STATUS;

    mes_message_t msg;
    if (mes_send_data3(&bcast.head, sizeof(msg_broadcast_data_t), (void *)&btree_data) != GS_SUCCESS) {
        GS_LOG_RUN_ERR("[DTC] dtc_get_btree_split_status send message failed");
        return GS_ERROR;
    }

    if (mes_recv(session->id, &msg, GS_FALSE, GS_INVALID_ID32, DTC_GET_BTREE_SPLIT_STATUS_TIMEOUT) != GS_SUCCESS) {
        GS_LOG_RUN_ERR("[DTC] dtc_get_btree_split_status get result timeout");
        return GS_ERROR;
    }
    ...
    *is_splitting = split_ack->is_splitting;
    ...
    GS_LOG_DEBUG_INF("[DTC][get btree split status]: uid/table_id/part/subpart:[%d-%d-%u-%u], split_owner:%d, "
...
void dtc_process_btree_split_status(knl_session_t *session, mes_message_t *req_msg, char *data)
{
    ...
    if (entity == NULL) {
        cm_reset_error();
        GS_LOG_RUN_WAR("[DTC] broadcast btree entity is null, uid/table_id/index_id/part/subpart:[%d-%d-%d-%u-%u]",
                       bcast->uid, bcast->table_id, bcast->index_id, bcast->part_loc.part_no,
                       bcast->part_loc.subpart_no);
        msg.split_owner = GS_INVALID_ID8;
        msg.is_splitting = GS_FALSE;
    } else {
        btree_t *btree = dc_get_btree_by_id(session, entity, bcast->index_id, bcast->part_loc, bcast->is_shadow);
    ...
    if (mes_send_data(&msg) != GS_SUCCESS) {
        GS_LOG_RUN_ERR("[DTC DC] failed to send split btree status,");
        return;
    }
}
...
void dtc_broadcast_data_send_status_ack(knl_session_t *session, mes_message_t *msg)
{
    ...
    status_t ret = GS_SUCCESS;
    SYNC_POINT_GLOBAL_START(CANTIAN_DTC_BCAST_ACK_FAIL, &ret, GS_ERROR);
    ret = mes_send_data(&ack_head);
    SYNC_POINT_GLOBAL_END;
    if (ret != GS_SUCCESS) {
        GS_LOG_DEBUG_ERR("[DTC] failed to send broadcast ack msg: cmd=%u, dest_id=%u, dest_sid=%u",
...
status_t dtc_broadcast_heap_extend(knl_session_t *session, heap_t *heap, knl_part_locate_t part_loc)
{
    ...
    heap->extend_owner = heap->extending ? session->kernel->id : GS_INVALID_ID8;
    GS_LOG_RUN_RET_INFO(
        ret,
...
status_t dtc_process_heap_extend(knl_session_t *session, char *data, uint8 src_inst)
{
    ...
    if (knl_try_open_dc_by_id(session, bcast->uid, bcast->table_id, &dc) != GS_SUCCESS) {
        cm_reset_error();
        GS_LOG_RUN_ERR("[DTC] process heap extend, failed to open dc user id %u, table id %u",
    ...
    if (entity == NULL) {
        cm_reset_error();
        GS_LOG_DEBUG_INF("[DTC] broadcast heap extend entity is null, uid/table_id/part/subpart:[%d-%d-%u-%u]",
            bcast->uid, bcast->table_id, bcast->part_loc.part_no, bcast->part_loc.subpart_no);
        return GS_SUCCESS;
    }

    heap = dc_get_heap(session, bcast->uid, bcast->table_id, bcast->part_loc, &dc);
    if (bcast->extending == GS_TRUE && heap->extending) {
        GS_LOG_RUN_ERR(
    ...
    heap->extend_owner = heap->extending ? src_inst : GS_INVALID_ID8;
...
status_t dtc_get_heap_extend_status(knl_session_t *session, heap_t *heap, knl_part_locate_t part_loc, bool8 *extending)
{
    ...
    uint16 msg_size = sizeof(msg_broadcast_data_t) + sizeof(msg_broadcast_heap_data_t);
    mes_init_send_head(&bcast.head, MES_CMD_BROADCAST_DATA, msg_size, GS_INVALID_ID32, session->kernel->id,
                       heap->extend_owner, session->id, GS_INVALID_ID16);
    bcast.type = HEAP_EXTEND_STATUS;

    mes_message_t  msg;

    if (mes_send_data3(&bcast.head, sizeof(msg_broadcast_data_t), (void *)&heap_data) != GS_SUCCESS) {
        GS_LOG_RUN_ERR("[DTC] dtc_get_heap_extend_status failed");
        return GS_ERROR;
    }

    if (mes_recv(session->id, &msg, GS_FALSE, GS_INVALID_ID32, DTC_GET_HEAP_EXTEND_STATUS_TIMEOUT) != GS_SUCCESS) {
        GS_LOG_RUN_ERR("[DTC] dtc_get_heap_extend_status get result timeout");
        return GS_ERROR;
    }
    ...
    *extending = extend_ack->is_extending;
    ...
    GS_LOG_DEBUG_INF("[DTC][get heap extend status]: uid/table_id/part/subpart:[%d-%d-%u-%u], "
...
void dtc_process_heap_extend_status(knl_session_t *session, mes_message_t *req_msg, char *data)
{
    ...
    if (knl_try_open_dc_by_id(session, bcast->uid, bcast->table_id, &dc) != GS_SUCCESS) {
        cm_reset_error();
        knl_panic_log(0, "[DTC] process heap extend, failed to open dc user id %u, table id %u",
            bcast->uid, bcast->table_id);
        mes_release_message_buf(req_msg->buffer);
        return;
    }
    ...
    if (entity == NULL) {
        cm_reset_error();
        GS_LOG_RUN_WAR("[DTC] broadcast heap extend entity is null, uid/table_id/part/subpart:[%d-%d-%u-%u]",
                       bcast->uid, bcast->table_id, bcast->part_loc.part_no, bcast->part_loc.subpart_no);
        msg.is_extending = GS_FALSE;
        msg.extend_owner = GS_INVALID_ID8;
    } else {
        heap_t *heap = dc_get_heap(session, bcast->uid, bcast->table_id, bcast->part_loc, &dc);
    ...
    mes_release_message_buf(req_msg->buffer);
    if (mes_send_data(&msg) != GS_SUCCESS) {
        GS_LOG_RUN_ERR("[DTC DC] failed to send heap extend status,");
        return;
    }
}
...
void dtc_broadcast_user_status(knl_session_t *session, uint32 uid, user_status_t status)
{
    ...
    if (status == USER_STATUS_LOCKED) {
        ...
        user_data.user_locked_owner = GS_INVALID_ID32;
    }

    mes_init_send_head(&bcast.head, MES_CMD_BROADCAST_USER,
                       sizeof(msg_broadcast_data_t) + sizeof(msg_broadcast_user_data_t), GS_INVALID_ID32,
...
void dtc_process_user_status(knl_session_t * session, char *data)
{
    ...
    if (dtc_modify_drop_uid(session, user_data->uid) != GS_SUCCESS ||
        dc_open_user_by_id(session, user_data->uid, &user) != GS_SUCCESS) {
        cm_reset_error();
        knl_panic_log(0, "[DDL] failed to open user id %u", user_data->uid);
        return;
    }
...
void dtc_broadcast_invalidate_dc(knl_session_t * session, uint32 uid, uint32 oid)
{
    ...
    dc_info.uid = uid;
    dc_info.oid = oid;
    mes_init_send_head(&bcast.head, MES_CMD_BROADCAST_DATA,
                       sizeof(msg_broadcast_data_t) + sizeof(msg_broadcast_invalidate_dc_t), GS_INVALID_ID32,
                       session->kernel->id, GS_INVALID_ID8, session->id, GS_INVALID_ID16);
...
void dtc_process_invalidate_dc(knl_session_t* session, char* data)
{
    ...
    if (dc_open_table_directly(session, dc_info->uid, dc_info->oid, &dc) != GS_SUCCESS) {
        GS_LOG_RUN_ERR("[DC]invalidate dc failed, uid:%u, oid:%u", dc_info->uid, dc_info->oid);
        return;
    }
    GS_LOG_DEBUG_INF("invalidate dc: uid: %u, tid: %u, valid: %u", dc_info->uid, dc_info->oid, DC_ENTITY(&dc)->valid);
...
void dtc_process_broadcast_data(void *sess, mes_message_t * msg)
{
    ...
    if (!DC_IS_READY(session)) {
        GS_LOG_RUN_INF("[DTC] process broadcast data, dc not ready, status=%d", DB_STATUS(session));
...
status_t dtc_ddl_enabled(knl_handle_t knl_session, bool32 forbid_in_rollback)
{
    ...
    if (RC_REFORM_IN_PROGRESS) {
        GS_LOG_RUN_WAR("reform is preparing, refuse to ddl operation");
        GS_THROW_ERROR(ERR_CLUSTER_DDL_DISABLED, "reform is preparing");
        return GS_ERROR;
    }
    GS_LOG_DEBUG_INF("dtc check ddl enabled");
    mes_message_head_t head;
    mes_init_send_head(&head, MES_CMD_CHECK_DDL_ENABLED, sizeof(mes_message_head_t) + sizeof(bool32), GS_INVALID_ID32,
        session->kernel->dtc_attr.inst_id, 0, session->id, GS_INVALID_ID16);
    uint64 target_bits = 0;
    status_t status = dtc_get_alive_bitmap(&target_bits);
    if (status != GS_SUCCESS) {
        return status;
    ...
    if (status != GS_SUCCESS) {
        GS_LOG_RUN_ERR("[DDL] recv check ddl enabled failed from instance");
    }
    return status;
}
...
void dtc_process_check_ddl_enabled(void *sess, mes_message_t *msg)
{
    ...
    mes_release_message_buf(msg->buffer);
    if (mes_send_data2(&ack_head, &ddl_status) != GS_SUCCESS) {
        GS_LOG_RUN_ERR("[DDL] send check ddl enabled ack failed");
        return;
    }
}
...
status_t db_write_ddl_op_internal(knl_session_t* session, char *log, uint32 log_size)
{
    ...
    if (knl_row_put_lob(session, cursor, lob_column, &log_bin, &ra) != GS_SUCCESS) {
        CM_RESTORE_STACK(session->stack);
        return GS_ERROR;
    }
...
status_t db_write_ddl_op_for_parents(knl_session_t* session, table_t* table)
{
    ...
    for (i = 0; i < table->cons_set.ref_count; i++) {
        ...
        if (db_write_ddl_op_internal(session, buf, ((log_entry_t*)buf)->size) != GS_SUCCESS) {
            CM_RESTORE_STACK(session->stack);
            GS_LOG_DEBUG_ERR("[DDL]failed to write cluster ddl op for parent table (%d:%d).", ref->ref_uid, ref->ref_oid);
            return GS_ERROR;
        }
        CM_RESTORE_STACK(session->stack);
    }
...
status_t db_write_ddl_op_for_constraints(knl_session_t* session, uint32 uid, uint32 id, galist_t* constraints)
{
    ...
    for (i = 0; i < constraints->count; i++) {
        ...
        if (dc->handle != NULL) {
            ...
            if (db_write_ddl_op_internal(session, buf, ((log_entry_t*)buf)->size) != GS_SUCCESS) {
                CM_RESTORE_STACK(session->stack);
                GS_LOG_DEBUG_ERR("[DDL]failed to write cluster ddl op for cons table (%d:%d).", dc->uid, dc->oid);
                knl_panic(0);
            }
            CM_RESTORE_STACK(session->stack);
        }
...
status_t db_write_ddl_op_for_children(knl_session_t* session, table_t* table)
{
    ...
    for (i = 0; i < table->index_set.count; i++) {
        ...
        while (dep != NULL) {
            ...
            if (db_write_ddl_op_internal(session, buf, ((log_entry_t*)buf)->size) != GS_SUCCESS) {
                CM_RESTORE_STACK(session->stack);
                GS_LOG_DEBUG_ERR("[DDL]failed to write cluster ddl op for child table (%d:%d).", dep->uid, dep->oid);
                knl_panic(0);
            }
            CM_RESTORE_STACK(session->stack);
...
static inline void db_convert_ddl_op_desc(knl_session_t *session, knl_cursor_t* cursor, ddl_op_desc_t* desc)
{
    ...
    if (knl_read_lob(session, lob, 0, desc->logic_log, KNL_LOGIC_LOG_BUF_SIZE, &desc->log_size) != GS_SUCCESS) {
        knl_panic(0);
    }
...
status_t db_clean_ddl_op(knl_session_t *session, clean_ddl_op_t clean_op)
{
    while (!DB_IS_OPEN(session) && clean_op == DDL_REFORM_REPLAY) {
        ...
        cm_sleep(10); // 10ms
        GS_LOG_RUN_WAR("[DDL] wait db open current status %d", (session)->kernel->db.status);
    }
    knl_cursor_t* cursor = NULL;
    ...
    if (clean_op != DDL_CLEAN_SESSION) {
        knl_open_sys_cursor(session, cursor, CURSOR_ACTION_DELETE, SYS_CLUSTER_DDL_TABLE, GS_INVALID_ID32);
    } else {
        ...
        knl_set_scan_key(INDEX_DESC(cursor->index), &cursor->scan_range.l_key, GS_TYPE_INTEGER, &id, sizeof(uint32),
    ...
    if (cursor->eof) {
        CM_RESTORE_STACK(session->stack);
        GS_LOG_DEBUG_WAR("[DDL]clean ddl op record, found no record for session(%d), op(%d).", id, clean_op);
        return GS_SUCCESS;
    }
...
void db_clean_ddl_op_garbage(knl_session_t *session)
{
    ...
    if (status != GS_SUCCESS) {
        GS_LOG_DEBUG_WAR("[DDL]failed to clean ddl op garbage.");
    }
    knl_commit(session);
}
...
status_t dtc_modify_drop_uid(knl_session_t *knl_session, uint32 uid)
{
    ...
    if (uid >= GS_MAX_USERS) {
        GS_LOG_RUN_ERR("dtc_modify_drop_uid failed, invalid uid %u", uid);
...
status_t dtc_get_user_status(knl_session_t *session, dc_user_t *dc_user, bool8 *is_user_normal)
{
    ...
    mes_init_send_head(&bcast.head, MES_CMD_BROADCAST_DATA, msg_size, GS_INVALID_ID32, session->kernel->id,
        dc_user->user_locked_owner, session->id, GS_INVALID_ID16);
    bcast.type = USER_LOCK_STATUS;
    mes_message_t msg;
    if (mes_send_data3(&bcast.head, sizeof(msg_broadcast_data_t), (void *)&user_data) != GS_SUCCESS) {
        return GS_ERROR;
    }
    if (mes_recv(session->id, &msg, GS_FALSE, GS_INVALID_ID32, DTC_GET_BTREE_SPLIT_STATUS_TIMEOUT) != GS_SUCCESS) {
        GS_LOG_RUN_ERR("[DTC DC] dtc_get_user_lock_status get result timeout");
        return GS_ERROR;
    }

    if (SECUREC_UNLIKELY(msg.head->cmd != MES_CMD_BROADCAST_DATA_ACK)) {
        GS_LOG_RUN_ERR("[DTC DC] Recive unmatched message expect %d real %d", MES_CMD_BROADCAST_DATA_ACK,
            msg.head->cmd);
        mes_release_message_buf(msg.buffer);
        return GS_ERROR;
    }
    ...
    *is_user_normal = (user_data_ack->status == USER_STATUS_NORMAL) ? GS_TRUE : GS_FALSE;
    GS_LOG_RUN_INF("[DTC DC] Get user lock status uid %d user status %d", dc_user->desc.id, user_data_ack->status);
    mes_release_message_buf(msg.buffer);
    return GS_SUCCESS;
}
...
void dtc_process_get_user_lock_status(knl_session_t *session, mes_message_t *req_msg, char *data)
{
    ...
    if (uid >= GS_MAX_USERS) {
        GS_LOG_RUN_ERR("process_get_user_lock_status failed, invalid uid %u", uid);
        mes_release_message_buf(req_msg->buffer);
        return;
    }
    dc_user_t *dc_user = ctx->users[uid];
    ...
    if (dc_user) {
        ...
        msg.user_locked_owner = GS_INVALID_ID32;
    ...
    SYNC_POINT_GLOBAL_END;
    if (mes_send_data(&msg) != GS_SUCCESS) {
        GS_LOG_RUN_ERR("[DTC DC] failed to send lock user status");
        return;
    }
    GS_LOG_RUN_INF("[DTC DC] owner return user(%d) lock status %d", uid, msg.status);
}
...
status_t dtc_try_clean_user_lock(knl_session_t *knl_session, dc_user_t *dc_user)
{
    ...
    if (!rc_bitmap64_exist(&alive_inst, dc_user->user_locked_owner)) {
        need_clean = dc_user->status == USER_STATUS_LOCKED ? GS_TRUE : GS_FALSE;
    } else {
        bool8 is_user_normal = GS_FALSE;
        ret = dtc_get_user_status(knl_session, dc_user, &is_user_normal);
        need_clean = (ret == GS_SUCCESS) && (is_user_normal == GS_TRUE);
    }
    if (need_clean) {
        text_t username;
        cm_str2text(dc_user->desc.name, &username);
        GS_LOG_RUN_WAR("[DTC DC] Need clean user lock uid %d, lock owner %d, current status %d, alive inst %llu",
    ...
    GS_LOG_RUN_INF("[DTC DC] Clean lock status uid %d, lock owner %d, current status %d, alive inst %llu need clean %d",
