...
static bool32 cs_create_ssl_link(socket_t sock_ready, cs_pipe_t *pipe)
{
    ...
    link->tcp.sock = (socket_t)accept(sock_ready, SOCKADDR(&link->tcp.remote), &link->tcp.remote.salen);
    ...
    /* set default options of sock */
    cs_set_io_mode(link->tcp.sock, GS_TRUE, GS_TRUE);
    cs_set_buffer_size(link->tcp.sock, GS_TCP_DEFAULT_BUFFER_SIZE, GS_TCP_DEFAULT_BUFFER_SIZE);
    cs_set_keep_alive(link->tcp.sock, GS_TCP_KEEP_IDLE, GS_TCP_KEEP_INTERVAL, GS_TCP_KEEP_COUNT);
    cs_set_linger(link->tcp.sock, 1, 1);
    
    link->tcp.closed = GS_FALSE;
    return GS_TRUE;
}
...
static bool32 cs_create_rdma_link(socket_t sock_ready, cs_pipe_t *pipe)
{
    ...
    if (link->sock == CS_INVALID_SOCKET) {
        cs_check_file_error();
        GS_LOG_RUN_INF("Failed to accept RDMA connection request, OS error:%d",
                       cm_get_os_error());
        return GS_FALSE;
    }
...
void cs_try_rdma_accept(rdma_lsnr_t *lsnr, cs_pipe_t *pipe)
{
    ...
    /* try poll one connect request in time GS_POLL_WAIT */
    ret = cm_rdma_poll(&rdma_fd, 1, GS_POLL_WAIT);
    if (ret > 0 && ((uint16)rdma_fd.revents & (POLLERR | POLLHUP))) {
        GS_LOG_DEBUG_ERR("Failed to poll RDMA connection");
        return;
    }
    ...
    if (ret < 0) {
        if (cm_get_os_error() != EINTR) {
            ...
            GS_LOG_RUN_ERR("Failed to wait for RDMA connection request, OS error:%d", cm_get_os_error());
        }
        return;
    }
    ...
    if (lsnr->status != LSNR_STATUS_RUNNING) {
        ...
        GS_LOG_RUN_WAR("RDMA listener not in running status");
        return;
    }
    (void)lsnr->action(lsnr, pipe);
}
...
static void srv_ssl_lsnr_proc(thread_t *thread)
{
    ...
    GS_LOG_RUN_INF("ssl-lsnr thread started");
    ...
    GS_LOG_RUN_INF("ssl-lsnr thread closed");
}
...
static status_t cs_create_rdma_lsnr_sock(rdma_lsnr_t *lsnr, const char *host)
{
    ...
    *sock = CS_INVALID_SOCKET;
    ...
    if (cs_create_rdma_socket(SOCKADDR_FAMILY(&sock_addr), sock) != GS_SUCCESS) {
        return GS_ERROR;
    }
    cs_rdma_set_io_mode(*sock, GS_TRUE, GS_TRUE);
    option = 1;
...
status_t cs_create_rdma_lsnr(rdma_lsnr_t *lsnr)
{
    ...
    // for RDMA, multiple listenning sockets are not supported now
    if (host[0] != '\0') {
        if (cs_create_rdma_lsnr_sock(lsnr, host) != GS_SUCCESS) {
...
status_t cs_start_ssl_lsnr(tcp_lsnr_t *lsnr, connect_action_t action, bool32 is_replica)
{
    ...
    if (cm_create_thread(srv_ssl_lsnr_proc, 0, lsnr, &lsnr->thread) != GS_SUCCESS) {
        cs_close_lsnr_socks(lsnr);
