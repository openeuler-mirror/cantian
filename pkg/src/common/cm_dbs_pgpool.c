...
{
    if (origin != SEEK_END) {
        GS_LOG_RUN_ERR("Unsupported seek type(%d).", origin);
        return -1;
    }
    PagePoolAttr attr = { 0 };
    cm_dbs_map_item_s obj = { 0 };
    if (cm_dbs_map_get(handle, &obj) != GS_SUCCESS) {
        GS_LOG_RUN_ERR("Failed to find page pool by index %d", handle);
        return -1;
    }
    attr.nameSpaceId = obj.ns_id;
    uint64_t capacity = 0;
    int32 ret = dbs_global_handle()->get_pagepool_logic_capacity(&obj.obj_id, &attr, &capacity);
    if (ret != 0) {
        GS_LOG_RUN_ERR("Failed(%d) to get pagepool logic capacity.", ret);
        return -1;
    }
...
status_t cm_dbs_pg_create(const char *name, int64 size, uint32 flags, int32 *handle)
{
    ...
    ret = cm_dbs_get_ns_id(DEV_TYPE_PGPOOL, &attr.nameSpaceId);
    if (ret != GS_SUCCESS) {
        GS_LOG_RUN_ERR("Failed to get ns id");
        return GS_ERROR;
    }
    ret = dbs_global_handle()->create_pagepool((char *)name, &attr, &obj.obj_id);
    if (ret != GS_SUCCESS) {
        GS_LOG_RUN_ERR("Failed to create page pool %s", name);
        return GS_ERROR;
    }
    ret = dbs_global_handle()->open_pagepool((char *)name, &attr, &obj.obj_id);
    if (ret != GS_SUCCESS) {
        GS_LOG_RUN_ERR("Failed to open page pool %s", name);
        return GS_ERROR;
    }
    ...
    if (ret != GS_SUCCESS) {
        GS_LOG_RUN_ERR("Failed to insert page pool to map");
        return GS_ERROR;
    }
    GS_LOG_DEBUG_INF("Create page pool %s, page size %u, pool size %llu success", name, pageSize, pgPoolSize);
    return GS_SUCCESS;
}
...
{
    PagePoolAttr attr = { 0 };
    int32 ret = cm_dbs_get_ns_id(DEV_TYPE_PGPOOL, &attr.nameSpaceId);
    if (ret != 0) {
        GS_LOG_RUN_ERR("Failed to get ns id");
        return GS_ERROR;
    }
    ...
    if (ret != 0) {
        GS_LOG_RUN_ERR("Failed to detroy page pool");
        return GS_ERROR;
    }
    return GS_SUCCESS;
}
...
status_t cm_dbs_pg_open(const char *name, int32 *handle)
{
    ...
    ret = cm_dbs_get_ns_id(DEV_TYPE_PGPOOL, &attr.nameSpaceId);
    if (ret == GS_ERROR) {
        GS_LOG_DEBUG_ERR("Failed to Get ns id");
        return GS_ERROR;
    }
    ret = dbs_global_handle()->open_pagepool((char *)name, &attr, &obj.obj_id);
    if (ret != 0) {
        GS_LOG_DEBUG_ERR("Failed to open page pool name=%s", name);
        return GS_ERROR;
    }
    ...
    GS_LOG_DEBUG_INF("Open page pool(%s, %u) at handle(%d) successfully.", name, attr.pageSize, *handle);
    return GS_SUCCESS;
}
...
void cm_dbs_pg_close(int32 handle)
{
    ...
    if (stat != GS_SUCCESS) {
        GS_LOG_RUN_WAR("Failed to get pagepool object from cache by handle(%d).", handle);
        return;
    }
    cm_dbs_map_remove(handle);
    int32 ret = dbs_global_handle()->close_pagepool(&obj.obj_id);
    if (ret != 0) {
        GS_LOG_RUN_WAR("Failed(%d) to close pagepool from dbstor.", ret);
        return;
    }
    GS_LOG_DEBUG_INF("Close pagepool at handle(%d) successfully.", handle);
}
...
status_t cm_dbs_pg_read(int32 handle, int64 offset, void *buf, int32 size, int32 *read_size)
{
    ...
    if ((offset % pageSize) != 0 || (size % pageSize) != 0) {
        GS_LOG_RUN_ERR("Param is exception, offset %llu size %u", offset, size);
        return GS_ERROR;
    }
    ...
    if (ret != 0) {
        ...
        GS_LOG_RUN_ERR("Read page %s fail, offset:0x%llx", name, offset);
        return GS_ERROR;
    }
...
void cm_dbs_print_err_info(int32 handle, uint32_t opcode, int64 offset, DbsPageId pgId, int32 size, int32 ret)
{
    char name[CSS_MAX_NAME_LEN + 1] = { 0 };
    cm_dbs_map_get_name(handle, name, sizeof(name));
    if (opcode == CS_PAGE_POOL_WRITE) {
        GS_LOG_RUN_ERR("Mput page %s fail, offset:0x%llx, pgId %lu, size %d, ret %d", name, offset, pgId, size, ret);
    } else {
        GS_LOG_RUN_ERR("Write page %s fail, offset:0x%llx, pgId %lu, size %d, ret %d", name, offset, pgId, size, ret);
    }
}
                                     ...
                                     uint32 *partid)
{
    ...
    if (obj.pagepool.page_size == 0) {
        GS_LOG_RUN_ERR("Page size is invalid, page pool by index %d", handle);
        return GS_ERROR;
    }
    pageSize = obj.pagepool.page_size;

    if (((offset % pageSize) != 0) || ((size % pageSize) != 0)) {
        GS_LOG_RUN_ERR("Offset %lld or size %d is invalid, page pool by index %d", offset, size, handle);
        return GS_ERROR;
    }
    ...
    if (opcode == CS_PAGE_POOL_WRITE) {
        ...
        GS_LOG_RUN_ERR("Opcode %u is invalid", opcode);
        return GS_ERROR;
    }
...
status_t cm_dbs_pg_cal_part_id(uint64 pgid, uint32 pageSize, uint32 *partid)
{
    ...
    if (cfg->partition_num <= 0) {
        GS_LOG_RUN_ERR("The part capacity(%u) should not smaller than 1.", cfg->partition_num);
        return GS_ERROR;
    }
...
status_t cm_dbs_sync_page(int32 handle, uint32 partid)
{
    cm_dbs_map_item_s obj = { 0 };
    if (cm_dbs_map_get(handle, &obj) != GS_SUCCESS) {
        GS_LOG_RUN_ERR("Failed to find pgpool obj by handle(%d).", handle);
        return GS_ERROR;
    }

    int32 ret = dbs_global_handle()->sync_page_by_part_index(&obj.obj_id, partid);
    if (ret != GS_SUCCESS) {
        GS_LOG_RUN_ERR("Failed(%d) to sync page, partid %u.", ret, partid);
        return GS_ERROR;
    }
    return GS_SUCCESS;
}
...
status_t cm_dbs_pg_extend(int32 handle, int64 offset, int64 size)
{
    ...
    PagePoolAttr attr = { 0 };
    if (cm_dbs_get_ns_id(DEV_TYPE_PGPOOL, &attr.nameSpaceId) != GS_SUCCESS) {
        GS_LOG_RUN_ERR("Failed to get namespace id for pagepool.");
        return GS_ERROR;
    }
    int32 ret = dbs_global_handle()->expand_pagepool_logic_capacity(&obj.obj_id, &attr, (uint64)offset, (uint64)size);
    if (ret != 0) {
        GS_LOG_RUN_ERR("Failed(%d) to extend pgpool(%lld) size(%lld).", ret, offset, size);
        return GS_ERROR;
    }
    return GS_SUCCESS;
}
...
{
    if (keep_size < 0) {
        GS_LOG_RUN_ERR("Failed to truncate pagepool because of invalid size(%lld).", keep_size);
        return GS_ERROR;
    }
    cm_dbs_map_item_s obj = { 0 };
    if (cm_dbs_map_get(handle, &obj) != GS_SUCCESS) {
    ...
    if (ret != 0) {
        GS_LOG_RUN_ERR("Failed(%d) to truncate PagePool size from %lu to %lu.", ret, capacity, new_size);
        return GS_ERROR;
    }
    return GS_SUCCESS;
}
...
status_t cm_dbs_pg_rename(const char *src_name, const char *dst_name)
{
    if (src_name == NULL || strlen(src_name) == 0) {
        GS_LOG_RUN_ERR("The src name is invalid.");
        return GS_ERROR;
    }
    if (dst_name == NULL || strlen(dst_name) == 0) {
        GS_LOG_RUN_ERR("The dst name is invalid.");
        return GS_ERROR;
    }
    if (strcmp(src_name, dst_name) == 0) {
        GS_LOG_RUN_WAR("The dst name is same as the src(%s).", src_name);
        return GS_SUCCESS;
    }
    PagePoolAttr attr = { 0 };
    if (cm_dbs_get_ns_id(DEV_TYPE_PGPOOL, &attr.nameSpaceId) != GS_SUCCESS) {
    ...
    int32_t ret = dbs_global_handle()->rename_pagepool((char *)src_name, (char *)dst_name, &attr);
    if (ret != 0) {
        GS_LOG_RUN_ERR("Failed(%d) to rename pagepool from %s to %s.", ret, src_name, dst_name);
        return GS_ERROR;
    }
    GS_LOG_DEBUG_INF("Rename pagepool from %s to %s successfully.", src_name, dst_name);
    return GS_SUCCESS;
}
...
bool32 cm_dbs_pg_exist(const char *name)
{
    ...
    int32 ret = cm_dbs_get_ns_id(DEV_TYPE_PGPOOL, &attr.nameSpaceId);
    if (ret != GS_SUCCESS) {
        GS_LOG_RUN_ERR("Failed(%d) to get namespace id for pagepool.", ret);
        return GS_FALSE;
    }
    PagePoolId id = { 0 };
    ret = dbs_global_handle()->open_pagepool((char *)name, &attr, &id);
    if (ret == -ENOENT) {
        GS_LOG_DEBUG_INF("The pagepool(%s) does not exist.", name);
        return GS_FALSE;
    } else if (ret != 0) {
        GS_LOG_RUN_ERR("Failed(%d) to open pagepool(%s)", ret, name);
        return GS_FALSE;
    }
    (void)dbs_global_handle()->close_pagepool(&id);
    GS_LOG_DEBUG_INF("The pagepool named %s exists.", name);
    return GS_TRUE;
}
