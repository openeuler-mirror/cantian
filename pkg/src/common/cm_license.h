...
typedef struct {
    ...
    char lic_conf_path[CT_FILE_NAME_BUFFER_SIZE];
} lic_cfg_t;
...
#define CT_LIC_FILE_PATH_ENV   (char *)"CANTIANHOME"
#define CT_LIC_DEADLINE_PERM   (char *)"PERMANENT"
#define CT_LIC_CONF_LINE_BUF   (uint32)100
#define CT_LIC_SLEEP_TIME      (uint32)5000

#define CT_CONTINUE_IF_ERROR(ret) \
    if ((ret) != CT_SUCCESS) { \
        continue;                 \
    }
...
#define SSC_CONTINUE_IF_ERR(ret, err_info)        \
    if ((ret) == -1) {                            \
        CT_LOG_RUN_ERR("[LICENSE]%s", err_info);  \
        continue;                                    \
    }
