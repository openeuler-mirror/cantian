...
static inline status_t cm_get_cidrmask(const char *cidr_str, int *mask)
{
    ...
    if (cidr_slash != NULL) {
        if (cm_str2int(cidr_slash + 1, mask) != GS_SUCCESS || *mask < 0 || (family == AF_INET && *mask > 32) ||
            (family == AF_INET6 && *mask > 128)) {
            GS_THROW_ERROR_EX(ERR_TCP_INVALID_IPADDRESS, "invalid CIDR mask in address \"%s\"", cidr_str);
            return GS_ERROR;
        }
        *cidr_slash = '\0';
    } else {
...
status_t cm_split_host_port(uint16 ports[], const char *port_addr)
{
    ...
    for (pos = (char *)port_addr; len > 0; len--) {
        ...
        *pos = '\0';
        if (cm_str2uint16(port, &ports[port_count]) != GS_SUCCESS) {
            cm_reset_error();
            GS_THROW_ERROR(ERR_INVALID_PARAMETER, "port");
            return GS_ERROR;
        }

        if (ports[port_count] < GS_MIN_PORT) {
            GS_THROW_ERROR(ERR_PARAMETER_TOO_SMALL, "port", (int64)GS_MIN_PORT);
            return GS_ERROR;
        }
