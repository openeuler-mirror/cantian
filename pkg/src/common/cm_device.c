...
status_t cm_query_file_num_device(device_type_t type, const char *name, uint32 *file_num)
{
    if (type == DEV_TYPE_FILE) {
        if (cm_query_file_num(name, file_num) != CT_SUCCESS) {
            CT_LOG_RUN_ERR("query file num failed, path %s.", name);
            return CT_ERROR;
        }
    } else if (type == DEV_TYPE_DBSTOR_FILE) {
        if (cm_dbs_query_file_num(name, file_num) != CT_SUCCESS) {
            CT_LOG_RUN_ERR("query file num failed, path %s.", name);
            return CT_ERROR;
        }
    } else {
        CT_LOG_RUN_ERR("query file num failed, error device type, path %s", name);
        return CT_ERROR;
    }
...
{
    if (cm_query_file_num_device(type, file_path, file_num) != CT_SUCCESS) {
        CT_LOG_RUN_ERR("query file num failed, device type %u, file_path %s", type, file_path);
        return CT_ERROR;
    }
