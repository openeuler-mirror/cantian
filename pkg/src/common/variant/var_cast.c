...
static const type_desc_t g_datatype_items[GS_MAX_DATATYPE_NUM] = {
    ...
    [GS_TYPE_I(GS_TYPE_NUMBER2)] = { GS_TYPE_NUMBER2, { (char *)"NUMBER2", 7 }, 1001 },
...
static const uint64 g_dest_cast_mask[GS_MAX_DATATYPE_NUM] = {
    ...
    GS_TYPE_MASK_BINARY,
    [GS_TYPE_I(GS_TYPE_UINT64)] = GS_TYPE_MASK_NUMERIC | GS_TYPE_MASK_STRING | GS_TYPE_MASK(GS_TYPE_BOOLEAN) |
    GS_TYPE_MASK_BINARY,
    ...
    [GS_TYPE_I(GS_TYPE_NUMBER2)] = GS_TYPE_MASK_NUMERIC | GS_TYPE_MASK_STRING | GS_TYPE_MASK_BINARY,
    ...
    [GS_TYPE_I(GS_TYPE_BLOB)] = GS_TYPE_MASK_BINARY | GS_TYPE_MASK_STRING | GS_TYPE_MASK(GS_TYPE_CLOB) |
    GS_TYPE_MASK(GS_TYPE_BLOB) | GS_TYPE_MASK(GS_TYPE_RAW) | GS_TYPE_MASK(GS_TYPE_IMAGE),
    ...
    [GS_TYPE_I(GS_TYPE_NUMBER3)] = GS_TYPE_MASK_NUMERIC | GS_TYPE_MASK_STRING | GS_TYPE_MASK_BINARY,
};
...
status_t var_text2num(const text_t *text, gs_type_t type, bool32 negative, variant_t *result)
{
    ...
    switch (type) {
        ...
        case GS_TYPE_UINT64:
            ...
            if (result->v_ubigint == (uint64)(GS_MIN_UINT32)) {
...
status_t var_to_round_bigint(const variant_t *var, round_mode_t rnd_mode, int64 *i64, int *overflow)
{
    ...
    switch (var->type) {
        ...
        case GS_TYPE_UINT64:
            GS_RETURN_IFERR(var_to_int64_check_overflow(var->v_ubigint));
        ...
        case GS_TYPE_STRING: {
            dec8_t dec;
            GS_RETURN_IFERR(cm_text_to_dec(&var->v_text, &dec));
...
status_t var_to_round_ubigint(const variant_t *uvar, round_mode_t rnd_mode, uint64 *ui64, int *uoverflow)
{
    ...
    switch (uvar->type) {
        ...
        case GS_TYPE_REAL: {
            ...
            *ui64 = (uint64)val;
            if (REAL2UINT64_IS_OVERFLOW(*ui64, val)) {
                GS_THROW_ERROR(ERR_TYPE_OVERFLOW, "UNSIGNED BIGINT");
                if (uoverflow != NULL) {
        ...
        case GS_TYPE_NUMBER2:
            ...
            if (uret != GS_SUCCESS && uoverflow != NULL) {
        ...
        case GS_TYPE_STRING: {
            dec8_t dec;
            GS_RETURN_IFERR(cm_text_to_dec(&uvar->v_text, &dec));
        ...
        case GS_TYPE_RAW:
        default:
            GS_SET_ERROR_MISMATCH(GS_TYPE_UINT64, uvar->type);
            return GS_ERROR;
    }
}
...
status_t var_to_round_int32(const variant_t *var, round_mode_t rnd_mode, int32 *i32)
{
    ...
    switch (var->type) {
        case GS_TYPE_UINT32:
            GS_RETURN_IFERR(var_to_int32_check_overflow(var->v_uint32));
...
static status_t var_to_text(const nlsparams_t *nls, variant_t *var, text_t *text)
{
    ...
    switch (var->type) {
        ...
        case GS_TYPE_DECIMAL:
        case GS_TYPE_NUMBER2:
            status = cm_dec_to_text(VALUE_PTR(dec8_t, var), GS_MAX_DEC_OUTPUT_PREC, text);
            break;
        case GS_TYPE_DATE:
        case GS_TYPE_TIMESTAMP:
        case GS_TYPE_TIMESTAMP_TZ_FAKE:
...
{
    GS_RETURN_IFERR(var_as_decimal_core(var)); // var->v_dec's range is [DEC2_EXPN_LOW, MAX_NUMERIC_EXPN]
    if (var->type == GS_TYPE_NUMBER2) { // number2, range is [DEC2_EXPN_LOW, DEC2_EXPN_UPPER]
        return GS_SUCCESS;
    }
    var->type = GS_TYPE_NUMBER; // number, range is [MIN_NUMERIC_EXPN, MAX_NUMERIC_EXPN]
    if (!var->is_null) {
        GS_RETURN_IFERR(cm_dec_check_overflow(&var->v_dec, var->type));
    }
    return GS_SUCCESS;
}
...
{
    GS_RETURN_IFERR(var_as_decimal_core(var));
...
status_t var_to_unix_timestamp(dec8_t *unix_ts, timestamp_t *ts_ret, int64 time_zone_offset)
{
    ...
    if (GS_SUCCESS != cm_int64_mul_dec((int64)MICROSECS_PER_SECOND, unix_ts, unix_ts)) {
        cm_reset_error();
        GS_SET_ERROR_TIMESTAMP_OVERFLOW();
        return GS_ERROR;
    }

    if (GS_SUCCESS != cm_dec_to_int64(unix_ts, &ts, ROUND_TRUNC)) {
        cm_reset_error();
        GS_SET_ERROR_TIMESTAMP_OVERFLOW();
        return GS_ERROR;
    }
