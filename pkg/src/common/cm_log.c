...
static void cm_log_build_normal_head(char *buf, uint32 buf_size, log_level_t log_level, const char *module_name)
{
    ...
    if (tz_hour >= 0) {
        ...
        errcode = snprintf_s(buf, (size_t)buf_size, GS_MAX_LOG_HEAD_LENGTH - 1, "UTC+%02d:%02d %s|%s|%05u|%u|%s>",
...
static status_t cm_log_add_backup_file(char *backup_file_name[GS_MAX_LOG_FILE_COUNT], uint32 *backup_file_count,
...
#ifdef _WIN32
static status_t cm_log_search_backup_file(char *backup_file_name[GS_MAX_LOG_FILE_COUNT], uint32 *backup_file_count,
    const char *log_dir, const char *log_file_name, const char *log_ext_name)
{
    char bak_file_fmt[GS_FILE_NAME_BUFFER_SIZE] = { 0 };
    WIN32_FIND_DATA data;

    PRTS_RETURN_IFERR(snprintf_s(bak_file_fmt, GS_FILE_NAME_BUFFER_SIZE, GS_MAX_FILE_NAME_LEN, "%s/%s*.%s", log_dir,
...
status_t cm_log_get_bak_file_list(char *backup_file_name[GS_MAX_LOG_FILE_COUNT], uint32 *backup_file_count,
    const char *log_file)
{
...
static void cm_log_remove_bak_file(char *backup_file_name[GS_MAX_LOG_FILE_COUNT], uint32 *remove_file_count,
...
*/
static status_t cm_rmv_and_bak_log_file(log_file_handle_t *log_file_handle, char *bak_file_name[GS_MAX_LOG_FILE_COUNT],
    char new_bak_file_name[GS_FILE_NAME_BUFFER_SIZE], uint32 *remove_file_count)
{
    ...
    if (need_bak_file_count == 0) {
        ...
        *remove_file_count = 1;
        MEMS_RETURN_IFERR(
            strncpy_s(bak_file_name[0], GS_FILE_NAME_BUFFER_SIZE, log_file_handle->file_name, (size_t)file_name_len));
        return GS_SUCCESS;
    }
    ...
    if (log_file_handle->log_id == LOG_OPER && cm_log_stat_file(log_file_handle, &file_size, &file_inode) == GS_TRUE) {
        if (file_size < g_log_param.max_log_file_size) {
            // multi ctclients write one ctclient.olog: ctclient.olog has already be renamed
            // double check ctclient.olog size
            return GS_SUCCESS;
        }
    }
...
static void cm_write_rmv_and_bak_file_log(char *bak_file_name[GS_MAX_LOG_FILE_COUNT], uint32 remove_file_count,
    ...
    const char *module_name, bool32 need_rec_filelog, const char *format, ...)
{
    ...
    if (log_param->log_instance_startup) {
        errcode = snprintf_s(new_format, GS_MAX_LOG_CONTENT_LENGTH, GS_MAX_LOG_CONTENT_LENGTH - 1, "%s [%s:%u]", format,
...
void cm_write_longsql_log(const char *format, ...)
{
    ...
    errno_t errcode;
    va_start(args, format);
    errcode = vsnprintf_s(buf + sizeof(uint32), GS_LOG_LONGSQL_LENGTH_16K + 1, GS_LOG_LONGSQL_LENGTH_16K, format, args);
...
void cm_write_max_longsql_log(const char *format, ...)
{
    ...
    va_start(args, format);
    rc_memzero =
        vsnprintf_s(buf + sizeof(uint32), GS_MAX_LOG_LONGSQL_LENGTH + 1, GS_MAX_LOG_LONGSQL_LENGTH, format, args);
...
void cm_dump_mem(void *dump_addr, uint32 dump_len)
{
    ...
    GS_LOG_BLACKBOX("\r\n[DUMP] dump_addr 0x%llx, dump_len %u", (uint64)dump_addr, dump_len);
...
uint64_t cm_print_memory_usage(void)
{
    ...
    uint64_t pid = getpid();
    char statm_path[GS_FILE_NAME_BUFFER_SIZE] = { 0 };
