...
status_t cm_set_file_iof_cfg(uint32 cluster_id, uint32 termid, const char* file_path)
{
    ...
    if (err != EOK) {
        GS_LOG_RUN_ERR("Set file iof cfg failed, strncpy err, %d:%s.", errno, strerror(errno));
        return GS_ERROR;
    }
    GS_LOG_RUN_INF("File set iof cfg succ, file path %s, cluster id %u.", g_file_cfg.file_path,
...
void iof_file_watcher(thread_t *thread)
{
    ...
    GS_LOG_RUN_INF("Iof file watcher, file name(%s).", file_name);
    for (;;) {
        if (cm_access_file(file_name, F_OK) != 0) {
            GS_LOG_RUN_ERR("Failed to access file(%s).", file_name);
            break;
        }
        cm_sleep(1);
    }
...
status_t cm_file_iof_register(uint32 inst_id, thread_t *file_iof_thd)
{
    if (strlen(g_file_cfg.file_path) == 0) {
        GS_LOG_RUN_WAR("Iof file path is empty, no need iofence.");
        return GS_SUCCESS;
    }
    ...
    if (file_name == NULL) {
        GS_LOG_RUN_ERR("Failed to malloc file name buffer(%u), %d:%s.", IO_FENCE_FILE_NAME_BUFFER_SIZE,
    ...
    if (err == -1) {
        GS_LOG_RUN_ERR("Failed to snprintf file name, %d:%s.", errno, strerror(errno));
        return GS_ERROR;
    }
    GS_RETURN_IFERR(cm_open_file(file_name, O_CREAT | O_RDWR | O_BINARY | O_CLOEXEC | O_SYNC | O_DIRECT, &iof_fd));
    cm_close_file(iof_fd);

    if (GS_SUCCESS != cm_create_thread(iof_file_watcher, 0, file_name, file_iof_thd)) {
        GS_LOG_RUN_ERR("Failed to create file(%s) watcher thread.", file_name);
        return GS_ERROR;
    }
    GS_LOG_RUN_INF("File iof register succ, file name %s, cluster id %u, inst id %u.", file_name,
...
status_t cm_file_iof_kick_by_inst_id(uint32 inst_id)
{
    ...
    if (cm_access_file(file_name, F_OK) != 0) {
        GS_LOG_RUN_WAR("Iof file not exist, file(%s).", file_name);
        return GS_SUCCESS;
    }
    if (cm_remove_file(file_name) != GS_SUCCESS) {
        GS_LOG_RUN_ERR("Failed to remove file(%s).", file_name);
        return GS_ERROR;
    }
    GS_LOG_RUN_INF("File iof kick succ, file name %s, cluster id %u, inst id %u.", file_name, g_file_cfg.cluster_id,
        inst_id);
    return GS_SUCCESS;
}