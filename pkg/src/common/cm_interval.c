...
static const char *g_itvl_errinfos[IERR__NOT_USED__] = {
    ...
    [IERR_FIELD_TOO_LONG] = "-- field text is too long (<=" CT_STR(ITVL_MAX_ISO_FIELD_LEN) ")",
...
#define CT_THROW_ITVL_ERROR(cond, err_no)         \
    if (cond) {                                   \
        CT_THROW_ERROR(ERR_INVALID_INTERVAL_TEXT, \
...
#define VERIFY_INTVL_UNIT_VALUE(unit_val, max_val, info)                \
    if ((unit_val) > (max_val)) {                                       \
        CT_THROW_ERROR(ERR_INTERVAL_FIELD_OVERFLOW, (info), (max_val)); \
        return CT_ERROR;                                                \
    }
...
static inline itvl_errno_t cm_fetch_iso8601_item(text_t *iso_text, itvl_item_t *item, bool32 in_datepart)
{
    if (CM_IS_EMPTY(iso_text)) {
        CT_THROW_ERROR(ERR_ASSERT_ERROR, "iso_text is not empty");
        return IERR_ERROR;
    }
    ...
    CT_RETVALUE_IFTRUE((item->len == 0), IERR_UNEXPECTED_CHAR);
    CT_RETVALUE_IFTRUE((iso_text->len == 0), IERR_NO_INDICATOR);

    // removing leading zeros
    cm_text_ltrim_zero((text_t *)item);
    CT_RETVALUE_IFTRUE((item->len > ITVL_MAX_ISO_FIELD_LEN), IERR_FIELD_TOO_LONG);

    // handle dot
    if (!CM_IS_DOT(CM_TEXT_FIRST(iso_text))) {
        item->has_dot = CT_FALSE;
        return IERR_SUCCESS;
    }

    // the decimal point is not allowed in date part
    CT_RETVALUE_IFTRUE(in_datepart, IERR_UNEXPECTED_DOT);
    ...
    CT_RETVALUE_IFTRUE((fsec_prec == 0), IERR_NO_FRAC_DIGIT);

    // no indicator
    CT_RETVALUE_IFTRUE((iso_text->len == 0), IERR_NO_INDICATOR);

    // the indicator must be 'S', i.e., the dot must allowed in second field
    CT_RETVALUE_IFTRUE((!IS_SECOND_INDICATOR(CM_TEXT_FIRST(iso_text))), IERR_UNEXPECTED_CHAR);

    /* SECOND is the last field, and no characters are allowed */
    CT_RETVALUE_IFTRUE((iso_text->len > 1), IEER_UNDESIRED_CHARS);

    // recompute the length
    item->len = (uint32)(iso_text->str - item->str);
    item->has_dot = CT_TRUE;
    return IERR_SUCCESS;
}
                                             ...
                                             uint32 *mask, text_t *isotext, itvl_item_t *item)
{
    if (CM_IS_EMPTY(isotext)) {
        CT_THROW_ERROR(ERR_ASSERT_ERROR, "isotext is not empty");
        return CT_ERROR;
    }
    ...
    switch (CM_TEXT_FIRST(isotext)) {
        ...
        case 'D': /* DAYS */
            ...
            break;
        default:
            CT_THROW_ERROR(ERR_INVALID_INTERVAL_TEXT, cm_get_itvl_errinfo(IERR_UNEXPECTED_CHAR));
            return CT_ERROR;
    }
                                             ...
                                             uint32 *mask, text_t *isotext, itvl_item_t *item)
{
    ...
    switch (CM_TEXT_FIRST(isotext)) {
        ...
        case 'S':  // parsing second without fractional second
            ...
            CT_RETURN_IFERR(cm_second_field2value(item, &idetail->sec, &idetail->fsec));
            break;
        default:
            CT_THROW_ERROR(ERR_INVALID_INTERVAL_TEXT, cm_get_itvl_errinfo(IERR_UNEXPECTED_CHAR));
            return CT_ERROR;
 ...
 * requirement.
 *
 * For CT_TYPE_INTERVAL_YM type, all fields can be allowed in the interval text.
 * However, if the DAY or TIME fields are specified in YM_INTERVAL, they will
 * be ignored. For CT_TYPE_INTERVAL_DS type, YEAR and MONTH fields are not allowed.
 * Therefore, the MASK is set by IU_DS_INTERVAL.
 * */
static inline uint32 cm_init_interval_rank(ct_type_t itype)
{
    return (itype == CT_TYPE_INTERVAL_YM) ? (uint32)IU_ALL : (uint32)IU_DS_INTERVAL;
}
...
static inline status_t cm_isotext2intvl_detail(text_t *iso_text,
                                               ct_type_t itype, interval_detail_t *idetail)
{
    itvl_item_t item;
    uint32 rank;
    bool32 in_datepart = CT_TRUE;
    itvl_errno_t err_no;

    CM_POINTER2(iso_text, idetail);

    // if text is too short
    CT_THROW_ITVL_ERROR((iso_text->len < 3), IERR_TEXT_TOO_SHORT);
    ...
    while (iso_text->len > 0) {
        ...
        if (err_no != IERR_SUCCESS) {
            CT_THROW_ERROR(ERR_INVALID_INTERVAL_TEXT, cm_get_itvl_errinfo(err_no));
            return CT_ERROR;
        }

        if (in_datepart) {
            CT_RETURN_IFERR(cm_parse_iso_dateitem(idetail, &rank, iso_text, &item));
        } else {
            CT_RETURN_IFERR(cm_parse_iso_timeitem(idetail, &rank, iso_text, &item));
        }
...
static inline status_t cm_fetch_sqlfmt_item(text_t *sqltext, itvl_item_t *item)
{
    ...
    CT_THROW_ITVL_ERROR(CM_IS_EMPTY(sqltext), IERR_INVALID_FORMAT);
    ...
    if (item->len == 0) {
        CT_THROW_ITVL_ERROR((sqltext->len > 0), IERR_UNEXPECTED_CHAR);
        CT_THROW_ITVL_ERROR((sqltext->len == 0), IERR_INVALID_FORMAT);
    }
    CT_THROW_ITVL_ERROR((item->len > ITVL_MAX_ISO_FIELD_LEN), IERR_FIELD_TOO_LONG);
...
static status_t cm_fetch_sqlfmt_field(text_t *sqltext, char split_char, uint32 *val)
{
    itvl_item_t item;
    uint32 i;
    bool32 is_found = CT_FALSE; /* record whether found the split char */

    CT_RETURN_IFERR(cm_fetch_sqlfmt_item(sqltext, &item));
    ...
    if (sqltext->len == 0) {
        ...
        *val = cm_field2value((text_t *)(&item));
        ...
        for (i = 0; i < sqltext->len; i++) {
            ...
            CT_THROW_ITVL_ERROR((sqltext->str[i] > ' '), IERR_UNEXPECTED_CHAR);
        }
        CT_THROW_ITVL_ERROR(!is_found, IERR_INVALID_FORMAT);
...
static status_t cm_sqltext2ymintvl_detail(text_t *sqltext, interval_detail_t *idetail, uint32 fmt)
{
    ...
    if (fmt & IU_YEAR) {
        CT_RETURN_IFERR(cm_fetch_sqlfmt_field(sqltext, '-', &idetail->year));
    ...
    if (fmt & IU_MONTH) {
        ...
        CT_THROW_ITVL_ERROR((month.len == 0), IERR_INVALID_FORMAT);
        CT_THROW_ITVL_ERROR((month.len > ITVL_MAX_ISO_FIELD_LEN), IERR_ILLEGAL_MONTH_FIELD);
...
static inline status_t cm_fetch_sqlfmt_second(text_t *sqltext, interval_detail_t *idetail)
{
    ...
    if (sqltext->len == 0) {
        ...
        CT_THROW_ITVL_ERROR((!CM_IS_DOT(CM_TEXT_FIRST(sqltext))), IERR_UNEXPECTED_CHAR);
        ...
        // if no digits in frac
        CT_THROW_ITVL_ERROR((fsec_prec == 0), IERR_NO_FRAC_DIGIT);
        /* SECOND is the last field, and no characters are allowed */
        CT_THROW_ITVL_ERROR((sqltext->len != 0), IEER_UNDESIRED_CHARS);
...
static status_t cm_sqltext2dsintvl_detail(text_t *sqltext, interval_detail_t *idetail, uint32 fmt)
{
    ...
    if (fmt & IU_DAY) {
        // fetch day field
        CT_RETURN_IFERR(cm_fetch_sqlfmt_field(sqltext, ' ', &idetail->day));
        VERIFY_INTVL_UNIT_VALUE(idetail->day, ITVL_MAX_SQL_DAY, "DAY");
    }
    if (fmt & IU_HOUR) {
        // fetch hour field
        CT_RETURN_IFERR(cm_fetch_sqlfmt_field(sqltext, ':', &idetail->hour));
    }
    if (fmt & IU_MINUTE) {
        // fetch minute field
        CT_RETURN_IFERR(cm_fetch_sqlfmt_field(sqltext, ':', &idetail->min));
    }
    if (fmt & IU_SECOND) {
        // fetch second field
        CT_RETURN_IFERR(cm_fetch_sqlfmt_second(sqltext, idetail));
    }
    // expected end
    cm_trim_text(sqltext);
    CT_THROW_ITVL_ERROR(!CM_IS_EMPTY(sqltext), IERR_INVALID_FORMAT);
...
static inline status_t cm_sqltext2intvl_detail(text_t *sqltext, ct_type_t itype, interval_detail_t *idetail,
                                               uint32 fmt)
{
    // if no text
    CT_THROW_ITVL_ERROR((sqltext->len == 0), IERR_TEXT_TOO_SHORT);

    if (itype == CT_TYPE_INTERVAL_YM) {
...
status_t cm_text2intvl_detail(const text_t *text, ct_type_t itype, interval_detail_t *idetail, uint32 fmt)
{
    ...
    // if no text
    CT_THROW_ITVL_ERROR((itv_text.len == 0), IERR_TEXT_TOO_SHORT);

    idetail->is_neg = CT_FALSE;
...
status_t cm_text2yminterval(const text_t *text, interval_ym_t *itvl)
{
    ...
    CT_RETURN_IFERR(cm_text2intvl_detail(text, CT_TYPE_INTERVAL_YM, &itvl_detail, IU_YM_INTERVAL));
...
status_t cm_text2dsinterval(const text_t *text, interval_ds_t *itvl)
{
    ...
    CT_RETURN_IFERR(cm_text2intvl_detail(text, CT_TYPE_INTERVAL_DS, &itvl_detail, IU_DS_INTERVAL));
...
status_t cm_encode_yminterval(const interval_detail_t *idetail, interval_ym_t *itvl)
{
    ...
    *itvl = (interval_ym_t)(idetail->mon + idetail->year * ITVL_MONTHS_PER_YEAR);
    CT_THROW_ITVL_ERROR((*itvl > CT_MAX_YMINTERVAL), IERR_YMINTERVAL_OVERFLOW);
...
status_t cm_encode_dsinterval(const interval_detail_t *idetail, interval_ds_t *itvl)
{
    ...
    *itvl = (interval_ds_t)((uint64)idetail->day * ITVL_UNITS_PER_DAY);
    ...
    CT_RETURN_IFERR(cm_dsinterval_add(*itvl, unit, itvl));
    ...
    // add frac_sec unit
    CT_RETURN_IFERR(cm_dsinterval_add(*itvl, idetail->fsec, itvl));
...
void cm_decode_dsinterval(interval_ds_t dsitvl_input, interval_detail_t *idetail)
{
    ...
    CM_ASSERT(dsitvl <= CT_MAX_DSINTERVAL);
...
static void cm_yminterval2text_ex(interval_ym_t ymitvl, uint32 year_prec, text_t *text)
{
    ...
    iret_sprintf = snprintf_s(CM_GET_TAIL(text), CT_MAX_YM_INTERVAL_STRLEN, CT_MAX_YM_INTERVAL_STRLEN - 1, "%0*u-%02u",
...
static void cm_dsinterval2text_ex(interval_ds_t dsitvl, uint32 day_prec, uint32 frac_prec, text_t *text)
{
    ...
    if (frac_prec == 0) {
        iret_sprintf = snprintf_s(CM_GET_TAIL(text), CT_MAX_DS_INTERVAL_STRLEN, CT_MAX_DS_INTERVAL_STRLEN - 1,
...
uint32 cm_dsinterval2str_ex(interval_ds_t dsitvl, uint32 day_prec, uint32 frac_prec, char *str, uint32 str_max_sz)
{
    ...
    if (text.len >= str_max_sz) {
        CT_THROW_ERROR(ERR_BUFFER_OVERFLOW, text.len, str_max_sz);
        return 0;
    }
    CM_NULL_TERM(&text);
...
status_t cm_adjust_yminterval(interval_ym_t *ymitvl, uint32 year_prec)
{
    ...
    if (year_prec > (uint32)ITVL_MAX_YEAR_PREC) {
        CT_THROW_ERROR_EX(ERR_ASSERT_ERROR, "year_prec(%u) <= ITVL_MAX_YEAR_PREC(%u)", year_prec,
    ...
    CT_THROW_ERROR(ERR_INVALID_INTERVAL_FIELD, "YEAR", year_prec);
    return CT_ERROR;
}
...
#define CM_VERIFY_DAY_FIELD(dsitvl, day_prec)                            \
    do {                                                                 \
        if ((uint32)CM_EXTRACT_DAY(dsitvl) >= g_1ten_powers[day_prec]) { \
            CT_THROW_ERROR(ERR_INVALID_INTERVAL_FIELD, "DAY", day_prec); \
            return CT_ERROR;                                             \
        }                                                                \
    } while (CT_FALSE)
...
status_t cm_adjust_dsinterval(interval_ds_t *dsitvl, uint32 day_prec, uint32 fsec_prec)
{
    if (day_prec > (uint32)ITVL_MAX_DAY_PREC) {
        CT_THROW_ERROR_EX(ERR_ASSERT_ERROR, "day_prec(%u) <= ITVL_MAX_DAY_PREC(%u)",
            day_prec, (uint32)ITVL_MAX_DAY_PREC);
        return CT_ERROR;
    }
    if (fsec_prec > (uint32)ITVL_MAX_SECOND_PREC) {
        CT_THROW_ERROR_EX(ERR_ASSERT_ERROR, "fsec_prec(%u) <= ITVL_MAX_SECOND_PREC(%u)", fsec_prec,
