...
static const char g_zero_page[GS_LARGE_PAGE_SIZE];
...
typedef struct st_miner_input_def {
    ...
    char *ctrlfile;
    char *inputfile;
    char input_name[GS_FILE_NAME_BUFFER_SIZE];
    char *logfiles[GS_REDO_LOG_NUM];
    bool32 has_xid;
    xid_t xids[GS_XID_NUM];
    uint8 xid_cnt;
    uint8 log_cnt;
    char *keya;
    char *keyb;
...
static status_t miner_log_init(void)
{
    ...
    if (g_log_desc == NULL) {
        GS_THROW_ERROR (ERR_ALLOC_MEMORY, (uint64)(sizeof(log_desc_t) * RD_TYPE_END), "miner log init");
        return GS_ERROR;
    }
...
static status_t miner_read_file_head(int32 handle, log_file_head_t *head, int64 *offset)
{
    ...
    if (cm_read_file(handle, head, sizeof(log_file_head_t), &size) != GS_SUCCESS) {
        return GS_ERROR;
    }

    if (size != sizeof(log_file_head_t)) {
        GS_THROW_ERROR(ERR_FILE_SIZE_MISMATCH, (int64)size, (uint64)sizeof(log_file_head_t));
        return GS_ERROR;
    }
...
static status_t miner_verify_batch_checksum(log_batch_t *batch, bool32 is_checksum)
{
    ...
    if (is_checksum && org_cks != GS_INVALID_CHECKSUM) {
                          ...
                          int64 *file_offset, log_batch_t **batch_ptr, bool32 *file_finish)
{
    ...
    if (miner_need_read_file(buf, data_offset, data_size)) {
        if (cm_seek_file(*handle, *file_offset, SEEK_SET) != *file_offset) {
            GS_THROW_ERROR(ERR_SEEK_FILE, *file_offset, SEEK_SET, errno);
            return GS_ERROR;
        }

        if (cm_read_file(*handle, buf, size, data_size) != GS_SUCCESS) {
            return GS_ERROR;
        }
        *data_offset = 0;
    }
    ...
    if ((uint32)(*data_size) < sizeof(log_batch_t) + sizeof(log_batch_tail_t)) {
        GS_THROW_ERROR(ERR_FILE_SIZE_MISMATCH, (int64)(*data_size),
    ...
    if (batch->space_size < batch->size || (uint32)(*data_size) < batch->size) {
        GS_THROW_ERROR(ERR_INVALID_BATCH, "batch size");
        *file_finish = GS_TRUE;
        return GS_SUCCESS;
    }

    if (batch->head.magic_num != LOG_MAGIC_NUMBER) {
        GS_THROW_ERROR(ERR_INVALID_BATCH, "head");
        *file_finish = GS_TRUE;
        return GS_SUCCESS;
    }
    ...
    if (tail->magic_num != LOG_MAGIC_NUMBER) {
        GS_THROW_ERROR(ERR_INVALID_BATCH, "tail");
        return GS_ERROR;
    }

    if (batch->head.point.lfn != tail->point.lfn) {
        GS_THROW_ERROR(ERR_INVALID_BATCH, "lfn");
        return GS_ERROR;
    }
...
static status_t batch_decrypt(log_batch_t *batch)
{
    if (batch->encrypted) {
        ...
        if (plain_buf == NULL) {
            GS_THROW_ERROR(ERR_ALLOC_MEMORY, (uint64)batch->space_size, "miner batch decrypt");
            return GS_ERROR;
        }
        uint32 plain_size = batch->space_size;

        if (log_decrypt(NULL, batch, plain_buf, plain_size) != GS_SUCCESS) {
            GS_THROW_ERROR(ERR_INVALID_BATCH, "decrypt batch");
...
{
    bool32 fetch_group_count_is_over = GS_FALSE;
    log_group_t *group = miner_fetch_group(cursor);
    while (group != NULL) {
        if (!is_lfn && start != GS_INVALID_ID64 && group->lsn < start) {
        ...
        if (!is_lfn && count != GS_INVALID_ID32 && *group_count >= count) {
            fetch_group_count_is_over = GS_TRUE;
        }
        group = miner_fetch_group(cursor);
    }
...
{
    GS_RETURN_IFERR(dbs_global_handle()->get_curr_log_offset(buf, (uint32_t)data_size, (uint32_t *)file_offset,
    ...
    int32 *data_size)
{
    ...
    bool32 had_error = GS_FALSE;

    if (miner_verify_batch_checksum(batch, is_checksum) != GS_SUCCESS) {
        had_error = GS_TRUE;
    ...
    bool32 is_checksum, bool32 use_dbstor)
{
    ...
    if (use_dbstor) {
        if (cm_read_file(*handle, buf, (int32)GS_MAX_LOG_BUFFER_SIZE, &data_size) != GS_SUCCESS) {
            return GS_ERROR;
        }
    }
    for (;;) {
        file_finish = GS_FALSE;
        if (!use_dbstor) {
            GS_RETURN_IFERR(miner_load_batch(handle, buf, (int32)GS_MAX_LOG_BUFFER_SIZE, &data_offset,
        ...
        if (is_lfn && start != GS_INVALID_ID64 && batch->head.point.lfn < start) {
            continue;
        }
        if (!is_checksum) {
            ...
            GS_RETURN_IFERR(batch_decrypt(batch));

            fetch_part(batch, &cursor);

            if (fetch_group(&cursor, is_lfn, start, count, &group_count) == GS_TRUE) {
                break;
            }
        }
        batch_count++;

        if (is_lfn && count != GS_INVALID_ID32 && batch_count >= count) {
            break;
        }
    }
    return GS_SUCCESS;
    ...
    bool32 is_lfn, bool32 is_checksum)
{
    ...
    if (cm_open_file(logpath, O_RDONLY | O_BINARY, &handle) != GS_SUCCESS) {
        return GS_ERROR;
    }

    if (cm_read_file(handle, &head, sizeof(log_file_head_t), &data_size) != GS_SUCCESS) {
    ...
    bool32 use_dbstor)
{
    ...
    if (miner_log_init() != GS_SUCCESS) {
        return GS_ERROR;
    }

    char *buf = (char *)malloc(GS_MAX_LOG_BUFFER_SIZE);
    if (buf == NULL) {
        GS_THROW_ERROR(ERR_ALLOC_MEMORY, (uint64)GS_MAX_LOG_BUFFER_SIZE, "miner parse logfile");
        return GS_ERROR;
    }

    errno_t rc_memzero = memset_s(buf, GS_MAX_LOG_BUFFER_SIZE, 0, GS_MAX_LOG_BUFFER_SIZE);
    if (rc_memzero != EOK) {
        CM_FREE_PTR(buf);
        GS_THROW_ERROR(ERR_ALLOC_MEMORY, (uint64)GS_MAX_LOG_BUFFER_SIZE, "miner parse logfile");
        return GS_ERROR;
    ...
    if (!use_dbstor && miner_read_file_head(handle, &head, &file_offset) != GS_SUCCESS) {
        cm_close_file(handle);
        CM_FREE_PTR(buf);
        return GS_ERROR;
    }
...
status_t miner_read_page(int32 handle, char *buf, int64 offset, uint32 page_size)
{
    ...
    if (cm_read_file(handle, buf, page_size, &size) != GS_SUCCESS) {
        return GS_ERROR;
    }
    ...
    if ((uint32)size < page_size) {
        ...
        printf("\n\tcurrent file finished\n");
        GS_THROW_ERROR(ERR_FILE_SIZE_MISMATCH, (uint64)size, (uint64)page_size);
        return GS_ERROR;
    }
...
static void miner_verify_datafile(page_head_t *head, miner_input_def_t *input, int64 i)
{
    ...
    if (tail->checksum != GS_INVALID_CHECKSUM && page_verify_checksum(head, input->page_size) == GS_FALSE) {
        had_error = GS_TRUE;
    ...
    if (input->use_dbstor) {
        uint32 page_id = (uint32)dbs_global_handle()->get_correct_page_id((uint32_t)input->part_num,
...
static status_t miner_parse_datafile(miner_input_def_t *input)
{
    ...
    if (cm_open_file(input->datafile, O_RDONLY | O_BINARY, &handle) != GS_SUCCESS) {
        return GS_ERROR;
    }

    if (input->page_size == 0) {
        cm_close_file(handle);
        GS_THROW_ERROR(ERR_ALLOC_MEMORY, (uint64)input->page_size, "data page");
        return GS_ERROR;
    }
    ...
    if (buf == NULL) {
        cm_close_file(handle);
        GS_THROW_ERROR(ERR_ALLOC_MEMORY, (uint64)input->page_size, "miner parse datafile");
        return GS_ERROR;
    }

    i = (input->start != GS_INVALID_ID64) ? (int64)input->start : 0;

    while (miner_read_page(handle, buf, i * input->page_size, input->page_size) == GS_SUCCESS) {
        ...
        if (input->count != GS_INVALID_ID32 && hack_count >= input->count) {
            break;
        }
...
void miner_init_ctrlfile(database_ctrl_t *ctrl)
{
    ...
    uint32 inst_count = ctrl->core.clustered ? GS_MAX_INSTANCES : 1;
    uint32 offset = ctrl->core.clustered ? (GS_MAX_INSTANCES + CTRL_LOG_SEGMENT) : CTRL_LOG_SEGMENT + 1;
    uint32 count;
...
static status_t miner_parse_ctrlfile(const char *filepath, bool32 is_checksum)
{
    ...
    if (cm_open_file(filepath, O_RDONLY | O_BINARY, &handle) != GS_SUCCESS) {
        return GS_ERROR;
    }
    ...
    if (miner_verify_ctrlfile(&ctrl, is_checksum) != GS_SUCCESS) {
        cm_close_file(handle);
        CM_FREE_PTR(ctrl.pages);
        return GS_ERROR;
    }
...
status_t open_inputfile(char *inputfile, FILE **file)
{
    ...
    if (realpathRes == NULL) {
        GS_THROW_ERROR(ERR_INVALID_FILE_NAME, inputfile, errno);
        return GS_ERROR; // 规范化的错误处理
    }
    *file = fopen(realpathRes, "rb+");
    free(realpathRes);
    realpathRes = NULL;
    if (*file == NULL) {
        GS_THROW_ERROR(ERR_OPEN_FILE, inputfile, errno);
        return GS_ERROR;
    }
    return GS_SUCCESS;
}
    ...
    uint32 buf_size, bak_head_t **bak_head)
{
    ...
    if (cm_open_file(input_file, O_RDWR | O_BINARY | O_SYNC, &handle) != GS_SUCCESS) {
        return GS_ERROR;
    }

    *bak_head = (bak_head_t *)(*read_buf);

    if (cm_read_file(handle, *read_buf, buf_size, &read_size) != GS_SUCCESS) {
        cm_close_file(handle);
        *bak_head = NULL;
        return GS_ERROR;
    }
    ...
    if ((uint32)read_size < sizeof(bak_head_t)) {
        GS_THROW_ERROR(ERR_READ_DEVICE_INCOMPLETE, read_size, sizeof(bak_head_t));
        cm_close_file(handle);
        *bak_head = NULL;
        return GS_ERROR;
    }
...
static status_t miner_parse_backup_head(const char *input_file)
{
    ...
    char *read_buf = (char *)malloc(GS_BACKUP_BUFFER_SIZE);

    if (read_buf == NULL) {
        GS_THROW_ERROR(ERR_ALLOC_MEMORY, (uint64)GS_BACKUP_BUFFER_SIZE, "zminer");
        return GS_ERROR;
    }

    if (miner_parse_backup_info(input_file, &read_buf, GS_BACKUP_BUFFER_SIZE, &bak_head) != GS_SUCCESS) {
                                         ...
                                         bool32 read_end, bak_t *bak)
{
    ...
    uint32 left_offset;
    uint32 write_size;
    uint32 align_size = GS_DFLT_LOG_BLOCK_SIZE;
    errno_t ret;
    ...
    for (;;) {
        ...
        knl_panic(compress_ctx->write_len + left_size <= GS_BACKUP_BUFFER_SIZE);
        ...
        if (cm_write_file(file, compress_buf, write_size) != GS_SUCCESS) {
            return GS_ERROR;
        }

        if (left_size > 0) {
            ret = memmove_s(compress_buf, GS_BACKUP_BUFFER_SIZE, compress_buf + left_offset, left_size);
            if (ret != EOK) {
                GS_THROW_ERROR(ERR_ALLOC_MEMORY, (uint64)GS_MAX_LOG_BUFFER_SIZE, "miner decompress to disk");
                return GS_ERROR;
            }
        }
...
{
    char head_path[GS_FILE_NAME_BUFFER_SIZE];
    text_t head_path_text;
    bak_head_t *bak_head = NULL;

    char output_file[GS_MAX_FILE_NAME_LEN] = { 0 };
    PRTS_PRINT_RETURN_IFERR(snprintf_s(output_file, GS_MAX_FILE_NAME_LEN, GS_MAX_FILE_NAME_LEN - 1,
        "%s%s", outputbase, MINER_BAK_FILE_TYPE));

    if (cm_file_exist(output_file)) {
        GS_THROW_ERROR(ERR_DATAFILE_ALREADY_EXIST, output_file);
        return GS_ERROR;
    }

    *read_buf = (char *)malloc(GS_BACKUP_BUFFER_SIZE + sizeof(bak_t));
    if (*read_buf == NULL) {
        GS_THROW_ERROR(ERR_ALLOC_MEMORY, (uint64)GS_BACKUP_BUFFER_SIZE + sizeof(bak_t), "zminer read buffer");
        return GS_ERROR;
    }

    cm_trim_filename(inputfile, GS_FILE_NAME_BUFFER_SIZE, head_path);
    if (strlen(inputfile) == strlen(head_path)) {
        ...
        cm_concat_string(&head_path_text, GS_FILE_NAME_BUFFER_SIZE, MINER_BACKUP_SET);
        head_path_text.str[head_path_text.len] = '\0';
    }

    if (miner_parse_backup_info(head_path_text.str, read_buf,
        GS_BACKUP_BUFFER_SIZE + sizeof(bak_t), &bak_head) != GS_SUCCESS) {
        CM_FREE_PTR(*read_buf);
        return GS_ERROR;
    }
    *bak = (bak_t *)(*read_buf + GS_BACKUP_BUFFER_SIZE);
    (*bak)->record.attr.compress = bak_head->attr.compress;
    if ((*bak)->record.attr.compress == COMPRESS_NONE) {
        GS_THROW_ERROR(ERR_DECOMPRESS_ERROR, "miner", COMPRESS_NONE, "input file is uncompressed");
        CM_FREE_PTR(*read_buf);
        return GS_ERROR;
    }

    if (cm_open_file(inputfile, O_RDWR | O_BINARY | O_SYNC, in_handle) != GS_SUCCESS) {
        CM_FREE_PTR(*read_buf);
        return GS_ERROR;
    }

    if (cm_create_file(output_file, O_RDWR | O_BINARY | O_SYNC, out_handle) != GS_SUCCESS) {
...
static status_t miner_decompress_log_head(const char *inputfile, char *read_buf, int32 in_handle, int32 out_handle)
{
    ...
    if (cm_text_str_contain_equal_ins(&path_text, "arch", (uint32)strlen("arch")) ||
        ...
        if (cm_read_file(in_handle, read_buf, sizeof(log_file_head_t), &read_size) != GS_SUCCESS) {
            return GS_ERROR;
        }
        ...
        if (cm_read_file(in_handle, read_buf + sizeof(log_file_head_t), fill_size, &read_size) != GS_SUCCESS) {
            return GS_ERROR;
        }
        ...
        if (cm_write_file(out_handle, read_buf, head.block_size) != GS_SUCCESS) {
            return GS_ERROR;
        }
    }
...
{
    int32 out_handle = GS_INVALID_HANDLE;
    int32 in_handle = GS_INVALID_HANDLE;
    int32 read_size;
    ...
    if (miner_decompress_parameters_init(inputfile, outputfile, &read_buf, &bak, &out_handle, &in_handle) == GS_ERROR) {
        return GS_ERROR;
    }

    if (miner_decompress_log_head(inputfile, read_buf, in_handle, out_handle) != GS_SUCCESS) {
        close_decompress_resource(in_handle, out_handle, read_buf, NULL);
        return GS_ERROR;
    }

    compress_ctx.compress_buf.aligned_buf = (char *)malloc(GS_BACKUP_BUFFER_SIZE);
    if (compress_ctx.compress_buf.aligned_buf == NULL) {
        GS_THROW_ERROR(ERR_ALLOC_MEMORY, (uint64)GS_BACKUP_BUFFER_SIZE, "zminer uncomperss buffer");
        close_decompress_resource(in_handle, out_handle, read_buf, NULL);
        return GS_ERROR;
    }

    status_t status = knl_compress_alloc(bak->record.attr.compress, &compress_ctx, GS_FALSE);
    if (status == GS_ERROR) {
        GS_THROW_ERROR(ERR_ALLOC_MEMORY, "zminer uncomperss alloc error");
        close_decompress_resource(in_handle, out_handle, read_buf, &compress_ctx);
        return GS_ERROR;
    }

    status = knl_compress_init(bak->record.attr.compress, &compress_ctx, GS_FALSE);

    while (!read_end && status == GS_SUCCESS) {
        if (cm_read_file(in_handle, read_buf, GS_BACKUP_BUFFER_SIZE, &read_size) != GS_SUCCESS) {
            status = GS_ERROR;
            break;
        }

        read_end = (bool32)((uint32)read_size < GS_BACKUP_BUFFER_SIZE);
    ...
    knl_compress_free(bak->record.attr.compress, &compress_ctx, GS_FALSE);
    close_decompress_resource(in_handle, out_handle, read_buf, &compress_ctx);
    return status;
}
...
static status_t miner_update_version(const char *filepath)
{
    ...
    uint32 offset = GS_DFLT_CTRL_BLOCK_SIZE + sizeof(page_head_t);
    ...
    if (cm_open_file(filepath, O_RDWR | O_BINARY | O_SYNC, &handle) != GS_SUCCESS) {
        return GS_ERROR;
    }
    ...
    if (cm_write_file(handle, &ctrl_version, size) != GS_SUCCESS) {
        cm_close_file(handle);
        return GS_ERROR;
    }
...
static status_t miner_ctrlfile_version(const char *filepath)
{
    ...
    if (ctrl.pages == NULL) {
        cm_close_file(handle);
        GS_THROW_ERROR(ERR_ALLOC_MEMORY, (uint64)size, "page");
        return GS_ERROR;
    }
...
{
    printf("Zminer is an analysis tool for Z-engine.\n"
           "\n"
           "Usage:\n"
           "  zminer [OPTIONS]\n"
           "\nRequired options:\n"
...
static status_t miner_init_input(miner_input_def_t *input)
{
    ...
    input->is_lfn = GS_FALSE;
    input->is_version = GS_FALSE;
    input->modify_version = GS_FALSE;
    input->is_backup = GS_FALSE;
    input->is_backupset = GS_FALSE;
    input->is_checksum = GS_FALSE;
    input->is_force = GS_FALSE;
    input->start = GS_INVALID_ID64;
    input->count = GS_INVALID_ID32;
    input->logfile = NULL;
    input->datafile = NULL;
    input->ctrlfile = NULL;
    input->inputfile = NULL;
    input->has_xid = GS_FALSE;
    input->is_decompress = GS_FALSE;
    input->log_cnt = 0;
    input->xid_cnt = 0;
    input->keya = NULL;
    input->keyb = NULL;
    input->part_num = 0;
    input->serial_num = 0;
    input->use_dbstor = GS_FALSE;
    for (uint8 i = 0; i < GS_REDO_LOG_NUM; i++) {
        input->logfiles[i] = NULL;
    }
    ret = memset_s(input->input_name, GS_FILE_NAME_BUFFER_SIZE, 0, GS_FILE_NAME_BUFFER_SIZE);
    if (ret != EOK) {
...
int32 miner_getopt(int nargc, char *nargv[], const char *ostr)
{
    ...
    GS_RETURN_IFERR(miner_validate_opt(nargc, nargv, &place_str));
...
static status_t miner_get_backup_file(miner_input_def_t *input)
{
    ...
    input->is_backup = GS_TRUE;
    if (input->inputfile == NULL) {
        printf("failed to get file name of backup file");
        return GS_ERROR;
    }
    cm_trim_dir(input->inputfile, GS_FILE_NAME_BUFFER_SIZE, input->input_name);
    if (cm_str_equal(input->input_name, MINER_BACKUP_SET)) {
        input->is_backupset = GS_TRUE;
    }
...
static status_t miner_get_xid(miner_input_def_t *input)
{
    ...
    uint64 xid_number[GS_XID_NUM] = { 0 };
    int cnt = 0;
    while (*xid_str != '\0') {
        if (*xid_str == ',') {
            ++cnt;
...
{
    if (miner_get_logfile(input) != GS_SUCCESS) {
        return GS_ERROR;
    }
    if (miner_get_logfiles(input) != GS_SUCCESS) {
        return GS_ERROR;
    }
    if (input->log_cnt > GS_REDO_LOG_NUM) {
...
{
    if (miner_get_xid(input) == GS_SUCCESS) {
        input->has_xid = GS_TRUE;
    }
    return GS_SUCCESS;
}
...
static inline status_t miner_opt_proc_of_F(miner_input_def_t *input)
{
    input->is_force = GS_TRUE;
    return GS_SUCCESS;
}

static inline status_t miner_opt_proc_of_d(miner_input_def_t *input)
{
    input->is_decompress = GS_TRUE;
    return GS_SUCCESS;
}
...
static status_t miner_get_input(int argc, char *argv[], miner_input_def_t *input)
{
    ...
    while (c != -1) {
        ...
        GS_RETURN_IFERR(opt_proc_func(input));
        c = miner_getopt(argc, argv, "bvuCFdDc:f:l:z:n:p:s:t:A:B:x:P:S:");
    }

    // for kmc keys, need both null or both not null
    if (input->keya != NULL && input->keyb != NULL) {
        GS_RETURN_IFERR(cm_kmc_init(GS_SERVER, input->keya, input->keyb));
        GS_RETURN_IFERR(cm_kmc_init_domain(GS_KMC_KERNEL_DOMAIN));
        return GS_SUCCESS;
    }
    ...
    GS_THROW_ERROR(ERR_INVALID_PARAMETER, "-A or -B");
    return GS_ERROR;
}
...
status_t miner_verify_datafile_version(char *file_name, uint32 size)
{
    ...
    if (cm_open_file(file_name, O_RDONLY | O_BINARY, &handle) != GS_SUCCESS) {
        return GS_ERROR;
    }

    if (size == 0) {
        cm_close_file(handle);
        GS_THROW_ERROR(ERR_ALLOC_MEMORY, (uint64)size, "datafile page");
        return GS_ERROR;
    }
    ...
    if (buf == NULL) {
        cm_close_file(handle);
        GS_THROW_ERROR(ERR_ALLOC_MEMORY, (uint64)size, "parse datafile");
        return GS_ERROR;
    }
    ...
    if (status == GS_SUCCESS) {
        ...
        if (TBOX_DATAFILE_VERSION != ctrl_bk->version) {
            status = GS_ERROR;
            GS_THROW_ERROR(ERR_PARAMETER_NOT_MATCH, "ZTBOX DATAFILE STRUCTURE VERSION",
...
status_t miner_verify_logfile_version(char *file_name, uint32 size)
{
    ...
    if (size == 0) {
        cm_close_file(handle);
        GS_THROW_ERROR(ERR_ALLOC_MEMORY, (uint64)size, "logfile page");
        return GS_ERROR;
    }
...
status_t miner_verify_corefile_version(char *file_name, uint32 page_size)
{
    ...
    int32 read_size;
    int32 handle;
    uint32 size = (CORE_CTRL_PAGE_ID + 1) * GS_DFLT_CTRL_BLOCK_SIZE;
...
status_t miner_check_file_version(miner_input_def_t *input)
{
    ...
    // core ctrl
        ...
        ctrl_head_size = (uint32)(CORE_CTRL_PAGE_ID + 1) * GS_DFLT_CTRL_BLOCK_SIZE;
    ...
    // several redo logs
        ...
        for (int i = 0; i < GS_REDO_LOG_NUM; i++) {
            if (input->logfiles[i] != NULL && ret == GS_SUCCESS) {
...
static status_t miner_check_input(miner_input_def_t *input)
{
    if (input->datafile == NULL && input->logfile == NULL && input->ctrlfile == NULL && !input->is_backupset) {
        if (input->inputfile != NULL && input->is_backup) {
            printf("must set output filename to decompress, zminer -z backup file -f output file\n");
        }
        printf("try use \"--help\" for more information.\n");
        GS_THROW_ERROR(ERR_INVALID_PARAMETER, "zminer input parameter");
        return GS_ERROR;
    }
    ...
    if (input->page_size == 0) {
        GS_THROW_ERROR(ERR_INVALID_PARAMETER, "page size");
        return GS_ERROR;
    }

    // force excute, ignore verify version
    if (input->is_force == GS_TRUE) {
        return GS_SUCCESS;
    }
    ...
    if (input->inputfile != NULL) {
        if (input->is_backup) {
            if (miner_check_backupset_version(input->inputfile) == GS_ERROR) {
                return GS_ERROR;
            }
        }
        return GS_SUCCESS;
    ...
    if (miner_need_backupset_file(input, &input_file) == GS_TRUE) {
...
{
    msg->buf = (char *)malloc(GS_MAX_LOG_BUFFER_SIZE);
    ...
    errno_t rc_memzero = memset_s(msg->buf, GS_MAX_LOG_BUFFER_SIZE, 0, GS_MAX_LOG_BUFFER_SIZE);
    if (rc_memzero != EOK) {
        CM_FREE_PTR(msg->buf);
        GS_THROW_ERROR(ERR_ALLOC_MEMORY, (uint64)GS_MAX_LOG_BUFFER_SIZE, "miner parse logfile");
        return GS_ERROR;
    }
    if (cm_open_file(msg->filedir, O_RDONLY | O_BINARY, &msg->handle) != GS_SUCCESS) {
        CM_FREE_PTR(msg->buf);
        return GS_ERROR;
    }

    if (miner_read_file_head(msg->handle, &msg->head, file_offset) != GS_SUCCESS) {
...
{
    bool32 fetch_group_is_over = GS_FALSE;
    log_group_t *group = miner_fetch_group(cursor);
    uint32 group_count = 0;
    ...
    bool32 use_dbstor)
{
    ...
    for (;;) {
        file_finish = GS_FALSE;
        ret = miner_load_batch(&msg->handle, msg->buf, (int32)GS_MAX_LOG_BUFFER_SIZE, &data_offset,
                               &data_size, &file_offset, &batch, &file_finish);
        if (ret == GS_ERROR) {
            break;
        }
...
{
    miner_log_file_t log_files_msg[GS_REDO_LOG_NUM];
    int64 file_offset[GS_REDO_LOG_NUM] = {0};
    tx_msg_t tx_msg[GS_XID_NUM];
    ...
    for (uint8 i = 0; i < input->log_cnt; i++) {
        ...
        if (miner_parse_log_file_msg(&log_files_msg[i], &file_offset[i]) != GS_SUCCESS) {
            return GS_ERROR;
        }
    }
    ...
    for (uint8 i = 0; i < input->log_cnt; i++) {
        if (miner_parse_each_logfile(log_files_msg[i].filedir, start, count, is_lfn, is_checksum, &log_files_msg[i],
            file_offset[i], input->has_xid, tx_msg, input->xid_cnt, input->use_dbstor) != GS_SUCCESS) {
            return GS_ERROR;
        }
    }
    return GS_SUCCESS;
...
status_t miner_execute(int argc, char *argv[])
{
    ...
    if (miner_init_input(&input) != GS_SUCCESS) {
        return GS_ERROR;
    }
    ...
    if (miner_get_input(argc, argv, &input) != GS_SUCCESS) {
        GS_THROW_ERROR(ERR_INVALID_PARAMETER, "zminer input parameter");
    ...
    if (miner_check_input(&input) != GS_SUCCESS) {
        miner_free_input(&input);
        return GS_ERROR;
    }

    if (input.ctrlfile != NULL && input.is_version) {
        if (miner_ctrlfile_version(input.ctrlfile) == GS_ERROR) {
    ...
    if (input.ctrlfile != NULL && input.modify_version) {
        if (miner_update_version(input.ctrlfile) == GS_ERROR) {
            printf("%s is not a ctrlfile", input.ctrlfile);
            miner_free_input(&input);
            return GS_ERROR;
        }
    ...
    if (miner_parse_input(&input) != GS_SUCCESS) {
        miner_free_input(&input);
        return GS_ERROR;
    }
