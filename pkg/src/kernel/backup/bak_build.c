                                 ...
                                 build_param_ctrl_t *ctrl)
{
    ...
    attr->level = (ctrl->is_increment || ctrl->is_repair) ? CT_TRUE : CT_FALSE;
    ...
    errno_t ret = strncpy_s(bak->peer_host, CT_HOST_NAME_BUFFER_SIZE, cm_inet_ntop(
        (struct sockaddr *)&pipe->link.tcp.remote.addr, ipstr, CM_MAX_IP_LEN), CT_HOST_NAME_BUFFER_SIZE - 1);
    knl_securec_check(ret);

    CT_LOG_RUN_INF("[BUILD] is increment %u, base lsn %llu ", ctrl->is_increment, ctrl->base_lsn);
}
...
bool32 knl_brain_repair_check(knl_session_t *session)
{
    ...
    if ((kernel->lrcv_ctx.dbid != 0) && (kernel->lrcv_ctx.dbid != kernel->db.ctrl.core.dbid)) {
        CT_LOG_RUN_ERR("[REPAIR] primary dbid [%u] is not equal to standby dbid [%u], need repair",
            kernel->lrcv_ctx.dbid, kernel->db.ctrl.core.dbid);
        CT_THROW_ERROR(ERR_INVALID_OPERATION, ", primary dbid is not equal to standby dbid.");
        return CT_FALSE;
    }

    if (reset_log->last_asn > rcy->asn || rcy->rst_id >= reset_log->rst_id) {
        CT_LOG_RUN_ERR("[REPAIR] reset_log and rcy point not matched. reset_log rst_id-asn-lfn: %u-%u-%llu,"
            "rcy point rst_id-asn-lfn: %u-%u-%llu.", reset_log->rst_id, reset_log->last_asn, reset_log->last_lfn,
            rcy->rst_id, rcy->asn, (uint64)rcy->lfn);
        CT_THROW_ERROR(ERR_INVALID_OPERATION, ", asn of reset_log is larger than asn of rcy or rst_id of rcy is larger "
    ...
    if (rcy->asn - reset_log->last_asn >= log_count) {
        CT_LOG_RUN_ERR("[REPAIR] the count of log between rcy(asn: %u) and reset_log(asn: %u) is larger than "
            "log_count %u.", rcy->asn, reset_log->last_asn, log_count);
        CT_THROW_ERROR(ERR_INVALID_OPERATION, ", log generated by standby is too large.");
        return CT_FALSE;
    }
    ...
    if (reset_log->rst_id - rcy->rst_id > 1) {
        CT_LOG_RUN_ERR("[REPAIR] reset_log and rcy point not matched. reset_log rst_id-asn-lfn: %u-%u-%llu,"
            "rcy point rst_id-asn-lfn: %u-%u-%llu.", reset_log->rst_id, reset_log->last_asn, reset_log->last_lfn,
            rcy->rst_id, rcy->asn, (uint64)rcy->lfn);
        CT_THROW_ERROR(ERR_INVALID_OPERATION, ", rst_id of reset_log and rcy is not mapped.");
        return CT_FALSE;
    }

    if (reset_log->last_lfn > rcy->lfn) {
        CT_LOG_RUN_ERR("[REPAIR] primary lfn [%llu] is larger than standby rcy lfn [%llu]", reset_log->last_lfn, (uint64)rcy->lfn);
        CT_THROW_ERROR(ERR_INVALID_OPERATION, ", primary lfn is larger than standby rcy lfn.");
        return CT_FALSE;
    }
...
status_t build_analyze_mem_init(knl_session_t *session)
{
    ...
    if (cm_aligned_malloc(buf_size, "build log analysis", &bak->build_aly_mem) != CT_SUCCESS) {
        return CT_ERROR;
    }
    ...
    if (cm_aligned_malloc(CT_MAX_BATCH_SIZE, "build analyze log buffer", &bak->log_buf) != CT_SUCCESS) {
...
static void brain_repair_invalidate_page(bak_t *bak, page_id_t *page_id)
{
    ...
    while (item != NULL) {
        if (IS_SAME_PAGID(*item->page_id, *page_id)) {
            item->page_id->file = CT_INVALID_FILEID;
            break;
        }
        item = item->next;
    }
...
status_t build_aly_add_page(knl_session_t *session, page_id_t page_id)
{
    ...
    if (new_item == NULL) {
        CT_LOG_RUN_ERR("[BUILD] pages changed by standby is too much.");
        CT_THROW_ERROR(ERR_INVALID_OPERATION, ", pages changed by standby is too much.");
        return CT_ERROR;
    }

    CT_LOG_DEBUG_INF("[REPAIR] standby find page %u-%u", page_id.file, page_id.page);
...
status_t build_find_batch(knl_session_t *session, device_type_t type, int32 *handle, log_file_head_t *head,
                          log_point_t *point, uint32 *data_size, aligned_buf_t *align_buf)
{
    ...
    if (file_size == -1) {
        CT_LOG_RUN_ERR("[BUILD] failed to get log [%u-%u] size ", point->rst_id, point->asn);
    ...
    do {
        ...
        if (cm_read_device(type, *handle, (int64)offset, buf, (int32)size) != CT_SUCCESS) {
            CT_LOG_RUN_ERR("[BUILD] Failed to read log [%u-%u] file", point->rst_id, point->asn);
            return CT_ERROR;
        }
        ...
        while (remain_size >= sizeof(log_batch_t)) {
            if (!rcy_validate_batch(batch, tail) || batch->head.point.rst_id != head->rst_id ||
                ...
                finished = CT_TRUE;
                CT_LOG_RUN_INF("[BUILD] log [%u-%u] offset %llu invalid batch size %u "
            ...
            if (batch->head.point.lfn == point->lfn) {
                ...
                if (session->kernel->lrcv_ctx.primary_reset_log_scn == batch->scn ||
                    ...
                    *data_size = 0;
                    CT_LOG_RUN_ERR("[BUILD] For same lfn [%llu] in resetlog, scn from primary and standby [%llu/%llu] "
    ...
    aligned_buf_t *align_buf)
{
    ...
    if (cm_open_device(file->ctrl->name, file->ctrl->type, knl_redo_io_flag(session), &handle) != CT_SUCCESS) {
        CT_LOG_RUN_ERR("[BUILD] failed to open %s", file->ctrl->name);
        return CT_ERROR;
    }
    ...
    if (cm_read_device(file->ctrl->type, handle, 0, tmp_head, (int32)size) != CT_SUCCESS) {
        cm_close_device(file->ctrl->type, &handle);
        CT_LOG_RUN_ERR("[BUILD] failed to read %s ", file->ctrl->name);
        return CT_ERROR;
    }
    ...
    if (build_find_batch(session, file->ctrl->type, &handle, &head, point, data_size, align_buf) != CT_SUCCESS) {
        cm_close_device(file->ctrl->type, &handle);
        return CT_ERROR;
    }
...
static status_t bak_build_prepare(knl_session_t *session)
{
    ...
    if (buf == NULL) {
        CT_THROW_ERROR(ERR_ALLOC_MEMORY, (uint64)size, "backup");
        return CT_ERROR;
    }
     ...
     * so remote->send_pack is offset by 2 * CT_BACKUP_BUFFER_SIZE compared to bak->backup_buf
     */
     ...
     * remote->send_pack, so remote->recv_pack is offset by 2 * CT_BACKUP_BUFFER_SIZE and sizeof(cs_packet_t) compared
     * to bak->backup_buf
     */
    ...
    // if return ct_error, free ctrl_data_buf in bak_build_end
    bak->ctrl_data_buf = (char *)malloc(CTRL_MAX_PAGES(session) * CT_DFLT_CTRL_BLOCK_SIZE);
    if (bak->ctrl_data_buf == NULL) {
        CT_THROW_ERROR(ERR_ALLOC_MEMORY, (uint32)(CTRL_MAX_PAGES(session) * CT_DFLT_CTRL_BLOCK_SIZE), "build");
        return CT_ERROR;
    }
...
void bak_build_end(knl_session_t *session)
{
    ...
    CT_LOG_RUN_INF("[BUILD] remote backup finished, release resources");
    ...
    bak_free_compress_context(session, CT_FALSE);
    ...
    if (bak->failed && !bak->need_retry) {
        ...
        CT_LOG_RUN_INF("[BUILD] build failed, error code :%d, %s", error_info->err_code, error_info->err_msg);
    }
    ...
    if (rst_delete_track_file(session, bak, CT_TRUE) != CT_SUCCESS) {
        CT_LOG_RUN_WAR("[BUILD] failed to delete bak process file.");
    }
...
static status_t bak_build_connect(knl_session_t *session)
{
    ...
    if (lrcv_get_primary_server(session, CT_BACKUP_RETRY_COUNT, server_host, CT_HOST_NAME_BUFFER_SIZE,
        &port) != CT_SUCCESS) {
        return CT_ERROR;
    }
    ...
    CT_LOG_RUN_INF("[BUILD] remote backup, standby connect to primary %s", url);

    remote->pipe->connect_timeout = REPL_CONNECT_TIMEOUT;
    remote->pipe->socket_timeout = REPL_SOCKET_TIMEOUT;
    arch_get_bind_host(session, server_host, bind_host, CT_HOST_NAME_BUFFER_SIZE);
    if (cs_connect(url, remote->pipe, bind_host, NULL, NULL) != CT_SUCCESS) {
        CT_LOG_RUN_ERR("[BACKUP] failed to connect %s", url);
        return CT_ERROR;
    }

    if (knl_login(session, remote->pipe, REP_LOGIN_BACKUP, (const char *)bind_host, NULL) != CT_SUCCESS) {
        CT_LOG_RUN_ERR("[BACKUP] login failed");
        return CT_ERROR;
    }
...
static status_t bak_build_start(knl_session_t *session)
{
    ...
    if (bak_build_prepare(session) != CT_SUCCESS) {
        bak->failed = CT_TRUE;
        return CT_ERROR;
    }

    for (uint32 i = 0; i < BUILD_MAX_RETRY; i++) {
        if (bak_build_connect(session) == CT_SUCCESS) {
            return CT_SUCCESS;
        }
    }

    bak->need_retry = CT_TRUE;
    return CT_ERROR;
}
...
static status_t rst_read_start_msg(knl_session_t *session, bak_start_msg_t *start_msg)
{
    ...
    if (rst_agent_read_head(bak, BAK_PKG_FILE_START, &bak->remote.remain_data_size, &read_end) != CT_SUCCESS) {
        return CT_ERROR;
    }

    if (bak_agent_recv(bak, (char *)start_msg, sizeof(bak_start_msg_t)) != CT_SUCCESS) {
        return CT_ERROR;
    }
    ...
    CT_LOG_RUN_INF("[RESTORE]: type:%u, file_id:%u, curr_file_index:%u", start_msg->type, start_msg->file_id,
        start_msg->curr_file_index);
    if (start_msg->type != BAK_MSG_TYPE_HEAD) {
        if (rst_restart_from_file_end(bak, start_msg)) {
            CT_LOG_RUN_INF("[BUILD] break-point build restart from end of file [%u/%u]", bak->file_count,
                start_msg->curr_file_index);
            if (bak->file_count != start_msg->curr_file_index - 1) {
                CT_LOG_RUN_INF("[BUILD] break-point build restart from unexpected position");
                return CT_ERROR;
            }
            bak->file_count++;
        }
...
static status_t rst_build_receive_files(knl_session_t *session)
{
    ...
    while (!bak->failed && stage != BACKUP_HEAD_STAGE) {
        ...
        if (type >= BACKUP_DATA_FILE && type <= BACKUP_ARCH_FILE && bak_paral_task_enable(session)) {
            if (rst_stream_read_file(session) != CT_SUCCESS) {
                return CT_ERROR;
            }
        } else {
...
static status_t rst_restore_keyfile(knl_session_t *session, const char *buf, uint32 buf_size, char *file_name)
{
    ...
    if (cm_file_exist(file_name)) {
        if (cm_remove_file(file_name) != CT_SUCCESS) {
            CT_LOG_RUN_ERR("[RESTORE] failed to remove key file %s", file_name);
            return CT_ERROR;
        }
    }

    if (cm_open_file(file_name, O_RDWR | O_CREAT | O_SYNC, &handle) != CT_SUCCESS) {
        return CT_ERROR;
    }
    if (cm_chmod_file(S_IRUSR | S_IWUSR, handle) != CT_SUCCESS) {
        cm_close_file(handle);
        CT_LOG_RUN_ERR("[RESTORE] failed to modify key file %s permissions", file_name);
        return CT_ERROR;
    }
    ...
    if (file_size < 0) {
        cm_close_file(handle);
        CT_LOG_RUN_ERR("[RESTORE] file seek fail %s", file_name);
        return CT_ERROR;
    }

    file_size = ((keyfile_ctrl_t *)buf)->size;
    knl_panic(sizeof(keyfile_ctrl_t) + file_size <= CT_KMC_MAX_KEY_SIZE);

    if (cm_write_file(handle, buf + sizeof(keyfile_ctrl_t), (int32)file_size) != CT_SUCCESS) {
        cm_close_file(handle);
        CT_LOG_RUN_ERR("[RESTORE] fail to write file %s", file_name);
        return CT_ERROR;
    }
    cm_close_file(handle);
...
static status_t rst_build_keyfile(knl_session_t *session, bak_head_t *head, int32 read_size)
{
    ...
    if (file_size <= 0) {
        CT_LOG_RUN_INF("[BUILD] build without ssl, won't reset keyfile");
        return CT_SUCCESS;
    }

    for (uint32 i = 0; i < CT_KMC_MAX_KEYFILE_NUM; i++) {
        if (rst_restore_keyfile(session, (char *)head, (uint32)read_size,
            session->kernel->attr.kmc_key_files[i].name) != CT_SUCCESS) {
            return CT_ERROR;
        }
    }
...
static status_t rst_build_head(knl_session_t *session)
{
    ...
    if (bak->is_building) {
        buf_size += CT_KMC_MAX_KEY_SIZE;
        expect_size += CT_KMC_MAX_KEY_SIZE;
    }

    if (rst_read_data(ctx, (void *)head, buf_size, &read_size, &read_end, 0) != CT_SUCCESS) {
        return CT_ERROR;
    }

    if (rst_read_check_size(read_size, expect_size, "backupset head") != CT_SUCCESS) {
        return CT_ERROR;
    }

    if (bak->is_building) {
        if (rst_build_keyfile(session, head, read_size) != CT_SUCCESS) {
            CT_LOG_RUN_ERR("[BUILD] rst build keyfile failed.");
            return CT_ERROR;
        }
        head = (bak_head_t *)((char *)head + CT_KMC_MAX_KEY_SIZE);
    }

    if (rst_set_head(session, head, CT_FALSE) != CT_SUCCESS) {
        return CT_ERROR;
    }
...
static status_t build_analyze_entry(knl_session_t *session, log_entry_t *log)
{
    if (RD_TYPE_IS_ENTER_PAGE(log->type)) {
        ...
        if (build_aly_add_page(session, page_id) != CT_SUCCESS) {
            return CT_ERROR;
        }
    }
    return CT_SUCCESS;
...
static status_t build_analyze_batch(knl_session_t *session, log_batch_t *batch)
{
    ...
    while (group != NULL) {
        if (build_analyze_group(se, group) != CT_SUCCESS) {
            return CT_ERROR;
        }
    ...
    uint32 data_size_input, bool32 *need_more_log)
{
    ...
    while (data_size >= sizeof(log_batch_id_t)) {
        ...
        if (data_size < batch->space_size) {
            if (first_batch) {
                *need_more_log = CT_FALSE;
                CT_LOG_RUN_ERR("[BUILD] analyze failed, invalid batch(%u,%u).", point->asn, point->block_id);
                return CT_ERROR;
            }
            *need_more_log = CT_TRUE;
            return CT_SUCCESS;
        ...
        if (rcy_verify_checksum(session, batch) != CT_SUCCESS) {
            *need_more_log = CT_FALSE;
            CT_LOG_RUN_ERR("[BUILD] analyze failed");
            return CT_ERROR;
        }

        if (batch->encrypted) {
            if (log_decrypt(session, batch, ctx->logwr_cipher_buf, ctx->logwr_cipher_buf_size) != CT_SUCCESS) {
                *need_more_log = CT_FALSE;
                CT_LOG_RUN_ERR("[BUILD] analyze failed");
                return CT_ERROR;
            }
        ...
        if (build_analyze_batch(session, batch) != CT_SUCCESS) {
            *need_more_log = CT_FALSE;
            CT_LOG_RUN_ERR("[BUILD] analyze failed");
            return CT_ERROR;
        }
...
static status_t build_analyze_redo(knl_session_t *session, log_point_t *curr_point)
{
    ...
    while (bak_load_log_batch(session, curr_point, &data_size, &log_buf, &block_size) == CT_SUCCESS) {
        batch = (log_batch_t *)log_buf.aligned_buf;

        if (build_analyze(session, curr_point, batch, block_size, data_size, &need_more_log) != CT_SUCCESS) {
            cm_aligned_free(&log_buf);
            CT_THROW_ERROR(ERR_INVALID_OPERATION, ", analyze log failed");
            return CT_ERROR;
        }
...
static status_t build_repair_prepare(knl_session_t *session, uint64 *base_lsn)
{
    ...
    CT_LOG_RUN_INF("[REPAIR] analysis primary resetlog rstid-asn-lfn: %u-%u-%llu", resetlog.rst_id, resetlog.last_asn,
        resetlog.last_lfn);
    if (log_load(session) != CT_SUCCESS) {
        return CT_ERROR;
    }

    if (build_load_batch(session, &target_point, &data_size, &log_buf) != CT_SUCCESS) {
        CT_THROW_ERROR(ERR_INVALID_OPERATION, ", can not find resetlog point in standby.");
        return CT_ERROR;
    }
    ...
    CT_LOG_RUN_INF("[REPAIR] analysis start point rstid-asn-lfn: %u-%u-%llu", target_point.rst_id, target_point.asn,
        (uint64)target_point.lfn);
    if (build_analyze_redo(session, &target_point) != CT_SUCCESS) {
        return CT_ERROR;
    }
    CT_LOG_RUN_INF("[REPAIR] analysis end point rstid-asn-lfn: %u-%u-%llu", target_point.rst_id, target_point.asn,
        (uint64)target_point.lfn);

    if (target_point.lfn < lrp->lfn) {
        CT_THROW_ERROR(ERR_INVALID_OPERATION, ", analysis end lfn is smaller than lrp lfn.");
        return CT_ERROR;
    }
    return CT_SUCCESS;
}
...
bool32 rst_logfile_not_changed(knl_instance_t *kernel, log_file_ctrl_t *log_old, log_file_ctrl_t *log_new)
{
    ...
    ret = strcpy_sp(name, CT_FILE_NAME_BUFFER_SIZE, log_new->name);
    knl_securec_check(ret);

    if (LOG_IS_DROPPED(log_old->flg) != LOG_IS_DROPPED(log_new->flg)) {
        CT_LOG_RUN_ERR("[BUILD] primary's log flag has changed old: %u, new: %u", log_old->flg, log_new->flg);
        return CT_FALSE;
    }
    if (db_change_storage_path(&kernel->attr.log_file_convert, name, CT_FILE_NAME_BUFFER_SIZE) != CT_SUCCESS) {
        return CT_FALSE;
    }
    if (!cm_str_equal(log_old->name, name)) {
        CT_LOG_RUN_ERR("[BUILD] primary's log amount/name has changed old: %s, new: %s", log_old->name, name);
        return CT_FALSE;
    }
...
bool32 rst_datafile_not_changed(knl_instance_t *kernel, datafile_ctrl_t *df_old, datafile_ctrl_t *df_new)
{
    ...
    ret = strcpy_sp(name, CT_FILE_NAME_BUFFER_SIZE, df_new->name);
    knl_securec_check(ret);

    if (df_old->used != df_new->used || (df_old->flag & DATAFILE_FLAG_ONLINE) != (df_new->flag & DATAFILE_FLAG_ONLINE)) {
        CT_LOG_RUN_ERR("[BUILD] primary's data flag has changed old_used: %u, new_used: %u, old_flag: %u,"
            "new_flag: %u", df_old->used, df_new->used, df_old->flag, df_new->flag);
        return CT_FALSE;
    }

    if (df_old->create_version != df_new->create_version) {
        CT_LOG_RUN_ERR("[BUILD] primary's data version has changed old: %u, new: %u", df_old->create_version,
            df_new->create_version);
        return CT_FALSE;
    }
    if (db_change_storage_path(&kernel->attr.data_file_convert, name, CT_FILE_NAME_BUFFER_SIZE) != CT_SUCCESS) {
        return CT_FALSE;
    }
    if (!cm_str_equal(df_old->name, name)) {
        CT_LOG_RUN_ERR("[BUILD] primary's data amount/name has changed old: %s, new: %s", df_old->name, name);
        return CT_FALSE;
    }
...
bool32 rst_db_files_not_changed(knl_session_t *session, ctrl_page_t *new_ctrl)
{
    ...
    for (i = 0; i < CT_MAX_LOG_FILES; i++) {
        ...
        if (rst_logfile_not_changed(kernel, log_old, log_new) != CT_TRUE) {
            return CT_FALSE;
        }
    }

    count = CTRL_MAX_BUF_SIZE / sizeof(log_file_ctrl_t);
    offset = offset + (CT_MAX_LOG_FILES - 1) / count + 1;
    ...
    for (i = 0; i < CT_MAX_DATA_FILES; i++) {
        ...
        if (rst_datafile_not_changed(kernel, df_old, df_new) != CT_TRUE) {
            return CT_FALSE;
        }
    }
...
status_t rst_build_update_logfiles(knl_session_t *session, bak_process_t *ctx)
{
    ...
    for (uint32 i = 0; i < dtc_my_ctrl(session)->log_hwm; i++) {
        ...
        if (LOG_IS_DROPPED(logfile->flg)) {
            ...
            if (cm_remove_device(ctx->log_type[i], ctx->logfile_name[i]) != CT_SUCCESS) {
                return CT_ERROR;
            }
            continue;
        }

        if (cm_exist_device(logfile->type, logfile->name) && *handle != CT_INVALID_HANDLE) {
            cm_close_device(ctx->log_type[i], handle);
            if (cm_open_device(logfile->name, logfile->type, knl_redo_io_flag(session), handle) != CT_SUCCESS) {
                CT_LOG_RUN_ERR("[RESTORE] failed to open %s ", logfile->name);
                return CT_ERROR;
            }
            continue;
        }
...
static status_t rst_load_ctrlfile(knl_session_t *session)
{
    ...
    if (ctrlfiles.len == 0) {
        CT_LOG_RUN_INF("[BUILD] ctrl file does not exist");
    ...
    node->archived_end = 0;
    db->ctrl.core.build_completed = CT_FALSE;
...
static status_t rst_check_ctrl_file(knl_session_t *session)
{
    ...
    CT_LOG_RUN_INF("[BUILD] start read ctrl file");
    if (rst_read_data(ctx, bak->ctrl_data_buf, CTRL_MAX_PAGES(session) * CT_DFLT_CTRL_BLOCK_SIZE,
                      &read_size, &read_end, 0) != CT_SUCCESS) {
        return CT_ERROR;
    }
    CT_LOG_RUN_INF("[BUILD] ctrl file read_size : %u", read_size);

    if (bak_agent_send_pkg(bak, BAK_PKG_ACK) != CT_SUCCESS) {
        return CT_ERROR;
    }

    if (rst_load_ctrlfile(session) != CT_SUCCESS) {
        return CT_ERROR;
    }
    ...
    CT_LOG_RUN_INF("[BUILD] start check file amount/name");
...
static status_t rst_build_restore(knl_session_t *session)
{
    ...
    if (rst_check_ctrl_file(session) != CT_SUCCESS) {
        return CT_ERROR;
    }

    if (rst_build_receive_files(session) != CT_SUCCESS) {
        return CT_ERROR;
    }

    if (rst_build_head(session) != CT_SUCCESS) {
        return CT_ERROR;
    }
...
static status_t bak_obtain_build_progress(bak_t *bak, bak_process_t *proc, const char *file_name)
{
    ...
    if (cm_open_file(file_name, O_BINARY | O_RDWR, &handle) != CT_SUCCESS) {
        CT_LOG_RUN_ERR("[BACKUP] failed to open %s ", file_name);
        return CT_ERROR;
    }

    if (cm_read_file(handle, build_progress, sizeof(build_progress_t), NULL) != CT_SUCCESS) {
        cm_close_file(handle);
        CT_LOG_RUN_ERR("[BACKUP] failed to read %s ", file_name);
        return CT_ERROR;
    }
    cm_close_file(handle);
    CT_LOG_RUN_INF("[BUILD] write build progress [%u/%u/%llu/%u/%u/%u/%d]", build_progress->stage,
    ...
    if (!bak_parameter_is_valid(build_progress)) {
        CT_LOG_RUN_ERR("[BACKUP] parameters are not effective in %s ", file_name);
        return CT_ERROR;
    }
...
static status_t bak_agent_send_build_ctrl(knl_session_t *session, bak_t *bak, bak_process_t *proc)
{
    ...
    CT_LOG_DEBUG_INF("[BACKUP] send type %d", head.cmd);
    // send head
    if (bak_agent_send(bak, (char *)&head, sizeof(bak_agent_head_t)) != CT_SUCCESS) {
        return CT_ERROR;
    }
    // send compress param
    if (bak_agent_send(bak, (char *)&ctrl, sizeof(build_param_ctrl_t)) != CT_SUCCESS) {
        return CT_ERROR;
    }
    ...
    if (cm_file_exist((const char *)bak_process_file)) {
        if (bak_obtain_build_progress(bak, proc, bak_process_file) != CT_SUCCESS) {
            CT_THROW_ERROR(ERR_INVALID_OPERATION, ", building process file is not effective.");
            return CT_ERROR;
        }
        bak->need_check = CT_TRUE;
        bak->is_first_link = CT_FALSE;
    } else {
    ...
    if (bak_agent_send(bak, (char *)build_progress, sizeof(build_progress_t)) != CT_SUCCESS) {
        return CT_ERROR;
    }

    if (rst_agent_read(bak, (char *)&primary_start_time, sizeof(int32), &read_size, &read_end) != CT_SUCCESS) {
        return CT_ERROR;
    }

    if (bak->is_first_link) {
        build_progress->start_time = primary_start_time;
        CT_LOG_RUN_INF("[BUILD] first link [primary/standby] start time : [%d/%d]", primary_start_time,
            build_progress->start_time);
    } else {
        if (build_progress->start_time != primary_start_time) {
            CT_LOG_RUN_INF("[BUILD] [primary/standby] start time : [%d/%d]", primary_start_time,
                build_progress->start_time);
            CT_LOG_RUN_INF("[BUILD] primary has restart or timeout, only non-breakpoint building is allowed");
            if (rst_delete_track_file(session, bak, CT_FALSE) != CT_SUCCESS) {
                return CT_ERROR;
            }
...
static status_t rst_build_prepare_basefiles(knl_session_t *session, bak_process_t *ctx)
{
    ...
    for (i = 0; i < CT_MAX_DATA_FILES; i++) {
        ...
        if (cm_open_device(df->ctrl->name, df->ctrl->type, knl_io_flag(session), &ctx->datafiles[i]) != CT_SUCCESS) {
            CT_LOG_RUN_ERR("[BUILD] failed to open %s ", df->ctrl->name);
            return CT_ERROR;
        }
    ...
    for (i = 0; i < MY_LOGFILE_SET(session)->logfile_hwm; i++) {
        ...
        ret = strcpy_sp(ctx->logfile_name[i], CT_FILE_NAME_BUFFER_SIZE, log->name);
        knl_securec_check(ret);
...
static status_t bak_check_keep_alive(knl_session_t *session)
{
    ...
    if (bak->build_retry_time == build_keep_alive_timeout) {
        bak->need_retry = CT_FALSE;
        CT_THROW_ERROR(ERR_INVALID_OPERATION, ", please check run log for information of building timeout.");
        return CT_ERROR;
    }
...
static status_t bak_agent_send_diff_pages(knl_session_t *session)
{
    ...
    CT_LOG_RUN_INF("[BUILD] start to send page list to primary, page_count: %u", page_count);
    head.ver = BAK_AGENT_PROTOCOL;
    ...
    if (bak_agent_send(bak, (char *)bak->build_aly_pages, page_count * sizeof(page_id_t)) != CT_SUCCESS) {
        return CT_ERROR;
    }
    if (bak_agent_wait_pkg(bak, BAK_PKG_ACK) != CT_SUCCESS) {
        return CT_ERROR;
    }
    CT_LOG_RUN_INF("[BUILD] send page list to primary successfully");
    return CT_SUCCESS;
}
...
{
    int32 handles[CT_MAX_DATA_FILES];
    errno_t ret = memset_sp(handles, CT_MAX_DATA_FILES * sizeof(int32), -1, CT_MAX_DATA_FILES * sizeof(int32));
    knl_securec_check(ret);
    build_analyse_item_t *item = NULL;
    bak_t *bak = &session->kernel->backup_ctx.bak;
    ...
    for (uint32 i = 0; i < bak->page_count; i++) {
        item = &bak->build_aly_items[i];
        if (item->page_id->file == CT_INVALID_FILEID) {
            continue;
        }

        datafile_t *df = DATAFILE_GET(session, item->page_id->file);
        if (cm_open_device(df->ctrl->name, df->ctrl->type, knl_io_flag(session), &handles[item->page_id->file]) != CT_SUCCESS) {
            CT_LOG_RUN_ERR("[REPAIR] failed to open datafile %s", df->ctrl->name);
            return CT_ERROR;
        }
        int64 offset = (int64)item->page_id->page * DEFAULT_PAGE_SIZE(session);
        if (cm_write_device(df->ctrl->type, handles[item->page_id->file], offset, buf, DEFAULT_PAGE_SIZE(session)) != CT_SUCCESS) {
            CT_LOG_RUN_ERR("[REPAIR] failed to write datafile %s", df->ctrl->name);
            bak_brain_close_file(handles, CT_MAX_DATA_FILES);
            return CT_ERROR;
        }
        CT_LOG_DEBUG_INF("[REPAIR] brain reset page %u-%u", item->page_id->file, item->page_id->page);
        item->page_id->file = CT_INVALID_FILEID;
    }

    for (uint32 i = 0; i < CT_MAX_DATA_FILES; i++) {
        if (handles[i] == CT_INVALID_HANDLE) {
            continue;
        }
        if (db_fdatasync_file(session, handles[i]) != CT_SUCCESS) {
            bak_brain_close_file(handles, CT_MAX_DATA_FILES);
            CT_LOG_RUN_ERR("[REPAIR] failed to fdatasync datafile");
            return CT_ERROR;
        }
    }
...
static status_t bak_build_from_remote(knl_session_t *session)
{
    ...
    if (bak_check_keep_alive(session) != CT_SUCCESS) {
        return CT_ERROR;
    }

    if (bak_build_start(session) != CT_SUCCESS) {
        return CT_ERROR;
    }

    if (bak_agent_send_build_ctrl(session, bak, proc) != CT_SUCCESS) {
        return CT_ERROR;
    }

    if (bak->record.is_repair && bak_agent_send_diff_pages(session) != CT_SUCCESS) {
        return CT_ERROR;
    }

    if (bak->record.attr.level > 0) {
        if (rst_build_prepare_basefiles(session, &ctx->process[BAK_COMMON_PROC]) != CT_SUCCESS) {
            return CT_ERROR;
        }
    }

    if (rst_build_restore(session) != CT_SUCCESS) {
        if (!bak->need_retry) {
            CT_LOG_RUN_INF("[RESTORE] restore failed, no more retry");
    ...
    if (bak->record.is_repair) {
        if (bak_brain_reset_page(session) != CT_SUCCESS) {
            return CT_ERROR;
        }
    }
...
static status_t rst_build_clean_arch(knl_session_t *session)
{
    ...
    for (uint32 i = 0; i < arch_num; i++) {
        clean_locator = (node->archived_start + i) % CT_MAX_ARCH_NUM;
        ...
        if (!cm_exist_device(cm_device_type(arch_ctrl->name), arch_ctrl->name)) {
            CT_LOG_RUN_INF("[RESTORE] archive file %s does not exist", arch_ctrl->name);
        } else {
            if (cm_remove_device(cm_device_type(arch_ctrl->name), arch_ctrl->name) != CT_SUCCESS) {
                CT_LOG_RUN_ERR("[RESTORE] Failed to remove archive file %s", arch_ctrl->name);
                return CT_ERROR;
            }
        }
...
status_t bak_build_prepare_attr(knl_session_t *session, build_param_ctrl_t *ctrl)
{
    ...
    if (ctrl->base_lsn != CT_INVALID_LSN) {
        attr->base_lsn = ctrl->base_lsn;
        return CT_SUCCESS;
    }

    if (ctrl->is_increment) {
        if (rst_build_get_base_lsn(session, &ctrl->base_lsn) != CT_SUCCESS) {
            return CT_ERROR;
        }

        if (rst_build_clean_arch(session) != CT_SUCCESS) {
            return CT_ERROR;
        }
    } else if (ctrl->is_repair) {
        if (build_analyze_mem_init(session) != CT_SUCCESS) {
            return CT_ERROR;
        }

        if (build_repair_prepare(session, &ctrl->base_lsn) != CT_SUCCESS) {
...
status_t bak_build_restore(knl_session_t *session, build_param_ctrl_t *ctrl)
{
    ...
    CT_LOG_RUN_INF("[BUILD] build start, compress method %s, compress level %u, is increment %u, is repair %u, "
    ...
    if (lrcv_ctx->thread.closed) {
        CT_THROW_ERROR(ERR_LRCV_NOT_READY);
        return CT_ERROR;
    }
    ...
    if (bak_build_prepare_attr(session, ctrl) != CT_SUCCESS) {
        bak_unset_running(session, ctx);
        return CT_ERROR;
    }
    ...
    if (!bak->need_retry) {
        CT_LOG_RUN_INF("[BUILD] backup remote finished, failed :%d", bak->failed);
    }
...
static void bak_send_error(bak_t *bak, cs_pipe_t *pipe, int32 max_pkg_size)
{
    ...
    CT_LOG_RUN_WAR("[BACKUP] build failed, error:%d, %s", err_code, error_msg);

    if (cs_write_stream(pipe, (char *)&head, sizeof(bak_agent_head_t), max_pkg_size) != CT_SUCCESS) {
        return;
    }

    if (cs_write_stream(pipe, (char *)&err_code, sizeof(int32), max_pkg_size) != CT_SUCCESS) {
        return;
    }

    if (cs_write_stream(pipe, (char *)error_msg, (uint32)strlen(error_msg) + 1, max_pkg_size) != CT_SUCCESS) {
        return;
    }

    (void)cs_read_stream(pipe, (char *)&head, CT_NETWORK_IO_TIMEOUT, sizeof(bak_agent_head_t), (int32 *)&receive_size);
}
    ...
    build_progress_t *build_progress)
{
    ...
    if (cs_read_stream(pipe, (char *)&head, CT_NETWORK_IO_TIMEOUT, sizeof(bak_agent_head_t),
                       (int32 *)&receive_size) != CT_SUCCESS) {
        return CT_ERROR;
    }

    if (receive_size != sizeof(bak_agent_head_t)) {
        CT_THROW_ERROR(ERR_TCP_TIMEOUT_REMAIN, (int32)(sizeof(bak_agent_head_t) - receive_size));
        return CT_ERROR;
    }

    if (head.cmd != BAK_PKG_START) {
        CT_THROW_ERROR(ERR_NOT_EXPECTED_BACKUP_PACKET, BAK_PKG_START, head.cmd);
        return CT_ERROR;
    }

    if (cs_read_stream(pipe, (char *)ctrl, CT_NETWORK_IO_TIMEOUT, sizeof(build_param_ctrl_t),
                       (int32 *)&receive_size) != CT_SUCCESS) {
        return CT_ERROR;
    }

    if (receive_size != sizeof(build_param_ctrl_t)) {
        CT_THROW_ERROR(ERR_TCP_TIMEOUT_REMAIN, (int32)(sizeof(build_param_ctrl_t) - receive_size));
        return CT_ERROR;
    }

    if (cs_read_stream(pipe, (char *)build_progress, CT_NETWORK_IO_TIMEOUT, sizeof(build_progress_t),
        (int32 *)&receive_size) != CT_SUCCESS) {
        return CT_ERROR;
    }
    CT_LOG_RUN_INF("[BUILD] received standby build progress [%u/%u/%llu/%u/%u/%u/%d]", build_progress->stage,
    ...
    if (receive_size != sizeof(build_progress_t)) {
        CT_THROW_ERROR(ERR_TCP_TIMEOUT_REMAIN, (int32)(sizeof(build_progress_t) - receive_size));
        return CT_ERROR;
    }
...
static status_t bak_set_break_point_params(knl_session_t *session, bak_context_t *ctx, build_progress_t *build_progress)
{
    ...
    switch (build_progress->stage) {
        case BACKUP_START:
            ctx->bak.is_first_link = CT_TRUE;
            ctx->bak.need_check = CT_FALSE;
        ...
        case BACKUP_HEAD_STAGE:
            ctx->bak.is_first_link = CT_FALSE;
            ctx->bak.need_check = CT_TRUE;
            ...
            *local_build_progress = *build_progress;
            ...
            break;
        default:
            CT_LOG_RUN_ERR("[BUILD] primary DB received unexpected build process stage, %u",
...
static status_t bak_get_page_list(knl_session_t *session)
{
    ...
    CT_LOG_RUN_INF("[BUILD] start to get page list from standby");
    if (rst_agent_read_head(bak, BAK_PKG_DATA, &read_size, &read_end) != CT_SUCCESS) {
        return CT_ERROR;
    }

    if (read_size > BUILD_ALY_MAX_PAGE_SIZE) {
        CT_LOG_RUN_ERR("[BUILD] get page list from standby failed, received size: %u, max size: %llu", read_size,
            BUILD_ALY_MAX_PAGE_SIZE);
        return CT_ERROR;
    }

    if (bak_agent_recv(bak, (char *)bak->build_aly_pages, (int32)read_size) != CT_SUCCESS) {
        return CT_ERROR;
    }
    ...
    CT_LOG_RUN_INF("[BUILD] get page list from standby successfully, page count: %u", bak->page_count);
    return CT_SUCCESS;
}
...
status_t bak_build_backup(knl_session_t *session, cs_pipe_t *pipe, cs_packet_t *send_pack, cs_packet_t *recv_pack)
{
    ...
    if (bak_build_wait_start(session, pipe, &ctrl, &build_progress) != CT_SUCCESS) {
        return CT_ERROR;
    }
    ...
    if (bak_set_build_running(session, ctx, &build_progress) != CT_SUCCESS) {
        bak_send_error(bak, pipe, (int32)cm_atomic_get(&session->kernel->attr.repl_pkg_size));
        return CT_ERROR;
    }
    ...
    if (bak_set_break_point_params(session, ctx, &build_progress) != CT_SUCCESS) {
        bak_unset_build_running(session, ctx);
        bak_send_error(bak, pipe, (int32)cm_atomic_get(&session->kernel->attr.repl_pkg_size));
        return CT_ERROR;
    }

    if (bak_agent_write(bak, (char *)&local_build_progress->start_time, sizeof(int32)) != CT_SUCCESS) {
        bak_unset_build_running(session, ctx);
        bak_send_error(bak, pipe, (int32)cm_atomic_get(&session->kernel->attr.repl_pkg_size));
        return CT_ERROR;
    }
    ...
    if (ctrl.is_repair) {
        ...
        if (bak_get_page_list(session) != CT_SUCCESS) {
            bak_unset_build_running(session, ctx);
            build_analyze_mem_free(session);
            bak_send_error(bak, pipe, (int32)cm_atomic_get(&session->kernel->attr.repl_pkg_size));
            return CT_ERROR;
...
void bak_stream_read_prepare(knl_session_t *session, bak_process_t *process, datafile_t *datafile, uint32 sec_id)
{
    ...
    ret = strcpy_sp(ctrl->name, CT_FILE_NAME_BUFFER_SIZE, datafile->ctrl->name);
    knl_securec_check(ret);
...
status_t bak_send_stream_data(knl_session_t *session, bak_t *bak, bak_assignment_t *assign_ctrl)
{
    ...
    for (;;) {
        ...
        if (send_size > 0) {
            start = g_timer()->now;
            if (bak_agent_write(bak, send_buf, send_size) != CT_SUCCESS) {
                return CT_ERROR;
            }
                                 ...
                                 bool32 *ignore_logfile)
{
    ...
    if (bak->files[bak_index].type == BACKUP_DATA_FILE) {
        ...
        int32 read_size;
        bool32 log_end = CT_FALSE;

        if (rst_agent_read(bak, (char *)head, sizeof(log_file_head_t), &read_size, &log_end) != CT_SUCCESS) {
            return CT_ERROR;
        }

        if (read_size < sizeof(log_file_head_t)) {
            CT_LOG_RUN_ERR("[RESTOR] failed to read log file head, read size %d less than %u", read_size,
                           (uint32)sizeof(log_file_head_t));
            return CT_ERROR;
        }

        if (log_verify_head_checksum(session, head, "") != CT_SUCCESS) {
            return CT_ERROR;
        }
        ...
        if (read_size != fill_size) {
            CT_LOG_RUN_ERR("[RESTOR] failed to read log file head, read size %d less than %u", read_size, fill_size);
            return CT_ERROR;
        }
        common_proc->ctrl.handle = CT_INVALID_HANDLE;
        if (rst_set_logfile_ctrl(session, bak->curr_file_index, head, &common_proc->ctrl, &ignore_data) != CT_SUCCESS) {
            return CT_ERROR;
        }
        if (!ignore_data) {
...
status_t rst_stream_aligned_blocks(rst_stream_buf_t *stream, uint32 recv_size, bool32 ignore_data)
{
    ...
    while (block->block_size + usable_size <= recv_size) {
        if (block->magic_num != LOG_MAGIC_NUMBER) {
            CT_LOG_RUN_ERR("[RESTOR] invalid received block with id %u, size is %u", block->block_id,
                           block->block_size);
            return CT_ERROR;
        }
    ...
    if (usable_size < sizeof(bak_block_head_t)) {
        CT_LOG_RUN_ERR("[RESTOR] invalid received block with id %u, size is %u", block->block_id, block->block_size);
        return CT_ERROR;
    }
                                  ...
                                  bool32 *file_end)
{
    ...
    if (rst_agent_read(bak, buf, buf_size, recv_size, file_end) != CT_SUCCESS) {
        return CT_ERROR;
    }
...
status_t rst_recv_stream_data(knl_session_t *session, bool32 ignore_data)
{
    ...
    for (;;) {
        ...
        CT_LOG_DEBUG_INF("[BACKUP] read size :%u, last block tail size %u", recv_size, last_tail_size);
        recv_size += last_tail_size;
        if (rst_stream_aligned_blocks(stream, (uint32)recv_size, ignore_data) != CT_SUCCESS) {
            return CT_ERROR;
        }
    }
