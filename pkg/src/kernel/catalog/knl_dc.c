...
void dc_set_index_profile(knl_session_t *session, dc_entity_t *entity, index_t *index)
{
    ...
    profile->is_compart_table = profile->global_idx_for_part_table ? IS_COMPART_TABLE(table->part_table) : GS_FALSE;
    profile->is_shadow = index->btree.is_shadow;
    profile->null_first = DB_ATTR_COMPATIBLE_MYSQL(session) == 1 ? GS_TRUE : GS_FALSE;
...
status_t dc_reset_nologging_entry(knl_session_t *session, knl_handle_t desc, object_type_t type)
{
    ...
    switch (type) {
        ...
        case OBJ_TYPE_SHADOW_INDEX_PART:
            status = db_upd_shadow_idx_part_entry(session, (knl_index_part_desc_t *)desc, INVALID_PAGID, GS_FALSE);
            break;
        ...
        case OBJ_TYPE_SHADOW_INDEX_SUBPART:
            status = db_upd_shadow_idx_part_entry(session, (knl_index_part_desc_t *)desc, INVALID_PAGID, GS_TRUE);
            break;
        default:
            knl_panic(GS_FALSE);
            break;
...
bool32 dc_open_ltt(knl_session_t *session, dc_user_t *user, text_t *obj_name, knl_dictionary_t *dc)
{
    ...
    if (entity->cbo_table_stats == NULL) {
        if (cbo_alloc_temp_table_stats(session, entity, temp_cache, GS_TRUE) == GS_SUCCESS) {
...
status_t knl_try_open_dc_by_id(knl_handle_t handle, uint32 uid, uint32 oid, knl_dictionary_t *dc)
{
    ...
    if (IS_LTT_BY_ID(oid)) {
        GS_THROW_ERROR(ERR_OPERATIONS_NOT_ALLOW, "try open local temporary table");
        return GS_ERROR;
    }
    ...
    if (!dc_find_by_id(session, user, oid, GS_FALSE)) {
        GS_THROW_ERROR(ERR_TABLE_ID_NOT_EXIST, uid, oid);
        return GS_ERROR;
    }
...
status_t dc_build_ex_systables(knl_session_t *session)
{
    ...
    if (!core->build_completed) {
        ...
        if (session->kernel->attr.clustered) {
            if (dtc_build_completed(session) != GS_SUCCESS) {
                return GS_ERROR;
            }
        } else {
            if (db_build_completed(session) != GS_SUCCESS) {
                return GS_ERROR;
            }
        }
...
static status_t dc_init_extral_entries(knl_session_t *session, dc_context_t *ctx)
{
    ...
    if (dc_creat_spm_context(ctx) != GS_SUCCESS) {
        return GS_ERROR;
    }
#endif
...
status_t dc_init(knl_session_t *session)
{
    ...
    if (ctx->memory == NULL) {
        if (dc_context_init(session) != GS_SUCCESS) {
            GS_LOG_RUN_ERR("failed to dc context init");
            return GS_ERROR;
        }
    }
    ...
    if (dc_init_users(session, ctx) != GS_SUCCESS) {
        GS_LOG_RUN_ERR("failed to dc init users");
        return GS_ERROR;
    }

    if (dc_open_core_systbl(session) != GS_SUCCESS) {
        GS_LOG_RUN_ERR("failed to dc open core systbl");
        return GS_ERROR;
    }

    if (dc_init_table_entries(session, ctx, DB_SYS_USER_ID) != GS_SUCCESS) {
        GS_LOG_RUN_ERR("failed to dc init table entries");
        return GS_ERROR;
    }

    if (dc_load_systables(session, ctx) != GS_SUCCESS) {
        GS_LOG_RUN_ERR("failed to dc load systables");
        return GS_ERROR;
    }
    ...
    if (dc_init_extral_entries(session, ctx) != GS_SUCCESS) {
        GS_LOG_RUN_ERR("failed to dc init extral entries");
        return GS_ERROR;
    }
...
heap_t *dc_get_heap(knl_session_t *session, uint32 uid, uint32 oid, knl_part_locate_t part_loc, knl_dictionary_t *dc)
{
    ...
    if (DB_IS_CLUSTER(session) && part_loc.part_no == GS_INVALID_ID32) {
                            ...
                            bool32 is_shadow)
{
    ...
    if (is_shadow) {
        ...
        if (shadow_index->part_loc.part_no == GS_INVALID_ID32) {
            ...
            knl_panic(shadow_index->part_loc.part_no == part_loc.part_no);
        ...
        if (!IS_PART_INDEX(index)) {
            ...
            if (index_part->btree.segment == NULL && !IS_INVALID_PAGID(index_part->btree.entry)) {
                ...
                if (dc_load_table_part_segment(session, index->entity, table_part) != GS_SUCCESS) {
                    knl_panic_log(0,
                                  "load table part segment is failed, panic info: page %u-%u table %s table_part %s "
...
void dc_recycle_table_dls(knl_session_t *session, dc_entry_t *entry)
{
    ...
    drc_recycle_dls_res(session, &entry->serial_lock.drid);
    drc_recycle_dls_res(session, &entry->ddl_latch.drid);
}
