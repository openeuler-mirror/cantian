...
static status_t user_check_def_space(knl_session_t *session, knl_user_def_t *def, knl_user_desc_t *desc)
{
    ...
    if (strlen(def->default_space) == 0) {
        ...
        if (spc_get_space_id(session, &space_name, def->is_for_create_db, &desc->data_space_id) != GS_SUCCESS) {
            return GS_ERROR;
        }
    ...
    if (strlen(def->temp_space) != 0) {
        ...
        if (spc_get_space_id(session, &space_name, def->is_for_create_db, &desc->temp_space_id) != GS_SUCCESS) {
            return GS_ERROR;
        }
...
{
    strcpy_sp(rd->name, GS_NAME_BUFFER_SIZE, desc->name);
    strcpy_sp(rd->password, GS_PASSWORD_BUFFER_SIZE, desc->password);
    rd->op_type = op_type;
    rd->uid = desc->id;
    rd->ctime = desc->ctime;
    rd->ptime = desc->ptime;
...
status_t user_create(knl_session_t *session, knl_user_def_t *def)
{
    ...
    if (AUTO_INHERIT_ON(session) && user_create_auto_grant_inherit(session, ctx, &desc) != GS_SUCCESS) {
        CM_RESTORE_STACK(session->stack);
    ...
    dls_spin_unlock(session, &ctx->paral_lock);
    user_clear_password(desc.password, GS_PASSWORD_BUFFER_SIZE);
    GS_LOG_RUN_INF("[DB] Finish to create user, user_id %u", desc.id);
    ...
    if (!CM_IS_EMPTY_STR(def->old_password)) {
        if (GS_SUCCESS != cm_check_password(&plain_password, &cipher_password)) {
            ...
            if (GS_SUCCESS != user_process_change_password_failed(session, &user->desc)) {
                ...
                g_knl_callback.kill_session(session, GS_FALSE, session->serial_id);
            }
            return GS_ERROR;
        }
    }
...
status_t user_drop_core(knl_session_t *session, dc_user_t *user, bool32 purge)
{
    ...
    if (!db_user_has_objects(session, uid, &username)) {
        SYNC_POINT_GLOBAL_START(CANTIAN_DROP_USER_OBJECT_FAIL, &ret, GS_ERROR);
