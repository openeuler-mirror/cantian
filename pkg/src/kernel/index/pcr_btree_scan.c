...
static int32 pcrb_cmp_column_null_first(gs_type_t datatype, knl_scan_key_t *scan_key, uint32 idx_col_id,
                                        pcrb_key_t *key, uint16 *offset)
{
    ...
    if (flag == SCAN_KEY_NORMAL) {
        ...
        result = btree_cmp_column_data((void *)data1, data2, datatype, offset, GS_TRUE);
...
int32 pcrb_cmp_column(gs_type_t datatype, knl_scan_key_t *scan_key, uint32 idx_col_id, pcrb_key_t *key, uint16 *offset,
                      bool8 null_first)
{
    if (null_first == GS_TRUE) {
        return pcrb_cmp_column_null_first(datatype, scan_key, idx_col_id, key, offset);
    }
                       ...
                       bool32 *is_same)
{
    ...
    for (uint32 i = 0; i < profile->column_count; i++) {
        result = pcrb_cmp_column(profile->types[i], scan_key, i, key, &offset, profile->null_first);
        if (result != 0) {
            return result;
        }
    }
...
static void pcrb_cleanout_page(knl_session_t *session, knl_cursor_t *cursor, page_id_t page_id)
{
    ...
    if (page_soft_damaged(&page->head) ||
        btree_check_segment_scn(page, PAGE_TYPE_PCRB_NODE, cursor->key_loc.seg_scn) != GS_SUCCESS) {
    ...
    key_locator_t *locator, page_id_t page_id, uint32 level)
{
    ...
    if (bt_chk_keys_recycled(session, search_info->btree, page,
        search_info->isolevel, search_info->query_scn) != GS_SUCCESS) {
        buf_leave_page(session, GS_FALSE);
        return GS_ERROR;
    }
    ...
    */
    ...
    for (;;) {
        ...
        if (bt_chk_leaf_recycled(session, search_info->btree, page, search_info->parent_snap_scn)) {
            ...
            search_info->use_cr_pool = GS_FALSE;
            search_info->read_root_copy = GS_FALSE;
            continue;
        }
    ...
    key_locator_t *locator, pcrb_key_t *key)
{
    ...
    gs_type_t datatype = profile->types[0];
    uint16 offset = sizeof(pcrb_key_t);
    ...
    if (cursor->index_dsc) {
        ...
        if (pcrb_cmp_column(datatype, &scan_range->r_key, 0, key, &offset, cursor->null_first) > 0) {
            return GS_FALSE;
        }

        for (uint32 i = 1; i < index->desc.column_count; i++) {
            datatype = profile->types[i];
            result = pcrb_cmp_column(datatype, &scan_range->l_key, i, key, &offset, cursor->null_first);
            if (result > 0) {
                return GS_FALSE;
            } else if (result < 0) {
                return GS_TRUE;
        ...
        if (pcrb_cmp_column(datatype, &scan_range->l_key, 0, key, &offset, cursor->null_first) < 0) {
            return GS_FALSE;
        }

        for (uint32 i = 1; i < index->desc.column_count; i++) {
            datatype = profile->types[i];
            result = pcrb_cmp_column(datatype, &scan_range->r_key, i, key, &offset, cursor->null_first);
            if (result < 0) {
                return GS_FALSE;
            } else if (result > 0) {
                return GS_TRUE;
                               ...
                               undo_row_t *ud_row)
{
    ...
    bool32 cmp_rowid = (profile->primary || profile->unique) ? GS_FALSE : GS_TRUE;
                                      ...
                                      pcr_itl_t *itl, undo_row_t *ud_row)
{
    switch (ud_row->type) {
        ...
        default:
            knl_panic_log(0, "ud_row type is unknown, panic info: page %u-%u type %u table %d index %d ud_row type %u",
                          AS_PAGID(cr_page->head.id).file, AS_PAGID(cr_page->head.id).page, cr_page->head.type,
                          profile->table_id, profile->index_id, ud_row->type);
            break;
    }
}
...
static status_t pcrb_construct_cr_page(knl_session_t *session, cr_cursor_t *cursor, btree_page_t *cr_page)
{
    ...
    for (;;) {
        if (pcrb_get_invisible_itl(session, cursor, cr_page) != GS_SUCCESS) {
            return GS_ERROR;
        }
        ...
        if (pcrb_reorganize_with_undo_list(session, cursor, cr_page) != GS_SUCCESS) {
            return GS_ERROR;
        }
        constructed = GS_TRUE;
    }
    ...
    key_locator_t *locator, knl_scn_t snap_scn, bool32 *retry)
{
    ...
    *retry = GS_FALSE;
    ...
    if (pcrb_construct_cr_page(session, &cr_cursor, (btree_page_t *)cursor->page_buf) != GS_SUCCESS) {
        return GS_ERROR;
    }
    ...
    btree_path_info_t *path_info)
{
    ...
    GS_LOG_RUN_INF("index %s relocate next key rowid %u-%u-%u", search_info->btree->index->desc.name,
    ...
    knl_scan_key_t *scan_key, btree_path_info_t *path_info)
{
    ...
    bool32 cmp_rowid = search_info->is_dsc_scan ? GS_TRUE : asc_cmp_rowid;
    if (pcrb_find_leaf(session, search_info, locator, scan_key, path_info, cmp_rowid, &is_found) != GS_SUCCESS) {
        if (BTREE_LOCATE_NEXT_KEY(search_info, cursor)) {
            dls_unlatch(session, &btree->struct_latch, &session->stat_btree);
        }
        return GS_ERROR;
...
static status_t pcrb_locate_with_find(knl_session_t *session, knl_cursor_t *cursor, knl_scan_key_t *scan_key)
{
    ...
    for (;;) {
        ...
        if (search_info.is_equal) {
            ...
            if (pcrb_construct_cr_page(session, &cr_cursor, page) != GS_SUCCESS) {
            ...
            */
            ...
            if (pcrb_local_cache_page(session, cursor, locator, search_info.child_snap_scn, &retry) != GS_SUCCESS) {
                return GS_ERROR;
            }
...
static status_t pcrb_reloc_nextkey(knl_session_t *session, knl_cursor_t *cursor, knl_scan_key_t *key)
{
    ...
    cursor->key_loc.skip_cache = GS_TRUE;
    if (!cursor->index_dsc) {
        cursor->asc_relocate_next_key = GS_TRUE;
    }

    status_t status = pcrb_locate_with_find(session, cursor, key);
    cursor->asc_relocate_next_key = GS_FALSE;
    return status;
}
    ...
    key_locator_t *locator, knl_scn_t snap_scn)
{
    ...
    for (;;) {
        if (pcrb_local_cache_page(session, cursor, locator, snap_scn, &retry) != GS_SUCCESS) {
            return GS_ERROR;
        }
...
static status_t pcrb_relocate_curr_page(knl_session_t *session, knl_cursor_t *cursor, knl_scan_key_t *scan_key)
{
    ...
    for (;;) {
        ...
        if (is_same) {
            ...
            locator->slot = 0;
            if (pcrb_retry_local_cache(session, cursor, locator, search_info.child_snap_scn) != GS_SUCCESS) {
                return GS_ERROR;
            }
...
static bool32 pcrb_do_match_cond(knl_cursor_t *cursor, pcrb_key_t *key)
{
    ...
    if (locator->match_left) {
        ...
        for (uint8 i = locator->equal_cols; i < index->desc.column_count; i++) {
            ...
            if (pcrb_cmp_column(profile->types[i], l_key, i, key, &offset, cursor->null_first) > 0) {
                return GS_FALSE;
            }
        }
    }
    ...
    if (locator->match_right) {
        ...
        for (uint32 i = locator->equal_cols; i < index->desc.column_count; i++) {
            offset = curr_key.offsets[i];
            if (pcrb_cmp_column(profile->types[i], r_key, i, key, &offset, cursor->null_first) < 0) {
                return GS_FALSE;
            }
        }
    }
...
static status_t pcrb_locate_next_scan_key(knl_session_t *session, knl_cursor_t *cursor)
{
    ...
    if (pcrb_cmp_column(datatype, scan_key, 0, key, &offset, cursor->null_first) != 0) {
        cursor->eof = GS_FALSE;
        return GS_SUCCESS;
    }
    ...
    if (scan_key->flags[0] == SCAN_KEY_NORMAL) {
        char *data = btree_get_column(scan_key, datatype, 0, &len, GS_TRUE);
    ...
    search_info.tree_info.value = cm_atomic_get(&BTREE_SEGMENT(session, btree->entry, btree->segment)->tree_info.value);
...
static status_t pcrb_set_next_range(knl_session_t *session, knl_cursor_t *cursor, btree_t *btree)
{
    ...
    int32 result = pcrb_compare_key(profile, end_key, key, GS_TRUE, NULL);
    ...
    for (;;) {
        ...
        if (IS_INDEX_ONLY_SCAN(cursor)) {
            ...
            if (knl_match_cond(session, cursor, &cursor->is_found) != GS_SUCCESS) {
                cantian_record_io_stat_end(IO_RECORD_EVENT_PCRB_FETCH, &tv_begin, GS_ERROR);
                return GS_ERROR;
            }
        } else {
...
static bool32 pcrb_chk_key_txn_end(knl_session_t *session, pcr_itl_t *pcr_itl)
{
    ...
    tx_get_pcr_itl_info(session, GS_FALSE, pcr_itl, &txn_info);
...
status_t pcrb_check_key_exist(knl_session_t *session, btree_t *btree, char *data, bool32 *exists)
{
    ...
    *exists = GS_FALSE;
    ...
    if (bt_chk_exist_pre(session, btree, &search_info) != GS_SUCCESS) {
        return GS_ERROR;
    }

    btree_init_key_loc(&locator);
    bool32 cmp_rowid = search_info.is_dsc_scan ? GS_TRUE : !IS_UNIQUE_PRIMARY_INDEX(btree->index);

    for (;;) {
        if (pcrb_find_leaf(session, &search_info, &locator, &scan_key, &path_info, cmp_rowid, &found) != GS_SUCCESS) {
            return GS_ERROR;
        }
...
static status_t pcrb_fetch_depended_asc(knl_session_t *session, knl_cursor_t *cursor)
{
    ...
    while (locator->slot < page->keys) {
        ...
        if (locator->slot_end == INVALID_SLOT || locator->slot_end < locator->slot) {
            result = pcrb_compare_key(profile, filter_key, key, GS_FALSE, NULL);
            if (result < 0) {
                pcrb_leave_curr_page(session, locator);
                cursor->eof = GS_TRUE;
                return GS_SUCCESS;
...
status_t pcrb_fetch_depended(knl_session_t *session, knl_cursor_t *cursor)
{
    ...
    cursor->key_loc.skip_cache = GS_FALSE;
