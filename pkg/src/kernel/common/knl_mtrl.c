...
#define MTRL_SESSION(ctx)                   ((ctx)->session)
#define MTRL_POOL(ctx)                      ((ctx)->pool)
...
status_t mtrl_open_page(mtrl_context_t *ctx, uint32 vmid, vm_page_t **page)
{
    ...
    uint32 max_pos = ctx->open_hwm + 1 > CT_MAX_MTRL_OPEN_PAGES ? CT_MAX_MTRL_OPEN_PAGES : ctx->open_hwm + 1;
    for (i = 0; i < max_pos; i++) {
        ...
        if (ctx->open_pages[i] == CT_INVALID_ID32 && id == CT_INVALID_ID32) {
            id = i;
    ...
    if (id == CT_INVALID_ID32) {
        CT_THROW_ERROR(ERR_VM, "fail to open vm page, the pages are already full");
        cm_spin_unlock(&ctx->lock);
        return CT_ERROR;
    }

    if (vm_open(MTRL_SESSION(ctx), MTRL_POOL(ctx), vmid, page) != CT_SUCCESS) {
...
void mtrl_close_page(mtrl_context_t *ctx, uint32 vmid)
{
    ...
    for (i = 0; i < ctx->open_hwm; i++) {
        if (ctx->open_pages[i] == vmid) {
            ctrl = vm_get_ctrl(MTRL_POOL(ctx), vmid);
            ref_num = ctrl->ref_num;

            vm_close(MTRL_SESSION(ctx), MTRL_POOL(ctx), vmid, VM_ENQUE_TAIL);
    ...
    while (CT_TRUE) {
        ...
        if (ctx->open_pages[ctx->open_hwm - 1] == CT_INVALID_ID32) {
            --ctx->open_hwm;
        } else {
            break;
        }
...
void mtrl_release_segment(mtrl_context_t *ctx, uint32 id)
{
    if (ctx->segments[id] != NULL && ctx->segments[id]->is_used) {
        ...
        vm_free_list(MTRL_SESSION(ctx), MTRL_POOL(ctx), &ctx->segments[id]->vm_list);
...
{
    for (uint32 i = 0; i < CT_MAX_MATERIALS; i++) {
        ctx->segments[i] = NULL;
    }
}
...
status_t mtrl_recreate_segment(mtrl_context_t *ctx, mtrl_segment_t *curr_seg, mtrl_segment_t *segment)
{
    ...
    if (vm_alloc(MTRL_SESSION(ctx), MTRL_POOL(ctx), &vmid) != CT_SUCCESS) {
        return CT_ERROR;
    }
...
static mtrl_segment_t *mtrl_get_idle_segment(mtrl_context_t *ctx, uint32 *id)
{
    ...
    if (ctx->seg_count >= CT_MAX_MATERIALS) {
        CT_THROW_ERROR(ERR_SQL_TOO_COMPLEX);
        return NULL;
    }

    for (i = 0; i < CT_MAX_MATERIALS; ++i) {
        if (ctx->segments[i] == NULL) {
            if (vmc_alloc_mem(&ctx->vmc, sizeof(mtrl_segment_t), (void **)&ctx->segments[i]) != CT_SUCCESS) {
                return NULL;
            }
        }
        if (!ctx->segments[i]->is_used) {
            ctx->segments[i]->is_used = CT_TRUE;
            ctx->seg_count++;
            *id = i;
...
status_t mtrl_create_segment(mtrl_context_t *ctx, mtrl_segment_type_t type, handle_t cmp_items, uint32 *id)
{
    ...
    if (segment == NULL) {
        CT_LOG_RUN_ERR("[MTRL] failed to get idle segment");
        return CT_ERROR;
    }
    ...
    return vm_alloc_and_append(MTRL_SESSION(ctx), MTRL_POOL(ctx), &segment->vm_list);
}
...
status_t mtrl_extend_segment(mtrl_context_t *ctx, mtrl_segment_t *segment)
{
    ...
    CT_RETURN_IFERR(vm_alloc_and_append(MTRL_SESSION(ctx), MTRL_POOL(ctx), &segment->vm_list));
    if (segment->vm_list.count < segment->pages_hold) {
        return vm_open(MTRL_SESSION(ctx), MTRL_POOL(ctx), segment->vm_list.last, &page);
    }
    return CT_SUCCESS;
}
...
status_t mtrl_open_segment2(mtrl_context_t *ctx, mtrl_segment_t *segment)
{
    ...
    if (mtrl_open_page(ctx, vmid, &segment->curr_page) != CT_SUCCESS) {
        return CT_ERROR;
    }
...
static status_t mtrl_sort_single_page(mtrl_context_t *ctx, mtrl_segment_t *segment, mtrl_page_t *page)
{
    ...
    switch (segment->sort_type) {
        ...
        default:
            CT_THROW_ERROR(ERR_SQL_SYNTAX_ERROR, "unexpect sort type");
            return CT_ERROR;
    }
    return CT_SUCCESS;
}
...
static void mtrl_free_sorted_page(mtrl_context_t *ctx, mtrl_sort_cursor_t *cursor)
{
    if (!mtrl_sort_need_free_page(cursor->page)) {
        ...
        cursor->page->page_occupied = CT_FALSE;
        return;
    }
    ...
    vm_ctrl_t *ctrl = vm_get_ctrl(MTRL_POOL(ctx), cursor->vmid);
    CM_ASSERT(ctrl->ref_num >= 1);
    ctrl->ref_num = 1;
    mtrl_close_page(ctx, cursor->vmid);
    vm_remove(MTRL_POOL(ctx), &cursor->segment->vm_list, cursor->vmid);
    vm_free(MTRL_SESSION(ctx), MTRL_POOL(ctx), cursor->vmid);
}
...
static status_t mtrl_locate_sort_part(mtrl_context_t *ctx, mtrl_sort_cursor_t *cursor, uint32 level)
{
    ...
    if (level == 0) {
        cursor->part.next.vmid = vm_get_ctrl(MTRL_POOL(ctx), cursor->vmid)->prev;
    ...
    ctrl = vm_get_ctrl(MTRL_POOL(ctx), cursor->vmid);
    next = ctrl->next;
    mtrl_free_sorted_page(ctx, cursor);
    cursor->vmid = CT_INVALID_ID32;

    if (mtrl_open_page(ctx, next, &page) != CT_SUCCESS) {
        return CT_ERROR;
    }
...
status_t mtrl_move_group_cursor(mtrl_context_t *ctx, mtrl_sort_cursor_t *cursor)
{
    ...
    if (mtrl_open_page(ctx, ctrl->next, &page) != CT_SUCCESS) {
        return CT_ERROR;
    }
                                     ...
                                     bool32 *eof)
{
    ...
    *group_changed = CT_FALSE;
    ...
    if (mtrl_move_group_cursor(ctx, cursor) != CT_SUCCESS) {
        return CT_ERROR;
    }

    if (cursor->rownum > cursor->part.rows) {
        *group_changed = CT_TRUE;
        return CT_SUCCESS;
    }

    next_row = MTRL_GET_ROW(cursor->page, cursor->slot - 1);
    if (ctx->sort_cmp(cursor->segment, next_row, cursor->row, &result) != CT_SUCCESS) {
        return CT_ERROR;
    }
                            ...
                            mtrl_sort_cursor_t *cursor2, mtrl_sort_cursor_t **result_cur)
{
    ...
    if (ctx->sort_cmp(segment, cursor1->row, cursor2->row, &result) != CT_SUCCESS) {
        return CT_ERROR;
    }
                                         ...
                                         uint16 row_size, bool32 is_part, mtrl_rowid_t *rid)
{
    ...
    if (page->free_begin + row_size + sizeof(uint32) > CT_VMEM_PAGE_SIZE - MTRL_DIR_SIZE(page)) {
        page->is_sorted = CT_TRUE;
    ...
    if (is_part) {
        page->has_part_info = CT_TRUE;
        if (rid->slot != 0) {
            page->page_occupied = CT_TRUE;
        }
    }
                                    ...
                                    mtrl_sort_cursor_t *cursor2, mtrl_rowid_t *srid)
{
    ...
    part.next.vmid = CT_INVALID_ID32;
    part.next.slot = CT_INVALID_ID16;

    if (mtrl_insert_into_segment(ctx, segment, (char *)&part, part.size, CT_TRUE, srid) != CT_SUCCESS) {
        return CT_ERROR;
    }

    if (mtrl_merge_compare(ctx, segment, cursor1, cursor2, &result_cur) != CT_SUCCESS) {
        return CT_ERROR;
    }

    while (result_cur != NULL) {
        row_size = MTRL_ROW_SIZE(segment, result_cur->row);
        if (mtrl_insert_into_segment(ctx, segment, result_cur->row, row_size, CT_FALSE, &rid) != CT_SUCCESS) {
            return CT_ERROR;
        }
        ...
        if (mtrl_move_sort_cursor(ctx, result_cur, mtrl_free_sorted_page) != CT_SUCCESS) {
            return CT_ERROR;
        }
                                   ...
                                   mtrl_segment_t *segment, mtrl_sort_cursor_t *cursor, mtrl_rowid_t *srid)
{
    ...
    while (cursor->rownum <= part.rows) {
        ...
        if (mtrl_insert_into_segment(ctx, segment, cursor->row, row_size, CT_FALSE, &rid) != CT_SUCCESS) {
            return CT_ERROR;
        }

        cursor->rownum++;

        if (mtrl_move_sort_cursor(ctx, cursor, mtrl_free_sorted_page) != CT_SUCCESS) {
            return CT_ERROR;
        }
    }
    return CT_SUCCESS;
                               ...
                               uint32 level, mtrl_sort_cursor_t *cursor)
{
    ...
    if (mtrl_open_page(ctx, rid->vmid, &page) != CT_SUCCESS) {
        return CT_ERROR;
    }
...
{
    if (cursor->vmid != CT_INVALID_ID32) {
        mtrl_close_page(ctx, cursor->vmid);
        if (cursor->last_vmid != cursor->vmid && cursor->last_vmid != CT_INVALID_ID32) {
            mtrl_close_page(ctx, cursor->last_vmid);
            cursor->last_vmid = CT_INVALID_ID32;
        }
        cursor->vmid = CT_INVALID_ID32;
    }
    cursor->segment = NULL;
...
static status_t mtrl_concat_part_info(mtrl_context_t *ctx, mtrl_rowid_t *ex_srid, mtrl_rowid_t *srid)
{
    ...
    if (mtrl_open_page(ctx, srid->vmid, &vm_page) != CT_SUCCESS) {
        return CT_ERROR;
    }
                                ...
                                uint32 level, mtrl_rowid_t *start_rid, bool32 *finished)
{
    ...
    if (mtrl_recreate_segment(ctx, curr_seg, segment) != CT_SUCCESS) {
        return CT_ERROR;
    }

    if (mtrl_open_segment2(ctx, segment) != CT_SUCCESS) {
        return CT_ERROR;
    }

    parts = 0;
    ex_srid.vmid = CT_INVALID_ID32;
    ex_srid.slot = CT_INVALID_ID16;
    srid.vmid = start_rid->vmid;
    srid.slot = start_rid->slot;

    while (srid.vmid != CT_INVALID_ID32) {
        if (knl_check_session_status(session) != CT_SUCCESS) {
            return CT_ERROR;
        }

        if (mtrl_open_sort_cursor(ctx, curr_seg, &srid, level, &cursor1) != CT_SUCCESS) {
            return CT_ERROR;
        }

        if (cursor1.part.next.vmid == CT_INVALID_ID32) {
            if (mtrl_merge_1cursor(ctx, segment, &cursor1, &srid) != CT_SUCCESS) {
                mtrl_close_sort_cursor(ctx, &cursor1);
                return CT_ERROR;
            }

            if (mtrl_concat_part_info(ctx, &ex_srid, &srid) != CT_SUCCESS) {
                mtrl_close_sort_cursor(ctx, &cursor1);
                return CT_ERROR;
            }

            CM_ASSERT(cursor1.vmid == CT_INVALID_ID32);
            mtrl_close_sort_cursor(ctx, &cursor1);
            *start_rid = srid;
            page = (mtrl_page_t *)segment->curr_page->data;
            page->is_sorted = CT_TRUE;
        ...
        if (mtrl_open_sort_cursor(ctx, curr_seg, &cursor1.part.next, level, &cursor2) != CT_SUCCESS) {
            mtrl_close_sort_cursor(ctx, &cursor1);
            return CT_ERROR;
        }

        if (mtrl_merge_2cursors(ctx, segment, &cursor1, &cursor2, &srid) != CT_SUCCESS) {
            mtrl_close_sort_cursor(ctx, &cursor1);
        ...
        *start_rid = srid;
        ...
        CM_ASSERT(cursor1.vmid == CT_INVALID_ID32);
        CM_ASSERT(cursor2.vmid == CT_INVALID_ID32);
        mtrl_close_sort_cursor(ctx, &cursor1);
        mtrl_close_sort_cursor(ctx, &cursor2);
        parts++;
    }
...
status_t mtrl_split_segment(mtrl_context_t *ctx, uint32 seg_id, uint32 *new_seg_id)
{
    ...
    for (uint32 i = 1; i < page_count1; i++) {
        vm_ctrl = vm_get_ctrl(MTRL_POOL(ctx), vmid);
        vmid = vm_ctrl->next;
    }
    vm_ctrl = vm_get_ctrl(MTRL_POOL(ctx), vmid);
    ...
    vm_ctrl->next = CT_INVALID_ID32;
    vm_ctrl = vm_get_ctrl(MTRL_POOL(ctx), new_seg->vm_list.first);
    vm_ctrl->prev = CT_INVALID_ID32;
...
status_t mtrl_merge_2segments(mtrl_context_t *ctx, uint32 seg_id1, uint32 seg_id2)
{
    ...
    if (mtrl_open_page(ctx, segment1->vm_list.first, &page1) != CT_SUCCESS) {
        return CT_ERROR;
    }
    ...
    vm_ctrl = vm_get_ctrl(MTRL_POOL(ctx), segment1->vm_list.last);
    vm_ctrl->next = segment2->vm_list.first;
    vm_ctrl = vm_get_ctrl(MTRL_POOL(ctx), segment2->vm_list.first);
    ...
    mtrl_sort_cursor_t *cursor2)
{
    ...
    if (mtrl_insert_into_segment(ctx, segment, (char *)&part, part.size, CT_TRUE, &rid) != CT_SUCCESS) {
        return CT_ERROR;
    }
    ...
    while (rows != cursor1->part.rows + cursor2->part.rows) {
        ...
        result_cur->rownum++;
        rows++;
        if (mtrl_move_sort_cursor(result_cur->ctx, result_cur, mtrl_close_sorted_page) != CT_SUCCESS) {
            return CT_ERROR;
        }
...
status_t mtrl_merge_2pools(knl_handle_t paral_ctx, uint32 id1, uint32 id2)
{
    ...
    for (;;) {
        if (mtrl_recreate_segment(pool1->mtrl_ctx, &segment, pool1->segment) != CT_SUCCESS) {
            status = CT_ERROR;
            break;
        }

        if (mtrl_open_segment2(pool1->mtrl_ctx, pool1->segment) != CT_SUCCESS) {
            status = CT_ERROR;
            break;
        }

        if (mtrl_open_sort_cursor(pool1->mtrl_ctx, &segment, &rid1, segment.level, &cursor1) != CT_SUCCESS) {
            status = CT_ERROR;
            break;
        }

        cursor1.ctx = pool1->mtrl_ctx;
        if (mtrl_open_sort_cursor(pool2->mtrl_ctx, pool2->segment, &rid2, pool2->segment->level, &cursor2) != CT_SUCCESS) {
        ...
        if (mtrl_merge_2pools_cursor(pool1->mtrl_ctx, pool1->segment, &cursor1, &cursor2) != CT_SUCCESS) {
            mtrl_close_sort_cursor(pool1->mtrl_ctx, &cursor1);
...
status_t mtrl_sort_segment_parallel(mtrl_sort_ctrl_t *sort_ctrl, mtrl_context_t *ctx, uint32 seg_id)
{
    ...
    if (mtrl_split_segment(ctx, seg_id, &new_seg_id) != CT_SUCCESS) {
        return CT_ERROR;
    }
    sort_ctrl->cur_seg_id = seg_id;

    if (cm_create_thread(mtrl_sort_segment_proc, 0, sort_ctrl, &sort_ctrl->threads[0]) != CT_SUCCESS) {
        return CT_ERROR;
    }

    if (mtrl_sort_segment(ctx, new_seg_id) != CT_SUCCESS) {
        cm_close_thread(&sort_ctrl->threads[0]);
        return CT_ERROR;
    }
    cm_close_thread(&sort_ctrl->threads[0]);

    if (sort_ctrl->threads[0].result != CT_SUCCESS) {
        CT_THROW_ERROR(ERR_BUILD_INDEX_PARALLEL, ctx->err_msg);
        return CT_ERROR;
    }

    if (mtrl_merge_2segments(ctx, seg_id, new_seg_id) != CT_SUCCESS) {
        return CT_ERROR;
    }
...
static status_t mtrl_sort_segment_last_page(mtrl_context_t *ctx, mtrl_segment_t *segment)
{
    ...
    vm_page_t *page = NULL;
    uint32 vmid = segment->vm_list.last;
    if (mtrl_open_page(ctx, vmid, &page) != CT_SUCCESS) {
        return CT_ERROR;
    }
    status_t ret = mtrl_sort_single_page(ctx, segment, (mtrl_page_t *)page->data);
    mtrl_close_page(ctx, vmid);
    return ret;
}
...
status_t mtrl_sort_segment2(mtrl_context_t *ctx, mtrl_segment_t *segment)
{
    ...
    knl_session_t *session = (knl_session_t *)MTRL_SESSION(ctx);
    ...
    if (mtrl_sort_segment_last_page(ctx, segment) != CT_SUCCESS) {
        return CT_ERROR;
    }
    session->stat->sorts++;
    level = segment->level;
    ...
    knl_begin_session_wait(session, MTRL_SEGMENT_SORT, CT_TRUE);
    while (!finished) {
        curr_seg = *segment;
        status = mtrl_merge_sort(ctx, &curr_seg, segment, level, &start_rid, &finished);
        vm_free_list(MTRL_SESSION(ctx), MTRL_POOL(ctx), &curr_seg.vm_list);

        if (status != CT_SUCCESS) {
            cm_get_error(&err_code, &err_msg, NULL);
            ret = memcpy_sp(ctx->err_msg, CT_MESSAGE_BUFFER_SIZE, err_msg, CT_MESSAGE_BUFFER_SIZE);
            knl_securec_check(ret);
...
void mtrl_close_segment(mtrl_context_t *ctx, uint32 seg_id)
{
...
status_t mtrl_insert_row2(mtrl_context_t *ctx, mtrl_segment_t *segment, char *row, mtrl_rowid_t *rid)
{
    ...
    if (page->free_begin + row_size + sizeof(uint32) > CT_VMEM_PAGE_SIZE - MTRL_DIR_SIZE(page)) {
        if (segment->cmp_items != NULL) {
            if (mtrl_sort_single_page(ctx, segment, page) != CT_SUCCESS) {
                return CT_ERROR;
            }
        }
        mtrl_close_page(ctx, segment->vm_list.last);
...
status_t mtrl_insert_row_in_page(mtrl_context_t *ctx, uint32 page_id, const char *row, uint32 size, bool32 *is_full)
{
    uint32 slot;
    vm_page_t *vm_page = NULL;
    if (vm_open(ctx->session, ctx->pool, page_id, &vm_page) != CT_SUCCESS) {
        return CT_ERROR;
    }
    ...
    if (page->free_begin + row_size + sizeof(uint32) > CT_VMEM_PAGE_SIZE - MTRL_DIR_SIZE(page)) {
        *is_full = CT_TRUE;
        vm_close(ctx->session, ctx->pool, page_id, VM_ENQUE_TAIL);
        return CT_SUCCESS;
    }
    *is_full = CT_FALSE;
...
void mtrl_sort_proc(thread_t *thread)
{
    ...
    while (!thread->closed) {
        ...
        knl_panic(page != NULL);
        if (mtrl_sort_page(ctrl->ctx, ctrl->segment, (mtrl_page_t *)page->data) != CT_SUCCESS) {
...
status_t mtrl_sort_clean(mtrl_sort_ctrl_t *sort_ctrl)
{
    ...
    while (sort_ctrl->sort_pages.count > 0) {
        if (mtrl_sort_from_queue(sort_ctrl) != CT_SUCCESS) {
            status = CT_ERROR;
        }
    }
    ...
    for (uint32 i = 0; i < sort_ctrl->thread_count; i++) {
        ...
        if (sort_ctrl->threads[i].result != CT_SUCCESS) {
            status = CT_ERROR;
    ...
    sort_ctrl->initialized = CT_FALSE;
    sort_ctrl->insert_complete = CT_TRUE;
                                  ...
                                  mtrl_rowid_t *rid)
{
    ...
    if (page->free_begin + row_size + sizeof(uint32) > CT_VMEM_PAGE_SIZE - MTRL_DIR_SIZE(page)) {
        ...
        while (sort_ctrl->sort_pages.count + sort_ctrl->thread_count >= CT_MAX_MTRL_OPEN_PAGES) {
            if (mtrl_sort_from_queue(sort_ctrl) != CT_SUCCESS) {
                return CT_ERROR;
            }
        }
...
static inline status_t mtrl_get_array_value(ct_type_t datatype, char *ptr, uint32 len, variant_t *value)
{
    ...
    switch (lob_type) {
        case CT_LOB_FROM_KERNEL:
            ...
            value->v_array.value.knl_lob.is_hex_const = CT_FALSE;
...
status_t mtrl_get_column_value(mtrl_row_assist_t *row, bool32 eof, uint32 id, ct_type_t datatype, bool8 is_array,
    variant_t *value)
{
    char *ptr = NULL;
    uint32 len, lob_type;
    ...
    if (SECUREC_UNLIKELY(is_array == CT_TRUE)) {
    ...
    switch (datatype) {
        ...
        case CT_TYPE_DECIMAL:
            if (cm_dec_4_to_8(VALUE_PTR(dec8_t, value), (dec4_t *)ptr, len) != CT_SUCCESS) {
                return CT_ERROR;
            }
            break;
        case CT_TYPE_NUMBER2:
            if (cm_dec_2_to_8(VALUE_PTR(dec8_t, value), (const payload_t *)ptr, len) != CT_SUCCESS) {
                return CT_ERROR;
            }
            break;
        ...
        case CT_TYPE_IMAGE: {
            ...
            if (lob_type == CT_LOB_FROM_KERNEL) {
                ...
                CT_THROW_ERROR(ERR_UNKNOWN_LOB_TYPE, "do get column value from mtrl");
                return CT_ERROR;
            }
            break;
        }
...
{
    rid->vmid = CT_INVALID_ID32;
    rid->slot = CT_INVALID_ID32;
}

status_t mtrl_init_cursor(mtrl_cursor_t *cursor)
{
    cursor->row.data = NULL;
    cursor->rs_vmid = CT_INVALID_ID32;
    cursor->rs_page = NULL;
    cursor->eof = CT_FALSE;
    cursor->count = 0;
    cursor->slot = 0;
...
status_t mtrl_open_cursor2(mtrl_context_t *ctx, mtrl_segment_t *segment, mtrl_cursor_t *cursor)
{
    ...
    if (mtrl_open_sort_cursor(ctx, segment, &rid, segment->level, &cursor->sort) != CT_SUCCESS) {
        return CT_ERROR;
    }
...
void mtrl_close_cursor(mtrl_context_t *ctx, mtrl_cursor_t *cursor)
{
    ...
    if (cursor->rs_vmid != CT_INVALID_ID32) {
...
status_t mtrl_fetch_sort_key(mtrl_context_t *ctx, mtrl_cursor_t *cursor)
{
    ...
    if (cursor->sort.rownum > 1) {
        if (mtrl_move_sort_cursor(ctx, &cursor->sort, mtrl_close_sorted_page) != CT_SUCCESS) {
            return CT_ERROR;
        }
    }
                                ...
                                bool32 *ord_chged)
{
    ...
    if (mtrl_fetch_winsort_key(ctx, &cursor->sort, cmp_flag, grp_chged, ord_chged, &cursor->eof) != CT_SUCCESS) {
        return CT_ERROR;
    }
    ...
    if (rid->vmid != cursor->rs_vmid) {
        ...
        if (mtrl_open_page(ctx, rid->vmid, &vm_page) != CT_SUCCESS) {
            return CT_ERROR;
        }
...
status_t mtrl_move_rs_cursor(mtrl_context_t *ctx, mtrl_cursor_t *cursor)
{
    ...
    ctrl = vm_get_ctrl(MTRL_POOL(ctx), cursor->rs_vmid);
    next = ctrl->next;
    mtrl_close_page(ctx, cursor->rs_vmid);
    cursor->rs_vmid = next;
...
{
    if (mtrl_move_rs_cursor(ctx, cursor) != CT_SUCCESS) {
        return CT_ERROR;
    }
    ...
    sibl_sort_row_t *sibl_row)
{
    ...
    if (mtrl_fetch_sort_key(ctx, curr_level_cursor) != CT_SUCCESS) {
        return CT_ERROR;
    }
    ...
    *sibl_row = *(sibl_sort_row_t *)(row + row_size - sizeof(sibl_sort_row_t));
    ...
    if (sibl_row->rs_rid.vmid != cursor->rs_vmid) {
        ...
        if (mtrl_open_page(ctx, sibl_row->rs_rid.vmid, &vm_page) != CT_SUCCESS) {
            return CT_ERROR;
        }
...
{
    if (mtrl_fetch_group_row(ctx, &cursor->sort, group_changed, &cursor->eof) != CT_SUCCESS) {
        mtrl_close_cursor(ctx, cursor);
        return CT_ERROR;
    }
                                    ...
                                    mtrl_cursor_t *cursor)
{
    ...
    if (cursor->sort.vmid != savepoint->vm_row_id.vmid) {
        if (cursor->sort.vmid != CT_INVALID_ID32) {
            mtrl_close_page(ctx, cursor->sort.vmid);
            cursor->sort.vmid = CT_INVALID_ID32;
        }
        if (mtrl_open_page(ctx, savepoint->vm_row_id.vmid, &page) != CT_SUCCESS) {
            return CT_ERROR;
        }
...
{
    if (mtrl_open_savepoint(ctx, sort_seg, savepoint, cursor) != CT_SUCCESS) {
        return CT_ERROR;
    }
    return mtrl_fetch_multi_row(ctx, cursor, rows, count);
}
                             ...
                             mtrl_rowid_t *rid, bool32 need_close)
{
    ...
    if (mtrl_ensure_page_size(ctx, segment, var_size, need_close) != CT_SUCCESS) {
        return CT_ERROR;
    }
    page = (mtrl_page_t *)segment->curr_page->data;
...
bool32 mtrl_win_cur_page_is_enough(mtrl_context_t *ctx, uint32 seg_id, uint32 size)
{
    ...
    return (page->free_begin + size <= CT_VMEM_PAGE_SIZE - MTRL_DIR_SIZE(page));
}
...
status_t mtrl_win_aggr_append_data(mtrl_context_t *ctx, uint32 seg_id, const char *data, uint32 size)
{
    ...
    if (page->free_begin + size > CT_VMEM_PAGE_SIZE - MTRL_DIR_SIZE(page)) {
        return CT_ERROR;
    }
    if (size > 0) {
...
status_t mtrl_win_aggr_get(mtrl_context_t *ctx, uint32 seg_id, char **row, mtrl_rowid_t *rid, bool32 need_close)
{
    ...
    if (segment->curr_page->vmid != rid->vmid) {
        ...
        if (mtrl_open_page(ctx, rid->vmid, &segment->curr_page) != CT_SUCCESS) {
            return CT_ERROR;
        }
        page = (mtrl_page_t *)segment->curr_page->data;
    }
...
bool32 mtrl_fill_page_up(mtrl_context_t *ctx, mtrl_page_t *dst_page, mtrl_page_t *src_page)
{
    ...
    for (uint16 i = src_page->rows; i > 0; i--) {
        ...
        if (dst_page->free_begin + src_row->size + sizeof(uint32) > CT_VMEM_PAGE_SIZE - MTRL_DIR_SIZE(dst_page)) {
            return CT_TRUE;
        }
