...
{
    session->kernel->db.ctrl.core.log_mode = ARCHIVE_LOG_OFF;
    return;
}
...
status_t dbc_build_logfiles(knl_session_t *session, galist_t *logfiles, uint8 node_id)
{
    ...
    ctrl->rcy_point.asn = GS_FIRST_ASN;
    ctrl->lrp_point.asn = GS_FIRST_ASN;
    ...
    for (i = 0; i < logfiles->count; i++) {
        ...
        if (logfile.ctrl->type == DEV_TYPE_ULOG &&
            cm_device_capacity(logfile.ctrl->type, &logfile.ctrl->size) != GS_SUCCESS) {
            GS_LOG_RUN_ERR("[DB] failed to get device(%u, %s) capacity.", logfile.ctrl->type, logfile.ctrl->name);
            return GS_ERROR;
        }
        ...
        if (cm_exist_device(logfile.ctrl->type, logfile.ctrl->name)) {
            GS_THROW_ERROR(ERR_OBJECT_EXISTS, "redo log file", logfile.ctrl->name);
            return GS_ERROR;
        }

        if (cm_build_device(logfile.ctrl->name, logfile.ctrl->type, kernel->attr.xpurpose_buf, GS_XPURPOSE_BUFFER_SIZE,
                            logfile.ctrl->size, knl_io_flag(session), GS_FALSE, &logfile.handle) != GS_SUCCESS) {
        ...
        if (db_save_log_ctrl(session, logfile.ctrl->file_id, node_id) != GS_SUCCESS) {
...
static status_t dbc_init_space(knl_session_t *session, knl_database_def_t *def)
{
    ...
    def->sysaux_space.is_for_create_db = GS_FALSE;
...
static void dbc_init_doublewrite(knl_session_t *session)
{
    ...
    CM_ASSERT(DATAFILE_GET(session, dw_space->ctrl->files[0])->ctrl->size >=
              (dw_space->head->hwms[0] + GS_MIN_SYSAUX_DATAFILE_SIZE) * DEFAULT_PAGE_SIZE(session));
...
{
    row_assist_t ra;
    char action[GS_MAX_ACTION_LEN] = "INSTALL";
    timeval_t now;

    CM_SAVE_STACK(session->stack);
    knl_cursor_t *cursor = knl_push_cursor(session);
    knl_open_sys_cursor(session, cursor, CURSOR_ACTION_INSERT, SYS_UPGRADE_RECORD_ID, GS_INVALID_ID32);
...
status_t dbc_register_iof(knl_instance_t *kernel)
{
    if (knl_dbs_is_enable_dbs()) {
        if (cm_dbs_create_all_ns() != GS_SUCCESS) {
            GS_LOG_RUN_ERR("Failed to build dbstore namespace.");
            return GS_ERROR;
        }
...
status_t dbc_create_database(knl_handle_t session, knl_database_def_t *def, bool32 clustered)
{
    ...
    if (DB_ATTR_CLUSTER(knl_session) != clustered) {
        GS_THROW_ERROR(ERR_INVALID_DATABASE_DEF, "parameter CLUSTER_DATABASE is not equal to that of create database");
        return GS_ERROR;
    }
    ...
    if (dbc_register_iof(kernel) != GS_SUCCESS) {
        return GS_ERROR;
    }
    ...
    if (dtc_build_logfiles(knl_session, def) != GS_SUCCESS) {
        return GS_ERROR;
    }
    ...
    if (dtc_init_undo_spaces(knl_session, def) != GS_SUCCESS) {
        return GS_ERROR;
    }
    ...
    if (dtc_save_all_ctrls(knl_session, def->nodes.count) != GS_SUCCESS) {
        return GS_ERROR;
    }
    ...
    if (db_record_install_info(knl_session) != GS_SUCCESS) {
        return GS_ERROR;
    }
