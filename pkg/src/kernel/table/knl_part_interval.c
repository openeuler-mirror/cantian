    ...
    dec8_t *interval_dec)
{
    ...
    if (cm_dec_to_int32(&div_result, &interval_count, ROUND_FLOOR) != GS_SUCCESS) {
        cm_reset_error();
    ...
    uint32 trans_id, uint32 part_pos)
{
    ...
    errno_t ret = snprintf_s(new_name, GS_NAME_BUFFER_SIZE, GS_NAME_BUFFER_SIZE - 1, "_SYS_%llX_P%u",
    ...
    uint32 trans_id, uint32 part_pos)
{
    ...
    if (IS_COMPART_INDEX(index->part_index)) {
        if (db_upd_parent_idx_partid(session, &index->desc, old_partid, new_partid) != GS_SUCCESS) {
            return GS_ERROR;
        }
    }
                                           ...
                                           binary_t *bhiboundval, uint32 part_no)
{
    ...
    switch (part_table->keycols->datatype) {
        ...
        case GS_TYPE_DECIMAL:
            ...
            (void)cm_dec_to_text(&inter_bound.dec_val, GS_MAX_DEC_OUTPUT_ALL_PREC, hiboundval);
    ...
    part_table_t *part_table, uint32 partno)
{
    ...
    errno_t ret = snprintf_s(part_desc->name, GS_NAME_BUFFER_SIZE, GS_NAME_BUFFER_SIZE - 1, "_SYS_P%u", pos);
    knl_securec_check_ss(ret);
}
