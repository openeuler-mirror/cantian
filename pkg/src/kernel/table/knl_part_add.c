...
status_t db_create_hash_lob_part_segment(knl_session_t *session, knl_dictionary_t *dc, uint32 column_idx)
{
    ...
    if (dc_open_table_private(session, dc->uid, dc->oid, &new_dc) != GS_SUCCESS) {
        GS_LOG_RUN_ERR("open dc private fail, uid %u, oid %u", dc->uid, dc->oid);
        return GS_ERROR;
    }
    ...
    if (lob_create_part_segment(session, lob_part) != GS_SUCCESS) {
        dc_close_table_private(&new_dc);
        GS_LOG_RUN_ERR("create hash lob part segment fail, uid %u, oid %u, partno %u",
    ...
    if (lob_generate_create_undo(session, lob_part->desc.entry, lob_part->desc.space_id,
                                 SPACE_IS_LOGGING(SPACE_GET(session, lob_part->desc.space_id))) != GS_SUCCESS) {
        lob_drop_part_segment(session, lob_part);
        dc_close_table_private(&new_dc);
        GS_LOG_RUN_ERR("create hash lob part segment generate undo fail, uid %u, oid %u, partno %u",
                       dc->uid, dc->oid, part_loc.part_no);
        return GS_ERROR;
    }
    dc_close_table_private(&new_dc);
                            ...
                            knl_table_part_desc_t *part_desc)
{
    ...
    if (db_create_index_segment_of_hash_parts(session, dc, table) != GS_SUCCESS) {
        CM_RESTORE_STACK(session->stack);
        return GS_ERROR;
    }
    CM_RESTORE_STACK(session->stack);
...
status_t db_create_hash_part_segment(knl_session_t *session, knl_dictionary_t *dc)
{
    ...
    if (db_create_hash_part_heap_segment_impl(session, table_part, part_loc) != GS_SUCCESS) {
        dc_close_table_private(&new_dc);
        GS_LOG_RUN_ERR("create hash part segment fail, uid %u, oid %u, partno %u",
                       dc->uid, dc->oid, part_loc.part_no);
        return GS_ERROR;
    }
    ...
    if (heap_generate_create_undo(session, table_part->desc.entry, table_part->desc.space_id,
                                  SPACE_IS_LOGGING(SPACE_GET(session, table_part->desc.space_id))) != GS_SUCCESS) {
        heap_drop_part_segment(session, table_part);
        dc_close_table_private(&new_dc);
        GS_LOG_RUN_ERR("create hash part segment generate undo fail, uid %u, oid %u, partno %u",
                       dc->uid, dc->oid, part_loc.part_no);
        return GS_ERROR;
    }
                                               ...
                                               uint32 part_id)
{
    ...
    if (db_create_hash_part_segment(session, dc) != GS_SUCCESS) {
        return GS_ERROR;
    }
    for (uint32 i = 0; i < table->desc.column_count; i++) {
        if (db_create_hash_lob_part_segment(session, dc, i) != GS_SUCCESS) {
            return GS_ERROR;
        }
    }

    if (dc_open_table_private(session, dc->uid, dc->oid, &new_dc) != GS_SUCCESS) {
        GS_LOG_RUN_ERR("open dc private fail, uid %u, oid %u, part_id %u", dc->uid, dc->oid, part_id);
        return GS_ERROR;
    }
    knl_part_def_t *part_def = (knl_part_def_t *)cm_galist_get(&def->part_def.obj_def->parts, 0);
    if (db_addpart_redis_data(session, &new_dc, part_id, part_def->is_parent) != GS_SUCCESS) {
        dc_close_table_private(&new_dc);
        GS_LOG_RUN_ERR("open dc private fail, uid %u, oid %u, part_id %u", dc->uid, dc->oid, part_id);
        return GS_ERROR;
    }
...
status_t db_altable_add_part(knl_session_t *session, knl_dictionary_t *dc, knl_altable_def_t *def)
{
    ...
    if (db_add_index_parts(session, dc, table, &desc) != GS_SUCCESS) {
        return GS_ERROR;
    }
    ...
    if (DB_ATTR_COMPATIBLE_MYSQL(session)) {
        if (db_add_hash_part_redistribute2(session, dc, def, part_id) != GS_SUCCESS) {
            return GS_ERROR;
        }
    } else {
