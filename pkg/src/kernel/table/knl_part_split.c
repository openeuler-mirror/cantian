...
static bool32 part_generate_split_id(table_t *table, uint32 split_id, uint32 *new_id1, uint32 *new_id2)
{
    ...
    if (split_id == 1) {
        *new_id1 = split_id + CT_DFT_PARTID_STEP;

        while (*new_id1 < split_id + CT_MAX_PART_ID_GAP) {
            if (part_is_id_conflict(table, *new_id1)) {
                *new_id1 += CT_DFT_PARTID_STEP;
                continue;
            }
            break;
        }
        ...
        if (*new_id1 - split_id >= CT_MAX_PART_ID_GAP) {
            *new_id1 = split_id + CT_DFT_PARTID_STEP + 1;
            for (;;) {
                if (!part_is_id_conflict(table, *new_id1)) {
                    break;
                }
    ...
    uint32 split_pos, uint32 new_partid)
{
    ...
    while (!cursor->eof) {
        new_rpid = new_partid + (idx - split_pos) * CT_DFT_PARTID_STEP;
        ...
        if (IS_PARENT_IDXPART(&desc)) {
            if (db_upd_parent_idx_partid(session, &index->desc, desc.part_id, new_rpid) != CT_SUCCESS) {
                return CT_ERROR;
            }
        }
    ...
    uint32 org_partid, uint32 new_partid)
{
    ...
    for (uint32 i = 0; i < index_set->total_count; i++) {
        ...
        if (part_split_update_idx_rpids(session, cursor, index, split_pos, new_partid) != CT_SUCCESS) {
            return CT_ERROR;
        }
    }
    ...
    uint32 split_pos, uint32 new_partid)
{
    ...
    while (!cursor->eof) {
        uint32 new_rpid = new_partid + (idx - split_pos) * CT_DFT_PARTID_STEP;
        row_init(&ra, cursor->update_info.data, HEAP_MAX_ROW_SIZE(session), UPDATE_COLUMN_COUNT_ONE);
        ...
        if (IS_PARENT_LOBPART(&desc)) {
            if (db_update_parent_lobpartid(session, &lob->desc, desc.part_id, new_rpid) != CT_SUCCESS) {
                return CT_ERROR;
            }
        }
    ...
    uint32 org_partid, uint32 new_partid)
{
    ...
    for (uint32 i = 0; i < table->desc.column_count; i++) {
        ...
        if (part_split_update_lob_rpids(session, cursor, table, lob, split_pos, new_partid) != CT_SUCCESS) {
            return CT_ERROR;
        }
    }
    ...
    uint32 org_partid, uint32 new_partid)
{
    ...
    while (!cursor->eof) {
        ...
        if (IS_PARENT_TABPART(&desc)) {
            if (db_update_parent_tabpartid(session, desc.uid, desc.table_id, desc.part_id, new_rpid) != CT_SUCCESS) {
                return CT_ERROR;
            }
        }
    ...
    bool32 reuse_orgpart)
{
    ...
    if (db_altable_drop_part(session, dc, def, CT_FALSE) != CT_SUCCESS) {
        return CT_ERROR;
    }
    CT_LOG_DEBUG_INF("drop the left spliting part %s", T2S(&def->part_def.name));
    ...
    CT_LOG_DEBUG_INF("drop the right spliting part %s", T2S(&def->part_def.name));
    return CT_SUCCESS;
}
...
static status_t part_split_rebuild_global_index(knl_session_t *session, knl_dictionary_t *dc, knl_handle_t knl_table)
{
    ...
    for (uint32 i = 0; i < table->index_set.total_count; i++) {
        ...
        if (!IS_PART_INDEX(index) && index->desc.is_invalid) {
            ...
            def.rebuild.build_stats = CT_FALSE;
            def.rebuild.pctfree = CT_INVALID_ID32;
            def.rebuild.cr_mode = index->desc.cr_mode;
            def.rebuild.is_online = CT_FALSE;
            def.rebuild.parallelism = 0;
            def.rebuild.specified_parts = 0;
            def.rebuild.lock_timeout = LOCK_INF_WAIT;
            if (db_alter_index_rebuild(session, &def, dc, index) != CT_SUCCESS) {
                return CT_ERROR;
            }
        }
    }
    ...
    knl_altable_def_t *def, table_part_t *split_part)
{
    ...
    if (table->part_table->desc.partcnt + CT_SPLIT_PART_COUNT - 1 > CT_MAX_PART_COUNT) {
    ...
    if (part_compare_range_key(column_desc, &part_key.decoder, table_part->desc.groups) <= 0) {
        CT_THROW_ERROR(ERR_INVALID_PART_KEY, "split partition bound must collate higher than its previous partition.");
        return CT_ERROR;
    }
    ...
    uint32 new_partid)
{
    ...
    if (part_split_update_table_rpids(session, cursor, table, org_partid, new_partid) != CT_SUCCESS) {
        CM_RESTORE_STACK(session->stack);
        return CT_ERROR;
    }

    if (part_split_update_idxs_rpids(session, table, cursor, org_partid, new_partid) != CT_SUCCESS) {
        CM_RESTORE_STACK(session->stack);
        return CT_ERROR;
    }

    if (part_split_update_lobs_rpids(session, dc, cursor, org_partid, new_partid) != CT_SUCCESS) {
        CM_RESTORE_STACK(session->stack);
        return CT_ERROR;
    }
    ...
    table_part_t *split_part, knl_table_part_desc_t *parent_desc)
{
    ...
    for (uint32 i = 0; i < split_part->desc.subpart_cnt; i++) {
        ...
        cm_str2text("SYS", &sys);
        if (knl_seq_nextval(session, &sys, &name, &object_id) != CT_SUCCESS) {
            return CT_ERROR;
        }
        
        ret = snprintf_s(desc.name, CT_NAME_BUFFER_SIZE, CT_NAME_BUFFER_SIZE - 1, "SYS_SUBP%llX", object_id);
...
part table or add non-encrypt partition to encrypt part table.");
    ...
    CM_SAVE_STACK(session->stack);
    knl_cursor_t *cursor = knl_push_cursor(session);
    if (db_write_sys_tablepart(session, cursor, &table->desc, desc) != CT_SUCCESS) {
        CM_RESTORE_STACK(session->stack);
        return CT_ERROR;
    }

    if (desc->storaged && db_write_sysstorage(session, cursor, desc->org_scn, &desc->storage_desc) != CT_SUCCESS) {
        CM_RESTORE_STACK(session->stack);
        return CT_ERROR;
    }
    ...
    if (desc->is_parent) {
        if (part_split_create_new_subparts(session, cursor, table, split_part, desc) != CT_SUCCESS) {
            CM_RESTORE_STACK(session->stack);
            return CT_ERROR;
        }
    }
    ...
    knl_table_part_desc_t *part_desc)
{
    ...
    for (uint32 i = 0; i < table->index_set.total_count; i++) {
        ...
        knl_securec_check(ret);
        ret = memcpy_sp(desc.name, CT_NAME_BUFFER_SIZE, part_desc->name, CT_NAME_BUFFER_SIZE);
        knl_securec_check(ret);
        desc.entry = INVALID_PAGID;
        desc.org_scn = db_inc_scn(session);
        desc.seg_scn = desc.org_scn;
    ...
    if (part_split_create_new_subidxpart(session, cursor, table, split_part, part_desc) != CT_SUCCESS) {
        CM_RESTORE_STACK(session->stack);
        return CT_ERROR;
    }
    ...
    knl_table_part_desc_t *part_desc, table_part_t *split_part)
{
    ...
    for (uint32 i = 0; i < table->desc.column_count; i++) {
        ...
        if (part_desc->is_parent) {
            ...
            lob_desc.is_parent = CT_TRUE;
        }

        if (part_write_sys_lobpart(session, cursor, &lob_desc) != CT_SUCCESS) {
            CM_RESTORE_STACK(session->stack);
            return CT_ERROR;
        }
        ...
        if (part_desc->is_parent) {
            ...
            for (uint32 j = 0; j < part->desc.subpart_cnt; j++) {
                ...
                if (subpart_write_syslob(session, &lob_desc) != CT_SUCCESS) {
                    CM_RESTORE_STACK(session->stack);
                    return CT_ERROR;
                }
            }
                                        ...
                                        uint32 part_id, knl_table_part_desc_t *desc)
{
    ...
    ret = memcpy_sp(desc->name, CT_NAME_BUFFER_SIZE, part_def->name.str, part_def->name.len);
    knl_securec_check(ret);
    desc->name[part_def->name.len] = '\0';
    
    if (part_def->space.len != 0) {
        if (spc_get_space_id(session, &part_def->space, part_def->is_for_create_db, &desc->space_id) != CT_SUCCESS) {
            return CT_ERROR;
        }
    }
    ...
    if (part_def->pctfree != CT_INVALID_ID32) {
    ...
    desc->not_ready = CT_TRUE;
    desc->entry = INVALID_PAGID;
    if (desc->compress_algo > COMPRESS_NONE) {
        if (!IS_SPACE_COMPRESSIBLE(space)) {
            CT_THROW_ERROR(ERR_OPERATIONS_NOT_SUPPORT, "split part table",
    ...
    knl_table_part_desc_t *desc, table_part_t *split_part)
{
    ...
    if (part_split_create_new_tabpart(session, dc, split_part, desc) != CT_SUCCESS) {
        return CT_ERROR;
    }

    if (part_split_create_new_index_part(session, table, split_part, desc) != CT_SUCCESS) {
        return CT_ERROR;
    }

    if (part_split_create_new_lobpart(session, dc, part_def, desc, split_part) != CT_SUCCESS) {
        return CT_ERROR;
    }
    ...
    table_part_t *split_part, bool32 reuse_orgpart, uint32 *new_partid)
{
    ...
    if (update_part_ids) {
        if (part_split_update_rpids(session, dc, split_part->desc.part_id, right_partid) != CT_SUCCESS) {
            return CT_ERROR;
        }
    }
    ...
    *new_partid = right_partid;
    ...
    if (part_split_get_partdesc(session, part_def, split_part, left_partid, &desc) != CT_SUCCESS) {
        return CT_ERROR;
    }
    ...
    desc.is_nologging = CT_FALSE;
    if (part_split_create_newpart(session, dc, part_def, &desc, split_part) != CT_SUCCESS) {
        return CT_ERROR;
    }
    ...
    if (part_split_get_partdesc(session, part_def, split_part, right_partid, &desc) != CT_SUCCESS) {
        return CT_ERROR;
    }
    ...
    knl_cursor_t *cursor_delete, knl_cursor_t *cursor_insert)
{
    ...
    cursor_delete->is_splitting = CT_TRUE;
    ...
    cursor_insert->is_splitting = CT_TRUE;
    ...
    table_part_t **right_part, bool32 reuse_orgpart)
{
    ...
    if (*left_part == NULL) {
        CT_LOG_DEBUG_INF("could not find the part, name is %s", T2S(&left_part_def->name));
        CT_THROW_ERROR(ERR_PARTITION_NOT_EXIST, "table", T2S(&left_part_def->name));
        return CT_ERROR;
    }

    if (*right_part == NULL && !reuse_orgpart) {
        CT_LOG_DEBUG_INF("could not find the part, name is %s", T2S(&right_part_def->name));
        CT_THROW_ERROR(ERR_PARTITION_NOT_EXIST, "table", T2S(&right_part_def->name));
        return CT_ERROR;
    }
    ...
    knl_cursor_t *cursor_insert, knl_altable_def_t *def)
{
    ...
    if (part_split_redis_get_newparts(table, def, &left_part, &right_part, CT_TRUE) != CT_SUCCESS) {
        return CT_ERROR;
    }
    ...
    knl_cursor_t *cursor_insert, knl_altable_def_t *def, bool32 reuse_orgpart)
{
    ...
    if (part_split_redis_get_newparts(table, def, &left_part, &right_part, CT_FALSE) != CT_SUCCESS) {
        return CT_ERROR;
    }
    ...
    knl_cursor_t *cursor_insert, knl_altable_def_t *def, bool32 reuse_orgpart)
{
    ...
    if (IS_PARENT_TABPART(&split_part->desc)) {
        for (uint32 i = 0; i < split_part->desc.subpart_cnt; i++) {
            ...
            if (part_split_redis_entity(session, dc, cursor_delete, cursor_insert, def, reuse_orgpart) != CT_SUCCESS) {
                return CT_ERROR;
            }
        }
    } else {
    ...
    table_part_t *split_part, bool32 reuse_orgpart)
{
    ...
    if (part_split_redisdata_open_cursor(session, dc, cursor_delete, cursor_insert) != CT_SUCCESS) {
        CM_RESTORE_STACK(session->stack);
        return CT_ERROR;
    }
    ...
    if (part_split_redis_rows(session, dc, cursor_delete, cursor_insert, def, reuse_orgpart) != CT_SUCCESS) {
    ...
    const char *new_name)
{
    ...
    knl_set_scan_key(INDEX_DESC(cursor->index), key, CT_TYPE_INTEGER, &split_part->desc.uid,
                     sizeof(uint32), IX_COL_SYS_TABLEPART001_USER_ID);
    knl_set_scan_key(INDEX_DESC(cursor->index), key, CT_TYPE_INTEGER, &split_part->desc.table_id,
                     sizeof(uint32), IX_COL_SYS_TABLEPART001_TABLE_ID);
    knl_set_scan_key(INDEX_DESC(cursor->index), key, CT_TYPE_INTEGER, &split_part->desc.part_id,
                     sizeof(uint32), IX_COL_SYS_TABLEPART001_PART_ID);
    ...
    const char *new_name, uint32 index_id)
{
    ...
    knl_set_scan_key(INDEX_DESC(cursor->index), key, CT_TYPE_INTEGER, &index_id,
    ...
    table_part_t *split_part, const char *new_name)
{
    ...
    for (uint32 i = 0; i < table->index_set.total_count; i++) {
        ...
        if (part_split_refresh_idxpart(session, cursor, split_part, new_name, index->desc.id) != CT_SUCCESS) {
            return CT_ERROR;
        }
    }
    ...
    text_t *new_name)
{
    ...
    CM_SAVE_STACK(session->stack);
    knl_cursor_t *cursor = knl_push_cursor(session);
    (void)cm_text2str(new_name, name_buffer, CT_NAME_BUFFER_SIZE);
    if (part_split_refresh_tabpart(session, cursor, split_part, name_buffer) != CT_SUCCESS) {
        CM_RESTORE_STACK(session->stack);
        return CT_ERROR;
    }

    if (part_split_refresh_idxparts(session, dc, cursor, split_part, name_buffer) != CT_SUCCESS) {
        CM_RESTORE_STACK(session->stack);
        return CT_ERROR;
    }
    ...
    table_part_t *split_part, bool32 reuse_orgpart, uint32 new_partid)
{
    ...
    if (part_split_redis_get_newparts(table, def, &left_part, &right_part, reuse_orgpart) != CT_SUCCESS) {
        return CT_ERROR;
    }
    
    if (db_update_part_flag(session, dc, table->part_table, left_part->desc.part_id,
        PART_FLAG_TYPE_NOTREADY) != CT_SUCCESS) {
        return CT_ERROR;
    }
    ...
    if (!reuse_orgpart) {
        if (db_drop_part(session, dc, split_part, CT_TRUE) != CT_SUCCESS) {
            return CT_ERROR;
        }
    } else {
        knl_part_def_t *part_def = (knl_part_def_t *)cm_galist_get(&def->part_def.obj_def->parts, 1);
        if (part_split_refresh_orgpart(session, dc, split_part, &part_def->name) != CT_SUCCESS) {
            return CT_ERROR;
        }

        if (db_update_part_id(session, dc, split_part, new_partid) != CT_SUCCESS) {
            return CT_ERROR;
        }
    ...
    if (def->part_def.global_index_option) {
        if (part_split_rebuild_global_index(session, dc, table) != CT_SUCCESS) {
            return CT_ERROR;
        }
    }
    ...
    bool32 reuse_orgpart)
{
    ...
    if (part_split_drop_newpart(session, dc, def, reuse_orgpart) != CT_SUCCESS) {
        CT_LOG_RUN_ERR("[PART] Failed to drop the newly created partition when split the partition %s failed",
    ...
    table_part_t **org_part)
{
    ...
    if (i == part_table->desc.partcnt + part_table->desc.not_ready_partcnt) {
        CT_THROW_ERROR(ERR_OBJECT_NOT_EXISTS, "partition", T2S(part_name));
        return CT_ERROR;
    }
    ...
    bool32 *reuse)
{
    ...
    knl_part_def_t *part_def = (knl_part_def_t *)cm_galist_get(&def->part_def.obj_def->parts, 1);
    if (part_def->space.len != 0) {
        if (spc_get_space_id(session, &part_def->space, def->is_for_create_db, &space_id) != CT_SUCCESS) {
            return CT_ERROR;
        }
    }
...
{
    uint32 new_partid = CT_INVALID_ID32;
    table_part_t *split_part = NULL;
    table_t *table = DC_TABLE(dc);
    part_table_t *part_table = table->part_table;

    CT_LOG_DEBUG_INF("begin to split partition %s", T2S(&def->part_def.name));

    if (!part_table_find_by_name(part_table, &def->part_def.name, &split_part) || split_part->desc.not_ready) {
        CT_THROW_ERROR(ERR_OBJECT_NOT_EXISTS, "normal partition", T2S(&def->part_def.name));
        return CT_ERROR;
    }

    /* check input for splitting a part */
    if (part_split_precheck(session, table, def, split_part) != CT_SUCCESS) {
        return CT_ERROR;
    }
    ...
    if (part_split_refresh_dc(session, dc, &def->part_def.name, &split_part) != CT_SUCCESS) {
        return CT_ERROR;
    }

    bool32 reuse_orgpart = CT_FALSE;
    if (part_split_reuse_orgpart(session, def, split_part, &reuse_orgpart) != CT_SUCCESS) {
        return CT_ERROR;
    }
    
    if (part_split_create_newparts(session, dc, def, split_part, reuse_orgpart, &new_partid) != CT_SUCCESS) {
        return CT_ERROR;
    }

    table = DC_TABLE(dc);
    if (part_split_invalidate_global_index(session, table) != CT_SUCCESS) {
        return CT_ERROR;
    }
    ...
    if (part_split_redis_data(session, dc, def, split_part, reuse_orgpart) != CT_SUCCESS) {
        part_split_handle_error(session, dc, def, reuse_orgpart);
        return CT_ERROR;
    }
    
    if (part_split_finish_lastwork(session, dc, def, split_part, reuse_orgpart, new_partid) != CT_SUCCESS) {
        part_split_handle_error(session, dc, def, reuse_orgpart);
        return CT_ERROR;
    }

    CT_LOG_DEBUG_INF("successfully to split partition %s", T2S(&def->part_def.name));
    return CT_SUCCESS;
}
    ...
    table_part_t *compart, uint32 split_partno)
{
    ...
    if (compart->desc.subpart_cnt + CT_SPLIT_PART_COUNT - 1 > CT_MAX_SUBPART_COUNT) {
        CT_THROW_ERROR(ERR_EXCEED_MAX_SUBPARTCNT, (uint32)CT_MAX_SUBPART_COUNT);
        return CT_ERROR;
    }
    ...
    table_part_t *compart, uint32 split_pos, uint32 new_partid)
{
    ...
    cursor->index_dsc = CT_TRUE;
    knl_init_index_scan(cursor, CT_FALSE);
    knl_set_scan_key(INDEX_DESC(cursor->index), &cursor->scan_range.l_key, CT_TYPE_INTEGER, &part_index->desc.uid,
        sizeof(uint32), IX_COL_SYS_INDEXSUBPART001_USER_ID);
    knl_set_scan_key(INDEX_DESC(cursor->index), &cursor->scan_range.l_key, CT_TYPE_INTEGER, &part_index->desc.table_id,
        sizeof(uint32), IX_COL_SYS_INDEXSUBPART001_TABLE_ID);
    knl_set_scan_key(INDEX_DESC(cursor->index), &cursor->scan_range.l_key, CT_TYPE_INTEGER, &part_index->desc.index_id,
    ...
    knl_set_scan_key(INDEX_DESC(cursor->index), &cursor->scan_range.l_key, CT_TYPE_INTEGER, &old_partid,
        sizeof(uint32), IX_COL_SYS_INDEXSUBPART001_SUB_PART_ID);
    knl_set_scan_key(INDEX_DESC(cursor->index), &cursor->scan_range.r_key, CT_TYPE_INTEGER, &part_index->desc.uid,
        sizeof(uint32), IX_COL_SYS_INDEXSUBPART001_USER_ID);
    knl_set_scan_key(INDEX_DESC(cursor->index), &cursor->scan_range.r_key, CT_TYPE_INTEGER, &part_index->desc.table_id,
        sizeof(uint32), IX_COL_SYS_INDEXSUBPART001_TABLE_ID);
    knl_set_scan_key(INDEX_DESC(cursor->index), &cursor->scan_range.r_key, CT_TYPE_INTEGER, &part_index->desc.index_id,
        sizeof(uint32), IX_COL_SYS_INDEXSUBPART001_INDEX_ID);
    knl_set_scan_key(INDEX_DESC(cursor->index), &cursor->scan_range.r_key, CT_TYPE_INTEGER, &compart->desc.part_id,
        sizeof(uint32), IX_COL_SYS_INDEXSUBPART001_PARENT_PART_ID);
    ...
    while (!cursor->eof) {
        new_rpid = new_partid + (update_cnt - split_pos) * CT_DFT_PARTID_STEP;
        row_init(&ra, cursor->update_info.data, HEAP_MAX_ROW_SIZE(session), UPDATE_COLUMN_COUNT_ONE);
        (void)row_put_int32(&ra, new_rpid);
        cursor->update_info.count = UPDATE_COLUMN_COUNT_ONE;
    ...
    uint32 org_partid, uint32 new_partid)
{
    ...
    for (uint32 i = 0; i < index_set->total_count; i++) {
        ...
        if (subpart_split_update_idx_rpids(session, cursor, index, compart, split_pos, new_partid) != CT_SUCCESS) {
            CM_RESTORE_STACK(session->stack);
            return CT_ERROR;
        }
    }
    ...
    uint32 org_partid, uint32 new_partid)
{
    ...
    for (uint32 i = 0; i < table->desc.column_count; i++) {
        ...
        lob = (lob_t *)column->lob;
        if (subpart_split_update_lob_rpids(session, cursor, lob, compart, split_pos, new_partid) != CT_SUCCESS) {
            CM_RESTORE_STACK(session->stack);
            return CT_ERROR;
        }
    }
    ...
    uint32 org_partid, uint32 new_partid)
{
    ...
    knl_set_scan_key(INDEX_DESC(cursor->index), &cursor->scan_range.r_key, CT_TYPE_INTEGER, &compart->desc.uid,
        sizeof(uint32), IX_COL_SYS_TABLESUBPART001_USER_ID);
    knl_set_scan_key(INDEX_DESC(cursor->index), &cursor->scan_range.r_key, CT_TYPE_INTEGER, &compart->desc.table_id,
        sizeof(uint32), IX_COL_SYS_TABLESUBPART001_TABLE_ID);
    knl_set_scan_key(INDEX_DESC(cursor->index), &cursor->scan_range.r_key, CT_TYPE_INTEGER, &compart->desc.part_id,
        sizeof(uint32), IX_COL_SYS_TABLESUBPART001_PARENT_PART_ID);
    ...
    while (!cursor->eof) {
        uint32 new_rpid = new_partid + (update_cnt - split_pos) * CT_DFT_PARTID_STEP;
        row_init(&ra, cursor->update_info.data, HEAP_MAX_ROW_SIZE(session), UPDATE_COLUMN_COUNT_ONE);
        (void)row_put_int32(&ra, (int32)new_rpid);
        cursor->update_info.count = UPDATE_COLUMN_COUNT_ONE;
    ...
    uint32 org_partid, uint32 new_partid)
{
    ...
    if (subpart_split_update_table_rpids(session, table, compart, org_partid, new_partid) != CT_SUCCESS) {
        return CT_ERROR;
    }

    if (subpart_split_update_idxs_rpids(session, table, compart, org_partid, new_partid) != CT_SUCCESS) {
        return CT_ERROR;
    }

    if (subpart_split_update_lobs_rpids(session, dc, compart, org_partid, new_partid) != CT_SUCCESS) {
        return CT_ERROR;
    }
...
part table or add non-encrypt partition to encrypt part table.");
    ...
    if (db_update_subtabpart_count(session, desc->uid, desc->table_id, desc->parent_partid, CT_TRUE) != CT_SUCCESS) {
        CM_RESTORE_STACK(session->stack);
        return CT_ERROR;
    }
    ...
    knl_table_part_desc_t *new_part_desc, table_part_t *org_part)
{
    ...
    for (uint32 i = 0; i < table->index_set.total_count; i++) {
        ...
        knl_securec_check(ret);
        ret = memcpy_sp(&new_idx_desc.name, CT_NAME_BUFFER_SIZE, new_part_desc->name, CT_NAME_BUFFER_SIZE);
        knl_securec_check(ret);
        ...
        if (part_idx_find_by_name(index->part_index, &text, &index_part) ||
            subpart_idx_find_by_name(index->part_index, &text, &index_subpart)) {
            ret = snprintf_s(new_idx_desc.name, CT_NAME_BUFFER_SIZE,
                CT_NAME_BUFFER_SIZE - 1, "SYS_P%llX", new_idx_desc.org_scn);
            knl_securec_check_ss(ret);
        }

        if (db_write_sys_indsubpart(session, cursor, &new_idx_desc) != CT_SUCCESS) {
            CM_RESTORE_STACK(session->stack);
            return CT_ERROR;
        }

        if (db_update_subidxpart_count(session, &index->desc, new_part_desc->parent_partid, CT_TRUE) != CT_SUCCESS) {
            CM_RESTORE_STACK(session->stack);
            return CT_ERROR;
        }
    }
...
{
    if (subpart_split_create_new_tabpart(session, dc, desc) != CT_SUCCESS) {
        return CT_ERROR;
    }

    if (subpart_split_create_new_idxpart(session, dc, desc, org_part) != CT_SUCCESS) {
        return CT_ERROR;
    }

    if (subpart_split_create_new_lobpart(session, dc, desc) != CT_SUCCESS) {
        return CT_ERROR;
    }
    ...
    table_part_t *split_compart, table_part_t *split_subpart)
{
    ...
    if (part_split_get_partdesc(session, part_def, (table_part_t *)split_subpart, left_partid, &desc) != CT_SUCCESS) {
        return CT_ERROR;
    }
    ...
    desc.bhiboundval.size = part_def->partkey->size;
    desc.is_nologging = CT_FALSE;
    if (subpart_split_create_newpart(session, dc, &desc, split_subpart) != CT_SUCCESS) {
        return CT_ERROR;
    }

    part_def = (knl_part_def_t *)cm_galist_get(&parent_def->subparts, 1);
    if (part_split_get_partdesc(session, part_def, (table_part_t *)split_subpart, right_partid, &desc) != CT_SUCCESS) {
        return CT_ERROR;
    }
    ...
    knl_cursor_t *cursor_insert, knl_altable_def_t *def)
{
    ...
    if (subpart_split_redis_entity(session, dc, cursor_delete, cursor_insert, def) != CT_SUCCESS) {
        return CT_ERROR;
    }
    ...
    table_part_t *split_compart, table_part_t *split_subpart)
{
    ...
    if (subpart_split_redis_rows(session, dc, cursor_delete, cursor_insert, def) != CT_SUCCESS) {
        part_split_redisdata_close_cursor(session, cursor_delete, cursor_insert);
        CM_RESTORE_STACK(session->stack);
        return CT_ERROR;
    }
...
static status_t subpart_split_drop_newpart(knl_session_t *session, knl_dictionary_t *dc, knl_altable_def_t *def)
{
    ...
    if (db_altable_drop_subpartition(session, dc, def, CT_FALSE) != CT_SUCCESS) {
        return CT_ERROR;
    }
    CT_LOG_DEBUG_INF("drop the left spliting part %s", T2S(&def->part_def.name));
    ...
    table_part_t *compart, table_part_t *subpart)
{
    ...
    if (subpart_split_redis_get_newparts(table, def, compart->part_no, &left_part, &right_part) != CT_SUCCESS) {
        return CT_ERROR;
    }
        
    /* update the partition flag as ready */
    if (db_update_subpart_flag(session, dc, compart, left_part->desc.part_id, PART_FLAG_TYPE_NOTREADY) != CT_SUCCESS) {
        return CT_ERROR;
    }

    if (db_update_subpart_flag(session, dc, compart, right_part->desc.part_id, PART_FLAG_TYPE_NOTREADY) != CT_SUCCESS) {
        return CT_ERROR;
    }
...
{
    knl_rollback(session, NULL);
    if (subpart_split_drop_newpart(session, dc, def) != CT_SUCCESS) {
        CT_LOG_RUN_ERR("[PART] Failed to drop the newly created subpartition when split the subpartition %s failed",
            T2S(&def->part_def.name));
        knl_rollback(session, NULL);
    } else {
        knl_commit(session);
    ...
    table_part_t **compart, table_part_t **subpart)
{
    ...
    if (i == (*compart)->desc.subpart_cnt) {
        CT_THROW_ERROR(ERR_OBJECT_NOT_EXISTS, "subpartition", T2S(part_name));
        return CT_ERROR;
    }
...
status_t db_altable_split_subpart(knl_session_t *session, knl_dictionary_t *dc, knl_altable_def_t *def)
{
    ...
    CT_LOG_DEBUG_INF("begin to split subpartition %s", T2S(&def->part_def.name));
    ...
    /* check input for splitting a part */
    if (subpart_split_precheck(session, table, def, compart, subpart->part_no) != CT_SUCCESS) {
        return CT_ERROR;
    }
    ...
    if (subpart_split_refresh_dc(session, dc, &def->part_def.name, &compart, &subpart) != CT_SUCCESS) {
        return CT_ERROR;
    }

    if (subpart_split_create_newparts(session, dc, def, compart, subpart) != CT_SUCCESS) {
        return CT_ERROR;
    }
    ...
    /* redistribute the data of original part */
    if (subpart_split_redis_data(session, dc, def, compart, subpart) != CT_SUCCESS) {
        subpart_handle_split_error(session, dc, def);
        return CT_ERROR;
    }

    if (subpart_split_finish_lastwork(session, dc, def, compart, subpart) != CT_SUCCESS) {
        subpart_handle_split_error(session, dc, def);
        return CT_ERROR;
    }

    CT_LOG_DEBUG_INF("successfully to split subpartition %s", T2S(&def->part_def.name));
    return CT_SUCCESS;
}
