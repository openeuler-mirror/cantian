    ...
    text_t *column, uint16 col_id)
{
    ...
    switch (dc_column->datatype) {
        ...
        case GS_TYPE_INTEGER:
            ...
            break;
        case GS_TYPE_UINT64:
            if (cm_text2uint64(column, &val_ubigint) != GS_SUCCESS) {
                return GS_ERROR;
            }
        ...
        case GS_TYPE_NUMBER2:
            if (cm_text_to_dec(column, &tmp_value.v_dec) != GS_SUCCESS) {
                return GS_ERROR;
            }
            if (cm_adjust_dec(&tmp_value.v_dec, dc_column->precision, dc_column->scale) != GS_SUCCESS) {
                return GS_ERROR;
            }
            status = (GS_IS_NUMBER2_TYPE(dc_column->datatype)) ?
        ...
        case GS_TYPE_CHAR:
            ...
            if (g_knl_callback.convert_char(session, column, dc_column->size, is_char) != GS_SUCCESS) {
                return GS_ERROR;
            }
    ...
    *is_found = GS_FALSE;
    ...
    for (;;) {
        if (cursor->text.len == 0) {
            if (cm_read_file(cursor->fd, cursor->page_buf, DEFAULT_PAGE_SIZE(session), &read_size) != GS_SUCCESS) {
