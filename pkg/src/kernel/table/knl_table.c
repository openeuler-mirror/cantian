...
static status_t db_get_table_spaceid(knl_session_t *session, knl_table_def_t *def, knl_table_desc_t *desc)
{
    if (def->space.str == NULL) {
        ...
        if (spc_get_space_id(session, &def->space, def->is_for_create_db, &desc->space_id) != GS_SUCCESS) {
            return GS_ERROR;
        }
...
static status_t db_check_table_by_spc(knl_session_t *session, space_t *space, knl_table_def_t *def)
{
    ...
    if (def->type == TABLE_TYPE_NOLOGGING && DB_ATTR_CLUSTER(session)) {
        GS_THROW_ERROR(ERR_OPERATIONS_NOT_SUPPORT, "create nologging table", "cluster mode");
        return GS_ERROR;
    }
...
status_t db_init_table_desc_spc(knl_session_t *session, knl_table_desc_t *desc, knl_table_def_t *def)
{
    ...
    if (db_check_table_by_spc(session, space, def) != GS_SUCCESS) {
        return GS_ERROR;
    }
                                   ...
                                   galist_t *lob_stores, bool32 is_for_create_db, knl_lob_desc_t *lob_desc)
{
    ...
    for (i = 0; i < lob_stores->count; i++) {
        ...
        if (store->space.len != 0) {
            ...
            if (spc_get_space_id(session, &store->space, is_for_create_db, &lob_desc->space_id) != GS_SUCCESS) {
                return GS_ERROR;
            }
...
status_t db_create_lob(knl_session_t *session, table_t *table, knl_column_t *column, knl_table_def_t *def)
{
    ...
    if (def != NULL && def->lob_stores.count > 0) {
        ...
        if (db_parse_lob_store(session, column, &def->lob_stores, def->is_for_create_db, lob_desc) != GS_SUCCESS) {
            return GS_ERROR;
        }
    }
                                        ...
                                        uint32 column_idx)
{
    ...
    if (col_id[column_idx] == GS_INVALID_ID16) {
        GS_THROW_ERROR(ERR_COLUMN_NOT_EXIST, DC_ENTRY_NAME(child_dc), T2S(col_name));
        return GS_FALSE;
    }
    if (ref_col_id[column_idx] == GS_INVALID_ID16) {
        GS_THROW_ERROR(ERR_COLUMN_NOT_EXIST, DC_ENTRY_NAME(parent_dc), T2S(ref_col_name));
        return GS_FALSE;
    }
                                         ...
                                         knl_constraint_def_t *def, ref_cons_t *ref_cons)
{
    ...
    col_id = (uint16 *)cm_push(session->stack, sizeof(uint16) * GS_MAX_INDEX_COLUMNS * 3);
    ...
    knl_dictionary_t *ref_dc, ref_cons_t *ref_cons, bool32 is_same)
{
    ...
    if (def->cons_state.is_validate) {
        if (db_verify_reference_data(session, dc, ref_dc, ref_cons) != GS_SUCCESS && !ref->no_need_check) {
            return GS_ERROR;
        }
    }
...
static status_t db_make_col_list_impl(uint32 column_cnt, text_t *txt_cols, uint16 *column_ids)
{
    for (uint32 i = 0; i < column_cnt; i++) {
        cm_concat_int32(txt_cols, GS_MAX_INDEX_COLUMNS * MAX_COLUMN_ID_STR_LEN, column_ids[i]);
        if (i + 1 < column_cnt) {
...
status_t make_column_name_list_str(text_t *txt_column_name, uint32_t column_cnt, galist_t *columns)
{
    ...
    for (int i = 0; i < column_cnt; i++) {
        ...
        cm_concat_n_string(txt_column_name, GS_MAX_INDEX_COLUMNS * GS_NAME_BUFFER_SIZE,
                           col_name->str, col_name->len);
        if (i + 1 < column_cnt) {
            if (cm_concat_string(txt_column_name, GS_MAX_INDEX_COLUMNS * GS_NAME_BUFFER_SIZE, ",") != GS_SUCCESS) {
                return GS_ERROR;
            }
        }
    }
...
{
    knl_reference_def_t *ref = &def->ref;
    char str_col_names[GS_MAX_INDEX_COLUMNS * GS_NAME_BUFFER_SIZE];
    text_t ref_txt_col_names;
    ref_txt_col_names.len = 0;
    ref_txt_col_names.str = str_col_names;
    if (make_column_name_list_str(&ref_txt_col_names, def->columns.count, &def->ref.ref_columns) != GS_SUCCESS) {
        return GS_ERROR;
    }
    char col_names[GS_MAX_INDEX_COLUMNS * GS_NAME_BUFFER_SIZE];
    text_t txt_col_names;
    txt_col_names.len = 0;
    txt_col_names.str = col_names;
    if (make_column_name_list_str(&txt_col_names, def->columns.count, &def->columns) != GS_SUCCESS) {
        return GS_ERROR;
    }
                                  ...
                                  row_assist_t *ra)
{
    ...
    if (!is_same) {
        ...
        if (status != GS_SUCCESS && !ref->no_need_check) {
            return GS_ERROR;
        }
        if (status == GS_SUCCESS && SYNONYM_EXIST(ref_dc)) {
            dc_close(ref_dc);
            GS_THROW_ERROR(ERR_TABLE_OR_VIEW_NOT_EXIST, T2S(&ref->ref_user), T2S_EX(&ref->ref_table));
            return GS_ERROR;
        }
    ...
    CM_SAVE_STACK(session->stack);
    if (status == GS_SUCCESS && db_check_ref_dc(session, dc, def, ref_dc, &ref_cons, is_same) == GS_ERROR) {
    ...
    if (status == GS_SUCCESS) {
        if (fill_ref_info_when_reftab_exist(def, ra, &ref_cons) != GS_SUCCESS) {
            CM_RESTORE_STACK(session->stack);
            return GS_ERROR;
        }
    } else {
        if (fill_ref_info_when_reftab_not_exist(def, ra) != GS_SUCCESS) {
            CM_RESTORE_STACK(session->stack);
            return GS_ERROR;
        }
    }
                                         ...
                                         knl_dictionary_t *dc, text_t *name)
{
    ...
    while (is_found) {
        ret = sprintf_s(new_name, GS_NAME_BUFFER_SIZE, "%s_%u", name->str, id);
                                    ...
                                    knl_index_desc_t *desc)
{
    ...
    for (uint32 i = 0; i < desc->column_count; i++) {
        ...
        if (index_col->is_func) {
            ...
            is_double = GS_IS_DOUBLE_TYPE(type);
        } else {
            desc->columns[i] = knl_get_column_id(dc, &index_col->name);
        ...
        if (is_lob) {
            GS_THROW_ERROR(ERR_CREATE_INDEX_ON_TYPE, IS_REVERSE_INDEX(desc) ? "reverse" : "normal",
                get_datatype_name_str(type));
            return GS_ERROR;
        }

        if (IS_REVERSE_INDEX(desc) && is_double) {
            GS_THROW_ERROR(ERR_CREATE_INDEX_ON_TYPE, "reverse", get_datatype_name_str(type));
            return GS_ERROR;
        }
...
{
    uint32 space_id;
    if (spc_get_space_id(session, space_name, is_for_create_db, &space_id) != GS_SUCCESS) {
        return GS_ERROR;
    }
    ...
    knl_index_desc_t *index_desc)
{
    ...
    for (uint32 i = 0; i < part_count; i++) {
        ...
        if (db_check_spaces_compatable(session, &part_def->space, index_space, def->is_for_create_db) != GS_SUCCESS) {
            return GS_ERROR;
        }
        ...
        for (uint32 j = 0; j < subpart_count; j++) {
            ...
            if (db_check_spaces_compatable(session, &subpart_def->space,
                                           index_space, def->is_for_create_db) != GS_SUCCESS) {
                return GS_ERROR;
            }
        }
    }
                                     ...
                                     knl_index_def_t *def, knl_index_desc_t *desc)
{
    ...
    if (db_check_spaces_compatable(session, &def->space, table_space, def->is_for_create_db) != GS_SUCCESS) {
        return GS_ERROR;
    }
    ...
    knl_index_desc_t *idx_desc)
{
    ...
    idx_desc->cr_mode = (idx_def->cr_mode == GS_INVALID_ID8) ? table->desc.cr_mode : idx_def->cr_mode;
    idx_desc->org_scn = db_inc_scn(session);
    idx_desc->entry = INVALID_PAGID;
    idx_desc->parted = idx_def->parted;
    idx_desc->pctfree = (idx_def->pctfree == GS_INVALID_ID32) ? table->desc.pctfree : idx_def->pctfree;
    idx_desc->is_reverse = idx_def->is_reverse;

    if (idx_desc->initrans == 0) {
        idx_desc->initrans = cm_text_str_equal_ins(&idx_def->user, "SYS") ? GS_INI_TRANS : session->kernel->attr.initrans;
    }
    ...
    if (IS_NOLOGGING_BY_TABLE_TYPE(table->desc.type) && idx_def->nologging) {
        idx_def->nologging = GS_FALSE;
    }

    if (!dc_get_user_id(session, &idx_def->user, &idx_desc->uid)) {
        GS_THROW_ERROR(ERR_USER_NOT_EXIST, T2S(&idx_def->user));
        return GS_ERROR;
    }
...
{
    if (db_check_index_space(session, dc, table, idx_def, idx_desc) != GS_SUCCESS) {
        return GS_ERROR;
    }

    if (idx_def->columns.count > GS_MAX_INDEX_COLUMNS) {
                                    ...
                                    knl_index_desc_t *desc)
{
    ...
    if (db_chk_idx_def(session, dc, table, def, desc) != GS_SUCCESS) {
        return GS_ERROR;
    }
                                 ...
                                 index_t *index)
{
    ...
    if (!IS_REVERSE_INDEX(&index->desc) && def->index.is_reverse) {
        GS_THROW_ERROR(ERR_OPERATIONS_NOT_ALLOW, "add reverse constraint on normal index");
        return GS_ERROR;
    }
                                     ...
                                     uint32 *index_id)
{
    ...
    if (DB_ATTR_COMPATIBLE_MYSQL(session)) {
        if (db_find_matched_index_compatible_mysql(dc, &idx_def, &index, &matched) != GS_SUCCESS) {
            cm_pop(session->stack);
            return GS_ERROR;
        }
    } else {
                                ...
                                uint32 *ref_index)
{
    ...
    knl_dictionary_t child_dc;
    if (dc_open_table_private(session, child_uid, child_oid, &child_dc) != GS_SUCCESS) {
        GS_LOG_RUN_ERR("[for mysql] open dc private fail");
        return GS_FALSE;
    }
    if (dc_open_table_private(session, desc->uid, desc->id, &parent_dc) != GS_SUCCESS) {
        GS_LOG_RUN_ERR("[for mysql] open dc private fail");
        return GS_FALSE;
    }
    text_t col_name;
    text_t ref_col_name;
    uint16 *col_id = (uint16 *)cm_push(session->stack, sizeof(uint16) * GS_MAX_INDEX_COLUMNS * 3);
    uint16 *ref_col_id = col_id + GS_MAX_INDEX_COLUMNS;
    uint16 *match_id = ref_col_id + GS_MAX_INDEX_COLUMNS;
    uint32 column_cnt = 0;
    ...
    if (db_make_col_list_impl(column_cnt, ref_column_ids, ref_col_id) != GS_SUCCESS) {
        cm_pop(session->stack);
        return GS_FALSE;
    }
    cm_pop(session->stack);
                                 ...
                                 uint32 ref_index)
{
    ...
    row_init(&ra, info->data, GS_MAX_ROW_SIZE, UPDATE_COLUMN_COUNT_SIX);
                                            ...
                                            bool32 *is_referenced, char *desc_name)
{
    ...
    knl_init_index_scan(cursor, GS_TRUE);
    l_key = &cursor->scan_range.l_key;
    knl_set_scan_key(INDEX_DESC(cursor->index), l_key, GS_TYPE_INTEGER, &invalid_id, sizeof(uint32),
    ...
    while (!cursor->eof) {
        ...
        if (has_same_ref_cons(session, desc, child_uid, child_oid, &column_name, &ref_column_name,
            ...
            *is_referenced = GS_TRUE;
            fill_update_ref_info(cursor, desc, &ref_column_ids, ref_index);
            if (knl_internal_update(session, cursor) != GS_SUCCESS) {
                CM_RESTORE_STACK(session->stack);
                return GS_ERROR;
            }
            if (db_create_table_lock_child(session, child_uid, child_oid) != GS_SUCCESS) {
                CM_RESTORE_STACK(session->stack);
                return GS_ERROR;
            }
        } else {
            CM_RESTORE_STACK(session->stack);
            GS_THROW_ERROR(ERR_FK_NO_INDEX_PAREN_4MYSQL, CURSOR_COLUMN_DATA(cursor, CONSDEF_COL_NAME), desc_name);
            return GS_ERROR;
        }
        if (knl_fetch(session, cursor) != GS_SUCCESS) {
            CM_RESTORE_STACK(session->stack);
...
static status_t db_create_index_without_cons(knl_session_t *session, knl_table_desc_t *desc, knl_table_def_t *def)
{
    ...
    for (cur_index = 0; cur_index < def->indexs.count; cur_index++) {
        ...
        if (db_create_index(session, index_def, &dc, GS_FALSE, &index_id) != GS_SUCCESS) {
            dc_close_table_private(&dc);
            return GS_ERROR;
        }
        dc_close_table_private(&dc);
...
static status_t db_create_constraints(knl_session_t *session, knl_table_desc_t *desc, knl_table_def_t *def)
{
    ...
    for (i = 0; i < def->constraints.count; i++) {
        ...
        if (DB_ATTR_COMPATIBLE_MYSQL(session)) {
            cons_def->ref.no_need_check = def->options & CREATE_TYPE_NO_CHECK_CONS ? GS_TRUE : GS_FALSE;
        }
        if (db_create_cons(session, &dc, cons_def) != GS_SUCCESS) {
            dc_close_table_private(&dc);
            return GS_ERROR;
...
status_t db_create_table(knl_session_t *session, knl_table_def_t *def, table_t *table, bool32 *is_referenced)
{
    ...
    if (def->indexs.count > 0) {
        if (db_create_index_without_cons(session, &table->desc, def) != GS_SUCCESS) {
            dc_free_broken_entry(session, table->desc.uid, table->desc.id);
            CM_RESTORE_STACK(session->stack);
            return GS_ERROR;
        }
    ...
    if (DB_ATTR_COMPATIBLE_MYSQL(session) && !IS_SYS_TABLE(table)) {
        if (db_update_ref_syscons_index(session, &table->desc, is_referenced, table->desc.name) != GS_SUCCESS) {
            dc_free_broken_entry(session, table->desc.uid, table->desc.id);
            return GS_ERROR;
        }
    }
...
static status_t make_child_col_names_str(knl_dictionary_t *ref_dc, knl_dictionary_t *child_dc, text_t *child_col_names)
{
    ...
    if (get_ref_cons_for_invalid_ref_info(ref_dc, child_table, &ref_cons) != GS_SUCCESS) {
        return GS_ERROR;
    }
    text_t col_name;
    for (uint32 i = 0; i < ref_cons->col_count; i++) {
        ...
        cm_concat_n_string(child_col_names, GS_MAX_INDEX_COLUMNS * GS_NAME_BUFFER_SIZE, col_name.str, col_name.len);
        if (i + 1 < ref_cons->col_count) {
            if (cm_concat_string(child_col_names, GS_MAX_INDEX_COLUMNS * GS_NAME_BUFFER_SIZE, ",") != GS_SUCCESS) {
                return GS_ERROR;
            }
        }
    }
                                       ...
                                       knl_dictionary_t *ref_dc)
{
    ...
    knl_init_index_scan(cursor, GS_TRUE);
    knl_set_scan_key(INDEX_DESC(cursor->index), scan_key, GS_TYPE_INTEGER, &uid, sizeof(uint32),
                     IX_COL_SYS_INDEX_001_USER);
    knl_set_scan_key(INDEX_DESC(cursor->index), scan_key, GS_TYPE_INTEGER, &oid, sizeof(uint32),
                     IX_COL_SYS_INDEX_001_TABLE);
    knl_set_scan_key(INDEX_DESC(cursor->index), scan_key, GS_TYPE_INTEGER, &index_id, sizeof(uint32),
                     IX_COL_SYS_INDEX_001_ID);
    ...
    while (cm_fetch_text(&column_list, ',', '\0', &col_id_str)) {
        if (cm_text2int(&col_id_str, (int32 *)&col_id) != GS_SUCCESS) {
            CM_RESTORE_STACK(session->stack);
            return GS_ERROR;
        }
        ...
        col_name.str = column->name;
        col_name.len = (uint32)strlen(col_name.str);
        cm_concat_n_string(txt_column_name, GS_MAX_INDEX_COLUMNS * GS_NAME_BUFFER_SIZE, col_name.str, col_name.len);
        col_cnt++;
                                      ...
                                      knl_dictionary_t *child_dc)
{
    ...
    if (make_child_col_names_str(ref_dc, child_dc, &child_col_names) != GS_SUCCESS) {
        return GS_ERROR;
    }

    char ref_str_col_names[GS_MAX_INDEX_COLUMNS * GS_NAME_BUFFER_SIZE];
    ...
    if (make_ref_col_names_str(session, &ref_col_names, index_id, ref_dc) != GS_SUCCESS) {
        return GS_ERROR;
    }
...
status_t db_replace_view(knl_session_t *session, knl_handle_t stmt, knl_view_def_t *def, knl_dictionary_t *dc)
{
    ...
    errno_t err = strcpy_sp(depender.name, GS_NAME_BUFFER_SIZE, view.name);
    knl_securec_check(err);
...
status_t db_create_or_replace_view(knl_session_t *session, knl_handle_t stmt, knl_view_def_t *def)
{
    ...
    if (db_replace_view(session, stmt, def, &dc) != GS_SUCCESS) {
        dc_close(&dc);
        return GS_ERROR;
    }
...
status_t db_update_index_count(knl_session_t *session, knl_cursor_t *cursor, knl_table_desc_t *desc, bool32 is_add)
{
    ...
    if (knl_update(session, cursor) != GS_SUCCESS) {
        return GS_ERROR;
    }
                               ...
                               text_t *index_name, knl_cursor_action_t action, bool32 *is_found)
{
    ...
    *is_found = GS_TRUE;
    ...
    knl_init_index_scan(cursor, GS_TRUE);
    knl_set_scan_key(INDEX_DESC(cursor->index), lkey, GS_TYPE_INTEGER, &uid, sizeof(uint32),
                     IX_COL_SYS_INDEX_002_USER);
    knl_set_scan_key(INDEX_DESC(cursor->index), lkey, GS_TYPE_STRING, index_name->str, index_name->len,
                     IX_COL_SYS_INDEX_002_NAME);
    knl_set_scan_key(INDEX_DESC(cursor->index), lkey, GS_TYPE_INTEGER, &table_id, sizeof(uint32),
    ...
    btree_t *btree, bool32 need_redo)
{
    if (IS_PART_INDEX(index)) {
        ...
        if (IS_SUB_IDXPART(&index_part->desc)) {
            if (db_upd_sub_idx_part_entry(session, &index_part->desc, index_part->desc.entry) != GS_SUCCESS) {
                return GS_ERROR;
            }
        } else {
            if (db_upd_idx_part_entry(session, &index_part->desc, index_part->desc.entry) != GS_SUCCESS) {
                return GS_ERROR;
            }
        }
...
status_t db_fill_index_paral(knl_session_t *session, knl_dictionary_t *dc, index_t *index, knl_index_def_t *def)
{
    ...
    if (db_fill_index_entity_paral(session, dc, index, part_loc, def->parallelism, def->nologging) != GS_SUCCESS) {
        return GS_ERROR;
    }
    return GS_SUCCESS;
}
...
static status_t db_pre_create_idx(knl_session_t *session, knl_index_def_t *idx_def, knl_dictionary_t *dc, index_t *idx)
{
    ...
    if (dc->type == DICT_TYPE_TABLE) {
        if (IS_SYS_TABLE(table)) {
            if (GS_SUCCESS != btree_create_segment(session, idx)) {
                CM_RESTORE_STACK(session->stack);
                return GS_ERROR;
            }
        }
    ...
    if (GS_SUCCESS != db_write_sysindex(session, cursor, &idx->desc)) {
        int32 err_code = cm_get_error_code();
        if (err_code == ERR_DUPLICATE_KEY) {
            cm_reset_error();
            GS_THROW_ERROR(ERR_OBJECT_EXISTS, "index", idx->desc.name);
        }
        CM_RESTORE_STACK(session->stack);
        return GS_ERROR;
    }
    ...
    if (IS_PART_INDEX(idx)) {
        ...
        if (db_create_part_index(session, cursor, idx, idx_def->part_def) != GS_SUCCESS) {
            CM_RESTORE_STACK(session->stack);
            return GS_ERROR;
        }
    }
                         ...
                         uint32 *index_id)
{
    ...
    if (db_pre_create_idx(session, def, dc, &index) != GS_SUCCESS) {
        return GS_ERROR;
    }
    ...
    if (dc->type == DICT_TYPE_TEMP_TABLE_SESSION || dc->type == DICT_TYPE_TEMP_TABLE_TRANS) {
        ...
        if (GS_SUCCESS != db_fill_index(session, cursor, table->desc, index.desc.id, def)) {
            CM_RESTORE_STACK(session->stack);
            return GS_ERROR;
        }
    }
...
static status_t db_drop_empty_idxpart_segment(knl_session_t *session, knl_dictionary_t *new_dc, index_t *new_index)
{
    ...
    for (uint32 i = 0; i < table->part_table->desc.partcnt; i++) {
        ...
        for (uint32 j = 0; j < table_part->desc.subpart_cnt; j++) {
            ...
            if (db_upd_sub_idx_part_entry(session, &index_sub_part->desc, INVALID_PAGID) != GS_SUCCESS) {
                return GS_ERROR;
            }
    ...
    index_t *new_index, knl_parts_locate_t parts_loc)
{
    ...
    if (IS_PART_INDEX(new_index)) {
        if (db_switch_shadow_idx_partitions(session, cursor, new_index) != GS_SUCCESS) {
            CM_RESTORE_STACK(session->stack);
            return GS_ERROR;
        }
...
status_t db_create_index_online(knl_session_t *session, knl_index_def_t *def, knl_dictionary_t *dc)
{
    ...
    if (DB_IS_CLUSTER(session)) {
        GS_THROW_ERROR(ERR_OPERATIONS_NOT_SUPPORT, "create index online", " cluster database");
        return GS_ERROR;
    }
    ...
    if (status != GS_SUCCESS) {
        ...
        if (((dc_entity_t *)dc->handle)->valid) {
            if (lock_upgrade_table_lock(session, dc->handle, LOCK_INF_WAIT) != GS_SUCCESS) {
                return GS_ERROR;
            }
        }
                             ...
                             knl_index_desc_t *desc)
{
    ...
    cursor = knl_push_cursor(session);
    if (GS_SUCCESS != db_fetch_sysindex_row(session, cursor, uid, table_id, name, CURSOR_ACTION_SELECT, &is_found)) {
        CM_RESTORE_STACK(session->stack);
        return GS_ERROR;
    }
                                          ...
                                          uint32 deleted_cols, knl_cursor_t *cursor)
{
    ...
    for (i = old_col_count; i < new_col_count; i++) {
        ...
        if (KNL_COLUMN_IS_SERIAL(column)) {
            ...
            if (IS_COMPATIBLE_MYSQL_INST && column->datatype == GS_TYPE_DATETIME_MYSQL) {
...
status_t db_update_index_name(knl_session_t *session, uint32 uid, uint32 table_id, const char *name, text_t *new_name)
{
    ...
    knl_set_scan_key(INDEX_DESC(cursor->index), key, GS_TYPE_STRING, name, (uint16)strlen(name),
                     IX_COL_SYS_INDEX_002_NAME);
    knl_set_scan_key(INDEX_DESC(cursor->index), key, GS_TYPE_INTEGER, &table_id, sizeof(uint32),
                     IX_COL_SYS_INDEX_002_TABLE);
...
static status_t db_modify_column_in_syscolumn(knl_session_t *session, knl_column_t *column, knl_table_desc_t *desc)
{
    ...
    row_init(&ra, cursor->update_info.data, HEAP_MAX_ROW_SIZE(session), 8);
                                       ...
                                       bool32 compatible, bool32 *need_rebuild, uint32 index_count)
{
    ...
    for (i = 0; i < index_count; i++) {
        ...
        if (COLUMN_IS_LOB(new_column)) {
            GS_THROW_ERROR(ERR_CREATE_INDEX_ON_TYPE, IS_REVERSE_INDEX(&index->desc) ? "reverse" : "normal",
                get_datatype_name_str(new_column->datatype));
            return GS_ERROR;
        }
        
        if ((COLUMN_IS_REAL(new_column) && IS_REVERSE_INDEX(&index->desc))) {
            GS_THROW_ERROR(ERR_CREATE_INDEX_ON_TYPE, "reverse", get_datatype_name_str(new_column->datatype));
            return GS_ERROR;
        }
...
static inline bool32 db_is_number_compatible(knl_column_t *old_column, knl_column_t *new_column)
{
    ...
    if (GS_IS_NUMBER2_TYPE(old_column->datatype) && !GS_IS_NUMBER2_TYPE(new_column->datatype)) {
        return GS_FALSE;
    }
    if (!GS_IS_NUMBER2_TYPE(old_column->datatype) && GS_IS_NUMBER2_TYPE(new_column->datatype)) {
        return GS_FALSE;
...
static bool32 db_is_column_compatible(knl_column_t *old_column, knl_column_t *new_column, bool32 *type_changed)
{
    *type_changed = (old_column->datatype != new_column->datatype);
    ...
    if (*type_changed) {
        ...
        } else if (GS_IS_NUMBER_TYPE(old_column->datatype) && GS_IS_NUMBER_TYPE(new_column->datatype)) {
                                         ...
                                         bool32 *rebuild_indexes, uint32 index_count)
{
    ...
    if (entity->has_serial_col) {
        if (KNL_COLUMN_IS_SERIAL(old_column)) {
            if ((new_column->datatype != GS_TYPE_BIGINT) &&
                (new_column->datatype != GS_TYPE_INTEGER) &&
                (!DB_ATTR_COMPATIBLE_MYSQL(session) ||
                 (DB_ATTR_COMPATIBLE_MYSQL(session) && new_column->datatype != GS_TYPE_REAL))) {
...
{
    bool32 is_fk_col = GS_FALSE;
    text_t col_name;
    char new_col_names[GS_MAX_INDEX_COLUMNS * GS_NAME_BUFFER_SIZE];
    ...
    while (cm_fetch_text(column_name, ',', '\0', &col_name)) {
        if (cm_text_equal(&col_name, old_name)) {
            is_fk_col = GS_TRUE;
            cm_concat_n_string(new_column_name, GS_MAX_INDEX_COLUMNS * GS_NAME_BUFFER_SIZE,
                               new_name->str, new_name->len);
        } else {
            cm_concat_n_string(new_column_name, GS_MAX_INDEX_COLUMNS * GS_NAME_BUFFER_SIZE, col_name.str, col_name.len);
        }
        col_cnt++;
        if (col_cnt < column_cnt) {
            if (cm_concat_string(new_column_name, GS_MAX_INDEX_COLUMNS * GS_NAME_BUFFER_SIZE, ",")) {
                return GS_ERROR;
            }
        }
    }
                                             ...
                                             knl_table_desc_t *desc)
{
    ...
    knl_init_index_scan(cursor, GS_TRUE);
    l_key = &cursor->scan_range.l_key;
    knl_set_scan_key(INDEX_DESC(cursor->index), l_key, GS_TYPE_INTEGER, &child_uid, sizeof(uint32),
        IX_COL_SYS_CONSDEF001_USER_ID);
    knl_set_scan_key(INDEX_DESC(cursor->index), l_key, GS_TYPE_INTEGER, &child_oid, sizeof(uint32),
        IX_COL_SYS_CONSDEF001_TABLE_ID);
    ...
    while (!cursor->eof) {
        ...
        if (ref_uid == -1 && ref_oid == -1 && column_name.len > 0 &&
           is_fk_column_and_update_column(old_name, new_name, &column_name, &new_column_name, column_count)) {
            row_assist_t ra;
            row_init(&ra, cursor->update_info.data, GS_MAX_ROW_SIZE, 1);
            cursor->update_info.count = 1;
...
status_t db_altable_rename_column(knl_session_t *session, knl_dictionary_t *dc, knl_altable_def_t *def)
{
    ...
    if (DB_ATTR_COMPATIBLE_MYSQL(session) &&
        db_rename_ref_syscons_for_fk(session, old_name, new_name, &table->desc) != GS_SUCCESS) {
        return GS_ERROR;
    }
...
status_t db_invalidate_children_for_update_ref_cons(knl_session_t *session, bool32 is_referenced, table_t *table)
{
    if (DB_ATTR_COMPATIBLE_MYSQL(session) && is_referenced) {
        knl_dictionary_t dc;
        if (dc_open_table_private(session, table->desc.uid, table->desc.id, &dc) != GS_SUCCESS) {
            GS_LOG_RUN_ERR("[db_invalidate_children_for_update_ref_cons]: open dc private fail");
            return GS_ERROR;
        }
                                 ...
                                 void *trig_list)
{
    ...
    if (DB_ATTR_COMPATIBLE_MYSQL(session) && !IS_SYS_TABLE(table) &&
        db_update_ref_syscons_index(session, &table->desc, &is_referenced, def->table_def.new_name.str) != GS_SUCCESS) {
        return GS_ERROR;
    }
        ...
        case CONS_TYPE_REFERENCE:
            if (DB_ATTR_COMPATIBLE_MYSQL(session)) {
                def->cons_def.new_cons.ref.no_need_check = def->options &
                                                           CREATE_TYPE_NO_CHECK_CONS ? GS_TRUE : GS_FALSE;
            }
...
status_t db_drop_table_cascade(knl_session_t *session, text_t *user, knl_table_desc_t *desc)
{
    ...
    if (db_drop_table(session, NULL, &dc) != GS_SUCCESS) {
        dc_close(&dc);
        return GS_ERROR;
    }
...
status_t db_create_synonym(knl_session_t *session, knl_handle_t stmt, knl_synonym_def_t *def)
{
    ...
    errno_t err = strncpy_s(depender.name, GS_NAME_BUFFER_SIZE, def->name.str, def->name.len);
    knl_securec_check(err);
...
status_t db_analyze_index(knl_session_t *session, knl_analyze_index_def_t *def, bool32 is_dynamic)
{
    ...
    if (knl_open_dc_by_index(session, &user, &table_name, &idx_name, &dc) != GS_SUCCESS) {
        return GS_ERROR;
    }
    ...
    knl_dictionary_t *dc, knl_part_locate_t part_loc, uint32 timeout_s)
{
    ...
    if (DB_IS_CLUSTER(session)) {
        GS_THROW_ERROR(ERR_OPERATIONS_NOT_SUPPORT, "rebuild index online", " cluster database");
        return GS_ERROR;
    }
    ...
    if (lock_upgrade_table_lock(session, entity, timeout_s) != GS_SUCCESS) {
        knl_close_cursor(session, cursor);
        return GS_ERROR;
    }
                                     ...
                                     index_t *old_index, index_t *shadow_index)
{
    ...
    if (CM_IS_EMPTY(&def->space) || def->specified_parts > 0) {
        ...
        if (spc_get_space_id(session, &def->space, def->is_for_create_db, &spc_id) != GS_SUCCESS) {
            return GS_ERROR;
        }
    ...
    if (db_fetch_index_desc(session, old_index->desc.uid, old_index->desc.table_id, &idx_name, &desc) != GS_SUCCESS) {
        return GS_ERROR;
    }
    ...
    index_t *old_index, index_t *index)
{
    ...
    if (IS_AUTO_REBUILD(def->rebuild.lock_timeout) && (old_index->desc.org_scn != def->rebuild.org_scn)) {
        ...
        GS_LOG_RUN_INF("index_name %s, org_scn %llu does not exist", index->desc.name, def->rebuild.org_scn);
        return GS_ERROR;
    }
    ...
    index_part_t *shadow_part, knl_part_locate_t part_loc, knl_alindex_def_t *def)
{
    ...
    for (;;) {
        ...
        if (db_switch_shadow_idx_partitions(session, cursor, index) != GS_SUCCESS) {
            status = GS_ERROR;
            break;
        }
    ...
    index_t *index, int32 index_i)
{
    ...
    if (IS_AUTO_REBUILD(def->rebuild.lock_timeout) && (old_part->desc.org_scn != def->rebuild.org_scn)) {
        GS_THROW_ERROR(ERR_INDEX_NOT_EXIST, T2S(&def->user), T2S_EX(&def->name));
        GS_LOG_RUN_INF("index_name %s, org_scn %llu does not exist", old_part->desc.name, def->rebuild.org_scn);
        return GS_ERROR;
    }
    ...
    if (CM_IS_EMPTY(&rebuild->space)) {
        *spc_id = old_part->desc.space_id;
        ...
        if (spc_get_space_id(session, &rebuild->space, def->is_for_create_db, spc_id) != GS_SUCCESS) {
            return GS_ERROR;
        }
    ...
    index_part_t *compart, knl_part_locate_t *part_loc)
{
    ...
    for (uint32 i = 0; i < compart->desc.subpart_cnt; i++) {
        ...
        if (btree_coalesce(session, &subpart->btree, &stats, *part_loc, GS_FALSE) != GS_SUCCESS) {
            return GS_ERROR;
        }
    }
    ...
    index_t *index)
{
    ...
    if (!IS_PARENT_IDXPART(&index_part->desc)) {
        ...
        return btree_coalesce(session, &index_part->btree, &stats, part_loc, GS_FALSE);
    }
...
status_t db_alter_index_coalesce(knl_session_t *session, knl_dictionary_t *dc, index_t *index)
{
    ...
    part_loc.part_no = GS_INVALID_ID32;
    part_loc.subpart_no = GS_INVALID_ID32;
    return btree_coalesce(session, &index->btree, &stats, part_loc, GS_FALSE);
}
...
status_t db_altable_enable_nologging(knl_session_t *session, knl_dictionary_t *dc)
{
    ...
    GS_LOG_RUN_INF("[NOLOGGING INSERT] table %s.%s is set nologging", user->desc.name, table->desc.name);
    return GS_SUCCESS;
}
...
status_t db_altable_disable_nologging(knl_session_t *session, knl_dictionary_t *dc)
{
    ...
    dc_user_t *user = session->kernel->dc_ctx.users[table->desc.uid];
    GS_LOG_RUN_INF("[NOLOGGING INSERT] table %s.%s disable nologging", user->desc.name, table->desc.name);
    return GS_SUCCESS;
}
...
status_t db_altable_enable_part_nologging(knl_session_t *session, knl_dictionary_t *dc, knl_altable_def_t *def)
{
    ...
    dc_user_t *user = session->kernel->dc_ctx.users[table->desc.uid];
    GS_LOG_RUN_INF("[NOLOGGING INSERT] table part %s.%s.%s enable nologging", user->desc.name, table->desc.name,
...
status_t db_altable_disable_part_nologging(knl_session_t *session, knl_dictionary_t *dc, knl_altable_def_t *def)
{
    ...
    dc_user_t *user = session->kernel->dc_ctx.users[table->desc.uid];
    GS_LOG_RUN_INF("[NOLOGGING INSERT] table part %s.%s.%s disable nologging", user->desc.name, table->desc.name,
        table_part->desc.name);
    return GS_SUCCESS;
}
...
{
    if (serial_start >= GS_INVALID_ID64 - GS_SERIAL_CACHE_COUNT) {
        if (GS_INVALID_ID64 != entity->table.heap.segment->serial) {
                                    ...
                                    knl_altable_def_t *def)
{
    ...
    if (table->desc.type != TABLE_TYPE_HEAP) {
        GS_THROW_ERROR(ERR_TABLE_NOT_HEAP, table->desc.name);
        return GS_ERROR;
    }
    ...
    index_part_t *index_part, uint32 initrans)
{
    ...
    for (uint32 i = 0; i < index_part->desc.subpart_cnt; i++) {
        ...
        if (db_upd_idx_subpart_initrans(session, &index_subpart->desc, initrans) != GS_SUCCESS) {
            return GS_ERROR;
        }
...
status_t db_alter_index_initrans(knl_session_t *session, knl_alindex_def_t *def, index_t *index)
{
    ...
    for (uint32 id = 0; id < index->part_index->desc.partcnt; ++id) {
        ...
        index_part = INDEX_GET_PART(index, id);
        if (db_upd_idx_part_initrans(session, &index_part->desc, initrans) != GS_SUCCESS) {
            return GS_ERROR;
        }
    ...
    knl_indexes_def_t *def)
{
    ...
    if (IS_PART_INDEX(indexes[0])) {
        ...
        if (db_fill_multi_indexes_paral(session, &dc, indexes, index_cnt, def->indexes_def[0].parallelism,
            part_loc, def->indexes_def[0].nologging) != GS_SUCCESS) {
            dc_close_table_private(&dc);
            CM_RESTORE_STACK(session->stack);
            return GS_ERROR;
        }
...
status_t db_create_hash_part_index_segment(knl_session_t *session, knl_dictionary_t *dc, uint32 index_id)
{
    ...
    SYNC_POINT_GLOBAL_END;
    if (btree_generate_create_undo(session, index_part->desc.entry, index_part->desc.space_id,
                                   IS_LOGGING_TABLE_BY_TYPE(new_dc.type)) != GS_SUCCESS) {
        btree_drop_part_segment(session, index_part);
        dc_close_table_private(&new_dc);
        return GS_ERROR;
    }
                                               ...
                                               table_t *table)
{
    ...
    for (uint32 i = 0; i < table->index_set.total_count; i++) {
        ...
        if (db_create_hash_part_index_segment(session, dc, index->desc.id) != GS_SUCCESS) {
            GS_LOG_RUN_ERR("db create hash part index sg fail, i %u, index id %u", i, index->desc.id);
            return GS_ERROR;
        }
    }
    return GS_SUCCESS;
