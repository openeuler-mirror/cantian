...
{
    switch (attr) {
        case GSC_ATTR_PREFETCH_ROWS:
            stmt->prefetch_rows = *(uint32 *)data;
            break;

        case GSC_ATTR_PREFETCH_BUFFER:
...
status_t clt_get_stmt_attr(clt_stmt_t *stmt, int attr, const void *data, uint32 buf_len, uint32 *len)
{
    ...
    switch (attr) {
        ...
        case GSC_ATTR_FETCHED_ROWS:
            *(uint32 *)data = stmt->fetched_rows;
            break;

        case GSC_ATTR_AFFECTED_ROWS:
            *(uint32 *)data = stmt->affected_rows;
            break;

        case GSC_ATTR_RESULTSET_EXISTS:
            *(uint32 *)data = (stmt->column_count > 0);
            break;

        case GSC_ATTR_COLUMN_COUNT:
            *(uint32 *)data = stmt->column_count;
            break;

        case GSC_ATTR_STMT_TYPE:
        ...
        case GSC_ATTR_ACTUAL_BATCH_ERRS:
...
static status_t clt_put_param_value(clt_stmt_t *stmt, cs_packet_t *req, uint32 offset, clt_param_t *param)
{
    uint32 size;
    text_t text;
    gsc_lob_t *bnd_lob = NULL;
    date_t date;

    switch ((gsc_type_t)param->bnd_type) {
...
{
    return cs_put_data(req, &(((gsc_lob_t *)param->lob_ptr)[offset]), sizeof(gsc_lob_t));
}

static status_t clt_write_large_str_param_value(clt_stmt_t *stmt, uint32 pos, uint32 offset)
{
    clt_param_t *param = NULL;
    gsc_lob_t *vm_lob = NULL;
    text_t large_str;
    ...
    vm_lob = (gsc_lob_t *)(param->lob_ptr + sizeof(gsc_lob_t) * offset);
...
static void clt_set_param_isnull(clt_param_t *param, uint32 offset, uint8 *flag)
{
    ...
    if (param->curr_ptr == NULL || param->direction == GSC_OUTPUT) {
        *flag |= 0x01;
        return;
    }

    if (param->ind_ptr != NULL) {
        *flag |= (param->ind_ptr[offset] == GSC_NULL) ? 0x01 : 0x00;
    } else {
        *flag |= 0x00;
    }
}
...
{
    if (direction == GSC_INPUT) {
        *flag |= 0x40;
    } else if (direction == GSC_OUTPUT) {
        *flag |= 0x80;
    } else if (direction == GSC_INOUT) {
        *flag |= 0x40;
        *flag |= 0x80;
    }
}
    ...
    uint32 types_offset, bool32 add_types)
{
    ...
    for (uint32 i = 0; i < stmt->param_count; i++) {
        ...
        if (add_types) {
            ...
            types[i] = (param->bnd_type == GSC_TYPE_NATIVE_DATE) ? GSC_TYPE_DATE : param->bnd_type;
            if (large_str) {
                types[i] = GSC_TYPE_CLOB;
            }
        }
...
status_t gsc_execute(gsc_stmt_t pstmt)
{
    status_t status;
    clt_stmt_t *stmt = (clt_stmt_t *)pstmt;
...
int32 clt_prepare_fetch(gsc_stmt_t pstmt, unsigned int *rows, bool32 fetch_ori_row)
{
    status_t status;
    clt_stmt_t *stmt = (clt_stmt_t *)pstmt;
    uint32 temp_rows = 0;
...
status_t gsc_write_clob(gsc_stmt_t pstmt, uint32 id, const void *data, uint32 size, uint32 *nchars)
{
    status_t status;
    clt_stmt_t *stmt = (clt_stmt_t *)pstmt;
    ...
    GSC_CHECK_OBJECT_NULL_GS(stmt, "statement");
    GSC_CHECK_OBJECT_NULL_GS(stmt->conn, "connection");
    GSC_CHECK_OBJECT_NULL_CLT(stmt->conn, data, "clob data bound");
...
status_t gsc_write_blob(gsc_stmt_t pstmt, uint32 id, const void *data, uint32 size)
{
    status_t status;
    clt_stmt_t *stmt = (clt_stmt_t *)pstmt;
    ...
    GSC_CHECK_OBJECT_NULL_GS(stmt, "statement");
    GSC_CHECK_OBJECT_NULL_GS(stmt->conn, "connection");
    GSC_CHECK_OBJECT_NULL_CLT(stmt->conn, data, "blob data bound");
...
status_t gsc_write_batch_clob(gsc_stmt_t pstmt, uint32 id, uint32 piece, const void *data, uint32 size, uint32 *nchars)
{
    status_t status;
    clt_stmt_t *stmt = (clt_stmt_t *)pstmt;
...
status_t gsc_write_batch_blob(gsc_stmt_t pstmt, uint32 id, uint32 piece, const void *data, uint32 size)
{
    status_t status;
    clt_stmt_t *stmt = (clt_stmt_t *)pstmt;
...
status_t clt_get_outparam_by_id(clt_stmt_t *stmt, uint32 id, void **data, uint32 *size, bool32 *is_null)
{
    ...
    is_null_val = (outparam->size == GSC_NULL);
