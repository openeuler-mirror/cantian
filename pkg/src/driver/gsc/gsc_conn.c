...
static inline void clt_load_default_options(clt_options_t *options)
{
    ...
    options->ssl_mode = GSC_SSL_PREFERRED;
    options->client_flag = CS_FLAG_CLIENT_SSL;
}

status_t gsc_alloc_conn(gsc_conn_t *pconn)
{
...
static status_t clt_ssl_check(clt_conn_t *conn)
{
    ...
    */
    ...
    /*
    Attempt SSL connection if ssl_mode != GSC_SSL_DISABLED and the
    server supports SSL. Fallback on unencrypted connection otherwise.
    */
    if (conn->options.ssl_mode != GSC_SSL_DISABLED && (conn->server_capabilities & CS_FLAG_CLIENT_SSL)) {
        conn->client_flag |= CS_FLAG_CLIENT_SSL;
    } else {
        conn->client_flag &= ~CS_FLAG_CLIENT_SSL;
    }
...
void gsc_try_fetch_url_tenant(const char *str, text_t *url, text_t *tenant)
{
...
status_t gsc_connect(gsc_conn_t pconn, const char *url, const char *user, const char *password)
{
    return gsc_connect_inner(pconn, url, user, password, CS_LOCAL_VERSION);
}
...
status_t clt_set_conn_attr(clt_conn_t *conn, int32 attr, const void *data, uint32 len)
{
    ...
    switch (attr) {
        ...
        case GSC_ATTR_NLS_DATE_LANGUAGE:
        case GSC_ATTR_NLS_DUAL_CURRENCY:
        case GSC_ATTR_NLS_ISO_CURRENCY:
        case GSC_ATTR_NLS_LANGUAGE:
        case GSC_ATTR_NLS_LENGTH_SEMANTICS:
        case GSC_ATTR_NLS_NCHAR_CHARACTERSET:
        case GSC_ATTR_NLS_NCHAR_CONV_EXCP:
        case GSC_ATTR_NLS_NUMERIC_CHARACTERS:
        ...
        case GSC_ATTR_NLS_TIME_FORMAT:
        case GSC_ATTR_NLS_TIME_TZ_FORMAT:
            return clt_set_conn_nls(conn, (nlsparam_id_t)(attr - GSC_ATTR_NLS_CALENDAR), data, len);
        ...
        case GSC_ATTR_SSL_MODE:
            conn->options.ssl_mode = *(gsc_ssl_mode_t *)data;
            break;
...
static status_t gsc_get_conn_nls(clt_conn_t *conn, nlsparam_id_t id, const void *data, uint32 len, uint32 *attr_len)
{
    text_t nlsfmt;
...
uint32 gsc_get_shd_node_type(gsc_conn_t conn)
{
    return (conn == NULL) ? CS_RESERVED : ((struct st_clt_conn *)conn)->node_type;
}

static status_t gsc_get_attr_string(const char *attr, void *data, uint32 len, uint32 *attr_len)
{
...
static status_t clt_get_conn_attr(clt_conn_t *conn, int32 attr, void *data, uint32 len, uint32 *attr_len)
{
    switch (attr) {
        ...
        case GSC_ATTR_SSL_CA:
            return gsc_get_attr_string(conn->options.ssl_ca, data, len, attr_len);
        case GSC_ATTR_SSL_CERT:
            return gsc_get_attr_string(conn->options.ssl_cert, data, len, attr_len);
        case GSC_ATTR_SSL_KEY:
            return gsc_get_attr_string(conn->options.ssl_key, data, len, attr_len);
        case GSC_ATTR_SSL_KEYPWD:
            return gsc_get_attr_string(conn->options.ssl_keypwd, data, len, attr_len);
        case GSC_ATTR_SSL_CRL:
            return gsc_get_attr_string(conn->options.ssl_crl, data, len, attr_len);
        case GSC_ATTR_SSL_CIPHER:
            return gsc_get_attr_string(conn->options.ssl_cipher, data, len, attr_len);
        case GSC_ATTR_SSL_MODE:
        ...
        case GSC_ATTR_UDS_SERVER_PATH:
            return gsc_get_attr_string(conn->options.server_path, data, len, attr_len);

        case GSC_ATTR_UDS_CLIENT_PATH:
            return gsc_get_attr_string(conn->options.client_path, data, len, attr_len);

        case GSC_ATTR_TIMESTAMP_SIZE:
        case GSC_ATTR_TIMESTAMP_LTZ_SIZE:
        ...
        case GSC_ATTR_TIMESTAMP_TZ_SIZE:
...
status_t gsc_cancel(gsc_conn_t pconn, uint32 sid)
{
    status_t status;
    clt_conn_t *conn = (clt_conn_t *)pconn;
...
status_t gsc_rollback(gsc_conn_t pconn)
{
    status_t status;
    clt_conn_t *conn = (clt_conn_t *)pconn;
...
uint32 gsc_query_get_affected_rows(gsc_conn_t pconn)
{
    clt_conn_t *conn = (clt_conn_t *)pconn;
    clt_stmt_t *stmt = NULL;
...
uint32 gsc_query_get_column_count(gsc_conn_t pconn)
{
    clt_conn_t *conn = (clt_conn_t *)pconn;
    clt_stmt_t *stmt = NULL;
...
status_t gsc_query_describe_column(gsc_conn_t pconn, uint32 id, gsc_column_desc_t *desc)
{
    status_t status;
    clt_conn_t *conn = (clt_conn_t *)pconn;
    clt_stmt_t *stmt = NULL;
...
status_t gsc_query_get_column(gsc_conn_t pconn, uint32 id, void **data, uint32 *size, uint32 *is_null)
{
    status_t status;
    clt_conn_t *conn = (clt_conn_t *)pconn;
    clt_stmt_t *stmt = NULL;
...
static void gsc_reset_query(clt_conn_t *conn)
{
...
status_t gsc_query(gsc_conn_t pconn, const char *sql)
{
    status_t status;
    clt_conn_t *conn = (clt_conn_t *)pconn;
    text_t sql_text;
...
status_t gsc_query_multiple(gsc_conn_t pconn, const char *sql)
{
    status_t status;
    clt_conn_t *conn = (clt_conn_t *)pconn;
