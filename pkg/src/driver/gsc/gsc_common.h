...
#define GSC_IS_DATE_TYPE(type)                                                                     \
    ((type) == GSC_TYPE_DATE || (type) == GSC_TYPE_TIMESTAMP || (type) == GSC_TYPE_TIMESTAMP_TZ || \
        (type) == GSC_TYPE_TIMESTAMP_TZ_FAKE)
#define GSC_IS_NUMBER_TYPE(type)                                                              \
    ((type) == GSC_TYPE_NUMBER || (type) == GSC_TYPE_DECIMAL || (type) == GSC_TYPE_INTEGER || \
        (type) == GSC_TYPE_BIGINT || (type) == GSC_TYPE_REAL || (type) == GSC_TYPE_BOOLEAN || \
        (type) == GSC_TYPE_UINT32 || (type) == GSC_TYPE_NUMBER2)
#define GSC_IS_TYPE_NEED_EXTRA(type)                                                          \
    ((type) == GSC_TYPE_NUMBER || (type) == GSC_TYPE_DECIMAL || (type) == GSC_TYPE_NUMBER2 || \
        (type) == GSC_TYPE_CHAR || (type) == GSC_TYPE_VARCHAR || (type) == GSC_TYPE_STRING || \
        (type) == GSC_TYPE_BINARY || (type) == GSC_TYPE_VARBINARY || (type) == GSC_TYPE_RAW)

#define GSC_CHECK_FETCH_STATUS(stmt)                                                                              \
    do {                                                                                                          \
...
#define GSC_INLINE_LOB_ENCODE_LEN (uint32)(sizeof(clt_cache_lob_t))
...
typedef struct st_clt_stmt {
    ...
    uint32 fetched_times; /* times of gsc_fetch called */
    uint32 fetched_rows;  /* rows fetched from client */
    uint32 fetch_pos;     /* pos of batch fetch of once gsc_fetch */
...
#define GSC_CHECK_OBJECT_NULL_CLT(conn, obj_ptr, obj_name)             \
    {                                                                  \
        if (SECUREC_UNLIKELY((obj_ptr) == NULL)) {                     \
...
typedef struct st_clt_param {
    ...
    uint16 *ind_ptr;
    char *curr_ptr;
    char *lob_ptr;       // for string convert to lob to bind, malloc & free by GSC
...
static inline void clt_reset_lob(gsc_lob_t *lob)
{
    if (lob == NULL) {
        return;
    }
