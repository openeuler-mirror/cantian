...
#define GS_MAX_SSL_KEYPWD (GS_MAX_CIPHER_LEN + 4)
#define GS_MAX_FACTOR_KEY_LEN (GS_MAX_FACTOR_KEY_STR_LEN + 4)
#define GS_MAX_LOCAL_KEY_LEN (GS_MAX_LOCAL_KEY_STR_LEN_DOUBLE + 4)
...
typedef struct st_cluster_info {
    ...
    char cipher[GS_PARAM_BUFFER_SIZE];
    char local_key[GS_MAX_LOCAL_KEY_LEN];
    char factor_key[GS_FILE_NAME_BUFFER_SIZE];
    uint32 cipher_len;
    gsc_ssl_mode_t ssl_mode;
    char ssl_ca[GS_FILE_NAME_BUFFER_SIZE + 1];
    char ssl_cert[GS_FILE_NAME_BUFFER_SIZE + 1];
    char ssl_key[GS_FILE_NAME_BUFFER_SIZE + 1];
    char ssl_crl[GS_FILE_NAME_BUFFER_SIZE + 1];
    char ssl_cipher[GS_PARAM_BUFFER_SIZE + 1];
    char ssl_keypwd[GS_PASSWORD_BUFFER_SIZE + 1];
    uint32 keypwd_len;
} cluster_info_t;
...
status_t clt_cluster_connect(clt_conn_t *conn, text_t *text, const char *user, const char *password,
