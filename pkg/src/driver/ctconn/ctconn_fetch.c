 ...
 *
 * IDENTIFICATION
 * src/driver/ctconn/ctconn_fetch.c
 *
 * -------------------------------------------------------------------------
 */
...
static status_t clt_save_inline_lob(clt_stmt_t *stmt, clt_column_t *column)
{
    ...
    need_len = lob->used_pos + (CTCONN_INLINE_LOB_ENCODE_LEN + head->size);
    ...
    lob->used_pos += CTCONN_INLINE_LOB_ENCODE_LEN;
    ...
    const char *row_addr)
{
    ...
    if (bits == COL_BITS_4) {
        ...
        *pos += 4;
        ...
        if (not_shd_dn && CTCONN_IS_TIMESTAMP_LTZ_TYPE(column->def.datatype)) {
...
static status_t clt_read_row(clt_stmt_t *stmt, bool32 fetch_ori_row)
{
    ...
    for (uint32 i = 0; i < stmt->column_count; ++i) {
        ...
        if (stmt->conn->recv_trans_func != NULL && CTCONN_IS_STRING_TYPE(column->def.datatype)) {
        ...
        if (stmt->fetch_size > 1 && CTCONN_IS_LOB_TYPE(column->def.datatype)) {
...
static status_t copy_lob_val(clt_stmt_t *stmt, clt_column_t *column)
{
    ...
    if (CLT_LOB_INLINE((clt_lob_head_t *)column->ptr) && stmt->fetch_size > 1 &&
        ...
        col_len = (uint16)CTCONN_INLINE_LOB_ENCODE_LEN;
    } else {
        col_len = column->size;
    }
...
static status_t copy_val_2int32(clt_stmt_t *stmt, clt_column_t *column)
{
    ...
    if (column->bnd_type == CTCONN_TYPE_BOOLEAN) {
...
static status_t init_cp_func(clt_stmt_t *stmt, clt_column_t *column)
{
    ...
    } else if (CTCONN_IS_STRING_TYPE(column->bnd_type)) {
        return init_cp_func_2str(stmt, column, CT_TRUE);
    } else if (CTCONN_IS_BINARY_TYPE(column->bnd_type)) {
        return init_cp_func_2str(stmt, column, CT_FALSE);
    } else if (CTCONN_IS_NUMBER_TYPE(column->bnd_type)) {
        return init_cp_func_2digit(stmt, column);
    } else if (column->bnd_type == CTCONN_TYPE_DATE) {
        return init_cp_func_2date(stmt, column);
    } else if (column->bnd_type == CTCONN_TYPE_TIMESTAMP || column->bnd_type == CTCONN_TYPE_TIMESTAMP_TZ_FAKE) {
