 ...
 *
 * IDENTIFICATION
 * src/driver/ctconn/ctconn_lob.c
 *
 * -------------------------------------------------------------------------
 */
...
{
    uint32 loc_size;
    ctconn_column_desc_t desc = { 0 };

    CT_RETURN_IFERR(clt_desc_column_by_id(stmt, id, &desc));

    if (!CTCONN_IS_LOB_TYPE(desc.type)) {
...
static status_t clt_decode_inline_lob(clt_stmt_t *stmt, void *locator, char **data)
{
    ...
    actual_len = CTCONN_INLINE_LOB_ENCODE_LEN + cache_lob->lob_head.size;
    ...
    *data = column->inline_lob.cache_buf.str + cache_lob->offset + CTCONN_INLINE_LOB_ENCODE_LEN;
...
status_t ctconn_read_blob(ctconn_stmt_t pstmt, void *locator, uint32 offset, void *buffer, uint32 size, uint32 *nbytes,
    uint32 *eof)
{
    status_t status;
...
status_t ctconn_read_blob_by_id(ctconn_stmt_t pstmt, uint32 id, uint32 offset, void *buffer, uint32 size, uint32 *nbytes,
    uint32 *eof)
{
    status_t status;
    clt_stmt_t *stmt = (clt_stmt_t *)pstmt;
...
static status_t ctconn_read_inline_clob(clt_stmt_t *stmt, void *locator, uint32 offset, void *buffer, uint32 size,
    ...
    uint32 *nbytes, uint32 *eof)
{
    ...
    if (CLT_LOB_INLINE(head) && stmt->conn->call_version >= CS_VERSION_3) {
        ret = ctconn_read_inline_clob(stmt, locator, offset, buffer, size, nchars, nbytes, &actual_eof);
...
status_t ctconn_read_clob(ctconn_stmt_t pstmt, void *locator, uint32 offset, void *buffer, uint32 size, uint32 *nchars,
    uint32 *nbytes, uint32 *eof)
{
    status_t status;
    clt_stmt_t *stmt = (clt_stmt_t *)pstmt;
...
status_t ctconn_read_clob_by_id(ctconn_stmt_t pstmt, uint32 id, uint32 offset, void *buffer, uint32 size, uint32 *nchars,
    uint32 *nbytes, uint32 *eof)
{
    status_t status;
    clt_stmt_t *stmt = (clt_stmt_t *)pstmt;
...
status_t ctconn_get_lob_size_by_id(ctconn_stmt_t pstmt, uint32 id, uint32 *size)
{
    clt_stmt_t *stmt = (clt_stmt_t *)pstmt;
    void *locator = NULL;
    status_t status;
...
static inline status_t clt_recv_lob_write_ack(clt_stmt_t *stmt, cs_packet_t *ack_pack, ctconn_lob_t *bnd_lob_piece)
{
    char *buf = NULL;
    ...
    CT_RETURN_IFERR(cs_get_data(ack_pack, sizeof(ctconn_lob_t), (void **)&buf));
    *bnd_lob_piece = *(ctconn_lob_t *)buf;
    ...
    uint32 *write_size, transcode_func_t transcode_func)
{
    ...
    ctconn_lob_t *bnd_lob = (ctconn_lob_t *)param->bnd_ptr;
