 ...
 *
 * IDENTIFICATION
 * src/driver/ctconn/ctconn_shard.c
 *
 * -------------------------------------------------------------------------
 */
...
#ifdef Z_SHARDING
status_t ctconn_fetch_raw(ctconn_stmt_t pstmt)
{
...
status_t ctconn_fetch_data(ctconn_stmt_t pstmt, uint32 *rows)
{
    clt_stmt_t *stmt = (clt_stmt_t *)pstmt;
...
status_t ctconn_fetch_data_ack(ctconn_stmt_t pstmt, char **data, uint32 *size)
{
    clt_stmt_t *stmt = (clt_stmt_t *)pstmt;
...
status_t ctconn_fetch_data_attr_ack(ctconn_stmt_t pstmt, uint32 *options, uint32 *return_rows)
{
    clt_stmt_t *stmt = (clt_stmt_t *)pstmt;
    if (stmt == NULL) {
...
status_t ctconn_init_paramset_length(ctconn_stmt_t pstmt)
{
    clt_stmt_t *stmt = (clt_stmt_t *)pstmt;
...
status_t ctconn_bind_by_pos_batch(ctconn_stmt_t pstmt, uint32 pos, int32 type, const void *data, uint32 size, bool32 is_null)
{
    clt_stmt_t *stmt = (clt_stmt_t *)pstmt;
    ...
    head->type = type;
    head->flag = 0;
    clt_set_param_direction(CTCONN_INPUT, &head->flag);
    ...
    if (is_null == CT_TRUE) {
        ...
        if (type == CTCONN_TYPE_STRING || type == CTCONN_TYPE_VARCHAR || type == CTCONN_TYPE_CHAR) {
...
status_t ctconn_init_params(ctconn_stmt_t pstmt, uint32 param_count, bool32 is_batch)
{
    clt_stmt_t *stmt = (clt_stmt_t *)pstmt;
    ...
    for (uint32 i = 0; i < stmt->param_count; i++) {
        ...
        param->bnd_ptr = NULL;
        param->ind_ptr = NULL;
        param->bnd_type = CTCONN_TYPE_INTEGER;
        param->bnd_size = 0;
...
status_t ctconn_pe_prepare(ctconn_stmt_t pstmt, const char *sql, uint64 *scn)
{
    clt_stmt_t *stmt = (clt_stmt_t *)pstmt;
...
status_t ctconn_pe_async_execute(ctconn_stmt_t pstmt, uint32 *more_param)
{
    clt_stmt_t *stmt = (clt_stmt_t *)pstmt;
    cs_packet_t *req_pack = NULL;
    ...
    *more_param = CT_FALSE;
    ...
    CTCONN_INIT_PREP_EXEC_PARAM(exe_param, stmt);
...
status_t ctconn_pe_async_execute_ack(ctconn_stmt_t pstmt, const char *sql, uint64 *ack_scn)
{
    clt_stmt_t *stmt = (clt_stmt_t *)pstmt;
...
status_t ctconn_async_execute(ctconn_stmt_t pstmt, bool32 *more_param, uint64 *scn)
{
    cs_packet_t *req_pack = NULL;
...
status_t ctconn_async_execute_ack(ctconn_stmt_t pstmt, uint64 *ack_scn)
{
    clt_stmt_t *stmt = (clt_stmt_t *)pstmt;
    cs_packet_t *ack_pack = NULL;
...
static status_t send_cmd(ctconn_conn_t pconn, uint8 cmd)
{
...
status_t ctconn_async_commit(ctconn_conn_t pconn)
{
    return send_cmd(pconn, CS_CMD_COMMIT);
}

status_t ctconn_async_commit_ack(ctconn_conn_t pconn)
{
    return clt_async_get_ack((clt_conn_t *)pconn, &((clt_conn_t *)pconn)->pack);
}

status_t ctconn_async_rollback(ctconn_conn_t pconn)
{
    return send_cmd(pconn, CS_CMD_ROLLBACK);
}

status_t ctconn_async_rollback_ack(ctconn_conn_t pconn)
{
    return clt_async_get_ack((clt_conn_t *)pconn, &((clt_conn_t *)pconn)->pack);
}

status_t ctconn_statement_rollback(ctconn_conn_t pconn, uint32 dml_id)
{
    clt_conn_t *conn = (clt_conn_t *)pconn;
    cs_packet_t *req_pack = NULL;
    cs_packet_t *ack_pack = NULL;
...
status_t ctconn_gts(ctconn_conn_t pconn, uint64 *scn)
{
    clt_conn_t *conn = (clt_conn_t *)pconn;
    cs_packet_t *req_pack = NULL;
    cs_packet_t *ack_pack = NULL;
...
status_t ctconn_shard_prepare(ctconn_stmt_t pstmt, const char *sql)
{
    return ctconn_prepare(pstmt, sql);
}

status_t ctconn_shard_execute(ctconn_stmt_t pstmt)
{
    return ctconn_execute(pstmt);
}

status_t ctconn_fetch_sequence(ctconn_stmt_t pstmt, text_t *user, text_t *seq_name, ctconn_sequence_t *ctconn_seq)
{
    clt_stmt_t *stmt = (clt_stmt_t *)pstmt;
    cs_packet_t *req_pack = NULL;
    cs_packet_t *ack_pack = NULL;
    ...
    CT_RETURN_IFERR(cs_put_int32(req_pack, ctconn_seq->group_order));
    CT_RETURN_IFERR(cs_put_int32(req_pack, ctconn_seq->group_cnt));
    CT_RETURN_IFERR(cs_put_int32(req_pack, ctconn_seq->size));

    CT_RETURN_IFERR(clt_remote_call(stmt->conn, req_pack, ack_pack));

    cs_init_get(ack_pack);
    CT_RETURN_IFERR(cs_get_int64(ack_pack, (int64 *)&ctconn_seq->start_val));
    CT_RETURN_IFERR(cs_get_int64(ack_pack, (int64 *)&ctconn_seq->step));
    CT_RETURN_IFERR(cs_get_int64(ack_pack, (int64 *)&ctconn_seq->end_val));

    return CT_SUCCESS;
}

status_t ctconn_set_sequence_nextval(ctconn_stmt_t pstmt, text_t *user, text_t *seq_name, int64 currval)
{
...
status_t ctconn_get_sequence_nextval(ctconn_stmt_t pstmt, text_t *user, text_t *seq_name, int64 *value)
{
    clt_stmt_t *stmt = (clt_stmt_t *)pstmt;
    cs_packet_t *req_pack = NULL;
    cs_packet_t *ack_pack = NULL;
...
status_t ctconn_get_cn_nextval(ctconn_stmt_t pstmt, text_t *user, text_t *seq_name, bool32 *empty_cache, int64 *value)
{
    clt_stmt_t *stmt = (clt_stmt_t *)pstmt;
    cs_packet_t *req_pack = NULL;
    cs_packet_t *ack_pack = NULL;
...
status_t ctconn_notify_cn_update_cache(ctconn_stmt_t pstmt, text_t *user, text_t *seq_name)
{
    clt_stmt_t *stmt = (clt_stmt_t *)pstmt;
    cs_packet_t *req_pack = NULL;
    cs_packet_t *ack_pack = NULL;
...
status_t ctconn_decode_scn(clt_conn_t *conn, cs_packet_t *ack_pack, uint64 *scn)
{
    if (!CS_XACT_WITH_TS(ack_pack->head->flags)) {
        return CT_SUCCESS;
    }
...
static status_t ctconn_commit_with_ts_core(clt_conn_t *conn, uint64 *scn)
{
    cs_packet_t *req_pack = NULL;
    cs_packet_t *ack_pack = NULL;
    req_pack = &conn->pack;
    ...
    return ctconn_decode_scn(conn, ack_pack, scn);
}

status_t ctconn_commit_with_ts(ctconn_conn_t pconn, uint64 *scn)
{
    status_t status;
    clt_conn_t *conn = (clt_conn_t *)pconn;
    ...
    status = ctconn_commit_with_ts_core(conn, scn);
