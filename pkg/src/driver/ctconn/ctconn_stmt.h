 ...
 *
 * IDENTIFICATION
 * src/driver/ctconn/ctconn_stmt.h
 *
 * -------------------------------------------------------------------------
 */
...
static inline uint32 clt_get_total_row_bndsz(clt_stmt_t *stmt)
{
    ...
    for (uint32 i = 0; i < stmt->param_count; i++) {
        ...
        actual_bnd_size =
            CTCONN_IS_TYPE_NEED_EXTRA(param->bnd_type) ? (sizeof(uint32) + CM_ALIGN4(param->bnd_size)) : param->bnd_size;
        if (CTCONN_IS_STRING_TYPE(param->bnd_type) && param->bnd_size > CT_MAX_COLUMN_SIZE) {
            actual_bnd_size = sizeof(ctconn_lob_t);
        }
