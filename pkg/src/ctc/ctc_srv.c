...
EXTER_ATTACK int ctc_close_session(ctc_handler_t *tch)
{
    ...
    if (ret != CT_SUCCESS) {
        CT_LOG_RUN_ERR("[CTC_CLOSE_SESSION]:close mysql connection failed, ret:%d, conn_id:%u, ctc_instance_id:%u",
...
EXTER_ATTACK void ctc_kill_session(ctc_handler_t *tch)
{
    ...
    ctc_set_no_use_other_sess4thd(session);
    CT_LOG_DEBUG_INF("[CTC_KILL_SESSION]:conn_id=%u, ctc_inst_id:%u, session_id=%u",
                               ...
                               dml_flag_t flag, serial_t *serial, uint64_t *last_insert_id)
{
    ...
    */
    ...
    if (ctc_get_curr_serial_value_auto_inc(session, entity, &serial_value, flag.auto_inc_step,
...
int ctc_copy_cursor_row(knl_session_t *session, knl_cursor_t *cursor, row_head_t *src_row,
...
int ctc_bulk_write_rows(knl_cursor_t *cursor, knl_session_t *knl_session,
                        uint64_t rec_num, const record_info_t *record_info,
                        uint *err_pos, ctc_context_t *ctc_context, ctc_part_t *part_ids)
{
    int ret = CT_SUCCESS;
                                ...
                                uint32_t *err_pos, dml_flag_t flag, ctc_part_t *part_ids)
{
    ...
    if (flag.ignore || (((table_t *)cursor->table)->cons_set.ref_count > 0 && flag.no_foreign_key_check) ||
        ...
        ret = insert_and_verify_for_bulk_write(cursor, knl_session, rec_num, record_info,
                                               err_pos, ctc_context, flag, part_ids);
    } else {
        ...
        ret = ctc_bulk_write_rows(cursor, knl_session, rec_num,
                                  record_info, err_pos, ctc_context, part_ids);
    }
...
int insert_and_verify_for_write_row(knl_session_t *knl_session, knl_cursor_t *cursor,
    serial_t *serial, uint64_t last_insert_id, ctc_context_t *ctc_context, dml_flag_t flag)
{
    int ret = 0;
    knl_savepoint_t savepoint;
    if (flag.ignore) {
...
not_ret:
    if (flag.auto_inc_used) {
        ...
        if (status != CT_SUCCESS) {
            ret = ctc_get_and_reset_err();
            CT_LOG_RUN_ERR("ctc_write_row failed to set serial with ret %d, serial_col_value=%lld",
                               ...
                               uint16_t serial_column_offset, uint64_t *last_insert_id, dml_flag_t flag)
{
    ...
    do {
        ...
        if (flag.auto_inc_used) {
            ...
            if (ctc_update_serial_col(knl_session, cursor, serial_column_offset,
...
int ctc_check_update_constraint(knl_session_t *knl_session, knl_cursor_t *cursor, uint64_t conflicts,
...
not_ret:
    if (entity->has_serial_col && serial.max_serial_col_value > 0 && !flag.dd_update) {
        if (knl_update_serial_value_4mysql(knl_session, cursor->dc_entity, serial.max_serial_col_value,
            ...
            ret = ctc_get_and_reset_err();
            CT_LOG_RUN_ERR(
                "raw_ctc_update_row failed to set serial column value with ret %d, max_serial_col_value:%lld",
                ret, serial.max_serial_col_value);
            if (ret != CT_SUCCESS && flag.ignore) {
                knl_rollback(knl_session, &savepoint);
            }
...
int ctc_open_cursor_and_fetch_by_rowid(knl_session_t *knl_session, knl_cursor_t *cursor,
                                       ctc_context_t *ctc_context, bool32 *isFound, bool is_select)
{
    int ret = CT_SUCCESS;
...
static int ctc_index_init(session_t *session, ctc_context_t *ctc_context, ctc_handler_t *tch,
...
int ctc_check_partition_status(ctc_handler_t *tch, knl_cursor_t **cursor,
                               index_key_info_t *index_key_info, ctc_select_mode_t mode)
{
    int ret;
    session_t *session = ctc_get_session_by_addr(tch->sess_addr);
    CTC_LOG_RET_VAL_IF_NUL(session, ERR_INVALID_SESSION_ID, "session lookup failed");
    ...
    if (!index_key_info->sorted) {
        ...
        ret = ctc_index_init(session, ctc_context, tch, index_key_info->active_index,
...
static int ctc_get_correct_pos(ctc_handler_t *tch, session_t *session,
...
int get_correct_pos_by_fetch(ctc_handler_t *tch, knl_cursor_t *cursor,
...
{
    if (IS_CTC_PART(cursor->part_loc.part_no) && IS_CTC_PART(tch->part_id) && cursor->part_loc.part_no != tch->part_id) {
        return CT_TRUE;
                                ...
                                ctc_select_mode_t mode, ctc_conds *cond, const bool is_replace)
{
    ...
    if (index_info->need_init) {
        CT_RETURN_IFERR(ctc_index_init(session, ctc_context, tch, index_info->active_index,
...
EXTER_ATTACK int ctc_trx_begin(ctc_handler_t *tch, ctc_trx_context_t trx_context, bool is_mysql_local)
{
    ...
    if (is_select && !knl_db_is_primary(knl_session)) {
        CT_LOG_RUN_INF_LIMIT(LOG_PRINT_INTERVAL_SECOND_20, "ctc_trx_begin: select operation on read only mode in slave node.");
        return CT_SUCCESS;
    }
    ...
    if (knl_set_session_trans(knl_session, (isolation_level_t)trx_context.isolation_level, is_select) != CT_SUCCESS) {
        ...
        CT_LOG_RUN_ERR("ctc_trx begin: knl_set_session_trans failed, thd_id=%u, err=%d",
            tch->thd_id, err);
        return err;
    }
...
EXTER_ATTACK int ctc_srv_set_savepoint(ctc_handler_t *tch, const char *name)
{
    ...
    if (knl_set_savepoint(knl_session, &nm) != CT_SUCCESS) {
        int err = ctc_get_and_reset_err();
        CT_LOG_RUN_ERR("ctc_srv_set_savepoint: knl_set_savepoint failed, thd_id=%u, err=%d",
            tch->thd_id, err);
        return err;
    }
    return CT_SUCCESS;
...
EXTER_ATTACK int ctc_srv_rollback_savepoint(ctc_handler_t *tch, uint64_t *cursors, int32_t csize, const char *name)
{
    ...
    if (knl_rollback_savepoint(knl_session, &nm) != CT_SUCCESS) {
        int err = ctc_get_and_reset_err();
        CT_LOG_RUN_ERR("ctc_srv_rollback_savepoint: knl_rollback_savepoint failed, thd_id=%u, err=%d",
            tch->thd_id, err);
        return err;
    }
    return CT_SUCCESS;
...
EXTER_ATTACK int ctc_srv_release_savepoint(ctc_handler_t *tch, const char *name)
{
    ...
    if (knl_release_savepoint(knl_session, &nm) != CT_SUCCESS) {
        int err = ctc_get_and_reset_err();
        CT_LOG_RUN_ERR("ctc_srv_release_savepoint: knl_release_savepoint failed, thd_id=%u, err=%d",
            tch->thd_id, err);
        return err;
    }
    return CT_SUCCESS;
...
EXTER_ATTACK int ctc_knl_write_lob(ctc_handler_t *tch, char *locator, uint32_t locator_size,
...
EXTER_ATTACK int ctc_analyze_table(ctc_handler_t *tch, const char *db_name,
...
EXTER_ATTACK int ctc_get_cbo_stats(ctc_handler_t *tch, ctc_cbo_stats_t *stats, ctc_cbo_stats_table_t *ctc_cbo_stats_table, uint32_t first_partid, uint32_t num_part_fetch)
{
    status_t ret;
    session_t *session = ctc_get_session_by_addr(tch->sess_addr);
    CTC_LOG_RET_VAL_IF_NUL(session, ERR_INVALID_SESSION_ID, "session lookup failed");
    ...
    ret = get_cbo_stats(knl_session, DC_ENTITY(ctc_context->dc), stats, ctc_cbo_stats_table, first_partid, num_part_fetch);
...
EXTER_ATTACK int ctc_get_index_name(ctc_handler_t *tch, char *index_name)
{
    ...
    errno_t ret = memcpy_s(index_name, CTC_MAX_KEY_NAME_LENGTH + 1,
                           DC_INDEX(dc, ctc_context->dup_key_slot)->desc.name, len);
    knl_securec_check(ret);
    return CT_SUCCESS;
}
