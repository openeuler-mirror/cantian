...
void fill_cbo_stats_column(cbo_stats_column_t *cbo_column, ctc_cbo_stats_column_t *ctc_column, uint32 col_id,
                               dc_entity_t *entity)
{
    if (cbo_column == NULL) {
        CT_LOG_RUN_WAR("statistics of column in DC is NULL, col_id: %u", col_id);
        return;
    }
...
void fill_cbo_stats_index(cbo_stats_index_t *index_cbo, cbo_stats_column_t *col_stats , uint32_t *ndv_keys)
{
    ...
    for (uint32_t i = MAX_PERSIST_INDEX_CBO_COLUMES; i < MAX_KEY_COLUMNS; i++) {
        ndv_keys[i] = index_cbo->distinct_keys_arr[i] + ndv_keys[i - 1];
    }
}

void fill_cbo_stats_table_t(knl_handle_t handle, dc_entity_t *entity, ctc_cbo_stats_t *stats,
                                cbo_stats_table_t *table_stats, ctc_cbo_stats_table_t *ctc_cbo_stats_table)
{
    for (uint32 col_id = 0; col_id <= table_stats->max_col_id; col_id++) {
        ...
        fill_cbo_stats_column(column, &ctc_cbo_stats_table->columns[col_id], col_id, entity);
    }
                                           ...
                                           cbo_stats_table_t *table_stats, uint32 part_id, uint32 stats_idx)
{
    ...
    for (uint32 col_id = 0; col_id <= table_stats->max_col_id; col_id++) {
        ...
        fill_cbo_stats_column(column, &ctc_cbo_stats_table[stats_idx].columns[col_id], col_id, entity);
    }
    return CT_SUCCESS;
}
...
void fill_part_table_cbo_stats_index(knl_handle_t handle, dc_entity_t *entity, ctc_cbo_stats_t *stats)
{
    ...
    for (uint32 idx_id = 0; idx_id < entity->table.desc.index_count; idx_id++) {
        ...
        cbo_stats_column_t *col_stats = knl_get_cbo_column(handle, entity, index->desc.columns[0]);
        if (col_stats == NULL) {
            CT_LOG_RUN_WAR("statistics of index in DC is NULL, table: %s ,index: %s", entity->table.desc.name, index->desc.name);
            continue;
        }
        fill_cbo_stats_index(index_cbo, col_stats, ndv_index_keys);
    }
...
status_t get_cbo_stats(knl_handle_t handle, dc_entity_t *entity, ctc_cbo_stats_t *stats, ctc_cbo_stats_table_t *ctc_cbo_stats_table, uint32_t first_partid, uint32_t num_part_fetch)
{
    ...
    if (!knl_is_part_table(entity)) {
        ...
        fill_cbo_stats_table_t(handle, entity, stats, table_stats, ctc_cbo_stats_table);
    } else if (!knl_is_compart_table(entity)){
        fill_part_table_cbo_stats_index(handle, entity, stats);
        for (uint32 i = 0; i < num_part_fetch; i++) {
            ...
            if (table_stats != NULL) {
                ...
                CT_LOG_RUN_WAR("table part: %d statistics in DC is empty.", i);
            }
