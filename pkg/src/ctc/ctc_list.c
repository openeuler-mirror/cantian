...
static void ctc_list_reset(ctc_list_t *list)
{
    knl_panic(list != NULL);
...
static uint64_t ctc_list_calc_size(ctc_list_t *list)
{
    uint64_t size = 0;
    ctc_list_node_t *node = (ctc_list_node_t *)cm_atomic_get(&list->head);
    while (node != NULL) {
        if (node->is_delete == false) {
            size++;
        }
        node = (ctc_list_node_t *)cm_atomic_get(&node->next);
    }
    return size;
}

void triger_ctc_list_raw_delete_all(ctc_list_t *list)
{
    ...
    ctc_list_node_t *node = (ctc_list_node_t *)cm_atomic_get(&list->head);
    ctc_list_node_t *node_next = NULL;
    while (node != NULL) {
        node_next = (ctc_list_node_t *)cm_atomic_get(&node->next);
        if (node->is_delete == false) {
            real_size++;
        } else {
            real_del_cnt++;
    ...
    CT_LOG_DEBUG_INF("[CTC_LIST]:triger_ctc_list_raw_delete_all delete_cnt:%lu, size:%lu", delete_cnt, size);
}

void triger_ctc_list_raw_delete(ctc_list_t *list)
{
    uint64_t real_del_cnt = 0;  // 实际删除的节点数量
    uint64_t real_size = 0;     // 实际剩余的节点，用于校验删除的过程中有无丢失节点
    ctc_list_node_t new_list;
    init_list_node(&new_list);
    ctc_list_node_t *node_next = NULL;
    ctc_list_node_t *new_list_tail = &new_list;
    ...
    CM_ASSERT(real_size == ctc_list_calc_size(list));
    (void)pthread_rwlock_unlock(&list->rw_lock);
    CT_LOG_DEBUG_INF("[CTC_LIST]:triger_ctc_list_raw_delete: delete_cnt:%lu,size:%lu", delete_cnt, size);
}
...
{
    knl_panic(list != NULL);
    triger_ctc_list_raw_delete_all(list);
}
...
void ctc_list_insert(ctc_list_t *list, uint64_t search_key)
{
    ...
    ctc_list_node_t *node = (ctc_list_node_t *)search_key;
    node->is_delete = false;
    (void)pthread_rwlock_rdlock(&list->rw_lock);
    // 使用CAS保证 将node->next指向list->head 和 将list->head替换为node是原子发生的
    ctc_list_node_t *head = NULL;
    do {
        head = (ctc_list_node_t *)cm_atomic_get(&list->head);
...
void ctc_list_delete(ctc_list_t *list, uint64_t search_key)
{
    ...
    uint64_t delete_cnt = cm_atomic_get(&list->delete_cnt);
    if (delete_cnt >= CTC_LIST_DELETE_CACHE_CNT) {
        triger_ctc_list_raw_delete(list);
    }
}
