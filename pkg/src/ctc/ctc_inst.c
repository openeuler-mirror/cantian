...
thread_lock_t g_ctc_mes_lock;

uint32_t g_ctc_max_inst_num;
bool32 g_is_single_run_mode;

thread_lock_t *get_g_ctc_mes_lock(void)
{
    return &g_ctc_mes_lock;
}
...
void my_free_ctc_ctx(uint64_t ctx)
{
    ctc_context_t *my_ctx = (ctc_context_t *)ctx;
    free_ctc_ctx(&my_ctx, true);
}

int init_mysql_inst(void)
{
    cm_init_thread_lock(&g_ctc_mes_lock);
    ...
    for (int i = 0; i < g_ctc_max_inst_num; i++) {
        ...
        for (int j = 0; j < CTC_CTX_LIST_CNT; j++) {
            ctc_list_init(&inst->ctx_lists[j], my_free_ctc_ctx);
        }
    }
    return CT_SUCCESS;
}
...
int add_mysql_inst_ctx_res(uint32_t inst_id, ctc_context_t *ptr)
{
    ...
    ctc_list_t *ctx_list = &inst->ctx_lists[slot];
    ctc_list_insert(ctx_list, (uint64_t)ptr);
    return CT_SUCCESS;
}
...
int remove_mysql_inst_ctx_res(uint32_t inst_id, ctc_context_t *ptr)
{
    ...
    CM_ASSERT(slot < CTC_CTX_LIST_CNT);
    ctc_list_t *ctx_list = &inst->ctx_lists[slot];
    ctc_list_delete(ctx_list, (uint64_t)ptr);
    return CT_SUCCESS;
}
...
void clean_up_mysql_inst_ctx_list(mysql_inst_info_s *inst)
{
    for (int i = 0; i < CTC_CTX_LIST_CNT; i++) {
        ctc_list_t *list = &inst->ctx_lists[i];
        ctc_list_clear(list);
    }
}
...
int ctc_release_inst_id(uint32_t inst_id)
{
    ...
    if (inst->empty == true || inst->inst_id != inst_id) {
        CT_LOG_RUN_ERR("[CTC_CLEAN_UP]: ctc_release_inst_id error inst id:%x %x, empty:%d", inst->inst_id, inst_id, inst->empty);
        return CT_SUCCESS;  // 这个地方可能存在没有调用alloc然后来调用release的情况，检查到状态错误直接返回，做任何操作
    }
    clean_up_mysql_inst_ctx_list(inst);
    inst->empty = true;
    inst->inst_id = CT_INFINITE32;
    CT_LOG_RUN_INF("[CTC_CLEAN_UP]: ctc_release_inst_id release inst id:%x success.", inst_id);
    return CT_SUCCESS;
}
...
int ctc_alloc_inst_id(uint32_t *inst_id)
{
    ...
    if (inst->empty == false || inst->inst_id != CT_INFINITE32) {
        CT_LOG_RUN_ERR("ctc_alloc_inst_id slot in use inst id:%u, empty:%d, mysql_inst_pos:%u", *inst_id, inst->empty,
                       mysql_inst_pos);
        return CT_ERROR;
    }
    ...
    *inst_id = alloc_inst_id;
    inst->empty = false;
    CT_LOG_RUN_INF("ctc_alloc_inst_id:%x", alloc_inst_id);
    return CT_SUCCESS;
}

int ctc_get_inst_id(uint32_t shm_client_id, uint32_t *inst_id)
{
    uint32_t mysql_inst_pos = GET_MYSQL_INST_POS(shm_client_id); // shm_client_id in 2 to 20 integer
    if (mysql_inst_pos >= g_ctc_max_inst_num) {
        CT_LOG_RUN_ERR("ctc_get_inst_id  invaild mysql_inst_pos:%u", mysql_inst_pos);
        return CT_ERROR;
    }
    mysql_inst_info_s *inst = &g_mysql_inst_infos[mysql_inst_pos];
    if (inst->empty == false && inst->inst_id != CT_INFINITE32) {
