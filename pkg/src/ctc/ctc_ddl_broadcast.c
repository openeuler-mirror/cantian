...
void ctc_process_broadcast_ack_ex(void *session, mes_message_t *msg)
{
    if (msg->head->dst_sid >= CT_MAX_MES_ROOMS) {
        CT_LOG_RUN_ERR("[CTC_MES]:invalid msg dst_sid %u.", msg->head->dst_sid);
        return;
    }
...
bool ctc_handle_recv_error(mes_message_t *recv_msg, char *err_msg)
{
    ...
    if (cmd == MES_CMD_PREPARE_DDL_RSP ||  cmd == MES_CMD_REWRITE_OPEN_CONN_RSP || allow_fail == true) {
        ...
        CT_LOG_RUN_ERR("[CTC_MES]:remote node exec failed. cmd:%d", cmd);
        return true;
    }
    return false;
}

int ctc_broadcast_and_recv(knl_session_t *knl_session, uint64 target_bits, const void *req_data, char *err_msg)
{
    ...
    thread_lock_t *ctc_mes_lock = get_g_ctc_mes_lock();

    cm_thread_lock(ctc_mes_lock);
    ...
    while (target_inst < mes_inst->profile.inst_count) {
        ...
        status_t status = ctc_send_data_retry(req_data, head->dst_inst);
        ...
        SYNC_POINT_GLOBAL_START(CTC_MES_OVERVIEW_RECV_FAIL, &status, CT_ERROR);
        CT_LOG_DEBUG_INF("[Disaster Recovery] wait for recv rsp. target_inst:%d, total: %d",
                         target_inst, mes_inst->profile.inst_count);
        status = mes_recv(sid, &recv_msg, CT_FALSE, head->rsn, CTC_BROADCAST_WAIT_TIMEOUT);
        ...
        if (status != CT_SUCCESS) {
            ...
            CT_LOG_RUN_ERR("[CTC_MES]:recv msg fail. going to resend. cmd%d, rsn:%d.", head->cmd, head->rsn);
            cm_reset_error();
            continue;
        }
        ...
        if (room->err_code != CT_SUCCESS) {
            ...
            if (ctc_handle_recv_error(&recv_msg, err_msg)) {
                ...
                cm_thread_unlock(ctc_mes_lock);
                return room->err_code;
            }

            CT_LOG_RUN_ERR("[CTC_MES]:recv error from other node. inst_id:%d, error_code:%d, cmd:%d",
...
static void ctc_copy_lock_info_from_rd(rd_lock_info_4mysql_ddl *rd_lock_info, ctc_lock_table_info *lock_info)
{
...
status_t ctc_lock_table_in_slave_node(knl_handle_t session, void *buff)
{
    ...
    ret = ctc_lock_table_impl(&tch, session, db_name, &lock_info, &error_code);
    if (ret != CT_SUCCESS) {
...
status_t ctc_unlock_table_in_slave_node(knl_handle_t session, void *buff)
{
    ...
    ret = ctc_unlock_table_impl(&tch, session, mysql_inst_id, &lock_info);
    if (ret != CT_SUCCESS) {
...
status_t ctc_execute_ddl_in_slave_node(knl_handle_t session, char *sql_text, uint32 sql_len)
{
    ctc_handler_t tch;
    status_t ret;
    ret = memset_s(&tch, sizeof(ctc_handler_t), 0, sizeof(ctc_handler_t));
    knl_securec_check(ret);
    // Do not use 0 or 0xFFFFFFFF, or mysql will close all the connections.
    tch.thd_id = CT_INVALID_ID32 - 1;
    tch.inst_id = CT_INVALID_ID32 - 1;
    ...
    ctc_ddl_broadcast_request *broadcast_req = (ctc_ddl_broadcast_request *)cm_malloc(sizeof(ctc_ddl_broadcast_request));
...
status_t ctc_unlock_mdl_key_in_slave_node(knl_handle_t session)
{
    ...
    ret = ctc_unlock_mdl_key_impl(&tch, session, mysql_inst_id);
    if (ret != CT_SUCCESS) {
