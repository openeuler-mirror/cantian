 ...
 *
 * IDENTIFICATION
 * src/utils/ctsql/ctsql_export.c
 *
 * -------------------------------------------------------------------------
 */
#include "cm_base.h"
...
#define EXP_MAX_USERNAME (CT_NAME_BUFFER_SIZE + 4)
#define EXP_MAX_PASSWD (CT_PASSWORD_BUFFER_SIZE + 4)
...
static ctconn_z_stream g_lf_zstream;  // used for lob file compress.
static FILE *g_exp_dpfile = (FILE *)NULL;
static FILE *g_exp_dpbinfile = (FILE *)NULL;
static ctconn_z_stream g_df_zstream;  // used for data file compress.
static FILE *g_exp_logfile = (FILE *)NULL;
static uint64 g_exp_scn = CT_INVALID_ID64;
static uint64 g_exp_gts_scn = CT_INVALID_ID64;
...
typedef struct st_exp_obj_info {
    char obj_name[CTSQL_MAX_OBJECT_LEN];
    char obj_type[CTSQL_MAX_OBJECT_TYPE_LEN];
} exp_obj_info_t;
...
#define EXP_RESET_ERROR g_exp_err_info.local_error = CT_FALSE;

#define EXP_THROW_ERROR(err_no, ...)                                                                               \
    do {                                                                                                           \
        CT_THROW_ERROR(err_no, ##__VA_ARGS__);                                                                     \
        exp_copy_error();                                                                                          \
    } while (0)

#define EXP_THROW_ERROR_EX(err_no, format, ...)                                                                    \
    do {                                                                                                           \
        CT_THROW_ERROR_EX(err_no, format, ##__VA_ARGS__);                                                          \
        exp_copy_error();                                                                                          \
    } while (0)

#define EXP_SRC_THROW_ERROR_EX(src_loc, err_no, format, ...)                                                       \
    do {                                                                                                           \
        CT_SRC_THROW_ERROR_EX(src_loc, err_no, format, ##__VA_ARGS__);                                             \
        exp_copy_error();                                                                                          \
    } while (0)
...
typedef struct st_exp_par_conn {
    uint32_t par_num;
    ctconn_conn_t conn[MAX_PARALLEL_VALUE];
    ctconn_stmt_t stmt[MAX_PARALLEL_VALUE];
} exp_par_conn_t;

typedef struct st_exp_dn_info {
    ctsql_conn_info_t dn_conn_info;
    uint32   dn_group_id;
    uint64   dn_scn;
...
typedef struct st_exp_shd_info {
    uint32_t shd_node_type;  // the type of the current node connected by ctsql is or not CN

    list_t  dn_info_list;   // all dn nodes info
    ctsql_conn_info_t gts_conn_info;
...
typedef struct st_export_options {
    ...
    char dump_data_path[CT_MAX_FILE_PATH_LENGH];    // the path of all dump files
    ...
    char log_file[CT_MAX_FILE_PATH_LENGH];
    // session schema name used when exp_type = EXP_TABLE or EXP_ALL_TABLES
    char schema_name[CT_MAX_NAME_LEN + 1];
...
typedef struct {
    ...
    ctconn_stmt_t query_tab_column; // for query table column info, cached stmt, prepared first
    ctconn_stmt_t query_func_indexes;
    ctconn_stmt_t query_tab_has_intervalpart;
    ctconn_stmt_t query_index_partitioning;
    ctconn_stmt_t query_index_subpartition;
    ctconn_stmt_t query_has_subpartition;
...
static export_options_t g_export_opts = {
    ...
    .compress = CT_FALSE,
    .consistent = CT_FALSE,
    .index_partitions = CT_FALSE,
    .content = CT_EXP_ALL,
    .query = "\0",
    .skip_comments = CT_FALSE,
    .force = CT_FALSE,
    .skip_add_drop_table = CT_FALSE,
    .skip_triggers = CT_FALSE,
    .quote_names = CT_TRUE,
    .commit_batch = CTSQL_COMMIT_BATCH,
    .insert_batch = 1,
...
typedef struct st_exp_partition_info {
    char parent_partition_type[CT_MAX_NAME_LEN];
    char sub_partition_type[CT_MAX_NAME_LEN];
...
{
    const char* msg = NULL;
    ctsql_get_error(NULL, &g_exp_err_info.err_info.code, &msg, &g_exp_err_info.err_info.loc);
    MEMS_RETVOID_IFERR(memcpy_sp(g_exp_err_info.err_info.message, CT_MESSAGE_BUFFER_SIZE, msg, CT_MESSAGE_BUFFER_SIZE));
    g_exp_err_info.local_error = CT_TRUE;
}

static void exp_get_error(ctconn_conn_t conn, int32 *code, const char **msg, source_location_t *loc)
{
    if (g_exp_err_info.local_error) {
        ...
        *msg = g_exp_err_info.err_info.message;
        ...
        ctsql_get_error(conn, code, msg, loc);
    }
}
...
static inline void clean_exp_bin_env(export_options_t *opt)
{
    ...
    if (opt->exp_status == CT_SUCCESS) {
        if (opt->crypt_info.crypt_flag) {
            (void)ctsql_get_encrypt_file(&opt->crypt_info, &crypt_file, cm_fileno(g_exp_dpfile));
        }
...
typedef struct st_exp_bin_file_ctx {
    ...
    FILE *df_h;
    ctconn_z_stream *df_zstream;
    FILE *lf_h;
    ctconn_z_stream *lf_zstream;
...
typedef struct st_par_exp_param {
    ...
    char tab_name[CT_MAX_NAME_LEN + 1];
...
typedef struct st_par_exp_thread_ctrl {
    ...
    struct st_ctsql_conn_info_t conn_info;
...
typedef struct st_par_exp_mgr {
    ...
    struct st_export_options options;
    struct st_par_exp_thread_ctrl thread_ctrls[CT_MAX_PAR_EXP_VALUE];
    uint64 *bin_rec_total_add;
} par_exp_mgr_t;

#define EXP_MAX_GROUPS  (CT_SHARED_PAGE_SIZE / sizeof(pointer_t))
...
static status_t exp_get_user_procs_sql(char *sql_buffer, uint32 buff_size, exp_prepare_sql_param_t *param)
{
    ...
    if (exp_proc_param->exp_opts->skip_triggers) {
        ...
        CT_RETURN_IFERR(exp_get_exclude_cond(EXP_EXCLUDE_TABLE, exp_proc_param->exp_opts, &exclude_cond));
...
static status_t exp_get_db_tables_sql(char *sql_buffer, uint32 buff_size, exp_prepare_sql_param_t *param)
{
    ...
    PRTS_RETURN_IFERR(sprintf_s(sql_buffer, buff_size,
        ...
        (ctconn_get_call_version(CONN) >= CS_VERSION_22) ? ", COMPRESS_ALGO" : " ",
        exp_tabname(g_export_opts.consistent, EXP_TABAGENT_DB_TABLES)));
                                       ...
                                       size_t max_len)
{
    ...
    PRTS_RETURN_IFERR(sprintf_s(cmd_buf, CTSQL_MAX_TEMP_SQL,
    ...
    bool8 is_first = CT_TRUE;
    do {
        CT_RETURN_IFERR(ctconn_fetch(STMT, &rows));
        if (rows == 0) {
            break;
        }
        CT_RETURN_IFERR(ctconn_column_as_string(STMT, 0, temp_column_name, MAX_COLUMN_WIDTH));
        if (!is_first) {
                                            ...
                                            size_t max_len)
{
    ...
    do {
        ...
        CT_RETURN_IFERR(ctconn_column_as_string(STMT, 0, temp_storeinfo, MAX_COLUMN_WIDTH));
        if (!is_first) {
...
static status_t exp_verify_column_of_part_tables_agent(void)
{
    ...
    int iret_sprintf = sprintf_s(cmd_buf, CTSQL_MAX_TEMP_SQL,
...
static status_t exp_get_old_partition_key_sql(char *sql_buffer, uint32 buff_size, exp_prepare_sql_param_t *param)
{
    ...
    CT_RETURN_IFERR(exp_get_partition_cols(exp_partition_param, column_names, MAX_COLUMN_WIDTH));
    CT_RETURN_IFERR(exp_get_partition_storeinfo(exp_partition_param, storeinfo, MAX_COLUMN_WIDTH));
...
static status_t exp_get_new_partition_key_sql(char *sql_buffer, uint32 buff_size, exp_prepare_sql_param_t *param)
{
    ...
    CT_RETURN_IFERR(exp_get_partition_cols(exp_partition_param, parent_key_names, MAX_COLUMN_WIDTH));
    CT_RETURN_IFERR(exp_get_partition_storeinfo(exp_partition_param, storeinfo, MAX_COLUMN_WIDTH));
    CT_RETURN_IFERR(exp_get_subpartition_cols(exp_partition_param, sub_key_names, MAX_COLUMN_WIDTH));
...
static status_t exp_get_partition_value_sql(char *sql_buffer, uint32 buff_size, exp_prepare_sql_param_t *param)
{
    ...
    PRTS_RETURN_IFERR(sprintf_s(sql_buffer, buff_size,
        ...
        (ctconn_get_call_version(CONN) >= CS_VERSION_22) ? ", X.COMPRESS_ALGO" : " ",
...
static status_t exp_get_roles_sql(char *sql_buffer, uint32 buff_size, exp_prepare_sql_param_t *param)
{
    ...
    if (get_roles_param->exp_opts->exp_type == EXP_ALL_SCHEMAS) {
        ...
        CT_RETURN_IFERR(exp_append_users(sql_buffer, buff_size, &get_roles_param->exp_opts->obj_list));
...
static status_t exp_get_user_tables_sql(char *sql_buffer, uint32 buff_size, exp_prepare_sql_param_t *param)
{
    ...
    char tbs_name[CTSQL_MAX_TEMP_SQL + 1] = { 0 };
    text_t *exclude_cond = NULL;
    ...
    if (exp_opts->tbs_list.count > 0) {
        CT_RETURN_IFERR(list2str(&exp_opts->tbs_list, tbs_name, CTSQL_MAX_TEMP_SQL + 1));
        PRTS_RETURN_IFERR(sprintf_s(sql_buffer + strlen(sql_buffer), buff_size - strlen(sql_buffer),
            "AND TABLESPACE_NAME IN(%s) ", tbs_name));
    }

    CT_RETURN_IFERR(exp_get_exclude_cond(EXP_EXCLUDE_TABLE, exp_opts, &exclude_cond));
    if (exclude_cond != NULL) {
        PRTS_RETURN_IFERR(sprintf_s(sql_buffer + strlen(sql_buffer), buff_size - strlen(sql_buffer),
...
static status_t get_current_scn(ctconn_stmt_t stmt, uint64 *scn)
{
    ...
    get_par_sql = (char *)malloc(CTSQL_MAX_TEMP_SQL);
    if (get_par_sql == NULL) {
        CTSQL_PRINTF(ZSERR_EXPORT, "malloc databuf failed!");
        return CT_ERROR;
    }

    iret_sprintf = sprintf_s(get_par_sql, CTSQL_MAX_TEMP_SQL, "SELECT CURRENT_SCN FROM DV_DATABASE");
    if (iret_sprintf == -1) {
    ...
    if (ctconn_prepare(stmt, get_par_sql) != CT_SUCCESS) {
        CM_FREE_PTR(get_par_sql);
        return CT_ERROR;
    }
    ...
    do {
        ...
        if (ctconn_get_column_by_id(stmt, 0, (void **)&data, &size, &is_null) != CT_SUCCESS) {
            CM_FREE_PTR(get_par_sql);
            return CT_ERROR;
        }
...
static status_t exp_init_scn(export_options_t *exp_opts)
{
    if (exp_opts->consistent || exp_opts->parallel > 1 || exp_opts->filetype == FT_BIN) {
        if (get_current_scn_core(exp_opts, &g_exp_scn) != CT_SUCCESS) {
...
static int32 exp_generate_filename(const char *tabname, const char *user, char *outname, uint32 max_size)
{
    ...
    int ret_len = sprintf_s(buf, CT_FILE_NAME_BUFFER_SIZE, "%s%u%s%I64d", user, file_no, tabname, ts);
    ...
    *(uint16 *)&bin_buf[8] = (uint16)file_no;
    ...
    CT_RETURN_IFERR(cm_bin2text(&bin, CT_FALSE, &text));
...
static int32 exp_form_fullpath(const char *filepath, const char *filename, char *fullpath, uint32 max_path_len)
{
    ...
    PRTS_RETURN_IFERR(sprintf_s(pathbuf, CT_MAX_FILE_PATH_LENGH, "%s%s",  filepath, filename));
    CT_RETURN_IFERR(realpath_file(pathbuf, fullpath, max_path_len));
...
int par_exp_rename_file(par_exp_thread_ctrl_t *ctrl)
{
    ...
    CT_RETURN_IFERR(exp_generate_filename(ctrl->tab_param.tab_name, ctrl->tab_param.schema, fname,
...
int par_exp_save_filenname(par_exp_thread_ctrl_t *ctrl, list_t *files)
{
    ...
    CT_RETURN_IFERR(cm_list_new(files, (void **)&buf));
    PRTS_RETURN_IFERR(sprintf_s(buf, EXP_LOB_MAX_FILE_NAME_LEN, "%s", filename));
    return CT_SUCCESS;
}

status_t ctsql_exp_generate_encrypt_info(crypt_info_t *crypt_info, char *file_name, uint32 max_len, int32 fp)
{
    crypt_file_t *encrypt_ctx = NULL;

    CT_RETURN_IFERR(cm_list_new(&crypt_info->crypt_list, (void *)&encrypt_ctx));
    CT_RETURN_IFERR(ctsql_encrypt_prepare(&encrypt_ctx->crypt_ctx, crypt_info->crypt_pwd));
    MEMS_RETURN_IFERR(strncpy_s(encrypt_ctx->filename, CT_MAX_NAME_LEN, file_name, max_len));
    encrypt_ctx->fp = fp;
    return CT_SUCCESS;
}
...
status_t par_exp_dispatch_bin(par_exp_thread_ctrl_t *ctrl, char *path)
{
    if (ctrl->files_context.exp_dpfile != NULL) {
        if (g_export_opts.compress) {
            (void)ctconn_common_z_uninit_write(&ctrl->files_context.df_zstream);
        }
        fclose(ctrl->files_context.exp_dpfile);
        ctrl->files_context.exp_dpfile = NULL;
    }

    CT_RETURN_IFERR(cm_fopen(path, "wb+", FILE_PERM_OF_DATA, &ctrl->files_context.exp_dpfile));

    if (g_export_opts.crypt_info.crypt_flag) {
        CT_RETURN_IFERR(ctsql_exp_generate_encrypt_info(&g_export_opts.crypt_info, ctrl->options.bin_data_file,
            EXP_LOB_MAX_FILE_NAME_LEN, cm_fileno(ctrl->files_context.exp_dpfile)));
    }

    if (g_export_opts.compress &&
        ctconn_common_z_init_write(ctrl->files_context.exp_dpfile, &ctrl->files_context.df_zstream,
            g_export_opts.compress_level) != CT_SUCCESS) {
        CTSQL_PRINTF(ZSERR_EXPORT, "init compress file '%s' failed.\n", path);
        return CT_ERROR;
    }

    if (ctrl->files_context.lf_h != NULL) {
        if (g_export_opts.compress) {
            (void)ctconn_common_z_uninit_write(&ctrl->files_context.lf_zstream);
        }
        fclose(ctrl->files_context.lf_h);
        ctrl->files_context.lf_h = NULL;
    }

    CT_RETURN_IFERR(exp_form_fullpath(ctrl->options.dump_data_path, ctrl->options.lob_file_name, path,
        CT_MAX_FILE_PATH_LENGH));
    CT_RETURN_IFERR(cm_fopen(path, "wb+", FILE_PERM_OF_DATA, &ctrl->files_context.lf_h));

    if (g_export_opts.crypt_info.crypt_flag) {
        CT_RETURN_IFERR(ctsql_exp_generate_encrypt_info(&g_export_opts.crypt_info, ctrl->options.lob_file_name,
            EXP_LOB_MAX_FILE_NAME_LEN, cm_fileno(ctrl->files_context.lf_h)));
    }

    if (g_export_opts.compress &&
        ctconn_common_z_init_write(ctrl->files_context.lf_h, &ctrl->files_context.lf_zstream,
            g_export_opts.compress_level) != CT_SUCCESS) {
        CTSQL_PRINTF(ZSERR_EXPORT, "init compress file '%s' failed.\n", path);
        return CT_ERROR;
    }
    ...
    exp_cols_t *exp_cols)
{
    ...
    CT_RETURN_IFERR(par_exp_rename_file(ctrl));

    CT_RETURN_IFERR(exp_form_fullpath(ctrl->options.dump_data_path, ctrl->options.bin_data_file, path,
        CT_MAX_FILE_PATH_LENGH));
    if (g_export_opts.filetype == FT_TXT) {
        ...
        CT_RETURN_IFERR(cm_fopen(path, "w+", FILE_PERM_OF_DATA, &ctrl->files_context.exp_dpfile));
        ...
        CT_RETURN_IFERR(par_exp_dispatch_bin(ctrl, (char *)path));
    }
    ...
    if (ctrl->files_context.str_buf == NULL) {
        ctrl->files_context.str_buf = (char *)malloc(CT_MAX_PACKET_SIZE + 1);
        if (ctrl->files_context.str_buf == NULL) {
            EXP_THROW_ERROR(ERR_ALLOC_MEMORY, (uint64)CT_MAX_PACKET_SIZE + 1, "func: par_exp_dispatch");
            return CT_ERROR;
        }

        MEMS_RETURN_IFERR(memset_s(ctrl->files_context.str_buf, CT_MAX_PACKET_SIZE + 1, 0, CT_MAX_PACKET_SIZE + 1));
    }
...
void par_exp_release_thread_ctrl(par_exp_thread_ctrl_t *ctrl)
{
    ...
    if (ctrl->conn_info.stmt) {
        ctconn_free_stmt(ctrl->conn_info.stmt);
        ctrl->conn_info.stmt = NULL;
    }
    if (ctrl->conn_info.conn) {
        ctconn_disconnect(ctrl->conn_info.conn);
        ctrl->conn_info.conn = NULL;
    }
...
{
    uint32 i;
    bool32 is_end = CT_TRUE;

    cm_thread_lock(&mgr->lock_t);

    for (i = 0; i < mgr->options.parallel; i++) {
        if (mgr->thread_ctrls[i].execute_ret != CT_SUCCESS) {
...
static inline status_t exp_bin_write_shortstr(exp_bin_memory_mgr_t *mem_mgr, const char *str, uint16 size)
{
    ...
    CT_RETURN_IFERR(get_short_bin_bufer_addr(mem_mgr, &bin_buffer, size));
...
static inline status_t exp_bin_write_bytes(exp_bin_memory_mgr_t *mem_mgr, const char *str, uint32 size)
{
    ...
    if (size == 0) {
        CTSQL_PRINTF(ZSERR_EXPORT, "no data need to write");
        return CT_ERROR;
    }

    CT_RETURN_IFERR(get_mem_address(mem_mgr, &addr, size));
...
static status_t exp_bin_write_str(exp_bin_memory_mgr_t *mem_mgr, const char *str, uint32 size)
{
    ...
    CT_RETURN_IFERR(get_bin_bufer_addr(mem_mgr, &bin_buffer, size));
    MEMS_RETURN_IFERR(memcpy_s(bin_buffer.buffer, size, str, size));
    return CT_SUCCESS;
}
...
static status_t exp_bin_init_data_file(export_options_t *exp_opts, const char *table, const char *user)
{
    ...
    char fname[EXP_LOB_MAX_FILE_NAME_LEN2 + 1];
    char file_path_buf[CT_MAX_FILE_PATH_LENGH];

    CT_RETURN_IFERR(exp_generate_filename(table, user, fname, EXP_LOB_MAX_FILE_NAME_LEN2));
    ...
    CT_RETURN_IFERR(exp_form_fullpath(exp_opts->dump_data_path, exp_opts->bin_data_file, file_path_buf,
                                      CT_MAX_FILE_PATH_LENGH));
    
    if (cm_fopen(file_path_buf, "wb+", FILE_PERM_OF_DATA, &g_exp_dpbinfile) != CT_SUCCESS) {
        return CT_ERROR;
    }

    if (g_export_opts.crypt_info.crypt_flag) {
        CT_RETURN_IFERR(ctsql_exp_generate_encrypt_info(&g_export_opts.crypt_info, exp_opts->bin_data_file,
            EXP_LOB_MAX_FILE_NAME_LEN, cm_fileno(g_exp_dpbinfile)));
    }

    if (exp_opts->compress &&
        ctconn_common_z_init_write(g_exp_dpbinfile, &g_df_zstream, exp_opts->compress_level) != CT_SUCCESS) {
        CTSQL_PRINTF(ZSERR_EXPORT, "init compress file '%s' failed.\n", file_path_buf);
        return CT_ERROR;
    }

    CT_RETURN_IFERR(exp_form_fullpath(exp_opts->dump_data_path, exp_opts->lob_file_name, file_path_buf,
                                      CT_MAX_FILE_PATH_LENGH));
    
    if (cm_fopen(file_path_buf, "wb+", FILE_PERM_OF_DATA, &g_lob_binfile) != CT_SUCCESS) {
        return CT_ERROR;
    }

    if (g_export_opts.crypt_info.crypt_flag) {
        CT_RETURN_IFERR(ctsql_exp_generate_encrypt_info(&g_export_opts.crypt_info, exp_opts->lob_file_name,
            EXP_LOB_MAX_FILE_NAME_LEN, cm_fileno(g_lob_binfile)));
    }

    if (exp_opts->compress &&
        ctconn_common_z_init_write(g_lob_binfile, &g_lf_zstream, exp_opts->compress_level) != CT_SUCCESS) {
        CTSQL_PRINTF(ZSERR_EXPORT, "init compress file '%s' failed.\n", file_path_buf);
        return CT_ERROR;
    }
...
void ctsql_display_export_version_info(void)
{
    ctsql_printf("\n");
    if (!IS_CONN) {
        ctsql_printf("Connection is not established. "
            "Please make sure connect is ready then execute \"exp -v\" or \"exp version\"\n\n");
        return;
    }
    
    ctsql_printf("New functions or features for EXP and IMP in this version:\n");
    ctsql_printf("---------------------------------------------------------"
        "------------------------------------------------------------------\n");
    uint32 curr_negotiate_version = ctconn_get_call_version(CONN);
    switch (curr_negotiate_version) {
        case CS_LOCAL_VERSION:
            ctsql_printf("* support jsonb and support subpartion format csf for EXP and IMP\n");
        /* fall through */
        case (uint32)CS_VERSION_23:
            ctsql_printf("* support export and import compress attribution of table or partition table\n");
        /* fall through */
        case (uint32)CS_VERSION_21:
            ctsql_printf("* support binary format of array datatype\n");
        /* fall through */
        case (uint32)CS_VERSION_20:
            ctsql_printf("* support subpartition\n");
        /* fall through */
        case (uint32)CS_VERSION_19:
            ctsql_printf("* support create or replace force view\n");
        /* fall through */
        default:
            ctsql_printf("\n");
    }
...
void ctsql_display_export_usage(void)
{
    ctsql_printf("The syntax of logic export is: \n\n");
    ctsql_printf("     Format:  EXP KEYWORD=value or KEYWORD=value1,value2,...,valueN;\n");
    ctsql_printf("     Example: EXP TABLES=EMP,DEPT,MGR;\n");
    ctsql_printf("               or EXP USERS=USER_A,USER_B;\n");
    ctsql_printf("               or EXP DIST_RULES=RULE_1,RULE_2;\n\n");
    ctsql_printf("Keyword                 Description (Default)\n");
    ctsql_printf("---------------------------------------------------------------------------------------------------------------------------\n");
    ctsql_printf("USERS                   List of schema names. Specify a percent sign (%%) to export all users.\n");
    ctsql_printf("TABLES                  List of table names. Specify a percent sign (%%) to export all tables.\n");
    ctsql_printf("DIST_RULES              List of distribute rule names. Specify a percent sign (%%) to export all distribution rules. Supported only for sharding.\n");
    ctsql_printf("TABLESPACE_FILTER       List of tablespace names, the data or objects in these tablespaces will be exported. Case-sensitive words enclosed by '`' or '\"'.\n");
    ctsql_printf("FILE                    Output file (EXPDAT.DMP) \n");
    ctsql_printf("FILETYPE                Output file type: (TXT), BIN\n");
    ctsql_printf("LOG                     Log file of screen output\n");
    ctsql_printf("COMPRESS                Compress output file (0), only for FILETYPE=BIN, value range: 0-9, the smaller the value, the faster the speed, 0: no compression.\n");
    ctsql_printf("CONTENT                 Specifies data to unload where the valid keyword, values are: (ALL), DATA_ONLY, and METADATA_ONLY. \n");
    ctsql_printf("QUERY                   Predicate clause used to export a subset of a table, eg. \"where rownum <= 10\" \n");
    ctsql_printf("SKIP_COMMENTS           Do not add comments to dump file. (N)\n");
    ctsql_printf("FORCE                   Continue even if an SQL error occurs during a table dump. (N)\n");
    ctsql_printf("SKIP_ADD_DROP_TABLE     Do not add a DROP TABLE statement before each CREATE TABLE statement. (N)\n");
    ctsql_printf("SKIP_TRIGGERS           Do not dump triggers. (N)\n");
    ctsql_printf("QUOTE_NAMES             Quote identifiers. (Y)\n");
    ctsql_printf("TABLESPACE              Default transport all tablespaces except for system reserved. (N)\n");
    ctsql_printf("COMMIT_BATCH            Batch commit rows, commit once if set 0. (1000)\n");
    ctsql_printf("INSERT_BATCH            Batch insert rows. (1)\n");
    ctsql_printf("FEEDBACK                Feedback row count, feedback once if set 0 (10000)\n");
    ctsql_printf("PARALLEL                Table data export parallelism settings, range 2~16, The default value is 0\n");
    ctsql_printf("CONSISTENT              Cross - table consistency(N)\n");
    ctsql_printf("CREATE_USER             Export user definition(N),Used in conjunction with USERS.\n");
    ctsql_printf("ROLE                    Export user roles expect system preset roles (N),Used in conjunction with USERS.\n");
    ctsql_printf("GRANT                   Grant role and permission to USER (N),Used in conjunction with USERS and ROLE.\n");
    ctsql_printf("WITH_CR_MODE            Export tables and indexes with CR_MODE options (N)\n");
    ctsql_printf("WITH_FORMAT_CSF         Export tables and part tables with FORMAT CSF option (Y)\n");
    ctsql_printf("ENCRYPT                 Export files will be encrypted.\n");
    ctsql_printf("REMAP_TABLES            Table's name will remapped to another tablename.\n");
    ctsql_printf("PARTITIONS              Export tables's data within the input partition.\n");
    ctsql_printf("EXCLUDE                 Export exclude objects.\n");
    ctsql_printf("INDEX_PARTITIONS        Export index's partition informations (N).\n");
...
static int exp_open_writer(export_options_t *exp_opts)
{
    ...
    char dump_data_dir[CT_MAX_FILE_PATH_LENGH] = { 0x00 };
    char dump_file_name[CT_MAX_FILE_PATH_LENGH] = { 0x00 };

    cm_trim_filename(exp_opts->dump_file, CT_MAX_FILE_PATH_LENGH, dump_path);
    ...
    CT_RETURN_IFERR(realpath_file(exp_opts->dump_file, realfile, CT_MAX_FILE_PATH_LENGH));
    
    if (cm_fopen(realfile, (exp_opts->filetype == FT_BIN) ? "wb+" : "w+", FILE_PERM_OF_DATA,
                 &g_exp_dpfile) != CT_SUCCESS) {
        return CT_ERROR;
    }

    if (g_export_opts.crypt_info.crypt_flag) {
        CT_RETURN_IFERR(ctsql_exp_generate_encrypt_info(&g_export_opts.crypt_info, (char *)dump_file_name,
            CT_MAX_NAME_LEN - 1, cm_fileno(g_exp_dpfile)));
    }

    // Step 2. record the dump data path
    cm_trim_filename(realfile, CT_MAX_FILE_PATH_LENGH, dump_path);

    PRTS_RETURN_IFERR(snprintf_s(dump_data_dir, CT_MAX_FILE_PATH_LENGH, CT_MAX_FILE_PATH_LENGH - 1, "%s%s",
        dump_path, CTSQL_SEC_FILE_NAME));

    if (!cm_dir_exist((const char *)dump_data_dir)) {
        if (cm_create_dir(dump_data_dir) != CT_SUCCESS) {
            CTSQL_PRINTF(ZSERR_EXPORT, "failed to create dir %s, errno is %d.", dump_data_dir, cm_get_os_error());
            return CT_ERROR;
        }
    }

    PRTS_RETURN_IFERR(snprintf_s(exp_opts->dump_data_path, CT_FILE_NAME_BUFFER_SIZE, CT_FILE_NAME_BUFFER_SIZE - 1,
        "%s/", dump_data_dir));
...
static int exp_open_logger(const char *logfile)
{
    ...
    if (cm_fopen(path, "w+", FILE_PERM_OF_DATA, &g_exp_logfile) != CT_SUCCESS) {
        return CT_ERROR;
    }
...
static void exp_close_writer()
{
    ...
    if (g_exp_dpbinfile != NULL) {
        if (g_export_opts.compress) {
            (void)ctconn_common_z_uninit_write(&g_df_zstream);
        }
    ...
    if (g_lob_binfile != NULL) {
        if (g_export_opts.compress) {
            (void)ctconn_common_z_uninit_write(&g_lf_zstream);
        }
...
static int exp_writer_compress_s(char *buf, uint32 size, ctconn_z_stream *stream)
{
    ...
    if (stream == NULL) {
        ...
        if (ctconn_common_z_write(stream, &g_export_opts.crypt_info, swap_buffer, sizeof(swap_buffer), buf,
                               size, CT_FALSE) != CT_SUCCESS) {
...
static int exp_flush_compress_s(text_buf_t *dst, ctconn_z_stream *stream)
{
    char swap_buffer[SIZE_K(4)];

    if (stream == NULL) {
        ctsql_printf("%s", dst->str);
        return CT_ERROR;
    } else {
        if (ctconn_common_z_write(stream, &g_export_opts.crypt_info, swap_buffer, sizeof(swap_buffer), dst->str,
                               dst->len, CT_TRUE) != CT_SUCCESS) {
            EXP_THROW_ERROR(ERR_CLT_WRITE_FILE_ERR, errno);
            return CT_ERROR;
        }
        return CT_SUCCESS;
...
static int exp_writer_data_compress_s(char *buf, uint32 size, text_buf_t *dst, ctconn_z_stream *stream)
{
    ...
    CT_RETURN_IFERR(exp_writer_compress_s(dst->str, dst->len, stream));
    ...
    CT_RETURN_IFERR(exp_writer_compress_s(text.str, text.len, stream));
    return CT_SUCCESS;
}
...
static inline int exp_writer_encrypt_s(char *buf, uint32 size, FILE *filehand)
{
    ...
    CT_RETURN_IFERR(ctsql_get_encrypt_file(&g_export_opts.crypt_info, &encrypt_file, cm_fileno(filehand)));
    ...
    if (cm_encrypt_data_by_gcm(encrypt_file->crypt_ctx.gcm_ctx, encrypt_buf, buf, size) != CT_SUCCESS) {
...
static inline int exp_writer_s(char *buf, uint32 size, FILE *filehand)
{
    if (filehand == NULL) {
        ...
        if (g_export_opts.crypt_info.crypt_flag) {
            CT_RETURN_IFERR(exp_writer_encrypt_s(buf, size, filehand));
            return CT_SUCCESS;
        }
...
static inline int exp_write_text_s(const text_t *text, text_buf_t *dst, FILE *filehand)
{
    ...
    CT_RETURN_IFERR(exp_writer_s(dst->str, dst->len, filehand));
    ...
    CT_RETURN_IFERR(exp_writer_s(text->str, text->len, filehand));
    return CT_SUCCESS;
}
...
static inline status_t exp_write_bin_s(const char *str, uint32 size, text_buf_t *dst, FILE *filehand)
{
    ...
    CT_RETURN_IFERR(exp_write_text_s(&text, dst, filehand));
    return CT_SUCCESS;
}

static int exp_writer_bin_data_s(char *buf, uint32 size, text_buf_t *dst, FILE *fp, ctconn_z_stream *stream)
{
    if (stream != NULL) {
...
static int exp_flush_bin_data_s(text_buf_t *dst, FILE *fp, ctconn_z_stream *stream)
{
    if (stream != NULL) {
...
static inline status_t exp_write_schema_com(const char *str, text_buf_t *dst, FILE *filehand)
{
    ...
    if (g_export_opts.filetype == FT_BIN) {
        ...
        CT_RETURN_IFERR(get_mem_address(&g_export_opts.master_bin_mgr, &addr, size));
        MEMS_RETURN_IFERR(memcpy_s(addr, size, str, size));
        return CT_SUCCESS;
    } else {
...
static void exp_write_fmt(bool32 wr_bin_sch, uint32 max_fmt_sz, const char *fmt, ...)
{
    ...
    va_end(var_list);
    if (len < 0) {
        ctsql_printf("Copy var_list to EXP_FMT_BUFER failed under using export tool.\n");
        return;
    }
    text.str = EXP_FMT_BUFER;
    text.len = (uint32)len;
    if (wr_bin_sch == CT_FALSE) {
...
static bool8 exp_error_ignored(ctconn_conn_t conn)
{
    int32 error_code = 0;
    const char *err_msg = "";
...
static inline status_t exp_ignore_error(ctconn_conn_t conn)
{
    ...
    return exp_error_ignored(conn) ? CT_SUCCESS : CT_ERROR;
}
...
#define exp_write_dplog(bufsz, fmt, ...)                    \
    do {                                                    \
        exp_write_fmt(CT_FALSE, bufsz, fmt, ##__VA_ARGS__); \
...
#define EXP_RETURN_IFERR(ret)                                                                           \
    do {                                                                                                \
        ...
        if (_status_ != CT_SUCCESS) {                                                                   \
            if (exp_ignore_error(CONN) != CT_SUCCESS) {                                                 \
                return _status_;                                                                        \
            }                                                                                           \
        }                                                                                               \
    } while (0)
...
#define EXP_BREAK_IFERR(ret)                                                                           \
    {                                                                                                  \
        int _status_ = (ret);                                                                          \
        if ((_status_) != CT_SUCCESS && exp_ignore_error(CONN) != CT_SUCCESS) {                        \
                break;                                                                                 \
        }                                                                                              \
    }
...
static inline void exp_concat_str_quote(text_t *text, const char *part)
{
    if (g_export_opts.quote_names) {
        cm_concat_fmt(text, CTSQL_MAX_QUOTE_NAME_SIZE, "\"%s\"", part);
    } else {
        (void)cm_concat_string(text, CTSQL_MAX_QUOTE_NAME_SIZE, part);
    }
}
...
{
    if (g_export_opts.quote_names) {
        exp_write_fmt(CT_TRUE, EXP_FMT_BUF_SZ, "\"%s\"", str);
    } else {
...
{
    if (g_export_opts.quote_names) {
        exp_write_fmt(CT_FALSE, EXP_FMT_BUF_SZ, "\"%s\"", str);
    } else {
...
static inline int par_exp_init_bfile_ctx(exp_bin_file_ctx_t *bfile_ctx, par_exp_thread_ctrl_t *ctrl)
{
    ...
    bfile_ctx->wr_lob_flag = CT_FALSE;
    ...
    bfile_ctx->tab_name = (char*)exp_remap_table_name(&ctrl->options.table_maps, ctrl->tab_param.tab_name,
        ctrl->tab_param.tab_name, CT_MAX_NAME_LEN);
    return CT_SUCCESS;
}
...
static inline status_t exp_write_head(void)
{
    ...
    CT_RETURN_IFERR(exp_write_str_s("--** The script is dumped by *CTSQL/EXP* tool, Zenith@Huawei Cantian Dept.\n",
        &g_exp_txtbuf, g_exp_dpfile));
    if (g_exp_dpfile != NULL) {
        exp_write_fmt(CT_FALSE, CTSQL_MAX_QUOTE_NAME_SIZE, "--** Dumped time: %s\n", exp_now());
    }
    CT_RETURN_IFERR(exp_write_str_s("\n", &g_exp_txtbuf, g_exp_dpfile));

    return CT_SUCCESS;
}

void exp_tmlog_error(ctconn_conn_t conn)
{
    int code = 0;
    const char *message = "";
...
static int ctsql_insert_export_obj(export_options_t *exp_opts, const text_t *obj_name, bool32 to_upper)
{
    ...
    return ctsql_generate_obj(&exp_opts->obj_list, object_name);
}

static int ctsql_insert_export_table(export_options_t *exp_opts, const text_t *table_name, bool32 to_upper)
{
    char table_name_buf[CTSQL_MAX_OBJECT_LEN] = "";
    char *table_name_tmp = table_name_buf;

    if (table_name->len >= CTSQL_MAX_OBJECT_LEN) {
        EXP_THROW_ERROR_EX(ERR_SQL_SYNTAX_ERROR, "the object name is too long");
        return CT_ERROR;
    }
    ...
    // copy table name
    if (to_upper) {
        cm_text2str_with_upper(table_name, table_name_buf, CTSQL_MAX_OBJECT_LEN);
    } else {
        CT_RETURN_IFERR(cm_text2str(table_name, table_name_buf, CTSQL_MAX_OBJECT_LEN));
    }

    return ctsql_generate_obj(&exp_opts->exp_tables.table_list, table_name_tmp);
}

static int ctsql_insert_export_tabspace(export_options_t *exp_opts, const text_t *obj_name, bool32 to_upper)
{
    char *ptr = NULL;
    char *ptr_name = NULL;
    char obj_name_buf[CTSQL_MAX_OBJECT_LEN] = "";
    ...
    CT_RETURN_IFERR(cm_list_new(&exp_opts->tbs_list, (void **)&ptr));
    MEMS_EXP_RETURN_IFERR(strncpy_s(ptr, CTSQL_MAX_OBJECT_LEN, obj_name_buf, CTSQL_MAX_OBJECT_LEN - 1));
...
static int exp_parse_self_schema(const char *user_name, bool8 *is_myself)
{
    ...
    CT_RETURN_IFERR(ctconn_prepare(STMT, usr_exist_sql));
    CT_RETURN_IFERR(ctconn_bind_by_pos(STMT, 0, CTCONN_TYPE_STRING, user_name, (int32)strlen(user_name), NULL));
    CT_RETURN_IFERR(ctconn_execute(STMT));
    CT_RETURN_IFERR(ctconn_fetch(STMT, &rows));
...
static int exp_parse_schema(lex_t *lex, export_options_t *exp_opts)
{
    ...
    while (word.type != WORD_TYPE_EOF) {
        ...
        if (ctsql_insert_export_obj(exp_opts, &word.text.value, !IS_DQ_STRING(word.type)) != CT_SUCCESS) {
    ...
    if (exp_opts->obj_list.count == 1) {
        ...
        CT_RETURN_IFERR(exp_parse_self_schema(ptr, &exp_opts->is_myself));
    }
...
static int exp_parse_tablespace_filter(lex_t *lex, export_options_t *exp_opts)
{
    ...
    while (word.type != WORD_TYPE_EOF) {
        ...
        if (ctsql_insert_export_tabspace(exp_opts, &word.text.value,
            !IS_DQ_STRING(word.type) && exp_opts->is_case_insensitive) != CT_SUCCESS) {
...
static int exp_parse_tables(lex_t *lex, export_options_t *exp_opts)
{
    ...
    while (word.type != WORD_TYPE_EOF) {
        ...
        if (ctsql_insert_export_table(exp_opts, &word.text.value,
            !IS_DQ_STRING(word.type) && exp_opts->is_case_insensitive) != CT_SUCCESS) {
            cm_set_error_loc(word.loc);
            return CT_ERROR;
        }
...
static status_t exp_fetch_exclude(text_t *exclude, exp_exclude_obj_t *exclude_obj)
{
    ...
    if (lex_fetch(&lex, &word) != CT_SUCCESS || word.type != WORD_TYPE_DQ_STRING) {
...
static status_t exp_add_exclude(export_options_t *exp_opts, exp_exclude_obj_t *exclude_obj)
{
    ...
    // add exclude object
    CT_RETURN_IFERR(cm_list_new(exclude_list, (void*)&obj));
    *obj = *exclude_obj;
    obj->exclude_cond.str = (char*)malloc(exclude_obj->exclude_cond.len);
    if (obj->exclude_cond.str == NULL) {
        CT_THROW_ERROR(ERR_ALLOC_MEMORY, (uint64)exclude_obj->exclude_cond.len, "exclude memory");
        return CT_ERROR;
    }
...
static status_t exp_parse_one_exclude(export_options_t *exp_opts, text_t *exclude)
{
    ...
    CT_RETURN_IFERR(exp_fetch_exclude(exclude, &exclude_obj));
    CT_RETURN_IFERR(exp_add_exclude(exp_opts, &exclude_obj));
...
static status_t exp_parse_exclude(lex_t *lex, export_options_t *exp_opts)
{
    ...
    while (CT_TRUE) {
        ...
        CT_RETURN_IFERR(exp_parse_one_exclude(exp_opts, &exclude));
...
int exp_parse_opts(lex_t *lex, export_options_t *exp_opts)
{
    ...
    while (!lex_eof(lex)) {
        CT_RETURN_IFERR(lex_try_match_records(lex, eopt_records, EXP_OPT_SIZE, (uint32 *)&matched_id));
        ...
        switch (matched_id) {
            case EOPT_USERS:
                CT_RETURN_IFERR(exp_parse_schema(lex, exp_opts));
                break;

            case EOPT_TABLES:
                CT_RETURN_IFERR(exp_parse_tables(lex, exp_opts));
                break;

            case EOPT_TABLESPACE_FILTER:
                CT_RETURN_IFERR(exp_parse_tablespace_filter(lex, exp_opts));
                break;
            ...
            case EOPT_FILE:
                ...
                CT_RETURN_IFERR(cm_text2str(&word.text.value, exp_opts->dump_file, CT_MAX_FILE_PATH_LENGH));
                break;
            ...
            case EOPT_LOG:
                ...
                CT_RETURN_IFERR(cm_text2str(&word.text.value, exp_opts->log_file, CT_MAX_FILE_PATH_LENGH));
                break;

            case EOPT_COMPRESS:
                CT_RETURN_IFERR(lex_expected_fetch_uint32(lex, &(exp_opts->compress_level)));
                ...
                exp_opts->compress = (exp_opts->compress_level == EXP_COMPRESS_NONE) ? CT_FALSE : CT_TRUE;

                break;

            case EOPT_CONSISTENT:
                CT_RETURN_IFERR(lex_expected_fetch_1of2(lex, "Y", "N", &matched_id));
                exp_opts->consistent = (matched_id == 0) ? CT_TRUE : CT_FALSE;
            ...
            case EOPT_QUERY:
                CT_RETURN_IFERR(lex_expected_fetch_dqstring(lex, &word));
                CT_RETURN_IFERR(cm_text2str(&word.text.value, exp_opts->query, MAX_EXP_QUERY_SIZE));
                break;

            case EOPT_SKIP_COMMENTS:
                CT_RETURN_IFERR(lex_expected_fetch_1of2(lex, "Y", "N", &matched_id));
                exp_opts->skip_comments = (matched_id == 0) ? CT_TRUE : CT_FALSE;
                break;

            case EOPT_FORCE:
                CT_RETURN_IFERR(lex_expected_fetch_1of2(lex, "Y", "N", &matched_id));
                exp_opts->force = (matched_id == 0) ? CT_TRUE : CT_FALSE;
                break;

            case EOPT_SKIP_ADD_DROP_TABLE:
                CT_RETURN_IFERR(lex_expected_fetch_1of2(lex, "Y", "N", &matched_id));
                exp_opts->skip_add_drop_table = (matched_id == 0) ? CT_TRUE : CT_FALSE;
                break;

            case EOPT_SKIP_TRIGGERS:
                CT_RETURN_IFERR(lex_expected_fetch_1of2(lex, "Y", "N", &matched_id));
                exp_opts->skip_triggers = (matched_id == 0) ? CT_TRUE : CT_FALSE;
                break;

            case EOPT_QUOTE_NAMES:
                CT_RETURN_IFERR(lex_expected_fetch_1of2(lex, "Y", "N", &matched_id));
                exp_opts->quote_names = (matched_id == 0) ? CT_TRUE : CT_FALSE;
                break;
            case EOPT_TABLESPACE:
                CT_RETURN_IFERR(lex_expected_fetch_1of2(lex, "Y", "N", &matched_id));
                exp_opts->tablespace = (matched_id == 0) ? CT_TRUE : CT_FALSE;
                break;
            case EOPT_COMMIT_BATCH:
                CT_RETURN_IFERR(lex_expected_fetch_uint32(lex, &(exp_opts->commit_batch)));
                break;
            case EOPT_INSERT_BATCH:
                CT_RETURN_IFERR(lex_expected_fetch_uint32(lex, &(exp_opts->insert_batch)));
                if (exp_opts->insert_batch == 0 || exp_opts->insert_batch > CTSQL_INSERT_BATCH) {
                    EXP_SRC_THROW_ERROR_EX(LEX_LOC, ERR_SQL_SYNTAX_ERROR, "insert_batch should between 1 and 10000.");
                    return CT_ERROR;
                }
                break;
            case EOPT_FEEDBACK:
                CT_RETURN_IFERR(lex_expected_fetch_uint32(lex, &(exp_opts->feedback)));
                break;
            case EOPT_PARALLEL:
                CT_RETURN_IFERR(lex_expected_fetch_uint32(lex, &(exp_opts->parallel)));
                exp_opts->parallel = MIN(exp_opts->parallel, CT_MAX_PAR_EXP_VALUE);
                break;
            case EOPT_CREATE_USER:
                CT_RETURN_IFERR(lex_expected_fetch_1of2(lex, "Y", "N", &matched_id));
                exp_opts->create_user = (matched_id == 0) ? CT_TRUE : CT_FALSE;
                break;
            case EOPT_EXP_ROLE:
                CT_RETURN_IFERR(lex_expected_fetch_1of2(lex, "Y", "N", &matched_id));
                exp_opts->exp_role = (matched_id == 0) ? CT_TRUE : CT_FALSE;
                break;
            case EOPT_IS_GRANT:
                CT_RETURN_IFERR(lex_expected_fetch_1of2(lex, "Y", "N", &matched_id));
                exp_opts->is_grant = (matched_id == 0) ? CT_TRUE : CT_FALSE;
                break;

            case EOPT_WITH_CR_MODE:
                CT_RETURN_IFERR(lex_expected_fetch_1of2(lex, "Y", "N", &matched_id));
                exp_opts->with_cr_mode = (matched_id == 0) ? CT_TRUE : CT_FALSE;
                break;

            case EOPT_WITH_FORMAT_CSF:
                CT_RETURN_IFERR(lex_expected_fetch_1of2(lex, "Y", "N", &matched_id));
                exp_opts->with_format_csf = (matched_id == 0) ? CT_TRUE : CT_FALSE;
                break;

            case EOPT_ENCRYPT: {
                key_word_info = "Encrypt pwd string";
                CT_RETURN_IFERR(ctsql_get_crypt_pwd(lex, exp_opts->crypt_info.crypt_pwd,
                                                   CT_PASSWD_MAX_LEN + 1, key_word_info));
                CT_RETURN_IFERR(ctsql_gen_encrypt_hash(&exp_opts->crypt_info));
                exp_opts->crypt_info.crypt_flag = CT_TRUE;
                break;
            }

            case EOPT_SHOW_CREATE_TABLE:
                CT_RETURN_IFERR(lex_expected_fetch_1of2(lex, "Y", "N", &matched_id));
                exp_opts->show_create_table = (matched_id == 0) ? CT_TRUE : CT_FALSE;
                /* prevent EXP from dumping file under 'show create table' option */
                MEMS_EXP_RETURN_IFERR(strncpy_s(exp_opts->dump_file,
                    CT_MAX_FILE_PATH_LENGH, NULL_DUMP_FILE, strlen(NULL_DUMP_FILE)));
                break;

            case EOPT_INDEX_PARTITIONS:
                CT_RETURN_IFERR(lex_expected_fetch_1of2(lex, "Y", "N", &matched_id));
                exp_opts->index_partitions = (matched_id == 0) ? CT_TRUE : CT_FALSE;
                break;

            case EOPT_EXCLUDE:
                CT_RETURN_IFERR(exp_parse_exclude(lex, exp_opts));
                break;

            case EOPT_TENANT:
                CT_RETURN_IFERR(lex_expected_fetch_1of2(lex, "Y", "N", &matched_id));
                exp_opts->tenant = (matched_id == 0) ? CT_TRUE : CT_FALSE;
                break;
...
static status_t exp_reset_opts_from_server(export_options_t *exp_opts)
{
    if (!IS_CONN) {
        ...
        return CT_ERROR;  // the connection is broken
    }
    return ctsql_reset_case_insensitive(&exp_opts->is_case_insensitive);
}

static inline status_t exp_reset_part_opts(export_options_t *exp_opts)
{
    exp_opts->show_create_table = CT_FALSE;
    exp_opts->compress = CT_FALSE;
    exp_opts->consistent = CT_FALSE;
    exp_opts->content = CT_EXP_ALL;
    exp_opts->skip_comments = CT_FALSE;
    exp_opts->force = CT_FALSE;
    exp_opts->skip_add_drop_table = CT_FALSE;
    exp_opts->skip_triggers = CT_FALSE;
    exp_opts->quote_names = CT_TRUE;
    exp_opts->tablespace = CT_FALSE;
    exp_opts->commit_batch = CTSQL_COMMIT_BATCH;
    exp_opts->insert_batch = 1;
    exp_opts->feedback = CTSQL_FEEDBACK;
    exp_opts->parallel = 0;
    exp_opts->tenant = CT_FALSE;
    exp_opts->create_user = CT_FALSE;
    exp_opts->exp_role = CT_FALSE;
    exp_opts->is_grant = CT_FALSE;
    exp_opts->with_cr_mode = CT_FALSE;
    exp_opts->with_format_csf = CT_TRUE;
    MEMS_RETURN_IFERR(memset_s(&exp_opts->dn_info, sizeof(exp_shd_info_t), 0, sizeof(exp_shd_info_t)));
    exp_opts->dn_info.shd_node_type = ctconn_get_shd_node_type(CONN);
    exp_opts->dn_info.consistent = CT_FALSE;
    exp_opts->is_myself = CT_FALSE;
    exp_opts->is_dba = CT_FALSE;
    return CT_SUCCESS;
}
...
status_t exp_reset_opts(export_options_t *exp_opts)
{
    ...
    exp_create_objlist(&exp_opts->obj_list, CTSQL_MAX_OBJECT_LEN);
    cm_reset_list(&exp_opts->tbs_list);
    exp_create_objlist(&exp_opts->tbs_list, CTSQL_MAX_OBJECT_LEN);
    exp_opts->filetype = FT_TXT;
    MEMS_EXP_RETURN_IFERR(strncpy_s(exp_opts->dump_file, CT_MAX_FILE_PATH_LENGH, DEFAULT_DUMP_FILE,
        strlen(DEFAULT_DUMP_FILE)));
    MEMS_RETURN_IFERR(memset_s(exp_opts->log_file, CT_MAX_FILE_PATH_LENGH, 0, CT_MAX_FILE_PATH_LENGH));
    ...
    CT_RETURN_IFERR(exp_reset_part_opts(exp_opts));
    ...
    exp_create_objlist(&exp_opts->exp_tables.table_list, CTSQL_MAX_OBJECT_LEN);
    cm_reset_list(&exp_opts->exp_tables.partition_list);
    exp_create_objlist(&exp_opts->exp_tables.partition_list, CTSQL_MAX_OBJECT_LEN);

    g_file_no = 0;
    g_exp_scn = CT_INVALID_ID64;
    g_exp_gts_scn = CT_INVALID_ID64;
    ...
    ctsql_reset_crypt_info(&exp_opts->crypt_info);
...
static void exp_print_options_write_dplog(export_options_t *exp_opts, char *str)
{
    ...
    exp_write_dplog(CT_MAX_FILE_PATH_LENGH + 100, "-- DUMP FILE = %s\n", exp_opts->dump_file);
    exp_write_dplog(CT_MAX_FILE_PATH_LENGH + 100, "-- LOG FILE = %s\n", exp_opts->log_file);
...
{
    if (cm_str_equal("SYS", USER_NAME)) {
        exp_opts->is_dba = CT_TRUE;
        return CT_SUCCESS;
    }
     
    CT_RETURN_IFERR(ctsql_check_dba_user(&exp_opts->is_dba));
    return CT_SUCCESS;
}
...
static status_t exp_verify_dist_rule(export_options_t *exp_opts)
{
    ...
    CT_RETURN_IFERR(exp_set_dba_user(exp_opts));
    if (exp_opts->is_dba) {
    ...
    CT_RETURN_IFERR(ctconn_prepare(STMT, dist_rule_sql));
    for (uint32 i = 0; i < rule_list->count; i++) {
        uint32 rows;
        char *rule_name = (char *)cm_list_get(rule_list, i);
        CT_RETURN_IFERR(ctconn_bind_by_pos(STMT, 0, CTCONN_TYPE_STRING, rule_name, (int32)strlen(rule_name), NULL));
...
static int exp_verify_tables(export_options_t *exp_opts)
{
    ...
    CT_RETURN_IFERR(ctconn_prepare(STMT, table_exist_sql));

    for (uint32 i = 0; i < exp_opts->exp_tables.table_list.count; i++) {
        uint32 rows;
        char *tbl_name = (char *)cm_list_get(&exp_opts->exp_tables.table_list, i);
        CT_RETURN_IFERR(ctconn_bind_by_pos(STMT, 0, CTCONN_TYPE_STRING,
...
static int exp_verify_tbs_filter(list_t *tbs_list)
{
    ...
    CT_RETURN_IFERR(ctconn_prepare(STMT, tbs_exist_sql));

    for (uint32 i = 0; i < tbs_list->count; i++) {
        uint32 rows;
        char *tbs_name = (char *)cm_list_get(tbs_list, i);
        CT_RETURN_IFERR(ctconn_bind_by_pos(STMT, 0, CTCONN_TYPE_STRING, tbs_name, (int32)strlen(tbs_name), NULL));
...
static inline int exp_tablespace_filter(export_options_t *exp_opts, const char *user, const char *table)
{
    ...
    CT_RETURN_IFERR(ctconn_prepare(STMT, tablespace_filter_sql));

    for (uint32 i = 0; i < exp_opts->tbs_list.count; i++) {
        uint32 rows;
        char *tabspace = (char *)cm_list_get(&(exp_opts->tbs_list), i);

        CT_RETURN_IFERR(ctconn_bind_by_pos(STMT, 0, CTCONN_TYPE_STRING, table, (int32)strlen(table), NULL));
        CT_RETURN_IFERR(ctconn_bind_by_pos(STMT, 1, CTCONN_TYPE_STRING, tabspace, (int32)strlen(tabspace), NULL));

        if (user != NULL) {
            CT_RETURN_IFERR(ctconn_bind_by_pos(STMT, 2, CTCONN_TYPE_STRING, user, (int32)strlen(user), NULL));
        }
...
static int exp_verify_opts(export_options_t *exp_opts)
{
    ...
    CT_RETURN_IFERR(exp_verify_query_opts(exp_opts));
    CT_RETURN_IFERR(exp_verify_users_opts(exp_opts));

    if (exp_opts->tbs_list.count > 0) {
        // verify tablespace name exist or not
        CT_RETURN_IFERR(exp_verify_tbs_filter(&exp_opts->tbs_list));
    }
    CT_RETURN_IFERR(exp_verify_partitions_opts(exp_opts));
    if (exp_opts->exp_type == EXP_NONE) {
        text_t curr_user;
        ...
        CT_RETURN_IFERR(ctsql_insert_export_obj(exp_opts, &curr_user, CT_TRUE));
        ...
        CT_RETURN_IFERR(exp_verify_tables(exp_opts));
        ...
        if (!exp_opts->is_myself) {
            CT_RETURN_IFERR(exp_verify_schema(&exp_opts->obj_list));
        }
    } else if (exp_opts->exp_type == EXP_DIST_RULES) {
        exp_log(EXP_INDENT "verify distribute rule ...\n");
        CT_RETURN_IFERR(exp_verify_dist_rule(exp_opts));
    }
...
static void exp_free_dn_info(exp_shd_info_t *dn_info)
{
    ...
    if (dn_info->gts_conn_info.stmt != NULL) {
        ctconn_free_stmt(dn_info->gts_conn_info.stmt);
        dn_info->gts_conn_info.stmt = NULL;
    }
    if (dn_info->gts_conn_info.conn != NULL) {
        ctconn_free_conn(dn_info->gts_conn_info.conn);
    ...
    for (uint32 i = 0; i < dn_info->dn_info_list.count; i++) {
        ...
        for (uint32 j = 0; j < node_info->dn_par_conn.par_num; j++) {
            if (node_info->dn_par_conn.stmt[j] != NULL) {
                ctconn_free_stmt(node_info->dn_par_conn.stmt[j]);
                node_info->dn_par_conn.stmt[j] = NULL;
            }
            if (node_info->dn_par_conn.conn[j] != NULL) {
                ctconn_free_conn(node_info->dn_par_conn.conn[j]);
...
static inline void exp_free(export_options_t *exp_opts)
{
    ...
    ctsql_encrypt_end(&exp_opts->crypt_info, filename);
    exp_close_writer();
    exp_close_logger();
    cm_reset_list(&exp_opts->obj_list);
    cm_reset_list(&exp_opts->tbs_list);
...
status_t ctsql_exp_data_write_varchar_s(char *src, text_buf_t *dst, FILE *filehand)
{
    bool32 exist_flag = CT_FALSE;
    char local_buf[CT_MAX_PACKET_SIZE + 1] = { 0 };

    CT_RETURN_IFERR(exp_write_str_s("'", dst, filehand));
    CT_RETURN_IFERR(cm_replace_quotation(src, local_buf, CT_MAX_PACKET_SIZE + 1, &exist_flag));

    if (!exist_flag) {
        CT_RETURN_IFERR(exp_write_str_s(src, dst, filehand));
    } else {
        CT_RETURN_IFERR(exp_write_str_s(local_buf, dst, filehand));
    }
    CT_RETURN_IFERR(exp_write_str_s("'", dst, filehand));
...
status_t ctsql_exp_data_write_clob_s(uint32 column_id, ctconn_stmt_t stmt, char *str_buf, text_buf_t *dst,
                                    FILE *filehand)
{
    bool32 exist_flag = CT_FALSE;
    ...
    char par_dest_buf[CT_MAX_PACKET_SIZE + 1] = { 0 };
    uint32 size = 0;

    CT_RETURN_IFERR(ctconn_get_lob_size_by_id(stmt, column_id, &size));

    if (size > CT_MAX_COLUMN_SIZE) {
        EXP_THROW_ERROR_EX(ERR_LOB_SIZE_TOO_LARGE, "%u, clob column(%u) should use \"filetype=bin\" to export",
                          CT_MAX_COLUMN_SIZE, column_id);
        return CT_ERROR;
    }

    while (eof != CT_TRUE) {
        CT_RETURN_IFERR(ctconn_read_clob_by_id(stmt, column_id, byte_offset, str_buf, CLT_LOB_BUFFER_SIZE, &nchars,
                                            &nbytes, &eof));
        str_buf[nbytes] = '\0';
        ...
        CT_RETURN_IFERR(cm_replace_quotation(str_buf, par_dest_buf, CT_MAX_PACKET_SIZE + 1, &exist_flag));

        CT_RETURN_IFERR(exp_write_str_s((exist_flag ? par_dest_buf : str_buf), dst, filehand));
    }
...
int ctsql_exp_data_write_blob_core(uint32 column_id, ctconn_stmt_t stmt, char *str_buf, text_buf_t *dst,
                                  FILE *filehand, uint8 *buf)
{
    ...
    while (eof != CT_TRUE) {
        if (ctconn_read_blob_by_id(stmt, column_id, byte_offset, buf, CLT_LOB_BUFFER_SIZE,
            &nbytes, &eof) != CT_SUCCESS) {
            return CT_ERROR;
        }
        ...
        if (cm_bin2str(&bin, CT_FALSE, str_buf, nbytes * 2 + 1) != CT_SUCCESS) {
            return CT_ERROR;
        }
...
int ctsql_exp_data_write_blob_s(uint32 column_id, unsigned short col_type, ctconn_stmt_t stmt, char *str_buf,
                               text_buf_t *dst, FILE *filehand)
{
    ...
    ret = exp_write_str_s((col_type == CTCONN_TYPE_BLOB) ? "'" : "X'", dst, filehand);
    if (ret != CT_SUCCESS) {
        CM_FREE_PTR(buf);
        return ret;
    }

    ret = ctconn_get_lob_size_by_id(stmt, column_id, &size);
    if (ret != CT_SUCCESS) {
        CM_FREE_PTR(buf);
        return ret;
    }
    ...
    ret = ctsql_exp_data_write_blob_core(column_id, stmt, str_buf, dst, filehand, buf);
    if (ret != CT_SUCCESS) {
        CM_FREE_PTR(buf);
        return ret;
    }
...
static status_t ctsql_make_insert_sql_s(exporter_t *exporter, char *sql_buf, const char *table_name)
{
    uint32 i;
    int iret_snprintf;
    text_t sql_text = { .str = sql_buf };
    char table_name_s[CTSQL_MAX_QUOTE_NAME_SIZE];
    char col_name_s[CTSQL_MAX_QUOTE_NAME_SIZE];
    text_t table_name_t;
    ...
    if (exporter->col_num == 0) {
        CTSQL_PRINTF(ZSERR_EXPORT, "assert raised, expect: exporter->col_num(%u) > 0", exporter->col_num);
        return CT_ERROR;
    }

    (void)exp_concat_str_quote(&table_name_t, table_name);
    table_name_t.str[table_name_t.len] = '\0';
    iret_snprintf = snprintf_s(sql_text.str, MAX_SQL_SIZE, CTSQL_MAX_TEMP_SQL, "INSERT INTO %s (",
    ...
    for (i = 0; i < exporter->col_num; i++) {
        ...
        iret_snprintf = snprintf_s(sql_text.str + sql_text.len, MAX_SQL_SIZE - sql_text.len,
                                   CTSQL_MAX_TEMP_SQL, "%s", column_name_t.str);
        PRTS_RETURN_IFERR(iret_snprintf);
        sql_text.len += iret_snprintf;
    }

    iret_snprintf = snprintf_s(sql_text.str + sql_text.len, MAX_SQL_SIZE - sql_text.len,
                               CTSQL_MAX_TEMP_SQL, ") values\n");
    PRTS_RETURN_IFERR(iret_snprintf);
    sql_text.len += iret_snprintf;
...
static inline status_t exp_bin_write_tab_fields_df(exporter_t *exporter, text_buf_t *dst, FILE *filehand,
                                                   ctconn_z_stream *stream)
{
    uint16 i, shrt_len;
    uint16 count = (uint16)exporter->col_num;

    // total of table fields
    CT_RETURN_IFERR(exp_writer_bin_data_s((char *)&count, sizeof(int16), dst, filehand, stream));
    for (i = 0; i < count; i++) {
        shrt_len = (uint16)strlen(exporter->col_desc[i].name);
        // field name len
        CT_RETURN_IFERR(exp_writer_bin_data_s((char *)&shrt_len, sizeof(uint16), dst, filehand, stream));
        // field name
        CT_RETURN_IFERR(exp_writer_bin_data_s(exporter->col_desc[i].name, shrt_len, dst, filehand, stream));
        // field type
        CT_RETURN_IFERR(exp_writer_bin_data_s((char *)&(exporter->col_desc[i].type), sizeof(int16),
                                              dst, filehand, stream));
        // field size
        CT_RETURN_IFERR(exp_writer_bin_data_s((char *)&(exporter->col_desc[i].size), sizeof(int16),
                                              dst, filehand, stream));
        // field is_array
        CT_RETURN_IFERR(exp_writer_bin_data_s((char *)&(exporter->col_desc[i].is_array), sizeof(uchar),
                                              dst, filehand, stream));
    }
    return CT_SUCCESS;
}

static int exp_init_columns(exporter_t *exporter, ctconn_stmt_t stmt, exp_cache_t *table_cache)
{
    uint32 i;
    lob_col_desc_t *col_desc = NULL;
    exp_cache_column_info_t* column_info = NULL;

    CT_RETURN_IFERR(ctconn_get_column_count(stmt, &exporter->col_num));

    CT_RETURN_IFERR(exp_start_cache_unit(table_cache, EXP_CACHE_COLUMN_INFO));

    for (i = 0; i < exporter->col_num; i++) {
        CT_RETURN_IFERR(ctconn_desc_inner_column_by_id(stmt, i, &(exporter->col_desc[i])));

        if (g_export_opts.filetype == FT_BIN) {
            /* cache column info */
            CT_RETURN_IFERR(alloc_column_cache_info(table_cache, &column_info));
            ...
            if (CTSQL_IS_LOB_TYPE(exporter->col_desc[i].type) || exporter->col_desc[i].is_array) {
                CT_RETURN_IFERR(cm_list_new(&exporter->lob_cols, (void **)&col_desc));
...
{
    CT_RETURN_IFERR(cm_bin2str(bin, CT_TRUE, str_buf, CT_MAX_COLUMN_SIZE * 2 + 1));
    CT_RETURN_IFERR(exp_write_str_s(str_buf, exp_txtbuf, exp_dpfile));
    return CT_SUCCESS;
}
                                    ...
                                    exporter_t *exporter)
{
    ...
    for (i = 0; i < exporter->col_num; i++) {
        if (i != 0) {
            CT_RETURN_IFERR(exp_write_str_s(",", exp_txtbuf, exp_dpfile));
        }

        (void)ctconn_get_column_by_id(stmt, i, &data, &size, &is_null);
        if (is_null) {
            CT_RETURN_IFERR(exp_write_str_s("null", exp_txtbuf, exp_dpfile));
            continue;
        }
        if (size == 0) {
            CT_RETURN_IFERR(exp_write_str_s("''", exp_txtbuf, exp_dpfile));
        ...
        if (column->def.is_array) {
            CT_RETURN_IFERR(ctconn_column_as_array(stmt, i, str_buf, CT_MAX_PACKET_SIZE));
            CT_RETURN_IFERR(exp_write_str_s(str_buf, exp_txtbuf, exp_dpfile));
            continue;
        }
        ...
        switch (exporter->col_desc[i].type) {
            case CTCONN_TYPE_BINARY:
                ...
                CT_RETURN_IFERR(exp_data_write_binary(&bin, str_buf, exp_txtbuf, exp_dpfile));
                break;
            ...
            case CTCONN_TYPE_CLOB:
                CT_RETURN_IFERR(ctsql_exp_data_write_clob_s(i, stmt, str_buf, exp_txtbuf, exp_dpfile));
                break;

            case CTCONN_TYPE_BLOB:
            case CTCONN_TYPE_IMAGE:
                if (ctconn_get_call_version(CONN) >= CS_VERSION_24 && column->def.is_jsonb) {
                    EXP_THROW_ERROR_EX(ERR_JSONB_EXP_ERROR, "jsonb column should use \"filetype=bin\" to export");
                    return CT_ERROR;
                }
                CT_RETURN_IFERR(ctsql_exp_data_write_blob_s(i, exporter->col_desc[i].type, stmt, str_buf, exp_txtbuf,
                                                              exp_dpfile));
                break;
            ...
            case CTCONN_TYPE_INTERVAL:
            case CTCONN_TYPE_INTERVAL_DS:
            case CTCONN_TYPE_INTERVAL_YM:
                (void)ctconn_column_as_string(stmt, i, str_buf, CT_MAX_PACKET_SIZE);
                CT_RETURN_IFERR(ctsql_exp_data_write_varchar_s(str_buf, exp_txtbuf, exp_dpfile));
                break;
                                 ...
                                 uint64 *file_insert_num)
{
    ...
    while (rows > 0) {
        ...
        if (*file_insert_num % g_export_opts.insert_batch == 0) {
            CT_RETURN_IFERR(exp_write_str_s(sql_buf, exp_txtbuf, exp_dpfile));
            CT_RETURN_IFERR(exp_write_str_s(EXP_INDENT "(", exp_txtbuf, exp_dpfile));
        } else {
            CT_RETURN_IFERR(exp_write_str_s(",\n" EXP_INDENT "(", exp_txtbuf, exp_dpfile));
        }

        CT_RETURN_IFERR(exp_table_txt_row(exp_txtbuf, stmt, sql_buf, str_buf, exp_dpfile, exporter));
        ...
        *file_insert_num += 1;
        ...
        if (g_export_opts.commit_batch != 0 &&
            *file_insert_num % (g_export_opts.commit_batch * g_export_opts.insert_batch) == 0) {
            CT_RETURN_IFERR(exp_write_str_s("COMMIT;\n", exp_txtbuf, exp_dpfile));
        }

        if (par_flag == CT_FALSE) {
...
static int exp_bin_write_clob(uint32 column_id, ctconn_stmt_t stmt, char *locator, text_buf_t *dst,
                              exp_bin_file_ctx_t *file_ctx, text_buf_t *data_buffer)
{
    ...
    if (dest_buf == NULL) {
        CTSQL_PRINTF(ZSERR_EXPORT, "exp clob buffer not init !");
        return CT_ERROR;
    }

    while (eof != CT_TRUE) {
        status = ctconn_read_clob(stmt, (void *)locator, byte_offset, dest_buf, data_buffer->max_size, &nchars, &nbytes,
                               &eof);
...
static int exp_bin_write_blob(uint32 column_id, ctconn_stmt_t stmt, char *locator, text_buf_t *dst,
                              exp_bin_file_ctx_t *file_ctx, text_buf_t *data_buffer)
{
    uint32 nbytes = 0;
    uint32 eof = CT_FALSE;
    ...
    if (buf == NULL) {
        CTSQL_PRINTF(ZSERR_EXPORT, "exp blob buffer not init !");
        return CT_ERROR;
    }

    while (eof != CT_TRUE) {
        status = ctconn_read_blob(stmt, (void *)locator, byte_offset, buf, data_buffer->max_size, &nbytes, &eof);
        CT_BREAK_IF_ERROR(status);
...
#define CTSQL_MAX_LOCATOR_SIZE 128
                                                ...
                                                uint32 really_sz)
{
    uint32 name_size;
    uint32 loc_offset = 0;
    char tmp_loc[CTSQL_MAX_LOCATOR_SIZE];
    char *ptr_tmp = tmp_loc;
    int ret;
    if (loc_sz > CTSQL_MAX_LOCATOR_SIZE) {
        EXP_THROW_ERROR(ERR_BUFFER_OVERFLOW, loc_sz, CTSQL_MAX_LOCATOR_SIZE);
        return CT_ERROR;
    }

    MEMS_RETURN_IFERR(memcpy_s(tmp_loc, loc_sz, locator, loc_sz));

    *(uint32 *)locator = CT_INVALID_ID32;  // redirect flag:0xFFFFFFFF
    ...
    *(uint16 *)(locator + loc_offset) = name_size;
    ...
    CT_RETURN_IFERR(exp_writer_bin_data_s((char *)&really_sz, sizeof(uint32), &file_ctx->bin_lob_buf,
                                          file_ctx->lf_h, file_ctx->lf_zstream));

    if (clob_col->col_desc.type == CTCONN_TYPE_CLOB || clob_col->col_desc.is_array) {
...
static int exp_bin_write_lob(exp_bin_file_ctx_t *bin_file_ctx, ctconn_stmt_t stmt, exporter_t *exporter, char *row)
{
    uint16 size = 0;
    uint32 i;
    ...
    for (i = 0; i < exporter->lob_cols.count; ++i) {
        ...
        if ((clob_col->col_id >= row_cols) || bin_file_ctx->lens[clob_col->col_id] == CT_NULL_VALUE_LEN) {
            continue;
        }
        locator = (void *)(row + bin_file_ctx->offsets[clob_col->col_id]);
        (void)ctconn_get_locator_info(stmt, locator, &outline, &really_size, &locator_size);
        if (!outline) {
            continue;
        }

        CT_RETURN_IFERR(exp_redirect_lob_locator(bin_file_ctx, stmt, clob_col, locator, locator_size, really_size));
        bin_file_ctx->wr_lob_flag = CT_TRUE;
    }
                                 ...
                                 uint64 *file_insert_num)
{
    ...
    CT_RETURN_IFERR(exp_writer_bin_data_s((char *)&shrt_len, sizeof(uint16), &bfile_ctx->bin_data_buf,
        bfile_ctx->df_h, bfile_ctx->df_zstream));  // table name len
    CT_RETURN_IFERR(exp_writer_bin_data_s(bfile_ctx->tab_name, shrt_len, &bfile_ctx->bin_data_buf,
        bfile_ctx->df_h, bfile_ctx->df_zstream));  // table name

    CT_RETURN_IFERR(exp_bin_write_tab_fields_df(exporter, &bfile_ctx->bin_data_buf,
        bfile_ctx->df_h, bfile_ctx->df_zstream));  // table fields

    CT_RETURN_IFERR(ctconn_fetch_ori_row(stmt, &rows));
    while (rows > 0) {
        // if cancel, stop export
        EXP_RETRUN_IF_CANCEL;
        CT_RETURN_IFERR(ctconn_read_ori_row(stmt, &data, &size));
        *file_insert_num += 1;
        if (exporter->lob_cols.count == 0) {
            CT_RETURN_IFERR(exp_writer_bin_data_s((char *)data, size, &bfile_ctx->bin_data_buf,
                                                  bfile_ctx->df_h, bfile_ctx->df_zstream));
        } else {
            MEMS_RETURN_IFERR(memcpy_s(str_buf, CT_MAX_PACKET_SIZE, data, size));
            CT_RETURN_IFERR(exp_bin_write_lob(bfile_ctx, stmt, exporter, str_buf));
            CT_RETURN_IFERR(exp_writer_bin_data_s((char *)str_buf, size, &bfile_ctx->bin_data_buf,
                                                  bfile_ctx->df_h, bfile_ctx->df_zstream));
        }
    ...
    CT_RETURN_IFERR(exp_flush_bin_data_s(&bfile_ctx->bin_data_buf, bfile_ctx->df_h, bfile_ctx->df_zstream));

    if (ctsql_reset_crypfile(bfile_ctx->df_h, &g_export_opts.crypt_info) != CT_SUCCESS) {
        CTSQL_PRINTF(ZSERR_EXPORT, "failed to reset decrypt file fp !");
    }

    CT_RETURN_IFERR(exp_flush_bin_data_s(&bfile_ctx->bin_lob_buf, bfile_ctx->lf_h, bfile_ctx->lf_zstream));

    if (ctsql_reset_crypfile(bfile_ctx->lf_h, &g_export_opts.crypt_info) != CT_SUCCESS) {
        CTSQL_PRINTF(ZSERR_EXPORT, "failed to reset decrypt file fp !");
    }
...
static inline int exp_pre_query_sql(ctconn_stmt_t stmt, char *sql_buf, const char *schema_name, const char *table_name)
{
    int ret = 0;
    uint32 sql_len = 0;
    ...
    if (g_export_opts.filetype == FT_BIN) {
        ...
        CT_RETURN_IFERR(ctconn_prepare(stmt, sql_buf));

        CT_RETURN_IFERR(ctconn_bind_by_pos(stmt, 0, CTCONN_TYPE_INTEGER, &part_no, sizeof(uint32), NULL));
        CT_RETURN_IFERR(ctconn_bind_by_pos(stmt, 1, CTCONN_TYPE_BIGINT, &g_exp_scn, sizeof(uint64), NULL));
        CT_RETURN_IFERR(ctconn_bind_by_pos(stmt, 2, CTCONN_TYPE_STRING, "NULL", 4, NULL));
...
static inline int par_exp_pre_query_sql_normaltab(par_exp_thread_ctrl_t *thread_ctrl, ctconn_stmt_t stmt,
    char *sql_buf, char *schema_name, char *table_name)
{
    if (g_export_opts.filetype == FT_TXT) {
        ...
        CT_RETURN_IFERR(ctconn_bind_by_pos(stmt, 0, CTCONN_TYPE_BIGINT,
            &thread_ctrl->tab_param.scn, sizeof(uint64), NULL));
        CT_RETURN_IFERR(ctconn_bind_by_pos(stmt, 1, CTCONN_TYPE_BIGINT,
            &thread_ctrl->tab_param.scan_param.l_page, sizeof(uint64), NULL));
        CT_RETURN_IFERR(ctconn_bind_by_pos(stmt, 2, CTCONN_TYPE_BIGINT,
            &thread_ctrl->tab_param.scan_param.r_page, sizeof(uint64), NULL));
        CT_RETURN_IFERR(ctconn_bind_by_pos(stmt, 3, CTCONN_TYPE_INTEGER,
        ...
        CT_RETURN_IFERR(ctconn_bind_by_pos(stmt, 0, CTCONN_TYPE_INTEGER,
            &thread_ctrl->tab_param.scan_param.part_no, sizeof(uint32), NULL));
        CT_RETURN_IFERR(ctconn_bind_by_pos(stmt, 1, CTCONN_TYPE_BIGINT,
            &thread_ctrl->tab_param.scn, sizeof(uint64), NULL));
        CT_RETURN_IFERR(ctconn_bind_by_pos(stmt, 2, CTCONN_TYPE_STRING, "NULL", 4, NULL));
        CT_RETURN_IFERR(ctconn_bind_by_pos(stmt, 3, CTCONN_TYPE_BIGINT,
            &thread_ctrl->tab_param.scan_param.l_page, sizeof(uint64), NULL));
        CT_RETURN_IFERR(ctconn_bind_by_pos(stmt, 4, CTCONN_TYPE_BIGINT,
            &thread_ctrl->tab_param.scan_param.r_page, sizeof(uint64), NULL));
    }
    return CT_SUCCESS;
}
...
static inline int par_exp_pre_query_sql(par_exp_thread_ctrl_t *thread_ctrl)
{
    ...
    if (thread_ctrl->tab_param.scan_param.normal_tab) {
        CT_RETURN_IFERR(par_exp_pre_query_sql_normaltab(thread_ctrl, stmt, sql_buf, schema_name, table_name));
        ...
        if (g_export_opts.filetype == FT_TXT) {
            ...
            CT_RETURN_IFERR(ctconn_bind_by_pos(stmt, 0, CTCONN_TYPE_INTEGER, &part_no, sizeof(uint32), NULL));
            CT_RETURN_IFERR(ctconn_bind_by_pos(stmt, 1, CTCONN_TYPE_BIGINT, &thread_ctrl->tab_param.scn,
                                            sizeof(uint64), NULL));
            CT_RETURN_IFERR(ctconn_bind_by_pos(stmt, 2, CTCONN_TYPE_STRING, "NULL", 4, NULL));
        }
    }
...
static int par_exp_table_records_core(par_exp_thread_ctrl_t *thread_ctrl, uint64 *file_insert_num,
    bool32 *wr_lob_flg, exporter_t *exporter, ctconn_stmt_t stmt)
{
    char *sql_buf = thread_ctrl->files_context.sql_buf;
    ...
    if (g_export_opts.filetype == FT_TXT) {
        CT_RETURN_IFERR(ctsql_make_insert_sql_s(exporter, thread_ctrl->files_context.sql_buf,
        ...
        CT_RETURN_IFERR(exp_write_txt_records(&thread_ctrl->files_context.exp_txtbuf,
                                              stmt,
                                              sql_buf,
                                              str_buf,
        ...
        CT_RETURN_IFERR(par_exp_init_bfile_ctx(&bin_file_ctx, thread_ctrl));
        if (g_export_opts.compress) {
        ...
        CT_RETURN_IFERR(exp_write_bin_records(&bin_file_ctx, stmt, str_buf, exporter, thread_ctrl->lock_t,
            file_insert_num));
...
static int par_exp_table_records(par_exp_thread_ctrl_t *thread_ctrl, uint64 *file_insert_num, bool32 *wr_lob_flg)
{
    ...
    *wr_lob_flg = CT_FALSE;
    ...
    CT_RETURN_IFERR(ctconn_set_stmt_attr(stmt, CTCONN_ATTR_PREFETCH_ROWS, &prefetch_rows, 0));
    CT_RETURN_IFERR(par_exp_pre_query_sql(thread_ctrl));

    exporter->tab_record_total = thread_ctrl->bin_rec_total_add;

    CT_RETURN_IFERR(ctconn_execute(stmt));

    if (par_exp_get_init_cols(thread_ctrl) == CT_FALSE) {
        cm_thread_lock(thread_ctrl->lock_t);
        if (par_exp_get_init_cols(thread_ctrl) == CT_FALSE) {
            if (exp_init_columns(exporter, stmt, thread_ctrl->tab_param.table_cache) != CT_SUCCESS) {
                cm_thread_unlock(thread_ctrl->lock_t);
                return CT_ERROR;
            }
        }
        par_exp_set_init_cols(thread_ctrl, CT_TRUE);
        cm_thread_unlock(thread_ctrl->lock_t);
    }
                             ...
                             exporter_t *exporter, exp_cache_t* table_cache)
{
    ...
    CT_RETURN_IFERR(exp_start_cache_unit(table_cache, EXP_CACHE_TABLE_NAME));
    CT_RETURN_IFERR(exp_cache_append_str(table_cache,
    ...
    for (uint32 num = 0; num < exp_opts->dn_info.dn_info_list.count; num++) {
        ...
        CT_RETURN_IFERR(exp_bin_init_data_file(exp_opts, table_name, schema_name));
        CT_RETURN_IFERR(exp_pre_query_sql(stmt, g_sql_buf, schema_name, table_name));
        CT_RETURN_IFERR(ctconn_execute(stmt));

        // only once
        if (is_first) {
            is_first = CT_FALSE;
            CT_RETURN_IFERR(exp_init_columns(exporter, stmt, table_cache));
        }

        if (g_export_opts.filetype == FT_TXT) {
            CT_RETURN_IFERR(ctsql_make_insert_sql_s(exporter, g_sql_buf,
                exp_remap_table_name(&exp_opts->table_maps, table_name, NULL, 0)));
            CT_RETURN_IFERR(exp_write_txt_records(&g_exp_txtbuf, stmt, g_sql_buf, g_str_buf, g_exp_dpfile, exporter,
                CT_FALSE, &exporter->file_insert_num));
        } else {
            CT_RETURN_IFERR(exp_bin_init_bin_file_ctx(&bin_file_ctx, g_exp_fbuf, EXP_MAX_FILE_BUF, g_lob_fbuf,
            ...
            CT_RETURN_IFERR(exp_write_bin_records(&bin_file_ctx, stmt, g_str_buf, exporter, NULL,
                &exporter->file_insert_num));

            if (exporter->file_insert_num - tmp_insert_num > 0) {
                subfile = NULL;
                CT_RETURN_IFERR(exp_start_cache_unit(table_cache, EXP_CACHE_SUB_FILE_NAME));
                CT_RETURN_IFERR(alloc_column_subfile_info(table_cache, &subfile));
    ...
    CT_RETURN_IFERR(exp_table_records_deal(exporter));
...
static inline int ctsql_make_column_def(ctconn_stmt_t stmt, text_t *col_def)
{
    void *data = NULL;
    uint32 size;
    bool32 is_null = CT_FALSE;
    typmode_t col_type;
    bool32 is_arr;
    bool32 is_jsonb = CT_FALSE;

    // get the column name
    CT_RETURN_IFERR(ctconn_column_as_string(stmt, 0, g_str_buf, CT_MAX_PACKET_SIZE));
    exp_concat_str_quote(col_def, g_str_buf);

    // get the datatype
    CT_RETURN_IFERR(ctconn_column_as_string(stmt, 1, g_str_buf, CT_MAX_PACKET_SIZE));
    is_arr = is_arr_type();
    col_type.datatype = get_datatype_id(g_str_buf);

    if (ctconn_get_call_version(CONN) >= CS_VERSION_24) {
        CT_RETURN_IFERR(ctconn_get_column_by_id(stmt, 8, &data, &size, &is_null));
        is_jsonb = ((size == STR_TRUE_LEN) ? CT_TRUE : CT_FALSE);
    }

    CT_RETURN_IFERR(ctconn_get_column_by_id(stmt, 2, &data, &size, &is_null));
    col_type.size = (uint16)(*(int32 *)data);

    CT_RETURN_IFERR(ctconn_get_column_by_id(stmt, 3, &data, &size, &is_null));
    col_type.precision = is_null ? 0 : (uint8)(*(int32 *)data);

    CT_RETURN_IFERR(ctconn_get_column_by_id(stmt, 4, &data, &size, &is_null));
    col_type.scale = is_null ? 0 : (int8)(*(int32 *)data);

    if (CT_IS_STRING_TYPE(col_type.datatype)) {
        CT_RETURN_IFERR(ctconn_get_column_by_id(stmt, 7, &data, &size, &is_null));
        col_type.is_char = (((char *)data)[0] == 'C');
    }

    CT_RETURN_IFERR(cm_concat_string(col_def, EXP_MAX_DDL_BUF_SZ, " "));
    if (ctconn_get_call_version(CONN) >= CS_VERSION_24 && is_jsonb) {
        CT_RETURN_IFERR(cm_concat_string(col_def, EXP_MAX_DDL_BUF_SZ, "JSONB"));
    } else {
        if (cm_typmode2text(&col_type, col_def, EXP_MAX_DDL_BUF_SZ) != CT_SUCCESS) {
            return CT_ERROR;
        }
    ...
    CT_RETURN_IFERR(ctconn_column_as_string(stmt, 5, g_str_buf, CT_MAX_PACKET_SIZE));
    if (g_str_buf[0] == 'N') {
        CT_RETURN_IFERR(cm_concat_string(col_def, EXP_MAX_DDL_BUF_SZ, " NOT NULL"));
    }

    // add default text
    CT_RETURN_IFERR(ctconn_column_as_string(stmt, 6, g_str_buf, CT_MAX_PACKET_SIZE));
    if (strlen(g_str_buf) > 0) {
        CT_RETURN_IFERR(cm_concat_string(col_def, EXP_MAX_DDL_BUF_SZ, " DEFAULT "));
        CT_RETURN_IFERR(cm_concat_string(col_def, EXP_MAX_DDL_BUF_SZ, g_str_buf));
    }
    return CT_SUCCESS;
}
...
static status_t exp_table_columns(exp_tab_info_t *tab_info, exp_cache_t *table_cache)
{
    ...
    CT_RETURN_IFERR(ctconn_bind_by_pos(tab_info->ctx->query_tab_column, 0, CTCONN_TYPE_STRING, tab_info->user,
        (int32)strlen(tab_info->user), NULL));
    CT_RETURN_IFERR(ctconn_bind_by_pos(tab_info->ctx->query_tab_column, 1, CTCONN_TYPE_STRING, tab_info->table,
        (int32)strlen(tab_info->table), NULL));

    CT_RETURN_IFERR(ctconn_execute(tab_info->ctx->query_tab_column));

    do {
        CT_RETURN_IFERR(ctconn_fetch(tab_info->ctx->query_tab_column, &rows));
        if (rows == 0) {
            break;
        }
        ...
        CT_RETURN_IFERR(cm_concat_string(&col_def, EXP_MAX_DDL_BUF_SZ, EXP_INDENT));
        CT_RETURN_IFERR(ctsql_make_column_def(tab_info->ctx->query_tab_column, &col_def));
        CT_RETURN_IFERR(exp_cache_append_text(table_cache, &col_def));
    } while (CT_TRUE);
...
static int exp_table_constraints(exp_tabs_ctx_t *ctx, const char *table, exp_cache_t* table_cache, bool32 is_implicit)
{
    ...
    CT_RETURN_IFERR(ctconn_bind_by_pos(STMT, 0, CTCONN_TYPE_STRING, user, (int32)strlen(user), NULL));
    CT_RETURN_IFERR(ctconn_bind_by_pos(STMT, 1, CTCONN_TYPE_STRING, table, (int32)strlen(table), NULL));
    ...
    do {
        ...
        CT_RETURN_IFERR(exp_cache_append_str(table_cache, "ALTER TABLE "));
        CT_RETURN_IFERR(ctconn_column_as_string(STMT, 4, g_str_buf, CT_MAX_PACKET_SIZE));
        CT_RETURN_IFERR(exp_cache_append_str_quote(table_cache,
            exp_remap_table_name(&g_export_opts.table_maps, g_str_buf, NULL, 0)));
        CT_RETURN_IFERR(exp_cache_append_str(table_cache, " ADD "));
        if (!is_implicit) {
            CT_RETURN_IFERR(exp_cache_append_str(table_cache, "CONSTRAINT "));
            CT_RETURN_IFERR(ctconn_column_as_string(STMT, 3, g_str_buf, CT_MAX_PACKET_SIZE));
            CT_RETURN_IFERR(exp_cache_append_str_quote(table_cache, g_str_buf));
            CT_RETURN_IFERR(exp_cache_append_str(table_cache, " "));
        }
        ...
        if ((g_str_buf[0] == 'P') || (g_str_buf[0] == 'U')) {
            ...
            CT_RETURN_IFERR(exp_cache_append_str(table_cache, is_primary ? "PRIMARY KEY(" : "UNIQUE("));

            if (!g_export_opts.quote_names) {
                CT_RETURN_IFERR(ctconn_column_as_string(STMT, 1, g_str_buf, CT_MAX_PACKET_SIZE));
            } else {
                CT_RETURN_IFERR(ctconn_column_as_string(STMT, 9, g_str_buf, CT_MAX_PACKET_SIZE));
            }

            CT_RETURN_IFERR(exp_cache_append_str_quote(table_cache, g_str_buf));
            CT_RETURN_IFERR(exp_cache_append_str(table_cache, ")"));
            CT_RETURN_IFERR(ctconn_column_as_string(STMT, 11, g_str_buf, CT_MAX_PACKET_SIZE));
            if (is_primary && g_str_buf[0] == 'Y') {
                CT_RETURN_IFERR(exp_cache_append_str(table_cache, " USING INDEX LOCAL"));
            }
            if (ctx->reverse_index_available) {
                CT_RETURN_IFERR(ctconn_column_as_string(STMT, 12, g_str_buf, CT_MAX_PACKET_SIZE));
                if (g_str_buf[0] == 'Y') {
                    CT_RETURN_IFERR(exp_cache_append_str(table_cache, " REVERSE"));
                }
            }
        } else if (g_str_buf[0] == 'C') {
            CT_RETURN_IFERR(exp_cache_append_str(table_cache, "CHECK("));
            CT_RETURN_IFERR(ctconn_column_as_string(STMT, 5, g_str_buf, CT_MAX_PACKET_SIZE));
            CT_RETURN_IFERR(exp_cache_append_str(table_cache, g_str_buf));
            CT_RETURN_IFERR(exp_cache_append_str(table_cache, ")"));
        } else {
            continue;
        }
...
{
    CT_RETURN_IFERR(exp_cache_append_str(table_cache, "\nPARTITION BY "));
    CT_RETURN_IFERR(ctconn_column_as_string(STMT, 0, partition_info->parent_partition_type, CT_MAX_NAME_LEN));
    CT_RETURN_IFERR(exp_cache_append_str(table_cache, partition_info->parent_partition_type));
    CT_RETURN_IFERR(exp_cache_append_str(table_cache, " ("));

    if (!g_export_opts.quote_names) {
        CT_RETURN_IFERR(ctconn_column_as_string(STMT, 1, g_str_buf, CT_MAX_PACKET_SIZE));
    } else {
        CT_RETURN_IFERR(ctconn_column_as_string(STMT, 2, g_str_buf, CT_MAX_PACKET_SIZE));
    }
    ...
    if (cm_str_equal(partition_info->parent_partition_type, "RANGE")) {
        ...
        if (g_str_buf[0] != '\0') {
            CT_RETURN_IFERR(exp_cache_append_str(table_cache, "INTERVAL("));
            CT_RETURN_IFERR(exp_cache_append_str(table_cache, g_str_buf));
            CT_RETURN_IFERR(exp_cache_append_str(table_cache, ")\n"));
            ...
            if (g_str_buf[0] != '\0') {
                CT_RETURN_IFERR(exp_cache_append_str(table_cache, "STORE IN("));
                CT_RETURN_IFERR(exp_cache_append_str_quote(table_cache, g_str_buf));
                CT_RETURN_IFERR(exp_cache_append_str(table_cache, ")\n"));
            }
        }
...
{
    if (ctconn_get_call_version(CONN) < CS_VERSION_20) {
        return CT_SUCCESS;
    }

    CT_RETURN_IFERR(ctconn_column_as_string(STMT, 6, partition_info->sub_partition_type, CT_MAX_NAME_LEN));
    ...
    if (partition_info->has_sub_partition) {
        CT_RETURN_IFERR(exp_cache_append_str(table_cache, "SUBPARTITION BY "));
        CT_RETURN_IFERR(exp_cache_append_str(table_cache, partition_info->sub_partition_type));
        CT_RETURN_IFERR(exp_cache_append_str(table_cache, " ("));
        if (!g_export_opts.quote_names) {
            CT_RETURN_IFERR(ctconn_column_as_string(STMT, 7, g_str_buf, CT_MAX_PACKET_SIZE));
        } else {
            CT_RETURN_IFERR(ctconn_column_as_string(STMT, 8, g_str_buf, CT_MAX_PACKET_SIZE));
        }
        CT_RETURN_IFERR(exp_cache_append_str_quote(table_cache, g_str_buf));
        CT_RETURN_IFERR(exp_cache_append_str(table_cache, ")\n"));
    }
                                        ...
                                        exp_prepare_sql_param_t param)
{
    ...
    if (ctconn_get_call_version(CONN) >= CS_VERSION_20) {
        CT_RETURN_IFERR(EXP_PREPARE_SQL(EXP_GET_TABLE_NEW_PARTITION_KEY, cmd_buf, CTSQL_MAX_TEMP_SQL, &param));
    } else {
        CT_RETURN_IFERR(EXP_PREPARE_SQL(EXP_GET_TABLE_OLD_PARTITION_KEY, cmd_buf, CTSQL_MAX_TEMP_SQL, &param));
    }
    ...
    CT_RETURN_IFERR(exp_append_parent_partition_key(table_cache, partition_info));
    CT_RETURN_IFERR(exp_append_sub_partition_key(table_cache, partition_info));
                                          ...
                                          exp_prepare_sql_param_t param)
{
    ...
    CT_RETURN_IFERR(exp_cache_append_str(table_cache, "(\n"));
    if (!partition_info.has_sub_partition) {
        CT_RETURN_IFERR(EXP_PREPARE_SQL(EXP_GET_TABLE_PARTITION_VALUE, cmd_buf, CTSQL_MAX_TEMP_SQL, &param));
    } else {
        CT_RETURN_IFERR(EXP_PREPARE_SQL(EXP_GET_TABLE_SUBPARTITION_VALUE, cmd_buf, CTSQL_MAX_TEMP_SQL, &param));
    }
    ...
    do {
        ...
        CT_RETURN_IFERR(exp_cache_append_str(table_cache, EXP_INDENT2));
        CT_RETURN_IFERR(exp_cache_append_str(table_cache, "PARTITION "));
        // pos 0 is partition name
        CT_RETURN_IFERR(ctconn_column_as_string(STMT, pos++, g_str_buf, CT_MAX_PACKET_SIZE));
        CT_RETURN_IFERR(exp_cache_append_str(table_cache, g_str_buf));

        if (cm_str_equal(partition_info.parent_partition_type, "RANGE")) {
            CT_RETURN_IFERR(exp_cache_append_str(table_cache, " VALUES LESS THAN ("));
            // pos 1 is high value
            CT_RETURN_IFERR(ctconn_column_as_string(STMT, pos, g_str_buf, CT_MAX_PACKET_SIZE));
            CT_RETURN_IFERR(exp_cache_append_str(table_cache, g_str_buf));
            CT_RETURN_IFERR(exp_cache_append_str(table_cache, ")"));
        } else if (cm_str_equal(partition_info.parent_partition_type, "LIST")) {
            CT_RETURN_IFERR(exp_cache_append_str(table_cache, " VALUES("));
            // pos 1 is high value
            CT_RETURN_IFERR(ctconn_column_as_string(STMT, pos, g_str_buf, CT_MAX_PACKET_SIZE));
            CT_RETURN_IFERR(exp_cache_append_str(table_cache, g_str_buf));
            CT_RETURN_IFERR(exp_cache_append_str(table_cache, ")"));
        ...
        CT_RETURN_IFERR(ctconn_column_as_string(STMT, pos++, g_str_buf, CT_MAX_PACKET_SIZE));
        CT_RETURN_IFERR(exp_cache_append_str(table_cache, " TABLESPACE "));
        CT_RETURN_IFERR(exp_cache_append_str_quote(table_cache, g_str_buf));
        // pos 3 is initrans
        CT_RETURN_IFERR(ctconn_column_as_string(STMT, pos++, g_str_buf, CT_MAX_PACKET_SIZE));
        CT_RETURN_IFERR(exp_cache_append_str(table_cache, " INITRANS "));
        CT_RETURN_IFERR(exp_cache_append_str(table_cache, g_str_buf));
        // pos 4 is pctfree
        CT_RETURN_IFERR(ctconn_column_as_string(STMT, pos++, g_str_buf, CT_MAX_PACKET_SIZE));
        CT_RETURN_IFERR(exp_cache_append_str(table_cache, " PCTFREE "));
        CT_RETURN_IFERR(exp_cache_append_str(table_cache, g_str_buf));
        ...
        if (exp_partition_param->db_tab_partitions_has_flag) {
            if (g_export_opts.with_format_csf) {
                ...
                CT_RETURN_IFERR(ctconn_column_as_string(STMT, pos, g_str_buf, CT_MAX_PACKET_SIZE));
                if (ctconn_get_call_version(CONN) >= CS_VERSION_24 || cm_compare_str(g_str_buf, "CSF") == 0) {
                    CT_RETURN_IFERR(exp_cache_append_str(table_cache, " FORMAT "));
                    CT_RETURN_IFERR(exp_cache_append_str(table_cache, g_str_buf));
                }
            }
            pos++;
        ...
        if (ctconn_get_call_version(CONN) >= CS_VERSION_22) {
            ...
            if (cm_compare_str(g_str_buf, "NONE") != 0) {
                CT_RETURN_IFERR(exp_cache_append_str(table_cache, " COMPRESS"));
            }
        }
...
{
    if (exp_verify_column_of_part_tables_agent() == CT_SUCCESS) {
        *flag = CT_TRUE;
    } else {
        *flag = CT_FALSE;
    }
...
static int exp_lob_storage(const char *user, const char *table, exp_cache_t* table_cache)
{
    ...
    CT_RETURN_IFERR(EXP_PREPARE_SQL(EXP_GET_DB_LOB_STORAGE, cmd_buf, CTSQL_MAX_TEMP_SQL, &param));
    CT_RETURN_IFERR(ctconn_prepare(STMT, (const char *)cmd_buf));
    CT_RETURN_IFERR(ctconn_execute(STMT));
    ...
    do {
        ...
        CT_RETURN_IFERR(exp_cache_append_str(table_cache, "\n"));
        CT_RETURN_IFERR(ctconn_column_as_string(STMT, 0, cmd_buf, CTSQL_MAX_TEMP_SQL));
        CT_RETURN_IFERR(exp_cache_append_str(table_cache, "LOB ("));
        CT_RETURN_IFERR(exp_cache_append_str_quote(table_cache, cmd_buf));
        CT_RETURN_IFERR(exp_cache_append_str(table_cache, ") STORE AS (\n"));
        CT_RETURN_IFERR(ctconn_column_as_string(STMT, 1, cmd_buf, CTSQL_MAX_TEMP_SQL));
        CT_RETURN_IFERR(exp_cache_append_str(table_cache, "    TABLESPACE "));
        CT_RETURN_IFERR(exp_cache_append_str_quote(table_cache, cmd_buf));
        CT_RETURN_IFERR(exp_cache_append_str(table_cache, "\n)"));
    } while (CT_TRUE);
...
static inline int exp_table_partition(const char *user, const char *table, exp_cache_t* table_cache)
{
    ...
    partition_info.has_sub_partition = CT_FALSE;
    ...
    CT_RETURN_IFERR(exp_table_partition_key(table_cache, &partition_info, param));
    CT_RETURN_IFERR(exp_table_partition_value(table_cache, partition_info, param));
...
{
    bool32 is_consist = CT_FALSE;

    if (tab_info->table_type == TABLE_TYPE_TRANS_TEMP || tab_info->table_type == TABLE_TYPE_SESSION_TEMP) {
        CT_RETURN_IFERR(exp_cache_append_str(table_cache, "CREATE GLOBAL TEMPORARY TABLE "));
    } else {
        CT_RETURN_IFERR(exp_cache_append_str(table_cache, "CREATE TABLE "));
    }

    CT_RETURN_IFERR(exp_cache_append_str_quote(table_cache,
        exp_remap_table_name(&g_export_opts.table_maps, tab_info->table, NULL, 0)));
    CT_RETURN_IFERR(exp_cache_append_str(table_cache, "\n(\n"));
    CT_RETURN_IFERR(exp_table_columns(tab_info, table_cache));
    CT_RETURN_IFERR(exp_cache_append_str(table_cache, "\n)"));

    if (ctconn_get_call_version(CONN) >= CS_VERSION_23) {
        EXP_RETURN_IFERR(exp_lob_storage(tab_info->user, tab_info->table, table_cache));
    }

    if (tab_info->partitioned == 1 && !is_consist) {
        CT_RETURN_IFERR(exp_table_partition(tab_info->user, tab_info->table, table_cache));
    }

    if (tab_info->table_type == TABLE_TYPE_TRANS_TEMP) {
        CT_RETURN_IFERR(exp_cache_append_str(table_cache, "ON COMMIT DELETE ROWS"));
    } else if (tab_info->table_type == TABLE_TYPE_SESSION_TEMP) {
        CT_RETURN_IFERR(exp_cache_append_str(table_cache, "ON COMMIT PRESERVE ROWS"));
    } else if (tab_info->table_type == TABLE_TYPE_NOLOGGING) {
        CT_RETURN_IFERR(exp_cache_append_str(table_cache, "NOLOGGING"));
    }
...
static int exp_table_attrs(const char *user, const char *table, exp_cache_t* table_cache)
{
    ...
    if (exp_verify_column_of_tables_agent() == CT_SUCCESS) {
        param.get_db_tables_param.db_tables_has_flag = CT_TRUE;
    } else {
        param.get_db_tables_param.db_tables_has_flag = CT_FALSE;
    }

    CT_RETURN_IFERR(EXP_PREPARE_SQL(EXP_GET_DB_TABLES_LIST, cmd_buf, CTSQL_MAX_TEMP_SQL, &param));
    ...
    CT_RETURN_IFERR(exp_table_attrs_core(table_cache, param.get_db_tables_param.db_tables_has_flag));
...
static int exp_table_comments(const char *user, const char *table, exp_cache_t* table_cache)
{
    ...
    if (rows > 0) {
        CT_RETURN_IFERR(ctconn_column_as_string(STMT, 0, g_str_buf, CT_MAX_PACKET_SIZE));
        CT_RETURN_IFERR(exp_cache_append_str(table_cache, "COMMENT ON TABLE "));
        CT_RETURN_IFERR(exp_cache_append_str_quote(table_cache,
            exp_remap_table_name(&g_export_opts.table_maps, g_str_buf, NULL, 0)));
        CT_RETURN_IFERR(exp_cache_append_str(table_cache, " IS '"));

        CT_RETURN_IFERR(ctconn_column_as_string(STMT, 1, g_str_buf, CT_MAX_PACKET_SIZE));
        CT_RETURN_IFERR(exp_cache_append_escape_str(table_cache, g_str_buf, '\''));
        CT_RETURN_IFERR(exp_cache_append_str(table_cache, "';\n"));
    }
    ...
    do {
        ...
        CT_RETURN_IFERR(ctconn_column_as_string(STMT, 0, g_str_buf, CT_MAX_PACKET_SIZE));
        CT_RETURN_IFERR(exp_cache_append_str(table_cache, "COMMENT ON COLUMN "));
        CT_RETURN_IFERR(exp_cache_append_str_quote(table_cache, g_str_buf));
        CT_RETURN_IFERR(exp_cache_append_str(table_cache, "."));
        CT_RETURN_IFERR(ctconn_column_as_string(STMT, 1, g_str_buf, CT_MAX_PACKET_SIZE));
        CT_RETURN_IFERR(exp_cache_append_str_quote(table_cache, g_str_buf));
    ...
    const char *index_name, exp_cache_t* table_cache)
{
    ...
    uint32 rows;
    uint32 cursor = 0;
    ctconn_stmt_t stmt = ctx->query_func_indexes;

    CT_RETURN_IFERR(ctconn_bind_by_pos(stmt, 0, CTCONN_TYPE_STRING, ctx->user, (int32)strlen(ctx->user), NULL));
    CT_RETURN_IFERR(ctconn_bind_by_pos(stmt, 1, CTCONN_TYPE_STRING, table, (int32)strlen(table), NULL));
    CT_RETURN_IFERR(ctconn_bind_by_pos(stmt, 2, CTCONN_TYPE_STRING, index_name, (int32)strlen(index_name), NULL));
...
static int exp_index_subpartition(exp_tabs_ctx_t *ctx, exp_cache_t* table_cache, const char *partition_name, const char *index_name)
{
    ...
    uint32 cursor = 0;
    char cmd_buf[CTSQL_MAX_TEMP_SQL + 1];
    ctconn_stmt_t stmt = ctx->query_index_subpartition;

    CT_RETURN_IFERR(ctconn_bind_by_pos(stmt, 0, CTCONN_TYPE_STRING, partition_name, (int32)strlen(partition_name), NULL));
    CT_RETURN_IFERR(ctconn_bind_by_pos(stmt, 1, CTCONN_TYPE_STRING, index_name, (int32)strlen(index_name), NULL));
    CT_RETURN_IFERR(ctconn_bind_by_pos(stmt, 2, CTCONN_TYPE_STRING, ctx->user, (int32)strlen(ctx->user), NULL));
    CT_RETURN_IFERR(ctconn_execute(stmt));
    ...
    do {
        ...
        CT_RETURN_IFERR(exp_cache_append_str(table_cache, "           SUBPARTITION "));
        CT_RETURN_IFERR(ctconn_column_as_string(stmt, 0, cmd_buf, CTSQL_MAX_TEMP_SQL));
        CT_RETURN_IFERR(exp_cache_append_str(table_cache, cmd_buf));
...
static int exp_has_sub_partition(ctconn_stmt_t stmt, const char *user, const char *table, bool8 *has_sub_partition)
{
    uint32 rows;
    char cmd_buf[CTSQL_MAX_TEMP_SQL + 1] = { 0 };
    ...
    CT_RETURN_IFERR(ctconn_column_as_string(stmt, 0, cmd_buf, CT_MAX_NAME_LEN));
...
static inline bool8 exp_need_index_partition(void)
{
    return g_export_opts.index_partitions &&
        ctconn_get_call_version(CONN) >= CS_VERSION_23;
}
    ...
    const char *index_name, const char *tablespace_name)
{
    uint32 rows;
    uint32 cnt = 0;
    char partition_name[CTSQL_MAX_OBJECT_LEN] = { 0 };
    char cmd_buf[CTSQL_MAX_TEMP_SQL + 1];
    char partition_tbls[CTSQL_MAX_OBJECT_LEN] = { 0 };
    bool8 has_sub_partition = 0;
    ctconn_stmt_t stmt = ctx->query_index_partitioning;

    CT_RETURN_IFERR(exp_has_sub_partition(ctx->query_has_subpartition, ctx->user, table, &has_sub_partition));

    CT_RETURN_IFERR(ctconn_bind_by_pos(stmt, 0, CTCONN_TYPE_STRING, index_name, (int32)strlen(index_name), NULL));
    CT_RETURN_IFERR(ctconn_bind_by_pos(stmt, 1, CTCONN_TYPE_STRING, ctx->user, (int32)strlen(ctx->user), NULL));
    CT_RETURN_IFERR(ctconn_execute(stmt));
    ...
    do {
        ...
        CT_RETURN_IFERR(ctconn_column_as_string(stmt, 0, partition_tbls, CTSQL_MAX_OBJECT_LEN));
        ...
        CT_RETURN_IFERR(exp_cache_append_str(table_cache, "       PARTITION "));
        CT_RETURN_IFERR(ctconn_column_as_string(stmt, 0, cmd_buf, CTSQL_MAX_TEMP_SQL));
        MEMS_RETURN_IFERR(memcpy_s(partition_name, CTSQL_MAX_OBJECT_LEN, cmd_buf, CTSQL_MAX_OBJECT_LEN));
        partition_name[strlen(partition_name)] = '\0';
        CT_RETURN_IFERR(exp_cache_append_str(table_cache, cmd_buf));

        CT_RETURN_IFERR(exp_cache_append_str(table_cache, " TABLESPACE "));
        CT_RETURN_IFERR(ctconn_column_as_string(stmt, 1, cmd_buf, CTSQL_MAX_OBJECT_LEN));
        CT_RETURN_IFERR(exp_cache_append_str_quote(table_cache, cmd_buf));

        CT_RETURN_IFERR(exp_cache_append_str(table_cache, " INITRANS "));
        CT_RETURN_IFERR(ctconn_column_as_string(stmt, 2, cmd_buf, CTSQL_MAX_TEMP_SQL));
        CT_RETURN_IFERR(exp_cache_append_str(table_cache, cmd_buf));

        CT_RETURN_IFERR(exp_cache_append_str(table_cache, " PCTFREE "));
        CT_RETURN_IFERR(ctconn_column_as_string(stmt, 3, cmd_buf, CTSQL_MAX_TEMP_SQL));
        CT_RETURN_IFERR(exp_cache_append_str(table_cache, cmd_buf));

        if (has_sub_partition) {
            CT_RETURN_IFERR(exp_index_subpartition(ctx, table_cache,
...
{
    ctconn_stmt_t stmt = ctx->query_tab_has_intervalpart;
    uint32 rows = 0;
    uint64 cnt = 0;

    *has_interval_part = CT_FALSE;

    CT_RETURN_IFERR(ctconn_bind_by_pos(stmt, 0, CTCONN_TYPE_STRING, ctx->user, (int32)strlen(ctx->user), NULL));
    CT_RETURN_IFERR(ctconn_bind_by_pos(stmt, 1, CTCONN_TYPE_STRING, table, (int32)strlen(table), NULL));

    CT_RETURN_IFERR(ctconn_bind_column(stmt, 0, CTCONN_TYPE_BIGINT, sizeof(cnt), &cnt, NULL));
...
{
    uint32 rows;
    char org_ind_name[CTSQL_MAX_OBJECT_LEN] = { 0 };
    char cmd_buf[CTSQL_MAX_TEMP_SQL + 1];
    char tablespace_name[CTSQL_MAX_OBJECT_LEN] = { 0 };
    bool8 has_interval_part = CT_FALSE;
    ...
    PRTS_RETURN_IFERR(sprintf_s(cmd_buf, CTSQL_MAX_TEMP_SQL,
        ...
        (ctconn_get_call_version(CONN) >= CS_VERSION_25) ? ", IS_NOLOGGING" : " ",
    ...
    do {
        ...
        if (ctx->exp_opts->tbs_list.count > 0 &&
            exp_table_index_check_tabspace(ctx->exp_opts, g_str_buf) != CT_SUCCESS) {
            continue;
        }
        ...
        if (g_str_buf[0] == 'Y') {
            CT_RETURN_IFERR(exp_cache_append_str(table_cache, "CREATE UNIQUE INDEX "));
        } else {
            CT_RETURN_IFERR(exp_cache_append_str(table_cache, "CREATE INDEX "));
        }
        
        CT_RETURN_IFERR(ctconn_column_as_string(STMT, 3, g_str_buf, CT_MAX_PACKET_SIZE));
        MEMS_RETURN_IFERR(memcpy_s(org_ind_name, CTSQL_MAX_OBJECT_LEN, g_str_buf, CTSQL_MAX_OBJECT_LEN));
        org_ind_name[strlen(g_str_buf)] = '\0';

        CT_RETURN_IFERR(exp_cache_append_str_quote(table_cache, g_str_buf));
        CT_RETURN_IFERR(exp_cache_append_str(table_cache, " ON "));
        CT_RETURN_IFERR(exp_cache_append_str_quote(table_cache,
        ...
        if (!g_export_opts.quote_names) {
            ...
            CT_RETURN_IFERR(exp_table_func_indexes(ctx, table, org_ind_name, table_cache));
        }
        ...
        if (g_str_buf[0] == 'Y') {
            CT_RETURN_IFERR(exp_cache_append_str(table_cache, "LOCAL\n"));
        }

        CT_RETURN_IFERR(ctconn_column_as_string(STMT, 5, g_str_buf, CT_MAX_PACKET_SIZE));
        MEMS_RETURN_IFERR(memcpy_s(tablespace_name, CTSQL_MAX_OBJECT_LEN, g_str_buf, CTSQL_MAX_OBJECT_LEN));
        tablespace_name[strlen(tablespace_name)] = '\0';

        CT_RETURN_IFERR(exp_tab_has_interval_parts(ctx, table, &has_interval_part));
        if (exp_need_index_partition() && !has_interval_part) {
            CT_RETURN_IFERR(exp_index_partitioning(ctx, table_cache, table, org_ind_name, tablespace_name));
        }
        ...
        if (ctx->exp_opts->with_cr_mode) {
            CT_RETURN_IFERR(ctconn_column_as_string(STMT, 10, g_str_buf, CT_MAX_PACKET_SIZE));
            if (strlen(g_str_buf) > 0) {
                CT_RETURN_IFERR(exp_cache_append_str(table_cache, "\nCRMODE "));
                CT_RETURN_IFERR(exp_cache_append_str(table_cache, g_str_buf));
            }
        ...
        if (ctconn_get_call_version(CONN) >= CS_VERSION_25) {
            CT_RETURN_IFERR(ctconn_column_as_string(STMT, 12, g_str_buf, CT_MAX_PACKET_SIZE));
            if (g_str_buf[0] == 'Y') {
                CT_RETURN_IFERR(exp_cache_append_str(table_cache, "\nNOLOGGING"));
            }
...
static int exp_table_auto_increment(const char *user, const char *table, exp_cache_t *table_cache)
{
    ...
    CT_RETURN_IFERR(exp_cache_append_str(table_cache, "ALTER TABLE "));
    CT_RETURN_IFERR(exp_cache_append_str_quote(table_cache, table));
    CT_RETURN_IFERR(exp_cache_append_str(table_cache, " MODIFY "));
    CT_RETURN_IFERR(ctconn_column_as_string(STMT, 0, g_str_buf, CT_MAX_PACKET_SIZE));
    CT_RETURN_IFERR(exp_cache_append_str_quote(table_cache, g_str_buf));
    CT_RETURN_IFERR(exp_cache_append_str(table_cache, " AUTO_INCREMENT"));
    CT_RETURN_IFERR(exp_cache_append_str(table_cache, ";\n"));
    PRTS_RETURN_IFERR(sprintf_s(cmd_buf, CTSQL_MAX_TEMP_SQL,
...
static inline int exp_table_foreign_constraints(const char *user, const char *table)
{
    ...
    do {
        ...
        CT_RETURN_IFERR(exp_write_str_s("ALTER TABLE ", &g_exp_txtbuf, g_exp_dpfile));
        CT_RETURN_IFERR(ctconn_column_as_string(STMT, 4, g_str_buf, CT_MAX_PACKET_SIZE));
        exp_write_str_quote(g_str_buf);
        CT_RETURN_IFERR(ctconn_column_as_string(STMT, 13, g_str_buf, CT_MAX_PACKET_SIZE));

        if (g_str_buf[0] == 'N') {
            CT_RETURN_IFERR(exp_write_str_s(" ADD CONSTRAINT ", &g_exp_txtbuf, g_exp_dpfile));
            CT_RETURN_IFERR(ctconn_column_as_string(STMT, 3, g_str_buf, CT_MAX_PACKET_SIZE));
            exp_write_str_quote(g_str_buf);
        } else if (g_str_buf[0] == 'Y') {
            CT_RETURN_IFERR(exp_write_str_s(" ADD", &g_exp_txtbuf, g_exp_dpfile));
        } else {
            continue;
        }

        CT_RETURN_IFERR(exp_write_str_s(" ", &g_exp_txtbuf, g_exp_dpfile));
        ...
        if (g_str_buf[0] == 'R') {
            ...
            CT_RETURN_IFERR(exp_write_str_s("FOREIGN KEY(", &g_exp_txtbuf, g_exp_dpfile));
            exp_write_str_quote(g_str_buf);
            CT_RETURN_IFERR(exp_write_str_s(") REFERENCES ", &g_exp_txtbuf, g_exp_dpfile));
            if (!g_export_opts.quote_names) {
                CT_RETURN_IFERR(ctconn_column_as_string(STMT, 6, g_str_buf, CT_MAX_PACKET_SIZE));
            } else {
                CT_RETURN_IFERR(ctconn_column_as_string(STMT, 11, g_str_buf, CT_MAX_PACKET_SIZE));
            ...
            CT_RETURN_IFERR(exp_write_str_s(")", &g_exp_txtbuf, g_exp_dpfile));
            CT_RETURN_IFERR(ctconn_column_as_string(STMT, 8, g_str_buf, CT_MAX_PACKET_SIZE));
            CT_RETURN_IFERR(exp_write_str_s(g_str_buf, &g_exp_txtbuf, g_exp_dpfile));
        } else {
            continue;
        }
        CT_RETURN_IFERR(exp_write_str_s(";\n", &g_exp_txtbuf, g_exp_dpfile));
...
static status_t exp_tab_meta(export_options_t *exp_opts, exp_tab_info_t *tab_info, exp_cache_t *table_cache)
{
    ...
    CT_RETURN_IFERR(exp_start_cache_unit(table_cache, EXP_CACHE_CREATE_TABLE));
    ...
    if (!exp_opts->skip_add_drop_table && !g_export_opts.show_create_table) {
        CT_RETURN_IFERR(exp_cache_append_str(table_cache, "DROP TABLE IF EXISTS "));
        CT_RETURN_IFERR(exp_cache_append_str_quote(table_cache,
            exp_remap_table_name(&exp_opts->table_maps, tab_info->table, NULL, 0)));
        CT_RETURN_IFERR(exp_cache_append_str(table_cache, " CASCADE CONSTRAINTS;\n"));
    }

    CT_RETURN_IFERR(exp_create_table(tab_info, table_cache));

    if (!TABLE_IS_TEMP(tab_info->table_type)) {
        CT_RETURN_IFERR(exp_table_attrs(tab_info->user, tab_info->table, table_cache));
    }
    CT_RETURN_IFERR(exp_cache_append_str(table_cache, ";\n"));
...
static status_t exp_tab_indx_meta(exp_tabs_ctx_t *ctx, const char *table, exp_cache_t *table_cache)
{
    ...
    CT_RETURN_IFERR(exp_table_indexes(ctx, table, table_cache));
    if (!g_export_opts.show_create_table) {
        exp_log(EXP_INDENT "exporting constraints on %s.%s ...\n", ctx->user, table);
    }
    CT_RETURN_IFERR(exp_table_constraints(ctx, table, table_cache, CT_FALSE));
    CT_RETURN_IFERR(exp_table_constraints(ctx, table, table_cache, CT_TRUE));
    if (!g_export_opts.show_create_table) {
        exp_log("\n");
    }
...
static inline int exp_add_tblobj(ctconn_stmt_t stmt, uint32 col_id, list_t *table_list)
{
    uint32 rows;
    char *ptr = NULL;
    uint32 i = 0;
    ...
    do {
        ...
        CT_RETURN_IFERR(ctconn_column_as_string(STMT, col_id, g_str_buf, CT_MAX_PACKET_SIZE));
        if ((cm_list_new(table_list, (pointer_t *)&ptr) != CT_SUCCESS) || (ptr == NULL)) {
            return CT_ERROR;
        }
        MEMS_EXP_RETURN_IFERR(strncpy_s(ptr, CTSQL_MAX_OBJECT_LEN, g_str_buf, CTSQL_MAX_OBJECT_LEN - 1));
        i++;
        exp_log("%-64s ", ptr);
        exp_log("%-10u\n", i);
    } while (CT_TRUE);
...
static inline int exp_get_user_tables(export_options_t *exp_opts, const char *user, list_t *table_list)
{
    ...
    CT_RETURN_IFERR(EXP_PREPARE_SQL(EXP_GET_USER_TABLE_LIST, cmd_buf, CTSQL_MAX_TEMP_SQL, &param));
    ...
    CT_RETURN_IFERR(exp_add_tblobj(STMT, 0, table_list));
...
static inline int exp_get_users(export_options_t *exp_opts)
{
    ...
    do {
        ...
        CT_RETURN_IFERR(ctconn_column_as_string(STMT, 0, g_str_buf, CT_MAX_PACKET_SIZE));
        if (cm_list_new(&exp_opts->obj_list, (pointer_t *)&ptr) != CT_SUCCESS) {
            return CT_ERROR;
        }
        MEMS_EXP_RETURN_IFERR(strncpy_s(ptr, CTSQL_MAX_OBJECT_LEN, g_str_buf, CTSQL_MAX_OBJECT_LEN - 1));
    } while (CT_TRUE);
...
static status_t par_exp_connectdb(par_exp_thread_ctrl_t *ctrl)
{
    ...
    uint32 new_num_width = (uint32)CT_MAX_DEC_OUTPUT_ALL_PREC;
    /* Re-establishing connections is not supported */
    if (ctrl->conn_info.conn == NULL) {
        ctrl->execute_ret = ctsql_alloc_conn(&ctrl->conn_info.conn);
        if (ctrl->execute_ret != CT_SUCCESS) {
        ...
        (void)ctconn_set_conn_attr(ctrl->conn_info.conn, CTCONN_ATTR_INTERACTIVE_MODE, (void *)&interactive_clt, 0);
        (void)ctconn_set_conn_attr(ctrl->conn_info.conn, CTCONN_ATTR_NUM_WIDTH, (void *)&new_num_width, sizeof(uint32));
        CT_RETURN_IFERR(ctsql_get_saved_pswd(ctrl->conn_info.passwd, CT_PASSWORD_BUFFER_SIZE + 4));

        (void)ctsql_switch_user(&ctrl->conn_info);
        ctrl->execute_ret = ctsql_conn_to_server(&ctrl->conn_info, CT_FALSE, CT_TRUE);
        if (ctrl->execute_ret != CT_SUCCESS) {
...
{
    char full_fpath[CT_MAX_FILE_PATH_LENGH];

    if ((wr_lob_flg == CT_FALSE) && (ctrl->files_context.lf_h != NULL)) {
        ...
        CT_RETURN_IFERR(exp_form_fullpath(ctrl->options.dump_data_path, ctrl->options.lob_file_name, full_fpath,
            CT_MAX_FILE_PATH_LENGH));
        CT_RETURN_IFERR(cm_remove_file((const char *)full_fpath));
    }
    ...
    if ((file_insert_num == 0) && (ctrl->files_context.exp_dpfile != NULL)) {
        ...
        fclose(ctrl->files_context.exp_dpfile);
        ctrl->files_context.exp_dpfile = NULL;
        CT_RETURN_IFERR(exp_form_fullpath(ctrl->options.dump_data_path, ctrl->options.bin_data_file, full_fpath,
            CT_MAX_FILE_PATH_LENGH));
        CT_RETURN_IFERR(cm_remove_file((const char *)full_fpath));
    }
...
static void par_exp_proc(thread_t *thread)
{
    ...
    while (!thread->closed) {
        if (par_exp_get_ctrl_stat(ctrl) == PAR_EXP_PROC) {
            ...
            (void)ctsql_reset_crypfile(ctrl->files_context.exp_dpfile, &g_export_opts.crypt_info);
...
{
    char get_par_sql[CTSQL_MAX_TEMP_SQL];
    ctconn_stmt_t dn_stmt = mgr->par_proc_param.stmt;
    tab_par_param_t *tab_param = NULL;
    bool32 normal_tab = CT_FALSE;
    int32 ret_code = CT_SUCCESS;
    void *data = NULL;
    ...
    for (i = 0; i < exp_opts->exp_tables.partition_list.count; i++) {
        ...
        PRTS_RETURN_IFERR(sprintf_s(get_par_sql, CTSQL_MAX_TEMP_SQL,
            "SELECT * from table(get_tab_parallel('\"%s\".\"%s\"', ?, ?))", user, tab));
        CT_RETURN_IFERR(ctconn_prepare(dn_stmt, (const char *)get_par_sql));
        CT_RETURN_IFERR(ctconn_bind_by_pos(dn_stmt, 0, CTCONN_TYPE_INTEGER,
            &mgr->options.parallel, sizeof(uint32), NULL));
        CT_RETURN_IFERR(ctconn_bind_by_pos(dn_stmt, 1, CTCONN_TYPE_STRING,
            partition_name, (int)strlen(partition_name), NULL));
        ret_code = ctconn_execute(dn_stmt);
        if (ret_code != CT_SUCCESS) {
            const char *message = "";
            int exp_partion_err_code = 0;
            ctconn_get_error(mgr->par_proc_param.conn, &exp_partion_err_code, &message);
        ...
        do {
            CT_RETURN_IFERR(ctconn_fetch(dn_stmt, &rows));
            if (rows == 0) {
                break;
            }

            normal_tab = CT_TRUE;
            CT_RETURN_IFERR(cm_list_new(&mgr->tab_par_params, (void **)&tab_param));

            CT_RETURN_IFERR(ctconn_get_column_by_id(dn_stmt, 0, (void **)&data, &size, &is_null));
            tab_param->part_no = *(uint32 *)data;
            CT_RETURN_IFERR(ctconn_get_column_by_id(dn_stmt, 1, (void **)&data, &size, &is_null));
            tab_param->l_page = *(uint64 *)data;
            CT_RETURN_IFERR(ctconn_get_column_by_id(dn_stmt, 2, (void **)&data, &size, &is_null));
    ...
    exp_dn_info_t *node_info, par_exp_mgr_t *mgr, const char *user, const char *tab)
{
    ...
    PRTS_RETURN_IFERR(sprintf_s(get_par_sql, CTSQL_MAX_TEMP_SQL,
        "SELECT * from table(get_tab_parallel('\"%s\".\"%s\"', ?))", user, tab));
    CT_RETURN_IFERR(ctconn_prepare(dn_stmt, (const char *)get_par_sql));
    CT_RETURN_IFERR(ctconn_bind_by_pos(dn_stmt, 0, CTCONN_TYPE_INTEGER, &mgr->options.parallel, sizeof(uint32), NULL));
    CT_RETURN_IFERR(ctconn_execute(dn_stmt));
    ...
    if (normal_tab == CT_FALSE) {
        /* Temporary tables, views */
        CT_RETURN_IFERR(cm_list_new(&mgr->tab_par_params, (void **)&tab_param));
        tab_param->part_no = CT_INVALID_ID32;
        tab_param->l_page = CT_INVALID_ID64;
        tab_param->r_page = CT_INVALID_ID64;
        tab_param->normal_tab = normal_tab;
    }
    return CT_SUCCESS;
}
...
static status_t par_exp_get_tab_params(export_options_t *exp_opts, exp_dn_info_t *node_info, par_exp_mgr_t *mgr, const char *user, const char *tab)
{
    ...
    if (exp_opts->exp_tables.table_exp_type == EXP_TABLE_PARTITION) {
        CT_RETURN_IFERR(par_exp_get_partition_tab_params(exp_opts, node_info, mgr, user, tab));
        if (mgr->tab_par_params.count == 0) {
            return CT_SUCCESS;
        }
    } else if (exp_opts->exp_tables.table_exp_type == EXP_TABLE_FULL) {
        CT_RETURN_IFERR(par_exp_get_full_tab_params(exp_opts, node_info, mgr, user, tab));
    }
    ...
    for (i = 0; i < mgr->options.parallel; i++) {
        ...
        mgr->par_proc_param.scan_param = *(tab_par_param_t *)cm_list_get(&mgr->tab_par_params,
                                                                         mgr->tab_par_param_offset);
        mgr->tab_par_param_offset++;
        CT_RETURN_IFERR(par_exp_dispatch_s(&mgr->thread_ctrls[i], &mgr->par_proc_param, mgr->bin_rec_total_add,
                                           &mgr->exp_cols));
    }
    return CT_SUCCESS;
}
...
static status_t par_exp_data_file_name(par_exp_mgr_t *mgr, exp_cache_t *table_cache)
{
    ...
    if (g_export_opts.filetype == FT_BIN) {
        ...
        for (thread_no = 0; thread_no < mgr->options.parallel; thread_no++) {
            for (filename_pos = 0; filename_pos < mgr->thread_ctrls[thread_no].exp_files.count; filename_pos++) {
                CT_RETURN_IFERR(exp_write_str_s("@@ ", &g_exp_txtbuf, g_exp_dpfile));
                CT_RETURN_IFERR(exp_write_str_s(CTSQL_SEC_FILE_NAME, &g_exp_txtbuf, g_exp_dpfile));
                CT_RETURN_IFERR(exp_write_str_s("/", &g_exp_txtbuf, g_exp_dpfile));
                CT_RETURN_IFERR(exp_write_str_s((char *)cm_list_get(&mgr->thread_ctrls[thread_no].exp_files,
...
static status_t exp_prepare_column_info(ctconn_stmt_t *stmt)
{
    char cmd_buf[CTSQL_MAX_TEMP_SQL + 1];
    ...
    PRTS_RETURN_IFERR(sprintf_s(cmd_buf, CTSQL_MAX_TEMP_SQL,
        ...
        (ctconn_get_call_version(CONN) >= CS_VERSION_24) ? ", IS_JSONB" : " ",
        exp_tabname(g_export_opts.consistent, EXP_TABAGENT_DB_TAB_COLS)));

    CT_RETURN_IFERR(ctconn_alloc_stmt(CONN, stmt));
    CT_RETURN_IFERR(ctconn_prepare(*stmt, (const char *)cmd_buf));

    return CT_SUCCESS;
}

static status_t exp_prepare_table_func_indexes(ctconn_stmt_t *stmt)
{
    char cmd_buf[CTSQL_MAX_TEMP_SQL + 1];
...
static status_t exp_prepare_table_has_intervalpart(ctconn_stmt_t *stmt)
{
    char cmd_buf[CTSQL_MAX_TEMP_SQL + 1];
...
static status_t exp_prepare_index_partitioning(ctconn_stmt_t *stmt)
{
    char cmd_buf[CTSQL_MAX_TEMP_SQL + 1];
...
static status_t exp_prepare_index_subpartition(ctconn_stmt_t *stmt)
{
    char cmd_buf[CTSQL_MAX_TEMP_SQL + 1];
...
static int exp_prepare_has_subpartition(ctconn_stmt_t *stmt)
{
    char cmd_buf[CTSQL_MAX_TEMP_SQL + 1] = { 0 };
...
status_t exp_shd_decrypt_pwd_with_key(exp_shd_info_t *dn_info)
{
    ...
    uint32 passwd_len = CT_PASSWORD_BUFFER_SIZE;
    status_t ret = CT_SUCCESS;

    CT_RETURN_IFERR(ctsql_init_home());
    if (ctsql_get_local_server_kmc_ksf(CT_HOME) != CT_SUCCESS) {
        return CT_ERROR;
    }
    ...
    if (cm_decrypt_passwd_with_key_by_kmc(&aes_kmc) != CT_SUCCESS) {
        CTSQL_PRINTF(ZSERR_CTSQL, "exp failed");
        ret = CT_ERROR;
    }
...
status_t par_exp_create_dn_connectdb(ctsql_conn_info_t *dn_conn_info, bool32 is_get_pwd)
{
    uint16 charset_id;
    int32 execute_ret;
    bool32 interactive_clt = CT_FALSE;
    bool32 flag_with_ts = CT_TRUE;
    uint32 new_num_width = (uint32)CT_MAX_DEC_OUTPUT_ALL_PREC;
    /* Re-establishing connections is not supported */
    if (dn_conn_info->conn == NULL) {
        execute_ret = ctsql_alloc_conn(&dn_conn_info->conn);
        if (execute_ret != CT_SUCCESS) {
        ...
        /* set session interactive check disable */
        (void)ctconn_set_conn_attr(dn_conn_info->conn, CTCONN_ATTR_INTERACTIVE_MODE, (void *)&interactive_clt, 0);
        (void)ctconn_set_conn_attr(dn_conn_info->conn, CTCONN_ATTR_FLAG_WITH_TS, (void *)&flag_with_ts, 0);
        (void)ctconn_set_conn_attr(dn_conn_info->conn, CTCONN_ATTR_NUM_WIDTH, (void *)&new_num_width, sizeof(uint32));
        if (is_get_pwd) {
            // pwd , clean after conn_to_server
            CT_RETURN_IFERR(ctsql_get_saved_pswd(dn_conn_info->passwd, EXP_MAX_PASSWD));
        }

        execute_ret = ctsql_conn_to_server(dn_conn_info, CT_FALSE, CT_TRUE);
        if (execute_ret != CT_SUCCESS) {
            exp_tmlog("Connect to server failed\n");
            return execute_ret;
        }
        ctconn_get_charset(STMT, &charset_id);
        CT_RETURN_IFERR(ctconn_set_charset(dn_conn_info->stmt, charset_id));
    }

    return CT_SUCCESS;
}

status_t exp_clean_pwd_info(ctsql_conn_info_t *dn_conn_info)
{
    // clear user and pwd
...
static status_t exp_init_conn_info(export_options_t *opt)
{
    ...
    char login_username[CT_NAME_BUFFER_SIZE + CT_STR_RESERVED_LEN] = { 0 };
    ctsql_get_saved_user(login_username, EXP_MAX_USERNAME);

    {
        // using local connected info
        CT_RETURN_IFERR(cm_list_new(&opt->dn_info.dn_info_list, (void **)&node_info));
        node_info->dn_scn = CT_INVALID_ID64;
    ...
    exp_cache_t* table_cache)
{
    ...
    exp_tab_dist_info_t tab_dist_info;
    MEMS_RETURN_IFERR(memset_s(&tab_dist_info, sizeof(exp_tab_dist_info_t), 0, sizeof(exp_tab_dist_info_t)));
    par_mgr->par_proc_param.is_coordinator = CT_FALSE;
    ...
    for (uint32 num = 0; num < exp_opts->dn_info.dn_info_list.count; num++) {
        ...
        while (par_exp_check_thread_stat(par_mgr, &node_info->dn_par_conn, &ret) == CT_FALSE) {
            if (ret != CT_SUCCESS) {
                break;
            }
            cm_sleep(5);
        ...
        CT_RETURN_IFERR(par_exp_data_file_name(par_mgr, table_cache));
    }
    ...
    const char *table, exp_cache_t *table_cache)
{
    ...
    CT_RETURN_IFERR(exp_init_tab_info(ctx->user, table, ctx, &tab_info));

    // 1.Use global connect to export metadata
    if (ctx->exp_opts->content & CT_EXP_META) {
        CT_RETURN_IFERR(exp_tab_meta(ctx->exp_opts, &tab_info, table_cache));
        CT_RETURN_IFERR(table_cache_write_txt_tab_meta(table_cache));
    }
    ...
    if (par_mgr->exp_cols.init_cols == CT_TRUE) {
        exp_reset_exporter(&par_mgr->exp_cols.exporter);
    }
    par_mgr->exp_cols.init_cols = CT_FALSE;

    // 2.3.4 steps need to deal together
    CT_RETURN_IFERR(par_exp_table_dn_records(ctx->exp_opts, &tab_info, par_mgr, table_cache));

    // 5.Use global connect to export table index metadata
    if (ctx->exp_opts->content & CT_EXP_META) {
        CT_RETURN_IFERR(exp_start_cache_unit(table_cache, EXP_CACHE_TABLE_INDEX));
        CT_RETURN_IFERR(exp_table_auto_increment(ctx->user, table, table_cache));
        CT_RETURN_IFERR(exp_tab_indx_meta(ctx, table, table_cache));
        CT_RETURN_IFERR(table_cache_write_txt_tab_index_meta(table_cache));
    }
...
static status_t exp_single_table_serial(exp_tabs_ctx_t *ctx, const char *user, const char *table, exp_cache_t *table_cache)
{
    ...
    CT_RETURN_IFERR(exp_init_tab_info(user, table, ctx, &tab_info));
    ...
    if ((ctx->exp_opts->content & CT_EXP_DATA)) {
        ...
        if (TABLE_IS_TEMP(tab_info.table_type)) {
            ...
            CT_RETURN_IFERR(exp_table_records(ctx->exp_opts, user, table, &exporter, table_cache));
        }
    }
    ...
    if (ctx->exp_opts->content & CT_EXP_META) {
        CT_RETURN_IFERR(exp_start_cache_unit(table_cache, EXP_CACHE_TABLE_INDEX));
        CT_RETURN_IFERR(exp_table_auto_increment(user, table, table_cache));
        CT_RETURN_IFERR(exp_tab_indx_meta(ctx, table, table_cache));
        CT_RETURN_IFERR(table_cache_write_txt_tab_index_meta(table_cache));
    }
...
static int exp_tables_serial(exp_tabs_ctx_t *ctx, exp_cache_t* table_cache)
{
    ...
    for (uint32 i = 0; i < ctx->tables->count; i++) {
        ...
        if (ctx->exp_opts->exp_type == EXP_TABLE &&
            ...
            CT_SUCCESS != exp_tablespace_filter(ctx->exp_opts, ctx->user, one_table)) {
            continue;
        }
        ...
        if (g_export_opts.filetype == FT_TXT) {
            CT_BREAK_IF_ERROR(exp_write_str_s("\n", &g_exp_txtbuf, g_exp_dpfile));
        }
    }
    return ret;
}

static status_t reverse_index_available(ctconn_stmt_t stmt, bool32 *reverse_index_aval)
{
    uint32 rows;
    char *get_par_sql = (char *)malloc(CTSQL_MAX_TEMP_SQL);
    ...
    int iret_sprintf = sprintf_s(get_par_sql, CTSQL_MAX_TEMP_SQL,
...
static status_t exp_init_tabs_ctx(const char *user, list_t *table_list, export_options_t *exp_opts, exp_tabs_ctx_t *ctx)
{
    ...
    CT_RETURN_IFERR(reverse_index_available(STMT, &ctx->reverse_index_available));
    CT_RETURN_IFERR(exp_prepare_column_info(&ctx->query_tab_column));
    CT_RETURN_IFERR(exp_prepare_table_func_indexes(&ctx->query_func_indexes));
    CT_RETURN_IFERR(exp_prepare_table_has_intervalpart(&ctx->query_tab_has_intervalpart));
    if (exp_need_index_partition()) {
        CT_RETURN_IFERR(exp_prepare_index_partitioning(&ctx->query_index_partitioning));
        CT_RETURN_IFERR(exp_prepare_index_subpartition(&ctx->query_index_subpartition));
        CT_RETURN_IFERR(exp_prepare_has_subpartition(&ctx->query_has_subpartition));
    }
    return CT_SUCCESS;
}
...
{
    ctconn_free_stmt(ctx->query_tab_column);
    ctconn_free_stmt(ctx->query_func_indexes);
    ctconn_free_stmt(ctx->query_tab_has_intervalpart);
    if (exp_need_index_partition()) {
        ctconn_free_stmt(ctx->query_index_partitioning);
        ctconn_free_stmt(ctx->query_index_subpartition);
        ctconn_free_stmt(ctx->query_has_subpartition);
    }
}
...
static status_t exp_table_scripts(export_options_t *exp_opts, const char *user, list_t *table_list)
{
    ...
    */
    CT_RETURN_IFERR(init_exp_cache(&table_cache, EXP_MAX_UNIT_CNT_PER_TABLE + g_export_opts.dn_info.dn_info_list.count,
        EXP_CACHE_ALL_TABLE));

    // init context for all tables.
    if (exp_init_tabs_ctx(user, table_list, exp_opts, &ctx) != CT_SUCCESS) {
        exp_uninit_tabs_ctx(&ctx);
        return CT_ERROR;
    }

    // prompt
    if ((exp_opts->parallel > CT_MAX_PAR_EXP_VALUE) || (exp_opts->parallel == 1)) {
        exp_tmlog("Parallelism setting range is not between 2~%u, ignoring parallel execution parameters\n",
                  (uint32)CT_MAX_PAR_EXP_VALUE);
    }

    if ((exp_opts->content & CT_EXP_DATA) && (exp_opts->parallel > 1) &&
        (exp_opts->parallel <= CT_MAX_PAR_EXP_VALUE)) {
...
static inline int exp_user_tables(export_options_t *exp_opts, const char *user)
{
    ...
    exp_create_objlist(&user_tables, CTSQL_MAX_OBJECT_LEN);
    do {
...
static inline int exp_get_user_views(const char *user, list_t *user_views)
{
    ...
    CT_RETURN_IFERR(EXP_PREPARE_SQL(EXP_GET_USER_VIEW_LIST, cmd_buf, CTSQL_MAX_TEMP_SQL, &param));

    CT_RETURN_IFERR(ctconn_prepare(STMT, (const char *)cmd_buf));
    CT_RETURN_IFERR(ctconn_bind_by_pos(STMT, 0, CTCONN_TYPE_STRING, user, (int32)strlen(user), NULL));
    CT_RETURN_IFERR(ctconn_bind_by_pos(STMT, 1, CTCONN_TYPE_STRING, user, (int32)strlen(user), NULL));
    CT_RETURN_IFERR(ctconn_bind_by_pos(STMT, 2, CTCONN_TYPE_STRING, user, (int32)strlen(user), NULL));
    ...
    do {
        ...
        CT_RETURN_IFERR(ctconn_column_as_string(STMT, 0, g_str_buf, CT_MAX_PACKET_SIZE));
        if (cm_list_new(user_views, (pointer_t *)&ptr) != CT_SUCCESS) {
            return CT_ERROR;
        }
        MEMS_EXP_RETURN_IFERR(strncpy_s(ptr, CTSQL_MAX_OBJECT_LEN, g_str_buf, CTSQL_MAX_OBJECT_LEN - 1));
    } while (CT_TRUE);
...
static inline int exp_view_columns(const char *user, const char *view, exp_cache_t *view_cache)
{
    ...
    CT_RETURN_IFERR(ctconn_bind_by_pos(STMT, 0, CTCONN_TYPE_STRING, user, (int32)strlen(user), NULL));
    CT_RETURN_IFERR(ctconn_bind_by_pos(STMT, 1, CTCONN_TYPE_STRING, view, (int32)strlen(view), NULL));

    CT_RETURN_IFERR(ctconn_execute(STMT));

    CT_RETURN_IFERR(exp_cache_append_str(view_cache, "(\n"));
    do {
        CT_RETURN_IFERR(ctconn_fetch(STMT, &rows));
        if (rows == 0) {
            break;
        ...
        CT_RETURN_IFERR(exp_cache_append_str(view_cache, EXP_INDENT));
        CT_RETURN_IFERR(ctconn_column_as_string(STMT, 0, g_str_buf, CT_MAX_PACKET_SIZE));
        CT_RETURN_IFERR(exp_cache_append_str_quote(view_cache, g_str_buf));
    } while (CT_TRUE);
    CT_RETURN_IFERR(exp_cache_append_str(view_cache, "\n)"));
...
static inline int exp_write_schema_lob(ctconn_stmt_t stmt, int col_id, char *buf, uint32 buf_size, exp_cache_t *exp_cache)
{
    uint32 offset = 0;
    uint32 nchars = 0;
    uint32 nbytes;
    ...
    do {
        CT_RETURN_IFERR(ctconn_read_clob_by_id(stmt, col_id, offset,
                                            (void *)buf, buf_size, &nchars, &nbytes, &eof));

        src_text.len = nbytes;
        CM_NULL_TERM(&src_text);
        if (exp_cache == NULL) {
            CT_RETURN_IFERR(exp_write_schema_com(buf, &g_exp_txtbuf, g_exp_dpfile));
        } else {
            CT_RETURN_IFERR(exp_cache_append_str(exp_cache, buf));
        }
        offset += nbytes;
    } while (!eof);
    ...
    if (!CM_IS_EMPTY(&src_text) && CM_TEXT_END(&src_text) != '/') {
        if (exp_cache == NULL) {
            CT_RETURN_IFERR(exp_write_schema_com("\n/\n", &g_exp_txtbuf, g_exp_dpfile));
        } else {
            CT_RETURN_IFERR(exp_cache_append_str(exp_cache, "\n/\n"));
        }
    }
...
static inline int exp_single_view(export_options_t *exp_opts, const char *user, const char *view, exp_cache_t *view_cache)
{
    ...
    // record view name
    CT_RETURN_IFERR(exp_start_cache_unit(view_cache, EXP_CACHE_VIEW_NAME));
    exp_cache_append_str_quote(view_cache, view);
    // record view columns
    CT_RETURN_IFERR(exp_start_cache_unit(view_cache, EXP_CACHE_VIEW_COLUMNS));
    CT_RETURN_IFERR(exp_view_columns(user, view, view_cache));
    // record view source
    CT_RETURN_IFERR(exp_start_cache_unit(view_cache, EXP_CACHE_VIEW_SRC));
...
static inline int exp_view_scripts(export_options_t *exporter, const char *user, list_t *user_views)
{
    ...
    CT_RETURN_IFERR(init_exp_cache(&view_cache, EXP_MAX_UNIT_CNT_PER_VIEW, EXP_CACHE_VIEW));
...
static inline int exp_user_views(export_options_t *exp_opts, const char *user)
{
    ...
    exp_create_objlist(&user_views, CTSQL_MAX_OBJECT_LEN);
    do {
...
static status_t exp_user_synonyms(export_options_t *exp_opts, const char *user)
{
    ...
    CT_RETURN_IFERR(EXP_PREPARE_SQL(EXP_GET_USER_SYNONYM_LIST, cmd_buf, CTSQL_MAX_TEMP_SQL, &param));
    ...
    do {
        ...
        CT_RETURN_IFERR(exp_write_schema_com(g_str_buf, &g_exp_txtbuf, g_exp_dpfile));
        CT_RETURN_IFERR(exp_write_schema_com("\n/\n", &g_exp_txtbuf, g_exp_dpfile));
...
static inline int exp_get_user_procs(export_options_t *exp_opts, const char *user, list_t *user_procs)
{
    ...
    CT_RETURN_IFERR(EXP_PREPARE_SQL(EXP_GET_USER_PROC_LIST, cmd_buf, CTSQL_MAX_TEMP_SQL, &param));
    ...
    do {
        ...
        CT_RETURN_IFERR(ctconn_get_column_by_id(STMT, 0, (void**)&obj_name, &name_len, NULL));
        CT_RETURN_IFERR(ctconn_get_column_by_id(STMT, 1, (void**)&obj_type, &type_len, NULL));
        CT_RETURN_IFERR(cm_list_new(user_procs, (pointer_t *)&ptr));
    ...
    exp_cache_t *obj_cache)
{
    ...
    CT_RETURN_IFERR(ctconn_bind_by_pos(STMT, 0, CTCONN_TYPE_STRING, obj->obj_name, (int32)strlen(obj->obj_name), NULL));
    CT_RETURN_IFERR(ctconn_bind_by_pos(STMT, 1, CTCONN_TYPE_STRING, obj->obj_type, (int32)strlen(obj->obj_type), NULL));
    if (!exp_opts->is_myself) {
    ...
    CT_RETURN_IFERR(ctconn_column_as_string(STMT, 0, g_str_buf, CT_MAX_PACKET_SIZE));
    CT_RETURN_IFERR(exp_start_cache_unit(obj_cache, EXP_CACHE_OBJ_TYPE));
    CT_RETURN_IFERR(exp_cache_append_str(obj_cache, g_str_buf));

    exp_log(EXP_INDENT "exporting %s %s.%s\n", g_str_buf, user, obj->obj_name);

    CT_RETURN_IFERR(ctconn_column_as_string(STMT, 1, g_str_buf, CT_MAX_PACKET_SIZE));
    CT_RETURN_IFERR(exp_start_cache_unit(obj_cache, EXP_CACHE_OBJ_NAME));
    if (!CM_IS_EMPTY_STR(g_str_buf)) {
        CT_RETURN_IFERR(exp_cache_append_str_quote(obj_cache, g_str_buf));
        CT_RETURN_IFERR(exp_cache_append_str(obj_cache, "."));
    }
    CT_RETURN_IFERR(exp_cache_append_str_quote(obj_cache, obj->obj_name));

    CT_RETURN_IFERR(exp_start_cache_unit(obj_cache, EXP_CACHE_OBJ_SRC));
    CT_RETURN_IFERR(exp_write_schema_lob(STMT, 2, g_str_buf, CLT_LOB_BUFFER_SIZE, obj_cache));
...
static status_t exp_proc_scripts(export_options_t *exp_opts, const char *user, list_t *user_procs)
{
    ...
    CT_RETURN_IFERR(init_exp_cache(&obj_cache, EXP_MAX_UNIT_CNT_PER_OBJ, EXP_CACHE_OBJ));
...
static inline int exp_grant_role(export_options_t *exp_opts, const char *schema_name)
{
    ...
    CT_RETURN_IFERR(ctconn_bind_by_pos(STMT, 0, CTCONN_TYPE_STRING, schema_name, (int32)strlen(schema_name), NULL));
    CT_RETURN_IFERR(ctconn_execute(STMT));
...
static inline int exp_grant_privilege2role(export_options_t *exp_opts)
{
    ...
    if (exp_opts->exp_type == EXP_ALL_SCHEMAS) {
        ...
        for (uint32 i = 0; i < exp_opts->obj_list.count; i++) {
            if (i > 0) {
                MEMS_RETURN_IFERR(strcat_s(cmd_buf, CTSQL_MAX_TEMP_SQL, ", "));
            }

            user = (char *)cm_list_get(&exp_opts->obj_list, i);
            MEMS_RETURN_IFERR(strcat_s(cmd_buf, CTSQL_MAX_TEMP_SQL, "UPPER('"));
            MEMS_RETURN_IFERR(strcat_s(cmd_buf, CTSQL_MAX_TEMP_SQL, user));
            MEMS_RETURN_IFERR(strcat_s(cmd_buf, CTSQL_MAX_TEMP_SQL, "')"));
        }
        MEMS_RETURN_IFERR(strcat_s(cmd_buf, CTSQL_MAX_TEMP_SQL,
            ")  AND GRANTED_ROLE NOT IN ('DBA','RESOURCE','CONNECT') ORDER BY GRANTED_ROLE) ORDER BY PRIVILEGE"));
...
static inline int exp_user_definition(export_options_t *exp_opts, const char *schema_name, bool32 *alter_tenant)
{
    ...
    char schema_buf[CT_NAME_BUFFER_SIZE];
    ...
    if (strchr(schema_name, '$') != NULL) {
        ...
        CT_RETURN_IFERR(cm_text2str(&schema, schema_buf, CT_NAME_BUFFER_SIZE));
        CT_RETURN_IFERR(cm_text2str(&tenant, tenant_buf, CT_TENANT_BUFFER_SIZE));
        CT_RETURN_IFERR(exp_write_schema_com("ALTER SESSION SET TENANT = ", &g_exp_txtbuf, g_exp_dpfile));
        exp_write_fmt(CT_TRUE, EXP_WRITE_FMT_100, "%s;\n", tenant_buf);
        *alter_tenant = CT_TRUE;
    } else {
        MEMS_RETURN_IFERR(strcpy_s(schema_buf, CT_NAME_BUFFER_SIZE, schema_name));
    }
    ...
    CT_RETURN_IFERR(ctconn_column_as_string(STMT, 0, g_str_buf, CT_MAX_PACKET_SIZE));
    exp_write_fmt(CT_TRUE, EXP_WRITE_FMT_100, "CREATE USER \"%s\" ", schema_buf);
    CT_RETURN_IFERR(exp_write_schema_com(g_str_buf, &g_exp_txtbuf, g_exp_dpfile));
    CT_RETURN_IFERR(exp_write_schema_com("\n", &g_exp_txtbuf, g_exp_dpfile));
...
static inline int exp_user_role(export_options_t *exp_opts)
{
    ...
    CT_RETURN_IFERR(EXP_PREPARE_SQL(EXP_GET_ROLE_LIST, cmd_buf, CTSQL_MAX_TEMP_SQL, &param));
...
static int exp_user_profile(export_options_t *exp_opts, const char *user)
{
    ...
    CT_RETURN_IFERR(EXP_PREPARE_SQL(EXP_GET_USER_PROFILE_LIST, cmd_buf, CTSQL_MAX_TEMP_SQL, &param));
...
static int exp_user_package(export_options_t *exp_opts, const char *user)
{
    ...
    CT_RETURN_IFERR(EXP_PREPARE_SQL(EXP_GET_USER_PACKAGE_LIST, cmd_buf, CTSQL_MAX_TEMP_SQL, &param));
    ...
    do {
        ...
        (void)exp_bin_memory_mgr_sub_begin(&exp_opts->master_bin_mgr, exp_opts->filetype);
        CT_RETURN_IFERR(exp_write_schema_lob(STMT, 0, g_str_buf, CLT_LOB_BUFFER_SIZE, NULL));
        CT_RETURN_IFERR(exp_write_schema_com("\n", &g_exp_txtbuf, g_exp_dpfile));
        (void)exp_bin_write_int32(&exp_opts->master_bin_mgr, EXP_OBJECT_END_FLAG);
        (void)exp_bin_memory_mgr_sub_end(&exp_opts->master_bin_mgr, exp_opts->filetype);
...
static int exp_user_type(export_options_t *exp_opts, const char *user)
{
    ...
    CT_RETURN_IFERR(EXP_PREPARE_SQL(EXP_GET_USER_TYPE_LIST, cmd_buf, CTSQL_MAX_TEMP_SQL, &param));
    CT_RETURN_IFERR(ctconn_prepare(STMT, (const char *)cmd_buf));
    CT_RETURN_IFERR(ctconn_execute(STMT));
...
status_t exp_sequence(export_options_t *exp_opts, const char *schema_name, uint32 *total)
{
    ...
    while (rows > 0) {
        (void)exp_bin_memory_mgr_sub_begin(&exp_opts->master_bin_mgr, exp_opts->filetype);
        inc_total(total);
        (void)ctconn_column_as_string(STMT, SEQUENCE_NAME, g_str_buf, CT_MAX_PACKET_SIZE);
        // write drop sequence
        CT_RETURN_IFERR(exp_write_schema_com("DROP SEQUENCE IF EXISTS ", &g_exp_txtbuf, g_exp_dpfile));
        exp_write_schema_quote(g_str_buf);
        CT_RETURN_IFERR(exp_write_schema_com(";\n", &g_exp_txtbuf, g_exp_dpfile));

        CT_RETURN_IFERR(exp_write_schema_com("CREATE SEQUENCE ", &g_exp_txtbuf, g_exp_dpfile));
        exp_write_schema_quote(g_str_buf);

        (void)ctconn_column_as_string(STMT, MIN_VALUE, g_str_buf, CT_MAX_PACKET_SIZE);
        iret_sprintf = sprintf_s(cmd_buf, CTSQL_MAX_TEMP_SQL, " MINVALUE %s ", g_str_buf);
        if (iret_sprintf == -1) {
            EXP_THROW_ERROR(ERR_SYSTEM_CALL, iret_sprintf);
            return CT_ERROR;
        }
        CT_RETURN_IFERR(exp_write_schema_com(cmd_buf, &g_exp_txtbuf, g_exp_dpfile));

        (void)ctconn_column_as_string(STMT, MAX_VALUE, g_str_buf, CT_MAX_PACKET_SIZE);
        iret_sprintf = sprintf_s(cmd_buf, CTSQL_MAX_TEMP_SQL, "MAXVALUE %s ", g_str_buf);
        if (iret_sprintf == -1) {
            EXP_THROW_ERROR(ERR_SYSTEM_CALL, iret_sprintf);
            return CT_ERROR;
        }
        ...
        (void)ctconn_column_as_string(STMT, LAST_NUMBER, g_str_buf, CT_MAX_PACKET_SIZE);
        iret_sprintf = sprintf_s(cmd_buf, CTSQL_MAX_TEMP_SQL, "START WITH %s ", g_str_buf);
        if (iret_sprintf == -1) {
            EXP_THROW_ERROR(ERR_SYSTEM_CALL, iret_sprintf);
            return CT_ERROR;
        }
        ...
        (void)ctconn_column_as_string(STMT, INCREMENT_BY, g_str_buf, CT_MAX_PACKET_SIZE);
        iret_sprintf = sprintf_s(cmd_buf, CTSQL_MAX_TEMP_SQL, "INCREMENT BY %s ", g_str_buf);
        if (iret_sprintf == -1) {
            EXP_THROW_ERROR(ERR_SYSTEM_CALL, iret_sprintf);
            return CT_ERROR;
        }
        ...
        (void)ctconn_column_as_string(STMT, CYCLE_FLAG, g_str_buf, CT_MAX_PACKET_SIZE);

        if (g_str_buf[0] == '1') {
            iret_sprintf = sprintf_s(cmd_buf, CTSQL_MAX_TEMP_SQL, "CYCLE ");
            if (iret_sprintf == -1) {
                EXP_THROW_ERROR(ERR_SYSTEM_CALL, iret_sprintf);
                return CT_ERROR;
            }
        ...
        (void)ctconn_column_as_string(STMT, CACHE_SIZE, g_str_buf, CT_MAX_PACKET_SIZE);

        if (g_str_buf[0] == '0') {
            iret_sprintf = sprintf_s(cmd_buf, CTSQL_MAX_TEMP_SQL, "NOCACHE ");
        } else {
            iret_sprintf = sprintf_s(cmd_buf, CTSQL_MAX_TEMP_SQL, "CACHE %s ", g_str_buf);
        }
        if (iret_sprintf == -1) {
            EXP_THROW_ERROR(ERR_SYSTEM_CALL, iret_sprintf);
            return CT_ERROR;
        ...
        (void)ctconn_column_as_string(STMT, ORDER_FLAG, g_str_buf, CT_MAX_PACKET_SIZE);

        if (g_str_buf[0] == '1') {
            iret_sprintf = sprintf_s(cmd_buf, CTSQL_MAX_TEMP_SQL, "ORDER ");
            if (iret_sprintf == -1) {
                EXP_THROW_ERROR(ERR_SYSTEM_CALL, iret_sprintf);
                return CT_ERROR;
            }
...
status_t exp_single_schema(export_options_t *exp_opts, const char *schema_name)
{
    ...
    CT_RETURN_IFERR(exp_bin_memory_mgr_begin(&exp_opts->master_bin_mgr, exp_opts->filetype));  // total len
    obj_total = (uint32 *)exp_bin_write_int32(&exp_opts->master_bin_mgr, (uint32)1);              // object total
    (void)exp_bin_memory_mgr_sub_begin(&exp_opts->master_bin_mgr, exp_opts->filetype);

    if (exp_opts->create_user == CT_TRUE) {
    ...
    CT_RETURN_IFERR(exp_write_schema_com("\n", &g_exp_txtbuf, g_exp_dpfile));
    CT_RETURN_IFERR(exp_write_schema_com("ALTER SESSION SET CURRENT_SCHEMA = ", &g_exp_txtbuf, g_exp_dpfile));
    exp_write_fmt(CT_TRUE, EXP_WRITE_FMT_100, "%s;\n", schema_name);
    ...
    CT_RETURN_IFERR(exp_bin_memory_mgr_begin(&exp_opts->master_bin_mgr, exp_opts->filetype));
    ...
    if (alter_tenant) {
        CT_RETURN_IFERR(exp_write_schema_com("ALTER SESSION SET TENANT = TENANT$ROOT;\n", &g_exp_txtbuf,
            g_exp_dpfile));
    }
...
static inline int exp_schema(export_options_t *exp_opts)
{
    ...
    CT_RETURN_IFERR(exp_check_privilege(exp_opts));
...
static inline int exp_all_schemas(export_options_t *exp_opts)
{
    ...
    CT_RETURN_IFERR(exp_check_privilege(exp_opts));
    CT_RETURN_IFERR(exp_get_users(exp_opts));
...
{
    CT_RETURN_IFERR(ctconn_column_as_string(STMT, 0, g_str_buf, CT_MAX_PACKET_SIZE));
    exp_write_fmt(CT_FALSE, EXP_FMT_BUF_SZ, "'%s'", &g_str_buf);
    CT_RETURN_IFERR(exp_write_str_s(" SIZE ", &g_exp_txtbuf, g_exp_dpfile));
    ...
    if (ctconn_get_call_version(CONN) >= CS_VERSION_22) {
        ...
        if (strcmp((char *)"TRUE", g_str_buf) == 0) {
            CT_RETURN_IFERR(exp_write_str_s(" COMPRESS", &g_exp_txtbuf, g_exp_dpfile));
        }
    }

    CT_RETURN_IFERR(ctconn_column_as_string(STMT, 2, g_str_buf, CT_MAX_PACKET_SIZE));
    if (cm_compare_str((char *)"YES", g_str_buf) == 0) {
        CT_RETURN_IFERR(exp_write_str_s(" AUTOEXTEND ON NEXT ", &g_exp_txtbuf, g_exp_dpfile));
        CT_RETURN_IFERR(ctconn_column_as_string(STMT, 3, g_str_buf, CT_MAX_PACKET_SIZE));
        CT_RETURN_IFERR(exp_write_str_s(g_str_buf, &g_exp_txtbuf, g_exp_dpfile));
        CT_RETURN_IFERR(exp_write_str_s(" MAXSIZE ", &g_exp_txtbuf, g_exp_dpfile));
        CT_RETURN_IFERR(ctconn_column_as_string(STMT, 4, g_str_buf, CT_MAX_PACKET_SIZE));
        CT_RETURN_IFERR(exp_write_str_s(g_str_buf, &g_exp_txtbuf, g_exp_dpfile));
    }

    if (ctconn_get_call_version(CONN) >= CS_VERSION_22 && is_first) {
        CT_RETURN_IFERR(ctconn_column_as_string(STMT, 6, g_str_buf, CT_MAX_PACKET_SIZE));
        if (cm_compare_str((char *)"MAP", g_str_buf) == 0) {
            CT_RETURN_IFERR(exp_write_str_s(" EXTENT AUTOALLOCATE", &g_exp_txtbuf, g_exp_dpfile));
        }
    }
...
static inline int exp_single_tablespaces(char *tbspaces)
{
    ...
    CT_RETURN_IFERR(ctconn_bind_by_pos(STMT, 0, CTCONN_TYPE_STRING, tbspaces, (int32)strlen(tbspaces), NULL));
    CT_RETURN_IFERR(ctconn_execute(STMT));
    ...
    do {
        ...
        if (IS_FIRST) {
            CT_RETURN_IFERR(exp_write_str_s("CREATE TABLESPACE ", &g_exp_txtbuf, g_exp_dpfile));
            exp_write_fmt(CT_FALSE, EXP_WRITE_FMT_100, "\"%s\"", tbspaces);
            CT_RETURN_IFERR(exp_write_str_s(" DATAFILE ", &g_exp_txtbuf, g_exp_dpfile));
            CT_RETURN_IFERR(exp_create_tablespaces(IS_FIRST));
            IS_FIRST = CT_FALSE;
        } else {
            CT_RETURN_IFERR(exp_write_str_s("ALTER TABLESPACE ", &g_exp_txtbuf, g_exp_dpfile));
            exp_write_fmt(CT_FALSE, EXP_WRITE_FMT_100, "\"%s\"", tbspaces);
            CT_RETURN_IFERR(exp_write_str_s(" ADD DATAFILE ", &g_exp_txtbuf, g_exp_dpfile));
            CT_RETURN_IFERR(exp_create_tablespaces(IS_FIRST));
        }
    } while (CT_TRUE);
...
static inline int exp_get_all_tbspace(list_t *tmp_tbsname_list)
{
    ...
    do {
        ...
        CT_RETURN_IFERR(ctconn_column_as_string(STMT, 0, g_str_buf, CT_MAX_PACKET_SIZE));
        if (cm_list_new(tmp_tbsname_list, (pointer_t *)&ptr) != CT_SUCCESS) {
...
static inline int exp_all_tablespaces(void)
{
    ...
    exp_create_objlist(&tmp_tbsname_list, CTSQL_MAX_OBJECT_LEN);
...
static inline int exp_single_tenant(char *tenant)
{
    ...
    iret_sprintf = sprintf_s(cmd_buf, CTSQL_MAX_TEMP_SQL, "SELECT DEFAULT_TABLESPACE FROM ADM_TENANTS "
                             "WHERE NAME = :tenant");
    if (iret_sprintf == -1) {
        EXP_THROW_ERROR(ERR_SYSTEM_CALL, iret_sprintf);
        return CT_ERROR;
    ...
    CT_RETURN_IFERR(ctconn_prepare(STMT, (const char *)cmd_buf));
    CT_RETURN_IFERR(ctconn_bind_by_pos(STMT, 0, CTCONN_TYPE_STRING, tenant, (int32)strlen(tenant), NULL));
    CT_RETURN_IFERR(ctconn_execute(STMT));
    CT_RETURN_IFERR(ctconn_fetch(STMT, &rows));
    if (rows == 0) {
    ...
    CT_RETURN_IFERR(ctconn_column_as_string(STMT, 0, default_space, CT_NAME_BUFFER_SIZE));

    iret_sprintf = sprintf_s(cmd_buf, CTSQL_MAX_TEMP_SQL, "SELECT TABLESPACE_NAME FROM "EXP_DV_TENANT_TABLESPACES
    ...
    CT_RETURN_IFERR(ctconn_prepare(STMT, (const char *)cmd_buf));
    CT_RETURN_IFERR(ctconn_bind_by_pos(STMT, 0, CTCONN_TYPE_STRING, tenant, (int32)strlen(tenant), NULL));
    CT_RETURN_IFERR(ctconn_bind_by_pos(STMT, 1, CTCONN_TYPE_STRING, default_space, (int32)strlen(default_space), NULL));
    CT_RETURN_IFERR(ctconn_execute(STMT));

    CT_RETURN_IFERR(exp_write_str_s("CREATE TENANT ", &g_exp_txtbuf, g_exp_dpfile));
    exp_write_fmt(CT_FALSE, EXP_WRITE_FMT_100, "\"%s\"", tenant);
    CT_RETURN_IFERR(exp_write_str_s(" TABLESPACES(", &g_exp_txtbuf, g_exp_dpfile));
    exp_write_fmt(CT_FALSE, EXP_WRITE_FMT_100, "\"%s\"", default_space);
    do {
        CT_RETURN_IFERR(ctconn_fetch(STMT, &rows));
        if (rows == 0) {
            break;
        ...
        CT_RETURN_IFERR(ctconn_column_as_string(STMT, 0, g_str_buf, CT_MAX_PACKET_SIZE));
        exp_write_fmt(CT_FALSE, EXP_WRITE_FMT_100, ",\"%s\"", g_str_buf);
    } while (CT_TRUE);
    CT_RETURN_IFERR(exp_write_str_s(");\n", &g_exp_txtbuf, g_exp_dpfile));
    return CT_SUCCESS;
}
...
static inline int exp_get_all_tenant(list_t *tmp_tenant_list)
{
    ...
    do {
        ...
        CT_RETURN_IFERR(ctconn_column_as_string(STMT, 0, g_str_buf, CT_MAX_PACKET_SIZE));
        if (cm_list_new(tmp_tenant_list, (pointer_t *)&ptr) != CT_SUCCESS) {
...
static inline int exp_all_tenants(void)
{
    ...
    exp_create_objlist(&tmp_tenant_list, CT_TENANT_BUFFER_SIZE);
...
static inline int exp_prepare(export_options_t *exp_opts)
{
    ...
    CT_RETURN_IFERR(exp_open_writer(exp_opts));
    CT_RETURN_IFERR(exp_open_logger(exp_opts->log_file));
...
static inline int exp_print_nlsparams(void)
{
    ...
    do {
        ...
        /* 'show create table' option suppresses irrelevant info display */
        if (!g_export_opts.show_create_table) {
            CT_RETURN_IFERR(exp_write_str_s("ALTER SESSION SET ", &g_exp_txtbuf, g_exp_dpfile));

            CT_RETURN_IFERR(ctconn_column_as_string(STMT, 0, g_str_buf, CT_MAX_PACKET_SIZE));
            exp_write_fmt(CT_FALSE, CTSQL_MAX_QUOTE_NAME_SIZE, "%s = ", g_str_buf);

            CT_RETURN_IFERR(ctconn_column_as_string(STMT, 1, g_str_buf, CT_MAX_PACKET_SIZE));
            exp_write_fmt(CT_FALSE, CTSQL_MAX_QUOTE_NAME_SIZE, "'%s';\n", g_str_buf);
        }
    } while (CT_TRUE);
    if (!g_export_opts.show_create_table) {
        CT_RETURN_IFERR(exp_write_str_s("\n", &g_exp_txtbuf, g_exp_dpfile));
...
static inline int exp_single_trigger(export_options_t *exp_opts, const char *user, const char *trigger)
{
    ...
    CT_RETURN_IFERR(ctconn_prepare(STMT, (const char *)cmd_buf));
    CT_RETURN_IFERR(ctconn_bind_by_pos(STMT, 0, CTCONN_TYPE_STRING, user, (int32)strlen(user), NULL));
    CT_RETURN_IFERR(ctconn_bind_by_pos(STMT, 1, CTCONN_TYPE_STRING, trigger, (int32)strlen(trigger), NULL));
    CT_RETURN_IFERR(ctconn_execute(STMT));
    ...
    do {
        ...
        CT_RETURN_IFERR(exp_write_schema_com("CREATE OR REPLACE TRIGGER ", &g_exp_txtbuf, g_exp_dpfile));
                                      ...
                                      list_t *tbl_triggers)
{
    ...
    do {
        ...
        CT_RETURN_IFERR(ctconn_column_as_string(STMT, 0, g_str_buf, CT_MAX_PACKET_SIZE));
        if (cm_list_new(tbl_triggers, (pointer_t *)&ptr) != CT_SUCCESS) {
            return CT_ERROR;
        }
        MEMS_EXP_RETURN_IFERR(strncpy_s(ptr, CTSQL_MAX_OBJECT_LEN, g_str_buf, CTSQL_MAX_OBJECT_LEN - 1));
    } while (CT_TRUE);
                                          ...
                                          uint32 *trigger_total)
{
    ...
    exp_create_objlist(&tbl_triggers, CTSQL_MAX_OBJECT_LEN);
    do {
        status = exp_get_tbl_trigger(exp_opts, user, table, &tbl_triggers);
        if ((status != CT_SUCCESS) || (tbl_triggers.count == 0)) {
            break;
        }
...
static inline int exp_tbl_triggers(export_options_t *exp_opts, const char *user, list_t *table_list)
{
    ...
    for (uint32 i = 0; i < table_list->count; i++) {
        ...
        if (exp_opts->exp_type == EXP_TABLE &&
            ...
            CT_SUCCESS != exp_tablespace_filter(exp_opts, user, tmp_table)) {
            continue;
        }
...
static inline int exp_get_user_triggers(export_options_t *exp_opts, const char *user, list_t *user_triggers)
{
    ...
    param.get_proc_param.exp_opts = exp_opts;
    param.get_proc_param.user = user;
    CT_RETURN_IFERR(EXP_PREPARE_SQL(EXP_GET_USER_TRIGGERS_LIST, cmd_buf, CTSQL_MAX_TEMP_SQL, &param));
    ...
    do {
        ...
        CT_RETURN_IFERR(ctconn_column_as_string(STMT, 0, g_str_buf, CT_MAX_PACKET_SIZE));
        if (cm_list_new(user_triggers, (pointer_t *)&ptr) != CT_SUCCESS) {
            return CT_ERROR;
        }
...
static inline int exp_user_triggers(export_options_t *exp_opts, const char *user)
{
    ...
    do {
        ...
        if ((status != CT_SUCCESS) || (user_triggers.count == 0)) {
            (void)exp_bin_write_int32(&exp_opts->master_bin_mgr, (uint32)0);
            break;
        }
...
{
    CT_RETURN_IFERR(exp_prepare(exp_opts));

    CT_RETURN_IFERR(exp_write_head());
    /* 'show create table' option suppresses irrelevant info display */
    if (!exp_opts->show_create_table) {
        exp_print_options(exp_opts);
    }

    CT_RETURN_IFERR(exp_print_nlsparams());

    if (exp_opts->tablespace) {
        CT_RETURN_IFERR(exp_tablespace());
    }

    if (exp_opts->tenant) {
        CT_RETURN_IFERR(exp_all_tenants());
    }
    ...
    CT_RETURN_IFERR(exp_bin_write_str(&exp_opts->master_bin_mgr, g_exp_txtbuf.str,
    ...
    if (exp_opts->exp_type == EXP_TABLE) {
        ...
        CT_RETURN_IFERR(exp_table_scripts(exp_opts, exp_opts->schema_name, &exp_opts->exp_tables.table_list));
        CT_RETURN_IFERR(exp_tbl_triggers(exp_opts, exp_opts->schema_name, &exp_opts->exp_tables.table_list));  // fuctions/pl len/total
        ...
        CT_RETURN_IFERR(exp_schema(exp_opts));
    } else {
        CT_RETURN_IFERR(exp_all_schemas(exp_opts));
    }

    /* 'show create table' option suppresses irrelevant info display */
    if ((g_exp_dpfile != NULL) && (exp_opts->filetype == FT_TXT) && !exp_opts->show_create_table) {
        exp_write_fmt(CT_FALSE, CTSQL_MAX_QUOTE_NAME_SIZE, "-- end of exp: %s\n", exp_now());
    }
...
void exp_clear_resource(export_options_t *exp_opts)
{
    ...
    if (realpath_file(exp_opts->dump_file, realfile, CT_MAX_FILE_PATH_LENGH) != CT_SUCCESS) {
        return;
    }
...
status_t exp_get_current_schema(export_options_t *exp_opts)
{
    if (exp_opts->exp_type == EXP_TABLE || exp_opts->exp_type == EXP_ALL_TABLES) {
        ...
        if (ctsql_get_curr_schema(&curr_schema) != CT_SUCCESS) {
...
status_t ctsql_export_lex_prepare(lex_t *lex, text_t *cmd_text, uint32 *matched_id)
{
    sql_text_t sql_text;
    ...
    if (lex_expected_fetch_1of2(lex, "EXP", "EXPORT", matched_id) != CT_SUCCESS) {
        ctsql_print_error(NULL);
        return CT_ERROR;
    }

    if (lex_try_fetch_1ofn(lex, matched_id, 6, "help", "usage", "option", "-h", "version", "-v") != CT_SUCCESS) {
        ctsql_print_error(NULL);
        return CT_ERROR;
    }
...
status_t ctsql_export_result_dealing(int32 status)
{
    /* 'show create table' option suppresses irrelevant info display */
    if (status != CT_SUCCESS && g_export_opts.show_create_table) {
        exp_tmlog_error(CONN);
...
status_t ctsql_export_impl(void)
{
    int status;
    uint32 prefetch_rows = 2000;
    ...
    do {
        ...
        CT_BREAK_IF_ERROR(status);
        status = ctconn_set_stmt_attr(STMT, CTCONN_ATTR_PREFETCH_ROWS, &prefetch_rows, 0);
        CT_BREAK_IF_ERROR(status);
...
status_t ctsql_execute_export()
{
    uint32 num_width = 0;
    uint32 attr_len = 0;
    uint32 new_num_width = (uint32)CT_MAX_DEC_OUTPUT_ALL_PREC;
    (void)ctconn_get_conn_attr(CONN, CTCONN_ATTR_NUM_WIDTH, &num_width, sizeof(uint32), &attr_len);
    (void)ctconn_set_conn_attr(CONN, CTCONN_ATTR_NUM_WIDTH, &new_num_width, sizeof(uint32));

    int status = ctsql_export_impl();

    (void)ctconn_set_conn_attr(CONN, CTCONN_ATTR_NUM_WIDTH, &num_width, sizeof(uint32));
    return status;
}

status_t ctsql_export(text_t *cmd_text, uint8 show_parse_info)
{
    int status;
    uint32 matched_id;
    lex_t lex;

    CT_RETURN_IFERR(ctsql_export_lex_prepare(&lex, cmd_text, &matched_id));

    if (matched_id == EXP_DESC_VERSION || matched_id == EXP_DESC_HYPHEN_V) {
        ctsql_display_export_version_info();
        return CT_SUCCESS;
    }
    ...
    if (!IS_CONN) {
        CTSQL_PRINTF(ZSERR_EXPORT, "connection is not established");
        return CT_ERROR;
    }

    CT_RETURN_IFERR(exp_reset_opts(&g_export_opts));

    if (show_parse_info) {
        exp_tmlog("Parsing export options ... \n");
    }

    if (exp_parse_opts(&lex, &g_export_opts) != CTCONN_SUCCESS) {
        status = CT_ERROR;  // the lex error is in client
    } else {
        if (!g_export_opts.show_create_table && ctsql_check_tenant() != CT_SUCCESS) {
            return ctsql_export_result_dealing(CT_ERROR);
        }

        status = ctsql_execute_export();
    ...
    ctsql_export_result_dealing(status);
    if (status != CTCONN_SUCCESS) {
...
{
    CT_RETURN_IFERR(ctconn_common_init_fixed_memory_pool_ex(&exp_cache->fixed_mem_pool,
        MAX_SQL_SIZE + 1, unit_cnt, EXP_MAX_TABLE_CACHE_EXT_CNT));

    exp_cache->root_unit.type = root_type;
    exp_cache->root_unit.content.str = ctconn_common_alloc_fixed_buffer(&exp_cache->fixed_mem_pool);
...
status_t alloc_exp_cache_unit(exp_cache_t* exp_cache, exp_cache_unit_type_t type, exp_cache_unit_t** unit)
{
    ...
    cache_unit->content.str = ctconn_common_alloc_fixed_buffer(pool);
...
status_t alloc_column_cache_info(exp_cache_t* exp_cache, exp_cache_column_info_t** column_info)
{
    ...
    if (EXP_CACHE_REMAIN_SIZE(exp_cache) < sizeof(exp_cache_column_info_t) &&
        exp_extend_cache_unit(exp_cache) != CT_SUCCESS) {
...
status_t get_exp_cache_unit(exp_cache_t* exp_cache, exp_cache_unit_type_t type, exp_cache_unit_t** unit)
{
    ...
    return (i == cache_unit_cnt) ? CT_ERROR : CT_SUCCESS;
}

void uninit_exp_cache(exp_cache_t* exp_cache)
{
    ctconn_common_uninit_fixed_memory_pool(&exp_cache->fixed_mem_pool);
}
...
{
    if (g_export_opts.quote_names) {
        CT_RETURN_IFERR(cache_unit_append_str(unit, "\""));
        CT_RETURN_IFERR(cache_unit_append_str(unit, str));
        CT_RETURN_IFERR(cache_unit_append_str(unit, "\""));
    } else {
        CT_RETURN_IFERR(cache_unit_append_str(unit, str));
    }
...
void reset_exp_cache(exp_cache_t* exp_cache)
{
    ...
    for (uint32 i = 0; i < cache_unit_cnt; i++) {
        ...
        ctconn_common_free_fixed_buffer(&exp_cache->fixed_mem_pool, cache_unit->content.str);
    }
    exp_cache->root_unit.content.len = 0;
    exp_cache->record_cnt = 0;
}
...
static status_t table_cache_write_create_table(exp_cache_t* table_cache)
{
    if (g_export_opts.content & CT_EXP_META) {
        CT_RETURN_IFERR(exp_bin_memory_mgr_sub_begin(&g_export_opts.master_bin_mgr, g_export_opts.filetype));
        CT_RETURN_IFERR(exp_cache_unit_write_file(table_cache, EXP_CACHE_CREATE_TABLE));
        exp_bin_memory_mgr_sub_end(&g_export_opts.master_bin_mgr, g_export_opts.filetype);
    } else {
...
static status_t table_cache_write_table_name(exp_cache_t* table_cache)
{
    ...
    if (g_export_opts.content & CT_EXP_DATA) {
        CT_RETURN_IFERR(get_exp_cache_unit(table_cache, EXP_CACHE_TABLE_NAME, &cache_unit));

        CT_RETURN_IFERR(exp_bin_write_shortstr(&g_export_opts.master_bin_mgr,
...
static status_t table_cache_write_record_cnt(exp_cache_t* table_cache)
{
    if (g_export_opts.content & CT_EXP_DATA) {
        return exp_bin_write_int64(&g_export_opts.master_bin_mgr,
            table_cache->record_cnt) == NULL ? CT_ERROR : CT_SUCCESS;
    }
...
static status_t table_cache_write_field(exp_cache_t* table_cache)
{
    ...
    if (g_export_opts.content & CT_EXP_DATA) {
        if (get_exp_cache_unit(table_cache, EXP_CACHE_COLUMN_INFO, &cache_unit) == CT_SUCCESS) {
            ...
            for (uint16 i = 0; i < column_cnt; i++) {
                ...
                CT_RETURN_IFERR(exp_bin_write_shortstr(&g_export_opts.master_bin_mgr, column_info->name,
...
static status_t table_cache_write_index(exp_cache_t* table_cache)
{
    ...
    if ((g_export_opts.content & CT_EXP_META) &&
        get_exp_cache_unit(table_cache, EXP_CACHE_TABLE_INDEX, &cache_unit) == CT_SUCCESS &&
        cache_unit->content.len > 0) {
        (void)exp_bin_memory_mgr_sub_begin(&g_export_opts.master_bin_mgr, g_export_opts.filetype);

        CT_RETURN_IFERR(exp_bin_write_bytes(&g_export_opts.master_bin_mgr,
            cache_unit->content.str, cache_unit->content.len));
...
status_t table_cache_write_file(exp_cache_t* table_cache)
{
    if (g_export_opts.filetype == FT_BIN) {
        ...
        CT_RETURN_IFERR(table_cache_write_create_table(table_cache));

        // 2. write table name
        CT_RETURN_IFERR(table_cache_write_table_name(table_cache));

        // 3. write record number
        CT_RETURN_IFERR(table_cache_write_record_cnt(table_cache));

        // 4. write field info
        CT_RETURN_IFERR(table_cache_write_field(table_cache));

        // 5. write subfile info
        CT_RETURN_IFERR(table_cache_write_subfile(table_cache));

        // 6. write index info
        CT_RETURN_IFERR(table_cache_write_index(table_cache));
    }
...
status_t exp_cache_unit_write_file(exp_cache_t* exp_cache, exp_cache_unit_type_t type)
{
    ...
    do {
        ...
        if (g_export_opts.filetype == FT_BIN) {
            ...
            CT_RETURN_IFERR(exp_write_text_s(&cache_unit->content, &g_exp_txtbuf, g_exp_dpfile));
        }
...
status_t view_cache_write_file(exp_cache_t* view_cache)
{
    ...
    // wirte head
    CT_RETURN_IFERR(exp_write_schema_com("CREATE OR REPLACE FORCE VIEW ", &g_exp_txtbuf, g_exp_dpfile));

    // write view name
    CT_RETURN_IFERR(exp_cache_unit_write_file(view_cache, EXP_CACHE_VIEW_NAME));

    // write view columns
    CT_RETURN_IFERR(exp_write_schema_com("\n", &g_exp_txtbuf, g_exp_dpfile));
    CT_RETURN_IFERR(exp_cache_unit_write_file(view_cache, EXP_CACHE_VIEW_COLUMNS));
    CT_RETURN_IFERR(exp_write_schema_com(" AS\n", &g_exp_txtbuf, g_exp_dpfile));

    // write view src
    CT_RETURN_IFERR(exp_cache_unit_write_file(view_cache, EXP_CACHE_VIEW_SRC));
    CT_RETURN_IFERR(exp_write_schema_com("\n", &g_exp_txtbuf, g_exp_dpfile));
...
status_t obj_cache_write_file(exp_cache_t* obj_cache)
{
    ...
    // wirte head
    CT_RETURN_IFERR(exp_write_schema_com("CREATE OR REPLACE ", &g_exp_txtbuf, g_exp_dpfile));
    CT_RETURN_IFERR(exp_cache_unit_write_file(obj_cache, EXP_CACHE_OBJ_TYPE));
    CT_RETURN_IFERR(exp_write_schema_com(" ", &g_exp_txtbuf, g_exp_dpfile));

    // write obj name
    CT_RETURN_IFERR(exp_cache_unit_write_file(obj_cache, EXP_CACHE_OBJ_NAME));
    CT_RETURN_IFERR(exp_write_schema_com("\n", &g_exp_txtbuf, g_exp_dpfile));

    // write obj src
    CT_RETURN_IFERR(exp_cache_unit_write_file(obj_cache, EXP_CACHE_OBJ_SRC));
    CT_RETURN_IFERR(exp_write_schema_com("\n", &g_exp_txtbuf, g_exp_dpfile));
...
{
    if (g_export_opts.filetype == FT_TXT) {
        CT_RETURN_IFERR(exp_cache_unit_write_file(table_cache, EXP_CACHE_TABLE_INDEX));
    }
