...
static inline void init_cur_buffer(exp_mem_block_t *block, char *buf, uint32 size)
{
    ...
    *(block->offset) = GSQL_EXP_BIN_MEM_BLOCK_RESERVED_SIZE;
}
...
static status_t extent_exp_bin_memory_block(exp_bin_memory_mgr_t *mem_mgr)
{
    ...
    GS_RETURN_IFERR(cm_list_new(&mem_mgr->mems, (void **)&buf));

    init_cur_buffer(&mem_mgr->cur_buf, buf, GSQL_EXP_BIN_MEM_BLOCK_SIZE);
...
status_t init_exp_bin_memory_mgr(exp_bin_memory_mgr_t *mem_mgr)
{
    ...
    cm_create_list2(&mem_mgr->mems, 1, MAX_LIST_EXTENTS, GSQL_EXP_BIN_MEM_BLOCK_SIZE);
...
status_t exp_bin_memory_mgr_begin(exp_bin_memory_mgr_t *mgr, exp_filetype_t filetype)
{
    ...
    GS_RETURN_IFERR(get_mem_address(mgr, &buf, sizeof(uint32)));
...
{
    *buf = NULL;
    if (required_size > GSQL_EXP_BIN_MEM_BLOCK_SIZE - 8) {
        return GS_ERROR;
    }

    if (required_size > GSQL_EXP_BIN_MEM_BLOCK_SIZE - *(mgr->cur_buf.offset)) {
        GS_RETURN_IFERR(extent_exp_bin_memory_block(mgr));
    }
...
static inline int mem_writer_encrypt_s(char *buf, uint32 size, FILE *filehand, EVP_CIPHER_CTX *gcm_ctx)
{
    ...
    if (cm_encrypt_data_by_gcm(gcm_ctx, encrypt_buf, buf, size) != GS_SUCCESS) {
        CM_FREE_PTR(encrypt_buf);
        return GS_ERROR;
    }

    if (fwrite(encrypt_buf, 1, size, filehand) == 0) {
        GS_THROW_ERROR(ERR_CLT_WRITE_FILE_ERR, errno);
        CM_FREE_PTR(encrypt_buf);
        return GS_ERROR;
    }
    ...
    for (pos = 0; pos < mgr->mems.count; ++pos) {
        ...
        if (buf != NULL) {
            if (encrypt_flag && crypt_file != NULL) {
                (void)mem_writer_encrypt_s(buf + GSQL_EXP_BIN_MEM_BLOCK_RESERVED_SIZE,
                    (*(uint32 *)(buf + sizeof(uint32)) - GSQL_EXP_BIN_MEM_BLOCK_RESERVED_SIZE), hand,
                    crypt_file->crypt_ctx.gcm_ctx);
            } else {
                (void)fwrite((buf + GSQL_EXP_BIN_MEM_BLOCK_RESERVED_SIZE), 1,
                    (*(uint32 *)(buf + sizeof(uint32)) - GSQL_EXP_BIN_MEM_BLOCK_RESERVED_SIZE), hand);
            }
        }
    }
