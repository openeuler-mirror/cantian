 ...
 *
 * IDENTIFICATION
 * src/utils/gsql/gsql_wsr_analyse.c
 *
 * -------------------------------------------------------------------------
 */
#include "gsql_wsr_analyse.h"

#define GS_WSR_DEFAULT_CPU_NUM             (uint32)8
#define GS_WSR_LOAD_RATE_VERY_HIGH         10
#define GS_WSR_LOAD_RATE_HIGH              1
#define GS_WSR_LOAD_RATE_MEDIUM            0.3
#define GS_WSR_LOAD_RATE_LOW               0.1
#define GS_WSR_CAPTURE_RATE                50
...
static void wsr_build_report_summary_load(wsr_options_t *wsr_opts, wsr_info_t *wsr_info)
{
    ...
    if (wsr_info->num_cpu == 0) {
        loadRate = (double)wsr_info->dbtime / WSR_MILLION / wsr_info->elapsed / GS_WSR_DEFAULT_CPU_NUM;
    } else {
    ...
    if (loadRate > GS_WSR_LOAD_RATE_VERY_HIGH) {
        descId = WSR_ITEM_VERY_HIGH;
        adviceId = WSR_ITEM_NEED_OPT;
    } else if (loadRate > GS_WSR_LOAD_RATE_HIGH) {
        descId = WSR_ITEM_HIGH;
        adviceId = WSR_ITEM_NEED_OPT;
    } else if (loadRate > GS_WSR_LOAD_RATE_MEDIUM) {
        descId = WSR_ITEM_MEDIUM;
    } else if (loadRate > GS_WSR_LOAD_RATE_LOW) {
        descId = WSR_ITEM_LOW;
    } else {
...
static status_t wsr_build_report_sql_capture(wsr_options_t *wsr_opts, wsr_info_t *wsr_info)
{
    ...
    GS_RETURN_IFERR(gsc_bind_by_pos(wsr_opts->curr_stmt, index++, GSC_TYPE_BIGINT,
...
static int wsr_build_report_event_analyse(wsr_options_t *wsr_opts, wsr_info_t *wsr_info)
{
    ...
    do {
        ...
        GS_RETURN_IFERR(gsc_column_as_string(wsr_opts->curr_stmt, 0, event, MAX_WSR_ENTITY_LEN));
        GS_RETURN_IFERR(gsc_column_as_string(wsr_opts->curr_stmt, 1, dbtime_percent, MAX_WSR_ENTITY_LEN));
...
static int wsr_build_report_sql_analyse(wsr_options_t *wsr_opts, wsr_info_t *wsr_info)
{
    ...
    do {
        ...
        GS_RETURN_IFERR(gsc_column_as_string(resultset, index++, sqlinfo.recommendations, WSR_FMT_SIZE_10000));
        GS_RETURN_IFERR(gsc_column_as_string(resultset, index++, sqlinfo.elapsed_time, MAX_WSR_ENTITY_LEN));
        GS_RETURN_IFERR(gsc_column_as_string(resultset, index++, sqlinfo.executions, MAX_WSR_ENTITY_LEN));
        GS_RETURN_IFERR(gsc_column_as_string(resultset, index++, sqlinfo.executings, MAX_WSR_ENTITY_LEN));
        GS_RETURN_IFERR(gsc_column_as_string(resultset, index++, sqlinfo.elapsed_per_exec, MAX_WSR_ENTITY_LEN));
        GS_RETURN_IFERR(gsc_column_as_string(resultset, index++, sqlinfo.total_percent, MAX_WSR_ENTITY_LEN));
        GS_RETURN_IFERR(gsc_column_as_string(resultset, index++, sqlinfo.cpu_percent, MAX_WSR_ENTITY_LEN));
        GS_RETURN_IFERR(gsc_column_as_string(resultset, index++, sqlinfo.net_percent, MAX_WSR_ENTITY_LEN));
        GS_RETURN_IFERR(gsc_column_as_string(resultset, index++, sqlinfo.io_percent, MAX_WSR_ENTITY_LEN));
        GS_RETURN_IFERR(gsc_column_as_string(resultset, index++, sqlinfo.sql_text_part, MAX_WSR_ENTITY_LEN));
        GS_RETURN_IFERR(gsc_column_as_string(resultset, index++, sqlinfo.sql_id, MAX_WSR_ENTITY_LEN));
        GS_RETURN_IFERR(gsc_column_as_string(resultset, index++, sqlinfo.sql_module, MAX_WSR_ENTITY_LEN));
        GS_RETURN_IFERR(gsc_column_as_string(resultset, index++, sqlinfo.buffer_gets_per, MAX_WSR_ENTITY_LEN));
        GS_RETURN_IFERR(gsc_column_as_string(resultset, index++, sqlinfo.disk_reads_per, MAX_WSR_ENTITY_LEN));
        GS_RETURN_IFERR(gsc_column_as_string(resultset, index++, sqlinfo.row_processed_per, MAX_WSR_ENTITY_LEN));
        GS_RETURN_IFERR(gsc_column_as_string(resultset, index++, sqlinfo.parse_time, MAX_WSR_ENTITY_LEN));
        GS_RETURN_IFERR(gsc_column_as_string(resultset, index++, sqlinfo.cr_gets_per, MAX_WSR_ENTITY_LEN));
        GS_RETURN_IFERR(gsc_column_as_string(resultset, index++, sqlinfo.schema, MAX_WSR_ENTITY_LEN));
        GS_RETURN_IFERR(gsc_column_as_string(resultset, index++, sqlinfo.line, MAX_WSR_ENTITY_LEN));
        GS_RETURN_IFERR(gsc_column_as_string(resultset, index++, sqlinfo.procedure, MAX_WSR_ENTITY_LEN));
        GS_RETURN_IFERR(gsc_column_as_string(resultset, index++, sqlinfo.wait_time, MAX_WSR_ENTITY_LEN));
        GS_RETURN_IFERR(gsc_column_as_string(resultset, index++, sqlinfo.vm_pages_used, MAX_WSR_ENTITY_LEN));
...
status_t wsr_build_report_summary(wsr_options_t *wsr_opts, wsr_info_t *wsr_info)
{
    ...
    GS_RETURN_IFERR(wsr_build_report_sql_capture(wsr_opts, wsr_info));
    GS_RETURN_IFERR(wsr_build_report_event_analyse(wsr_opts, wsr_info));
    GS_RETURN_IFERR(wsr_build_report_sql_analyse(wsr_opts, wsr_info));
