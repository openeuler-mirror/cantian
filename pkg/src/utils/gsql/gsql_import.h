...
#define GET_NEXT_STATUS(ret, next, err) ((ret) == GS_SUCCESS ? (next) : (err))
...
typedef struct st_imp_bind_info {
    ...
    uint16 *data_ind[GS_MAX_COLUMNS];     // store column bind ind array;
    char *col_data[GS_MAX_COLUMNS];       // store column data pointer
    uint16 bind_buf_len[GS_MAX_COLUMNS];  // each column length
    uint32 column_cnt;
...
typedef struct {
    ...
    gsc_lob_t lob[GS_MAX_COLUMNS];
    uint16 col_ind[GS_MAX_COLUMNS];
    gsql_conn_info_t conn_info;
...
typedef struct {
    int32 error_code;
    char  error_msg[GS_MESSAGE_BUFFER_SIZE];
} importer_error_t;
...
typedef struct {
    ...
    char currentSchema[GS_MAX_NAME_LEN + 1];
    char current_table[GS_MAX_NAME_LEN + 1];
} imp_ddl_worker_t;

typedef struct {
    FILE *fp;
    gsc_z_stream zstream;
    char *swap_buffer;
    uint32 swap_len;
} imp_compress_file_t;
...
typedef struct {
    ...
    uint32 tableInfoLen;
    uint16 tableNameLen;
    char tableName[GS_MAX_NAME_LEN + 1];
    uint64 recordNum;
    uint16 fieldNum;
    uint32 subFileNum;
    uint16 fileNameLen;
    char subFileName[GS_MAX_NAME_LEN + 1];
    uint32 indexLen;
...
typedef struct {
    ...
    char import_file[GS_MAX_FILE_PATH_LENGH];
    char log_file[GS_MAX_FILE_PATH_LENGH];
    char targetObj[GSQL_MAX_OBJECT_LEN]; /* use for remap */
    bool32 show;
    uint32 feedback;
    uint32 exp_content;
    uint32 content;
    ...
    uint32 sql_index;
    char singleSchema[GS_MAX_NAME_LEN + 1];
    bool32 timing;
    uint32 batchRowCnt;
    bool32 disable_trigger;
    char imp_file_path[GS_MAX_FILE_PATH_LENGH];
    char imp_subfile_path[GS_MAX_FILE_PATH_LENGH];
    crypt_info_t crypt_info;
} importer_t;

typedef struct {
    gsql_conn_info_t *conn_info; // execute connection
...
status_t gsql_get_saved_pswd(char *password, uint32 len);
