...
#define GSQL_DUMP_RETURN_IF_CLI_ERROR(ret)   \
    do {                                     \
        int _status_ = (ret);                \
...
typedef struct {
    ...
    uint32 col_num;
#ifdef USE_GSQL_COLUMN_DESC
    gsc_inner_column_desc_t *col_desc;
#else
    gsc_inner_column_desc_t col_desc[GS_MAX_COLUMNS];
#endif  // USE_GSQL_COLUMN_DESC
...
static inline void gsql_free_dumper(dumper_t *dumper);
static status_t gsql_dump_write_str(dumper_t *dumper, char *str, uint16 len, bool32 row_end);
static status_t gsql_dump_write_text(dumper_t *dumper, text_t *text, bool32 row_end);

static inline void gsql_dump_enclosed_char(dumper_t *dumper, int datatype)
{
    ...
    if (!GSQL_HAS_ENCLOSED_CHAR(g_dump_opt.fields_enclosed)) {
        return;
    }

    if (g_dump_opt.enclosed_optionally) {  // merely string and binary type are enclosed
        if (GSQL_IS_ENCLOSED_TYPE(datatype)) {
            (void)gsql_dump_write_str(dumper, fields_enclosed, enclosed_len, GS_FALSE);
        }
...
int gsql_dump_add_escape(text_t *buf, uint32 buf_len, int datatype, char enclosed_char)
{
    char *temp = NULL;
    uint32 i = 0;
    uint32 j = 0;
    errno_t errcode = 0;
    if (!GSQL_HAS_ENCLOSED_CHAR(enclosed_char) || !GSQL_IS_ENCLOSED_TYPE(datatype)) {
        return GSC_SUCCESS;
    }
...
static inline int gsql_dump_column_raw(const binary_t *bin, text_t *result)
{
    result->len = MAX_DUMP_CONVERT_SIZE;

    if (cm_bin2text(bin, GS_TRUE, result) != GS_SUCCESS) {
        gsql_print_error(NULL);
        return GSC_ERROR;
    }
...
static inline int gsql_dump_column_clob(dumper_t *dumper, uint32 col_id)
{
    uint32 offset = 0;
    bool32 eof = 0;
    uint32 nchars;
    ...
    do {
        GS_RETURN_IFERR(gsc_read_clob_by_id(STMT, col_id, offset, dumper->conver_buf, MAX_DUMP_CONVERT_SIZE / 2,
                                            &nchars, &nbytes, &eof));
        offset += nbytes;
        ...
        if (nbytes > 0) {
            ...
            result.len = nchars;
            GS_RETURN_IFERR(gsql_dump_add_escape(&result, MAX_DUMP_CONVERT_SIZE, dumper->col_desc[col_id].type,
                                                 g_dump_opt.fields_enclosed));
            GSQL_DUMP_RETURN_IF_CLI_ERROR(gsql_dump_write_text(dumper, &result, GS_FALSE));
        }
    } while (!eof);
...
static inline int gsql_dump_column_blob(dumper_t *dumper, uint32 col_id)
{
    uint32 offset = 0;
    bool32 eof = 0;
    uint32 nbytes;
    ...
    do {
        GS_RETURN_IFERR(gsc_read_blob_by_id(STMT, col_id, offset, dumper->conver_buf, MAX_DUMP_CONVERT_SIZE / 2,
                                            &nbytes, &eof));
        offset += nbytes;
...
static inline int gsql_dump_column_type_date(char *nlsbuf, text_t *fmt_text, char *data, text_t *result)
{
    GS_RETURN_IFERR(gsql_nlsparam_geter(nlsbuf, NLS_DATE_FORMAT, fmt_text));
    if (cm_date2text(*(date_t *)data, fmt_text, result, MAX_DUMP_CONVERT_SIZE + 1) != GS_SUCCESS) {
        gsql_print_error(NULL);
        return GSC_ERROR;
    }
    return GSC_SUCCESS;
...
static inline int gsql_dump_column_type_timestamp_ltz(char *nlsbuf, text_t *fmt_text, char *data, text_t *result)
{
    GS_RETURN_IFERR(gsql_nlsparam_geter(nlsbuf, NLS_TIMESTAMP_FORMAT, fmt_text));
    if (cm_timestamp2text(*(date_t *)data, fmt_text, result, MAX_DUMP_CONVERT_SIZE + 1) != GS_SUCCESS) {
        gsql_print_error(NULL);
        return GSC_ERROR;
    }
    return GSC_SUCCESS;
...
static inline int gsql_dump_column_type_timestamp_tz(char *nlsbuf, text_t *fmt_text, char *data, text_t *result)
{
    GS_RETURN_IFERR(gsql_nlsparam_geter(nlsbuf, NLS_TIMESTAMP_TZ_FORMAT, fmt_text));
    if (cm_timestamp_tz2text((timestamp_tz_t *)data, fmt_text, result, MAX_DUMP_CONVERT_SIZE + 1) != GS_SUCCESS) {
        gsql_print_error(NULL);
        return GSC_ERROR;
    }
    return GSC_SUCCESS;
...
static inline int gsql_dump_column_type_varbinary(dumper_t *dumper, text_t *result, uint32 col_id, char *data, uint32 size)
{
    if (size >= MAX_DUMP_CONVERT_SIZE - 2) {
        GSQL_PRINTF(ZSERR_DUMP, "assert raised, expect: size(%u) < MAX_DUMP_CONVERT_SIZE(%u) - 2", size,
...
static status_t gsql_dump_column_data_core(dumper_t *dumper, uint32 col_id, char *data, uint32 size)
{
    text_t fmt_text;
    ...
    switch (dumper->col_desc[col_id].type) {
        ...
        case GSC_TYPE_DATE: {
            GS_RETURN_IFERR(gsql_dump_column_type_date((char *)nlsbuf, &fmt_text, data, &result));
            break;
        }
        ...
        case GSC_TYPE_TIMESTAMP_LTZ: {
            GS_RETURN_IFERR(gsql_dump_column_type_timestamp_ltz((char *)nlsbuf, &fmt_text, data, &result));
            break;
        }

        case GSC_TYPE_TIMESTAMP_TZ: {
            GS_RETURN_IFERR(gsql_dump_column_type_timestamp_tz((char *)nlsbuf, &fmt_text, data, &result));
            break;
        }
        ...
        case GSC_TYPE_VARBINARY:
            GS_RETURN_IFERR(gsql_dump_column_type_varbinary(dumper, &result, col_id, data, size));
            break;

        case GSC_TYPE_NUMBER:
        case GSC_TYPE_DECIMAL: {
            GS_RETVALUE_IFTRUE(cm_dec4_to_text_all((dec4_t*)data, &result) != GSC_SUCCESS, GSC_ERROR);
            break;
        }
        ...
        case GSC_TYPE_NUMBER2: {
            ...
            GS_RETVALUE_IFTRUE(cm_dec2_to_text(&dec2, GSC_NUMBER_BOUND_SIZE, &result) != GSC_SUCCESS, GSC_ERROR);
            break;
        }

        case GSC_TYPE_RAW: {
            binary_t bin = { .bytes = (uint8 *)data, .size = size };
            GS_RETVALUE_IFTRUE(gsql_dump_column_raw(&bin, &result) != GSC_SUCCESS, GSC_ERROR);
            GS_RETURN_IFERR(gsql_dump_add_escape(&result, MAX_DUMP_CONVERT_SIZE, dumper->col_desc[col_id].type,
                                                 g_dump_opt.fields_enclosed));
            break;
        }
        ...
        case GSC_TYPE_CLOB:
            GS_RETVALUE_IFTRUE(gsql_dump_column_clob(dumper, col_id) != GSC_SUCCESS, GSC_ERROR);
            return GSC_SUCCESS; /* lob write to file in 'gsql_dump_column_clob' */

        case GSC_TYPE_BLOB:
        case GSC_TYPE_IMAGE:
            GS_RETVALUE_IFTRUE(gsql_dump_column_blob(dumper, col_id) != GSC_SUCCESS, GSC_ERROR);
            return GSC_SUCCESS; /* lob write to file in 'gsql_dump_column_blob' */
    ...
    if (result.len > MAX_DUMP_CONVERT_SIZE) {
        GSQL_PRINTF(ZSERR_DUMP, "the row buffer size is too small!");
        return GSC_ERROR;
    }
...
static int gsql_dump_column_data(gsc_stmt_t stmt, dumper_t *dumper, uint32 col_id)
{
    char *data = NULL;
    bool32 is_null = GS_FALSE;
    uint32 size;

    if (gsc_get_column_by_id(stmt, col_id, (void **)&data, &size, &is_null) != GS_SUCCESS) {
    ...
    gsql_dump_enclosed_char(dumper, dumper->col_desc[col_id].type);

    GS_RETURN_IFERR(gsql_dump_column_data_core(dumper, col_id, data, size));

    gsql_dump_enclosed_char(dumper, dumper->col_desc[col_id].type);
    return GSC_SUCCESS;
}

static int gsql_dump_row(gsc_stmt_t stmt, dumper_t *dumper)
{
    uint32 i;

    if (dumper->col_num == 0) {
        GSQL_PRINTF(ZSERR_DUMP, "assert raised, expect: dumper->col_num(%u) > 0", dumper->col_num);
        return GSC_ERROR;
    }

    for (i = 0; i < dumper->col_num; i++) {
        if (i != 0) {
            GSQL_DUMP_RETURN_IF_CLI_ERROR(gsql_dump_write_str(dumper, dumper->column_param.field_terminal,
                                                   dumper->column_param.field_terminal_len, GS_FALSE));
        }

        if (gsql_dump_column_data(stmt, dumper, i) != GSC_SUCCESS) {
            return GSC_ERROR;
        }
    }
    GSQL_DUMP_RETURN_IF_CLI_ERROR(gsql_dump_write_str(dumper, dumper->column_param.line_terminal,
                                           dumper->column_param.line_terminal_len, GS_TRUE));
    return GSC_SUCCESS;
}

static int gsql_dump_column_desc(dumper_t *dumper, gsc_stmt_t stmt)
{
    uint32 i;

    if (gsc_get_stmt_attr(STMT, GSC_ATTR_COLUMN_COUNT, &dumper->col_num, sizeof(uint32), NULL) != GSC_SUCCESS) {
        gsql_print_error(CONN);
        return GSC_ERROR;
    }

    for (i = 0; i < dumper->col_num; i++) {
        if (gsc_desc_inner_column_by_id(stmt, i, &dumper->col_desc[i]) != GSC_SUCCESS) {
            gsql_print_error(CONN);
            return GSC_ERROR;
        }
    }
...
static status_t gsql_dump_new_file(dumper_t *dumper)
{
    char filename[GS_MAX_FILE_PATH_LENGH];
    int iret_sprintf = 0;
    crypt_file_t *encrypt_ctx = NULL;
    char encrypt_filename[GS_MAX_NAME_LEN] = { 0 };

    if (dumper->file_idx < 0) {  // first file.
        GS_RETURN_IFERR(cm_create_file(dumper->dump_file, O_RDWR | O_TRUNC | O_BINARY | O_CREAT, &(dumper->file)));

        (void)cm_chmod_file(FILE_PERM_OF_DATA, dumper->file);
        if (g_dump_opt.crypt_info.crypt_flag) {
            GS_RETURN_IFERR(cm_list_new(&g_dump_opt.crypt_info.crypt_list, (void *)&encrypt_ctx));
            GS_RETURN_IFERR(gsql_encrypt_prepare(&encrypt_ctx->crypt_ctx, g_dump_opt.crypt_info.crypt_pwd));
            cm_trim_dir(dumper->dump_file, GS_MAX_NAME_LEN, encrypt_filename);
            MEMS_RETURN_IFERR(strncpy_s(encrypt_ctx->filename, GS_MAX_NAME_LEN, encrypt_filename, GS_MAX_NAME_LEN - 1));
            encrypt_ctx->fp = dumper->file;
        }
        dumper->file_size = 0;
        dumper->file_idx = 0;
        dumper->file_end = GS_FALSE;
    } else {
        cm_close_file(dumper->file);
        dumper->file = -1;
        dumper->file_idx++;

        iret_sprintf = sprintf_s(filename, GS_MAX_FILE_PATH_LENGH, "%s_%d", dumper->dump_file, dumper->file_idx);
        if (iret_sprintf == -1) {
            GS_THROW_ERROR(ERR_SYSTEM_CALL, iret_sprintf);
            return GS_ERROR;
        }

        GS_RETURN_IFERR(cm_create_file(filename, O_RDWR | O_TRUNC | O_BINARY | O_CREAT, &(dumper->file)));
        ...
        if (g_dump_opt.crypt_info.crypt_flag) {
            GS_RETURN_IFERR(cm_list_new(&g_dump_opt.crypt_info.crypt_list, (void *)&encrypt_ctx));
            GS_RETURN_IFERR(gsql_encrypt_prepare(&encrypt_ctx->crypt_ctx, g_dump_opt.crypt_info.crypt_pwd));
            cm_trim_dir(encrypt_filename, GS_MAX_NAME_LEN, (char *)filename);
            MEMS_RETURN_IFERR(strncpy_s(encrypt_ctx->filename, GS_MAX_NAME_LEN, filename, GS_MAX_NAME_LEN - 1));
            encrypt_ctx->fp = dumper->file;
        }
...
static status_t gsql_dump_write_file_core(dumper_t *dumper, char *write_str, bool32 row_end, uint32 len)
{
    GS_RETURN_IFERR(cm_write_file(dumper->file, write_str, len));
...
static status_t gsql_dump_encrypt_write_file(dumper_t *dumper, text_t *text, bool32 row_end, char *encrypt_buf)
{
    crypt_file_t *encrypt_file = NULL;

    GS_RETURN_IFERR(gsql_get_encrypt_file(&g_dump_opt.crypt_info, &encrypt_file, dumper->file));
    GS_RETURN_IFERR(cm_encrypt_data_by_gcm(encrypt_file->crypt_ctx.gcm_ctx, encrypt_buf, text->str, text->len));
    GS_RETURN_IFERR(gsql_dump_write_file_core(dumper, encrypt_buf, row_end, text->len));
    return GS_SUCCESS;
}

static status_t gsql_dump_write_file(dumper_t *dumper, text_t *text, bool32 row_end)
{
    char *encrypt_buf = NULL;

    // print screen.
    if (cm_str_equal_ins(dumper->dump_file, "stdout")) {
        gsql_printf("%.*s", text->len, text->str);
        return GSC_SUCCESS;
    }

    // need new a file.
    if (dumper->file_end) {
        GS_RETURN_IFERR(gsql_dump_new_file(dumper));
    }
    ...
    if (g_dump_opt.crypt_info.crypt_flag) {
        ...
        if (gsql_dump_encrypt_write_file(dumper, text, row_end, encrypt_buf) != GS_SUCCESS) {
        ...
        return gsql_dump_write_file_core(dumper, text->str, row_end, text->len);
    }
}

static status_t gsql_dump_flush_file(dumper_t *dumper)
{
    text_t complete_row;
    ...
    GS_RETURN_IFERR(gsql_dump_write_file(dumper, &complete_row, GS_TRUE));
...
static status_t gsql_dump_write_text(dumper_t *dumper, text_t *text, bool32 row_end)
{
    if (NEED_SPLIT_FILE(&g_dump_opt) && dumper->file_size + dumper->file_buf.row_end_pos >= g_dump_opt.file_size) {
        ...
        GS_RETURN_IFERR(gsql_dump_flush_file(dumper));
    }
    ...
    if (dumper->file_buf.txt.len + text->len <= g_dump_opt.max_filebuf_size) {
        ...
        if (dumper->file_buf.txt.len > 0) {
            if (dumper->file_buf.row_end_pos > 0) {
                ...
                GS_RETURN_IFERR(gsql_dump_write_file(dumper, &dumper->file_buf.txt, GS_FALSE));
                CM_TEXT_CLEAR(&dumper->file_buf.txt);
            }
            return gsql_dump_write_text(dumper, text, row_end); /* continue to write fille buffer */
        } else {                                                /* text larger than file buffer */
            return gsql_dump_write_file(dumper, text, row_end);
        }
    }
}
...
static status_t gsql_dump_write_str(dumper_t *dumper, char *str, uint16 len, bool32 row_end)
{
    ...
    return gsql_dump_write_text(dumper, &txt, row_end);
}

static status_t gsql_dump_rows(gsc_stmt_t stmt, dumper_t *dumper)
{
    uint32 rows;

    if (gsql_dump_column_desc(dumper, stmt) != GS_SUCCESS) {
        return GS_ERROR;
    }

    while (!GSQL_CANCELING) {
        ...
        if (gsql_dump_row(stmt, dumper) != GSC_SUCCESS) {
            return GS_ERROR;
        }
        dumper->dumped_rows++;

        if (dumper->dumped_rows % 5000 == 0) {  // print every 5000 lines
            gsql_printf("%-llu rows dumped.\n", dumper->dumped_rows);
        }
    }
    ...
    if (dumper->file_buf.txt.len > 0) {
        if (gsql_dump_write_file(dumper, &dumper->file_buf.txt, GS_TRUE) != GS_SUCCESS) {
            gsql_print_error(NULL);
            return GSC_ERROR;
        }
        gsql_printf("%-llu rows dumped.\n", dumper->dumped_rows);
    ...
    gsql_printf("  %-llu rows are totally dumped.\n\n", dumper->dumped_rows);
    return GS_SUCCESS;
}

static inline void gsql_free_dumper_file_buf(dumper_t *dumper)
{
...
static inline void gsql_free_dumper(dumper_t *dumper)
{
    ...
    gsql_encrypt_end(&g_dump_opt.crypt_info, filename);
    gsql_free_dumper_file_buf(dumper);
...
static status_t gsql_alloc_dumper_file_buf(dumper_t *dumper)
{
...
static status_t gsql_alloc_dumper(dumper_t *dumper)
{
    GS_RETURN_IFERR(gsql_alloc_dumper_file_buf(dumper));

    dumper->conver_buf = (char *)malloc(MAX_DUMP_CONVERT_SIZE + 1);
    if (dumper->conver_buf == NULL) {
        gsql_printf("can't allocate %u bytes for data conversion.\n", MAX_DUMP_CONVERT_SIZE);
        gsql_free_dumper_file_buf(dumper);
        return GSC_ERROR;
    }
...
static status_t gsql_dump_query(dumper_t *dumper)
{
    status_t status;
    ...
    if (gsc_prepare(STMT, g_dump_opt.query_sql) != GS_SUCCESS) {
        gsql_print_error(CONN);
        return GS_ERROR;
    }

    /* Set the charset based on the charset parameter of dump cmd */
    if (gsql_reset_charset(g_dump_opt.charset_id, g_local_config.charset_id) != GS_SUCCESS) {
        gsql_print_error(CONN);
        return GS_ERROR;
    }
    ...
    if (gsql_alloc_dumper(dumper) != GS_SUCCESS) {
        return GS_ERROR;
    }

    status = gsql_dump_rows(STMT, dumper);
    gsql_free_dumper(dumper);
    
    /* Restore CTCLIENT Client Character Set */
    if (gsql_reset_charset(g_local_config.charset_id, g_dump_opt.charset_id) != GS_SUCCESS) {
        gsql_print_error(CONN);
        return GS_ERROR;
    }
    return status;
...
void gsql_init_dumper(dumper_t *dumper)
{
#ifdef USE_GSQL_COLUMN_DESC
...
void gsql_show_dumper_usage(void)
{
    gsql_printf("The syntax of data dumper is: \n");
    gsql_printf("DUMP {TABLE table_name | QUERY \"select_query\"}\n");
    gsql_printf("    INTO FILE \"file_name\"\n");
    gsql_printf("    [FILE SIZE 'uint64_file_size']\n");
    ...
    gsql_printf("    [ENCRYPT BY 'password'];\n");
    gsql_printf("\n");
}

void gsql_show_dumper_opts(dumper_t dumper)
{
    gsql_printf("The global options for data dumper is: \n");
    if (GSQL_HAS_ENCLOSED_CHAR(dumper.column_param.enclosed_char)) {
...
*/
static inline int gsql_parse_dumping_object(lex_t *lex, dumper_t *dumper)
{
    uint32 matched_id;
    word_t word;
    ...
    if (lex_expected_fetch_1of2(lex, "TABLE", "QUERY", &matched_id) != GS_SUCCESS) {
        return GSC_ERROR;
    }
    ...
    if (matched_id == 0) {
        ...
        GS_RETURN_IFERR(cm_text2str(&word.text.value, g_dump_opt.query_sql, MAX_DUMP_SQL_SIZE));

        // verify query
        (void)cm_fetch_text(&word.text.value, ' ', '\0', &first);
        if (!cm_text_str_equal_ins(&first, "SELECT")) {
            GSQL_PRINTF(ZSERR_DUMP, "the query must be a SELECT statement");
            return GSC_ERROR;
        }
    }
 ...
 */
static inline int gsql_parse_dumping_file(lex_t *lex, dumper_t *dumper)
{
    word_t word;

    if (lex_expected_fetch_word2(lex, "INTO", "FILE") != GS_SUCCESS) {
        return GSC_ERROR;
    }
    ...
    return cm_text2str(&word.text.value, dumper->dump_file, GS_MAX_FILE_PATH_LENGH);
}
...
*/
int gsql_parse_dumping_options(lex_t *lex, dumper_t *dumper, dump_option_t *dump_opt)
{
...
#define DP_OPT_SIZE (sizeof(opt_records) / sizeof(word_record_t))
    ...
    do {
        if (lex_try_match_records(lex, opt_records, DP_OPT_SIZE, &matched_id) != GS_SUCCESS) {
            return GSC_ERROR;
        }
        ...
        switch (matched_id) {
            ...
            case DOPT_FILE_SIZE: {
                ...
                cm_trim_text(&text);
                if (cm_text2size(&text, &file_size) != GS_SUCCESS || file_size < 0) {
                    GS_SRC_THROW_ERROR(LEX_LOC, ERR_SQL_SYNTAX_ERROR, "invalid file size value!");
                    return GSC_ERROR;
                }
            ...
            case DOPT_ENCRYPT: {
                ...
                GS_RETURN_IFERR(gsql_get_crypt_pwd(lex, dump_opt->crypt_info.crypt_pwd, GS_PASSWD_MAX_LEN + 1, key_word_info));
                if (gsql_gen_encrypt_hash(&dump_opt->crypt_info) != GS_SUCCESS) {
                    GSQL_PRINTF(ZSERR_DUMP, "the encrypt pwd is not correct");
                    return GSC_ERROR;
                }
                dump_opt->crypt_info.crypt_flag = GS_TRUE;
                break;
            }

            default:
                if (strcmp(dump_opt->fields_terminated, dump_opt->lines_terminated) == 0) {
                    GS_SRC_THROW_ERROR(LEX_LOC, ERR_SQL_SYNTAX_ERROR,
...
static int gsql_parse_dumper(lex_t *lex, dumper_t *dumper)
{
    if (gsql_parse_dumping_object(lex, dumper) != GSC_SUCCESS) {
        return GSC_ERROR;
    }

    if (gsql_parse_dumping_file(lex, dumper) != GSC_SUCCESS) {
        return GSC_ERROR;
    }

    if (gsql_parse_dumping_options(lex, dumper, &g_dump_opt) != GSC_SUCCESS) {
        return GSC_ERROR;
    }
...
static inline int gsql_reset_dumper_opts(void)
{
    errno_t errcode;
    g_dump_opt.enclosed_optionally = GS_FALSE;
    g_dump_opt.fields_enclosed = GSQL_DEFAULT_ENCLOSED_CHAR;
...
status_t gsql_dump(text_t *cmd_text)
{
    ...
    gsql_init_dumper(&dumper);

    lex_trim(&sql_text);
    lex_init(&lex, &sql_text);

    if (lex_expected_fetch_word(&lex, "DUMP") != GS_SUCCESS) {
        gsql_print_error(NULL);
        return GS_ERROR;
    }
    ...
    if (matched_id != GS_INVALID_ID32) {
        gsql_show_dumper_opts(dumper);
        return GS_ERROR;
    }

    if (gsql_reset_dumper_opts() != GSC_SUCCESS) {
        return GS_ERROR;
    }

    if (gsql_parse_dumper(&lex, &dumper) != GS_SUCCESS) {
        gsql_print_error(NULL);
        return GS_ERROR;
    }

    return gsql_dump_query(&dumper);
}
