...
typedef enum {
    ...
    EXP_DESC_VERSION = 4,
    EXP_DESC_HYPHEN_V = 5,
    EXP_DESC_UNUSED = GS_INVALID_ID32
} exp_desc_type_t;

typedef struct {
    char name[GS_MAX_NAME_LEN + 4]; // align by 4 bytes
    uint16 type;
    uint16 size;
    uchar is_array;
    uchar unused[3];
...
#define EXP_MAX_SUBFILE_NAME_LEN (GS_MAX_NAME_LEN + 1)
