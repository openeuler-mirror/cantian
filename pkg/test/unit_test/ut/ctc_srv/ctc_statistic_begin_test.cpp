...
class TestCtcStatisticBegin : public testing::Test {
protected:
...
TEST_F(TestCtcStatisticBegin, Should_return_status_true_when_ctc_statistic_begin1)
{
    ...
    int ret = ctc_statistic_begin(&mock_tch,begin_time, &enable_stat);
...
TEST_F(TestCtcStatisticBegin, Should_return_status_true_when_ctc_statistic_begin2)
{
    mock_instance.sql.enable_sql_statistic_stat = CT_TRUE;
    bool enable_stat = true;
    MOCKER(ctc_get_or_new_session)
