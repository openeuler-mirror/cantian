...
extern "C" {
    int ctc_statistic_commit(ctc_handler_t *tch, char *sql_str);
    static int ctc_ddl_commit_log_put(knl_session_t *session, sql_stmt_t *stmt, ctc_ddl_def_node_t *def_node, ctc_ddl_dc_array_t *dc_node){
        return CT_SUCCESS;
    };
    static void ctc_ddl_table_after_commit_list(bilist_t *def_list, ctc_ddl_dc_array_t *dc_array, knl_session_t *session, bool *unlock_tables){
        return;
    };
}
...
protected:
    ...
    static void MockKnlCommit4Mysql(knl_session_t *session) {}
...
TEST_F(TestCtcTrxCommit, CommitSuccess) {
    ...
    int ret = ctc_statistic_commit(&mock_tch, sql_str);
