...
extern "C" {
    int ctc_mq_trx_begin(dsw_message_block_t *message_block);
}

using namespace std;

class TestCtcMqTrxBegin : public testing::Test {
protected:
    ...
    static int MockCtcTrxBegin(ctc_handler_t *tch, ctc_trx_context_t trx_context, bool is_mysql_local, timeval_t begin_time, bool *enable_stat) {
        *enable_stat = true; // 模拟 enable_stat 被正确设置
        return CT_SUCCESS;          // 返回模拟值
    }
};

TEST_F(TestCtcMqTrxBegin, Should_return_status_true_when_ctc_mq_trx_begin)
{
    // 模拟 ctc_trx_begin 的行为
    MOCKER(ctc_trx_begin)
        .stubs()
        .will(invoke(MockCtcTrxBegin));

    // 调用待测试函数
    int ret = ctc_mq_trx_begin(&message_block);
